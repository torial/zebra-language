...
const _ttag_IDEDiagnostic: u64 = 1088570404;
...
// zbr:IDE/ZebraIDE.zbr:20
        var argv = std.ArrayList([]const u8).empty;
// zbr:IDE/ZebraIDE.zbr:21
        argv.append(_allocator, "zebra") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:22
        argv.append(_allocator, "-c") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:23
        argv.append(_allocator, filepath) catch unreachable;
// zbr:IDE/ZebraIDE.zbr:24
        const run_out = _sys_run(argv);
// zbr:IDE/ZebraIDE.zbr:25
...
// zbr:IDE/ZebraIDE.zbr:29
        argv.append(_allocator, "zebra") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:30
        argv.append(_allocator, filepath) catch unreachable;
// zbr:IDE/ZebraIDE.zbr:31
        const run_out = _sys_run(argv);
// zbr:IDE/ZebraIDE.zbr:32
        if ((run_out.exit_code == 0)) {
// zbr:IDE/ZebraIDE.zbr:33
...
// zbr:IDE/ZebraIDE.zbr:37
        var diags = std.ArrayList(*IDEDiagnostic).empty;
// zbr:IDE/ZebraIDE.zbr:38
        {
            var _it_ln = std.mem.splitSequence(u8, raw, "\n");
            while (_it_ln.next()) |ln| {
// zbr:IDE/ZebraIDE.zbr:39
                if ((@as(i64, @intCast(std.mem.trim(u8, ln, &std.ascii.whitespace).len)) == 0)) {
                    continue;
                }
...
// zbr:IDE/ZebraIDE.zbr:44
        return diags;
    }

    pub fn parseLine(ln: []const u8) ?*IDEDiagnostic {
// zbr:IDE/ZebraIDE.zbr:47
        var colon1: i64 = (if (std.mem.indexOf(u8, ln, ":")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
// zbr:IDE/ZebraIDE.zbr:48
        if (_zebra_lt(colon1, 0)) {
// zbr:IDE/ZebraIDE.zbr:49
            return null;
        }
// zbr:IDE/ZebraIDE.zbr:50
        var searchFrom: i64 = (colon1 + 1);
// zbr:IDE/ZebraIDE.zbr:51
        if ((colon1 == 1)) {
// zbr:IDE/ZebraIDE.zbr:52
            const next = (if (std.mem.indexOf(u8, ln, ":")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
...
// zbr:IDE/ZebraIDE.zbr:57
        const colon2 = (if (std.mem.indexOf(u8, ln, ":")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
...
// zbr:IDE/ZebraIDE.zbr:60
        const colon3 = (if (std.mem.indexOf(u8, ln, ":")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
...
// zbr:IDE/ZebraIDE.zbr:63
        const lineStr = std.mem.trim(u8, ln[@intCast((colon1 + 1))..@intCast(colon2)], &std.ascii.whitespace);
...
// zbr:IDE/ZebraIDE.zbr:71
        var message: []const u8 = rest;
// zbr:IDE/ZebraIDE.zbr:72
        const colonPos = (if (std.mem.indexOf(u8, message, ":")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
// zbr:IDE/ZebraIDE.zbr:73
        if (_zebra_ge(colonPos, 0)) {
// zbr:IDE/ZebraIDE.zbr:74
            message = std.mem.trim(u8, message[@intCast((colonPos + 1))..@intCast(@as(i64, @intCast(message.len)))], &std.ascii.whitespace);
        }
...
const _ttag_CompilerBridge: u64 = 3241357169;
...
pub const Model = struct {
    _type_tag: u64 = _ttag_Model,
    filepath: []const u8 = "untitled.zbr",
    diags: std.ArrayList(*IDEDiagnostic) = std.ArrayList(*IDEDiagnostic).empty,
    output: []const u8 = "",
    dirty: bool = false,
    ...
    buildProc: ?*_SysProcess = null,
    buildTargets: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty,
    buildFile: []const u8 = "build.zbr",
    buildTmpFile: []const u8 = "",
    buildLastPos: i64 = 0,
    editor: ?*_CodeEditor = null,
    diagEditor: ?*_CodeEditor = null,
    outputEditor: ?*_CodeEditor = null,
    buildOutputEditor: ?*_CodeEditor = null,
    pub fn init() *Model {
        const self = _allocator.create(Model) catch @panic("OOM");
        self.* = .{};
        self._type_tag = _ttag_Model;
        return self;
    }

};

const _ttag_Model: u64 = 2189814010;
const _reflect_Model_name: []const u8 = "Model";
const _reflect_Model_fields: []const []const u8 = &.{"filepath", "diags", "output", "dirty", "debugProc", "debugPort", "buildOutput", "buildRunning", "buildProc", "buildTargets", "buildFile", "buildTmpFile", "buildLastPos", "editor", "diagEditor", "outputEditor", "buildOutputEditor"};
const _reflect_Model_field_types: []const []const u8 = &.{"str", "List(IDEDiagnostic)", "str", "bool", "?SysProcess", "int", "str", "bool", "?SysProcess", "List(str)", "str", "str", "int", "?CodeEditor", "?CodeEditor", "?CodeEditor", "?CodeEditor"};
...
pub const Msg = union(enum) {
    ...
    build_done,
    list_targets,
    filepath_changed: []const u8,
    buildfile_changed: []const u8,
    build_output_chunk: []const u8,
};
...
// zbr:IDE/ZebraIDE.zbr:129
        const idx = (if (std.mem.indexOf(u8, json, marker)) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
...
// zbr:IDE/ZebraIDE.zbr:133
        const endIdx = (if (std.mem.indexOf(u8, json, "\"")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1));
// zbr:IDE/ZebraIDE.zbr:134
        if (_zebra_lt(endIdx, 0)) {
            break;
        }
// zbr:IDE/ZebraIDE.zbr:136
        names.append(_allocator, json[@intCast(nameStart)..@intCast(endIdx)]) catch unreachable;
...
pub fn diagSummary(diags: std.ArrayList(*IDEDiagnostic)) []const u8 {
...
// zbr:IDE/ZebraIDE.zbr:148
    var lines: []const u8 = (std.fmt.allocPrint(_allocator, "Errors: {}  Warnings: {}\\n", .{errors, warnings}) catch @panic("OOM"));
// zbr:IDE/ZebraIDE.zbr:149
    for (diags.items) |d| {
// zbr:IDE/ZebraIDE.zbr:150
        var prefix: []const u8 = "W";
...
// zbr:IDE/ZebraIDE.zbr:153
        lines = _str_concat(lines, (std.fmt.allocPrint(_allocator, "[{s}] {}:{}  {s}\\n", .{prefix, d.line, d.col, d.message}) catch @panic("OOM")), _allocator);
    }
// zbr:IDE/ZebraIDE.zbr:154
    return lines;
}

pub fn ideInit() *Model {
// zbr:IDE/ZebraIDE.zbr:159
    var m = Model.init();
// zbr:IDE/ZebraIDE.zbr:160
    m.editor = _code_editor_new();
// zbr:IDE/ZebraIDE.zbr:161
    m.diagEditor = _code_editor_new();
// zbr:IDE/ZebraIDE.zbr:162
    m.outputEditor = _code_editor_new();
// zbr:IDE/ZebraIDE.zbr:163
    m.buildOutputEditor = _code_editor_new();
// zbr:IDE/ZebraIDE.zbr:164
    if ((blk: { std.Io.Dir.cwd().access(_io, m.filepath, .{}) catch break :blk false; break :blk true; })) {
// zbr:IDE/ZebraIDE.zbr:165
        _code_editor_set_text(m.editor.?, (std.Io.Dir.cwd().readFileAlloc(_io, m.filepath, _allocator, .unlimited) catch @panic("File.read error")));
    }
// zbr:IDE/ZebraIDE.zbr:166
    _code_editor_set_readonly(m.outputEditor.?, true);
// zbr:IDE/ZebraIDE.zbr:167
    _code_editor_set_readonly(m.diagEditor.?, true);
// zbr:IDE/ZebraIDE.zbr:168
    _code_editor_set_readonly(m.buildOutputEditor.?, true);
// zbr:IDE/ZebraIDE.zbr:169
    return m;
}

pub fn update(m: *Model, msg: Msg) *Model {
...
// zbr:IDE/ZebraIDE.zbr:178
                m.diags = std.ArrayList(*IDEDiagnostic).empty;
// zbr:IDE/ZebraIDE.zbr:179
                _code_editor_set_text(m.diagEditor.?, "");
// zbr:IDE/ZebraIDE.zbr:180
                m.output = _intern("");
// zbr:IDE/ZebraIDE.zbr:181
                _code_editor_set_text(m.outputEditor.?, "");
// zbr:IDE/ZebraIDE.zbr:182
                m.dirty = false;
            }
        },
        .save_file => {
// zbr:IDE/ZebraIDE.zbr:184
            (blk: {
                const _fw_path = m.filepath;
                const _fw_f = std.Io.Dir.cwd().createFile(_io, _fw_path, .{}) catch @panic("File.write error");
                defer _fw_f.close(_io);
                _fw_f.writeStreamingAll(_io, _code_editor_get_text(m.editor.?)) catch @panic("File.write error");
                break :blk {};
            });
...
// zbr:IDE/ZebraIDE.zbr:188
            (blk: {
                const _fw_path = tmpFile;
                const _fw_f = std.Io.Dir.cwd().createFile(_io, _fw_path, .{}) catch @panic("File.write error");
                defer _fw_f.close(_io);
                _fw_f.writeStreamingAll(_io, _code_editor_get_text(m.editor.?)) catch @panic("File.write error");
                break :blk {};
            });
// zbr:IDE/ZebraIDE.zbr:189
            m.diags = CompilerBridge.check(tmpFile);
// zbr:IDE/ZebraIDE.zbr:190
            _code_editor_set_text(m.diagEditor.?, diagSummary(m.diags));
// zbr:IDE/ZebraIDE.zbr:191
            _code_editor_set_error_markers(m.editor.?, m.diags);
// zbr:IDE/ZebraIDE.zbr:192
            m.dirty = true;
        },
        .run_requested => {
// zbr:IDE/ZebraIDE.zbr:194
            m.buildOutput = _intern("");
// zbr:IDE/ZebraIDE.zbr:195
            _code_editor_set_text(m.buildOutputEditor.?, "");
// zbr:IDE/ZebraIDE.zbr:196
            (blk: {
                const _fw_path = m.filepath;
                const _fw_f = std.Io.Dir.cwd().createFile(_io, _fw_path, .{}) catch @panic("File.write error");
                defer _fw_f.close(_io);
...
// zbr:IDE/ZebraIDE.zbr:197
            m.dirty = false;
// zbr:IDE/ZebraIDE.zbr:198
            m.output = _intern(CompilerBridge.runFile(m.filepath));
// zbr:IDE/ZebraIDE.zbr:199
            _code_editor_set_text(m.outputEditor.?, m.output);
        },
        .debug_start => {
// zbr:IDE/ZebraIDE.zbr:201
            if ((m.debugProc != null)) {
// zbr:IDE/ZebraIDE.zbr:202
                if (_sys_process_is_running(m.debugProc.?)) {
// zbr:IDE/ZebraIDE.zbr:203
                    return m;
                }
            }
...
// zbr:IDE/ZebraIDE.zbr:205
            m.dirty = false;
// zbr:IDE/ZebraIDE.zbr:206
            m.debugPort = 7777;
// zbr:IDE/ZebraIDE.zbr:207
            var argv = std.ArrayList([]const u8).empty;
// zbr:IDE/ZebraIDE.zbr:208
            argv.append(_allocator, "zebra") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:209
            argv.append(_allocator, "debug") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:210
            argv.append(_allocator, "--listen") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:211
            argv.append(_allocator, (std.fmt.allocPrint(_allocator, "{}", .{m.debugPort}) catch unreachable)) catch unreachable;
// zbr:IDE/ZebraIDE.zbr:212
            argv.append(_allocator, m.filepath) catch unreachable;
// zbr:IDE/ZebraIDE.zbr:213
            m.debugProc = _sys_spawn(argv);
// zbr:IDE/ZebraIDE.zbr:214
            m.output = _intern((std.fmt.allocPrint(_allocator, "Debug proxy on 127.0.0.1:{} — connect with DAP client.", .{m.debugPort}) catch @panic("OOM")));
...
// zbr:IDE/ZebraIDE.zbr:221
            m.output = _intern("Debugger stopped.");
...
// zbr:IDE/ZebraIDE.zbr:229
            m.buildTmpFile = _intern(".ide_build.tmp");
// zbr:IDE/ZebraIDE.zbr:230
            (blk: {
                const _fw_path = m.buildTmpFile;
                const _fw_f = std.Io.Dir.cwd().createFile(_io, _fw_path, .{}) catch @panic("File.write error");
                defer _fw_f.close(_io);
                _fw_f.writeStreamingAll(_io, "") catch @panic("File.write error");
                break :blk {};
            });
...
// zbr:IDE/ZebraIDE.zbr:232
            _code_editor_set_text(m.buildOutputEditor.?, "");
// zbr:IDE/ZebraIDE.zbr:233
            m.buildLastPos = 0;
// zbr:IDE/ZebraIDE.zbr:234
            m.buildRunning = true;
// zbr:IDE/ZebraIDE.zbr:235
            var cmd: []const u8 = "zebra build";
// zbr:IDE/ZebraIDE.zbr:236
            if (!std.mem.eql(u8, m.buildFile, "build.zbr")) {
// zbr:IDE/ZebraIDE.zbr:237
                cmd = _str_concat(_str_concat(cmd, " --build-file=", _allocator), m.buildFile, _allocator);
            }
// zbr:IDE/ZebraIDE.zbr:238
            var argv = std.ArrayList([]const u8).empty;
// zbr:IDE/ZebraIDE.zbr:239
            argv.append(_allocator, "cmd") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:240
            argv.append(_allocator, "/C") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:241
            argv.append(_allocator, _str_concat(_str_concat(_str_concat(cmd, " > ", _allocator), m.buildTmpFile, _allocator), " 2>&1", _allocator)) catch unreachable;
// zbr:IDE/ZebraIDE.zbr:242
            m.buildProc = _sys_spawn(argv);
        },
        .build_cancel => {
...
// zbr:IDE/ZebraIDE.zbr:247
        ...
        .build_done => {
// zbr:IDE/ZebraIDE.zbr:249
            m.buildRunning = false;
        },
        .list_targets => {
// zbr:IDE/ZebraIDE.zbr:251
            var argv = std.ArrayList([]const u8).empty;
...
// zbr:IDE/ZebraIDE.zbr:254
            argv.append(_allocator, "--list-targets") catch unreachable;
// zbr:IDE/ZebraIDE.zbr:255
            const out = _sys_run(argv);
// zbr:IDE/ZebraIDE.zbr:256
            if ((out.exit_code == 0)) {
// zbr:IDE/ZebraIDE.zbr:257
                m.buildTargets = parseBuildTargetNames(out.stdout);
            } else {
// zbr:IDE/ZebraIDE.zbr:259
                m.buildTargets = std.ArrayList([]const u8).empty;
            }
        },
        .filepath_changed => |path| {
// zbr:IDE/ZebraIDE.zbr:261
            m.filepath = _intern(path);
        },
        .buildfile_changed => |path| {
// zbr:IDE/ZebraIDE.zbr:263
            m.buildFile = _intern(path);
        },
        .build_output_chunk => |chunk| {
// zbr:IDE/ZebraIDE.zbr:265
            m.buildOutput = _intern(_str_concat(m.buildOutput, chunk, _allocator));
// zbr:IDE/ZebraIDE.zbr:266
            m.buildLastPos = (m.buildLastPos + @as(i64, @intCast(chunk.len)));
// zbr:IDE/ZebraIDE.zbr:267
            _code_editor_set_text(m.buildOutputEditor.?, m.buildOutput);
        },
    }
...
pub fn view(g: GuiContext, m: *Model) void {
// zbr:IDE/ZebraIDE.zbr:274
    if (m.debugProc) |proc| {
// zbr:IDE/ZebraIDE.zbr:275
        if ((!_sys_process_is_running(proc))) {
...
// zbr:IDE/ZebraIDE.zbr:278
        if (!std.mem.eql(u8, m.buildTmpFile, "")) {
// zbr:IDE/ZebraIDE.zbr:279
            if ((blk: { std.Io.Dir.cwd().access(_io, m.buildTmpFile, .{}) catch break :blk false; break :blk true; })) {
// zbr:IDE/ZebraIDE.zbr:280
                const content = (std.Io.Dir.cwd().readFileAlloc(_io, m.buildTmpFile, _allocator, .unlimited) catch @panic("File.read error"));
// zbr:IDE/ZebraIDE.zbr:281
                if (_zebra_gt(@as(i64, @intCast(content.len)), m.buildLastPos)) {
// zbr:IDE/ZebraIDE.zbr:282
                    g.send(Msg{ .build_output_chunk = content[@intCast(m.buildLastPos)..@intCast(@as(i64, @intCast(content.len)))] });
                }
            }
        }
// zbr:IDE/ZebraIDE.zbr:283
        if (m.buildProc) |proc| {
// zbr:IDE/ZebraIDE.zbr:284
            if ((!_sys_process_is_running(proc))) {
// zbr:IDE/ZebraIDE.zbr:285
                g.send(Msg.build_done);
            }
        }
    }
// zbr:IDE/ZebraIDE.zbr:287
    g.beginVBox("##root", true);
// zbr:IDE/ZebraIDE.zbr:290
    g.beginHBox("##toolbar", false);
// zbr:IDE/ZebraIDE.zbr:291
    if (g.button("Open")) {
// zbr:IDE/ZebraIDE.zbr:292
        g.send(Msg.open_file);
    }
// zbr:IDE/ZebraIDE.zbr:293
    if (g.button("Save")) {
// zbr:IDE/ZebraIDE.zbr:294
        g.send(Msg.save_file);
    }
// zbr:IDE/ZebraIDE.zbr:295
    if (g.button("Check")) {
// zbr:IDE/ZebraIDE.zbr:296
        g.send(Msg.check_requested);
    }
// zbr:IDE/ZebraIDE.zbr:297
    if (g.button("Run")) {
// zbr:IDE/ZebraIDE.zbr:298
        g.send(Msg.run_requested);
    }
// zbr:IDE/ZebraIDE.zbr:299
    if (g.button("Debug")) {
// zbr:IDE/ZebraIDE.zbr:300
...
// zbr:IDE/ZebraIDE.zbr:307
    const pathVal = g.input("##filepath", m.filepath);
// zbr:IDE/ZebraIDE.zbr:308
    if (!std.mem.eql(u8, pathVal, m.filepath)) {
// zbr:IDE/ZebraIDE.zbr:309
        g.send(Msg{ .filepath_changed = pathVal });
    }
// zbr:IDE/ZebraIDE.zbr:310
    var statusLine: []const u8 = "";
// zbr:IDE/ZebraIDE.zbr:311
    if (m.dirty) {
// zbr:IDE/ZebraIDE.zbr:312
        statusLine = "* ";
    }
// zbr:IDE/ZebraIDE.zbr:313
    if (_zebra_gt(m.debugPort, 0)) {
// zbr:IDE/ZebraIDE.zbr:314
        statusLine = _str_concat(statusLine, (std.fmt.allocPrint(_allocator, "DAP :{}  ", .{m.debugPort}) catch @panic("OOM")), _allocator);
    }
// zbr:IDE/ZebraIDE.zbr:315
    if (m.buildRunning) {
// zbr:IDE/ZebraIDE.zbr:316
        statusLine = _str_concat(statusLine, "Building...", _allocator);
    }
// zbr:IDE/ZebraIDE.zbr:317
    g.text(statusLine);
// zbr:IDE/ZebraIDE.zbr:318
    g.endHBox();
// zbr:IDE/ZebraIDE.zbr:321
    g.beginHBox("##body", true);
// zbr:IDE/ZebraIDE.zbr:323
    g.beginVBox("##editor_col", true);
// zbr:IDE/ZebraIDE.zbr:324
    _code_editor_render(m.editor.?, g, "##code", 0, 0);
// zbr:IDE/ZebraIDE.zbr:325
    g.endVBox();
// zbr:IDE/ZebraIDE.zbr:327
    g.beginVBox("##right_col", false);
// zbr:IDE/ZebraIDE.zbr:328
    g.text("Diagnostics");
// zbr:IDE/ZebraIDE.zbr:329
    _code_editor_render(m.diagEditor.?, g, "##diags", 340, 280);
// zbr:IDE/ZebraIDE.zbr:330
    g.separator();
// zbr:IDE/ZebraIDE.zbr:331
    g.text("Build file:");
// zbr:IDE/ZebraIDE.zbr:332
    const bfVal = g.input("##buildfile", m.buildFile);
// zbr:IDE/ZebraIDE.zbr:333
    if (!std.mem.eql(u8, bfVal, m.buildFile)) {
// zbr:IDE/ZebraIDE.zbr:334
        g.send(Msg{ .buildfile_changed = bfVal });
    }
// zbr:IDE/ZebraIDE.zbr:335
    if (g.button("List Targets")) {
// zbr:IDE/ZebraIDE.zbr:336
        g.send(Msg.list_targets);
    }
// zbr:IDE/ZebraIDE.zbr:337
    var targetsText: []const u8 = "";
// zbr:IDE/ZebraIDE.zbr:338
    for (m.buildTargets.items) |t| {
// zbr:IDE/ZebraIDE.zbr:339
        targetsText = _str_concat(_str_concat(_str_concat(targetsText, "  ", _allocator), t, _allocator), "\n", _allocator);
    }
// zbr:IDE/ZebraIDE.zbr:340
    g.text(targetsText);
// zbr:IDE/ZebraIDE.zbr:341
    g.endVBox();
// zbr:IDE/ZebraIDE.zbr:343
    g.endHBox();
// zbr:IDE/ZebraIDE.zbr:346
    g.beginHBox("##output_row", false);
// zbr:IDE/ZebraIDE.zbr:348
    g.beginVBox("##output_col", true);
// zbr:IDE/ZebraIDE.zbr:349
    g.text("Output");
// zbr:IDE/ZebraIDE.zbr:350
    _code_editor_render(m.outputEditor.?, g, "##output", 0, 120);
// zbr:IDE/ZebraIDE.zbr:351
    g.endVBox();
// zbr:IDE/ZebraIDE.zbr:353
    g.beginVBox("##buildout_col", true);
// zbr:IDE/ZebraIDE.zbr:354
    g.text("Build Output");
// zbr:IDE/ZebraIDE.zbr:355
    _code_editor_render(m.buildOutputEditor.?, g, "##buildout", 0, 120);
// zbr:IDE/ZebraIDE.zbr:356
    g.endVBox();
// zbr:IDE/ZebraIDE.zbr:358
    g.endHBox();
// zbr:IDE/ZebraIDE.zbr:360
    g.endVBox();
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
    _tui_env = _zinit.environ_map;
// zbr:IDE/ZebraIDE.zbr:365
    _gui_mvu_run("Zebra IDE", 1200, 800, ideInit, update, view);
}
