...
const Printer = struct {
    ...
    fn printStmt(p: *Printer, s: Ast.Stmt) anyerror!void {
        switch (s) {
            ...
            .assert_eq    => |n| { try p.w("(assert-eq ", .{}); try p.printExpr(n.lhs.*); try p.w(", ", .{}); try p.printExpr(n.rhs.*); try p.w(")", .{}); },
            .assert_ne    => |n| { try p.w("(assert-ne ", .{}); try p.printExpr(n.lhs.*); try p.w(", ", .{}); try p.printExpr(n.rhs.*); try p.w(")", .{}); },
    ...
    fn printExpr(p: *Printer, e: Ast.Expr) anyerror!void {
        switch (e) {
            ...
            .to_non_nil => |n| { try p.w("(force-unwrap ", .{}); try p.printExpr(n.expr.*); try p.w(")", .{}); },
