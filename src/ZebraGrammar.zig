...
pub const NT = enum {
    ...
    // ── general allocator-context block ───────────────────────────────────
    StmtAllocate,     // allocate Expr eol Block
...
const stmt_rules: []const Rule = &.{
    ...
    .{ .lhs = .StmtAssertEq,    .rhs = &.{ t(.kw_assert_eq),    n(.Expr), t(.comma), n(.Expr), t(.eol) } },
