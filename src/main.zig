...
fn cloneInterface(src: *const TypeChecker.ModuleInterface, alloc: std.mem.Allocator) !TypeChecker.ModuleInterface {
    ...
    {
        var it = src.optional_fields.keyIterator();
        while (it.next()) |k| try optional_fields.put(try alloc.dupe(u8, k.*), {});
    }
    var optional_method_returns = std.StringHashMap(void).init(alloc);
    errdefer optional_method_returns.deinit();
    {
        var it = src.optional_method_returns.keyIterator();
        while (it.next()) |k| try optional_method_returns.put(try alloc.dupe(u8, k.*), {});
    }
...
fn compileZbrToZig(
    ...
    if (gop.found_existing) {
        ...
        return TypeChecker.ModuleInterface{
            ...
            .optional_fields              = std.StringHashMap(void).init(alloc),
