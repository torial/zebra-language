...
// glfw (THE imgui backend), sdl2 and dx12 were removed 2026-08-29. glfw carried the
// imgui implementation; sdl2/dx12 were 178 lines of DUPLICATED STUB under a single
// "not yet implemented" comment -- reserved for something never built, which is the
// `aspect` shape from the reserved-word work: every --gui-backend value is a promise
// the CLI appears to make.
pub const GuiBackend = enum { stub, tui, libui_ng };
...
pub const GenerateResult = struct {
    ...
    /// external GUI dependency (e.g. the tui or libui_ng runtime).
    uses_gui: bool,
...
const Generator = struct {
    ...
    fn genModule(g: Generator, module: Ast.Module) anyerror!void {
        ...
        // deferred module-global whose init constructs a cross-module class needs
        // that class in class_names now, so `genExpr(Foo())` emits `Foo.init()`
        // (not `Foo()`, which is `type 'type' not a function`).  Idempotent with the
        // genUse registration.
        for (module.decls) |decl| {
            const u = switch (decl) { .use => |u| u, else => continue };
            const last_dot = std.mem.lastIndexOf(u8, u.path, ".");
            const alias = if (last_dot) |d| u.path[d + 1 ..] else u.path;
            const iface = if (g.imported_modules) |im| im.get(alias) else null;
            if (iface) |ifc| {
                for (u.exposing) |exp_name| {
                    if (ifc.types.getPtr(exp_name)) |kind_ptr| {
                        switch (kind_ptr.*) {
                            .union_ => try g.exposed_unions.put(exp_name, alias),
                            .class => {
                                try g.exposed_classes.put(exp_name, {});
                                try g.class_names.put(exp_name, {});
                            },
        ...
        try g.w.writeAll("\n\nconst std     = @import(\"std\");\nconst builtin = @import(\"builtin\");\npub const panic = std.debug.FullPanic(_zebra_panic);\nvar _io: std.Io = undefined;\nvar _args: std.process.Args = undefined;\n\n");
        ...
        // comptime-known: HashMap/Set/Atomic (reference the runtime _allocator) and
        // user class instances (`Foo.init()` allocates via _allocator.create — BUG-157).
        // They are declared `= undefined` at container scope; the once guard tolerates
        ...
        if (try findMainClass(module.decls, g.alloc, "")) |class_name| {
            ...
            "    _environ = _zinit.minimal.environ;\n" ++
                    "    _environ = _zinit.minimal.environ;\n" ++
    ...
    /// BUG-153/157: returns the TypeRef to use for the `= undefined` decl of a
    /// module-global `var` whose initializer is NOT comptime-known, or null if the
    /// var can be initialized inline at Zig global scope. Two deferred cases:
    ///   • generic containers (HashMap/Set/Atomic) — init references `_allocator`.
    ///   • user class instances (`var g: Foo = Foo()`) — `Foo.init()` allocates
    ///     via `_allocator.create`, which Zig cannot evaluate at global comptime.
    /// Both are emitted `= undefined` here and assigned in `_initModuleVars()`.
    /// Requires an explicit type annotation — that gives the `: T` for the decl
    /// and keeps both compilers symmetric.
    fn deferredModuleVarType(g: Generator, n: *Ast.DeclVar) ?Ast.TypeRef {
        if (n.is_const) return null;
        const init = n.init orelse return null;
        const tr = n.type_ orelse return null;
        switch (tr) {
            .generic => |gen| if (isDeferredContainerName(gen.name)) return tr,
            .named => |nm| {
                // A named user class instance (`var g: Foo = Foo()`) allocates via
                // `_allocator.create` in `Foo.init()`.  `class_names` catches locals
                // and TC-classified cross-module classes; but when the dep's TC
                // interface is unavailable, an exposed class isn't in `class_names`.
                // Fall back to the initializer shape: a named-type global whose init
                // is a constructor/factory *call* always allocates, so defer it.
                // (Deferring is semantically safe — assigned in `_initModuleVars`
                // before any use; `const` and comptime-value inits are excluded.)
                if (g.class_names.contains(nm.name)) return tr;
                if (init.* == .call) return tr;
            },
            else => {},
        }
        return null;
    ...
    fn genTopVar(g: Generator, n: *Ast.DeclVar) anyerror!void {
        ...
        // BUG-153/157: defer the init of allocating globals (containers + user
        // class instances) to _initModuleVars().
        if (g.deferredModuleVarType(n)) |dtr| {
    ...
    fn genMethod(g: Generator, n: *Ast.DeclMethod) anyerror!void {
        ...
        if (n.mods.extern_) {
            ...
            if (g.owner.len == 0 and std.mem.eql(u8, n.name, "main") and !g.test_mode) {
                ...
                try _eg.writeIndent(); try _eg.w.writeAll("_environ = _zinit.minimal.environ;\n");
        ...
        if (n.init) |e| {
            if (n.type_) |tr| {
                if (tr == .generic and std.mem.eql(u8, tr.generic.name, "List") and e.* == .list_lit) {
                    ...
                    // and --gui-backend=stub still delegates its whole build here, so a
