...
**Zebra** is a programming language whose compiler is written in Zebra itself
(self-hosted since the Phase 22 cutover; the original Zig-implemented compiler
was retired on 2026-09-16 — `docs/design/bootstrap_sunset.md`). The name is a
portmanteau of Zig and Cobra; language design draws on Python, Cobra, and Eiffel
(contracts, nil tracking) with a Zig runtime and error model.
...
- `selfhost/` — THE compiler, written in Zebra (`*.zbr`). Compiler module files are
  **PascalCase** (`Ast`, `AstWalk`, `AstBuilder`, `CgHelpers`, `Checker`, `CodeGen`,
  `Parser`, `Resolver`, `TypeChecker`, `Token`, `Lexer`; `main` stays lowercase per
  Zig convention), a naming inherited from the retired Zig `src/` tree. Paired `*.zig`
  files are generated artifacts (`Parser.zbr` → `Parser.zig`), committed so the repo
  builds with nothing but `zig`. `stdlib_preamble.zig` and the two `gui_*_section.zig`
  files are the hand-written runtime, read at codegen time.
  (`src/` — the Zig-implemented bootstrap, 37k lines — was deleted 2026-09-16; it is in
  `git log -- src/` and nowhere else.)
...
- `grammar.txt` — language grammar reference, FROZEN at its last export from the retired
  bootstrap's rule table (2026-09-15); `fuzz/gramgen.py` reads it. Not regenerated by anything.
...
**A `stdlib_preamble.zig` edit needs the full sequence.** The preamble is inlined at
emit time, so the compiler only picks up a preamble change after **regeneration** (this
is what made BUG-219's first fix look like it hadn't worked). `rebuild.sh --no-regen` is
for a `.zig`-only edit that is NOT the preamble.

(Until 2026-09-16 there was a second trap here, footgun 4: the Zig-implemented bootstrap
EMBEDDED the preamble files at its own build time, and the regen ran that binary, so a
preamble edit was invisible until the bootstrap was rebuilt — observed 2026-07-28 — and
on 2026-08-26 a `src/*.zig` edit failed the same way with a type error naming the very
feature being added. `rebuild.sh` pre-built the bootstrap when an input was newer, and
`rebuild_guard_check.sh` falsified that guard. The selfhost reads the preamble FROM DISK
at codegen time, repo-relative first, so there is no embedding binary to be stale against;
the guard and its check went to `tools/attic/` with the bootstrap, sunset Step 3.)
...
waste. It is sound because the regeneration is done by the compiler built from the
**committed** `.zig` (N-1), whose output for the other modules your edit cannot have
changed — verified by regenerating an untouched module and getting a byte-identical file. (Extracted from `mutation_check.py`'s `regen()`,
...
`src/CodeGen.zig` was rebuilt twice that night (falsifying BUG-124 needs a mutated <!-- doc-lint-ok: a dated observation about the bootstrap, deleted 2026-09-16 -->
...
bash tools/contract_mode_check.sh  # THE CONTRACT-STRIPPING CONTRACT (FULL tier, ~55s):
                                ...
                                #   all 11 checks ran.
                                #   (`grammar_export.py --check`, THE GRAMMAR-DRIFT GATE
                                #   `grammar-export`, sat here 2026-08-04 .. 2026-09-16:
                                #   grammar.txt was GENERATED from the bootstrap's Earley
                                #   rule table, src/ZebraGrammar.zig, and the gate asserted
                                #   the document matched the table. Its first export found
                                #   fuzz/gramgen.py generating 9 constructs the parser did
                                #   not have and never reaching 40 it did. The table left
                                #   with the bootstrap -- sunset Step 3 -- so grammar.txt is
                                #   FROZEN at the last export; the selfhost parses by hand
                                #   and nothing derives a grammar from it yet. gramgen still
                                #   reads the frozen file. tools/attic/grammar_export.py.)
...
python tools/lint_diag_columns.py  # THE DIAGNOSTIC-POSITION GATE (QUICK tier, ~2s).
                                ...
                                #   (`zig_test_check.sh`, THE ZIG-SIDE TEST GATE `zig-test`,
                                #   sat here 2026-08-09 .. 2026-09-16 -- BUG-279: the
                                #   bootstrap's 120 unit + 11 integration tests, which had
                                #   been in NO tier and NOT in the uncovered table either,
                                #   the one state that table exists to make impossible;
                                #   three failures sat on committed code because of it, two
                                #   of them stale tests asserting REMOVED syntax. The code
                                #   under test is gone -- sunset Step 3 -- and the compiler's
                                #   own tests are the .zbr fixtures smoke runs.
...
python tools/registration_check.py # THE UNASSERTED-FILE GATE (BUG-243, static, instant):
                                ...
                                #   QUICK tier.
                                #   (`lint_decl_exhaustive.py`, THE Ast.Decl EXHAUSTIVENESS
                                #   GATE `decl-exhaustive`, sat here 2026-08-26 .. 2026-09-16:
                                #   BUG-103's pin -- no `else => {}` in the bootstrap's
                                #   Ast.Decl switches, so adding a variant is a Zig COMPILE
                                #   ERROR rather than a silent skip. Oracle and subject were
                                #   both src/*.zig; retired with them, sunset Step 3. The
                                #   selfhost's `branch` over Decl is exhaustive by the
                                #   language's own rule. tools/attic/lint_decl_exhaustive.py.)
...
| **…and it is the RIGHT thing, per the reference** | **`boundary_check`** (intent-authored, not recorded) | 33 probes / 319 assertions | <!-- doc-gen: 33 = bash tools/corpus_ls.sh test/boundary | wc -l | tr -d ' ' --> <!-- doc-gen: 319 = cat test/boundary/*.expected | grep -c . -->
...
while the tracked corpus is **663** <!-- doc-gen: 663 = bash tools/corpus_ls.sh test | wc -l | tr -d ' ' -->.
...
— the emitted `src/main.zig` is the INLINE shape (3,947 lines, **zero** `zebra_rt.zig` <!-- doc-lint-ok: the GUI scaffold's own src/main.zig, not a path in this repo -->
...
The compiler lives in `selfhost/` and is written in Zebra. It regenerates itself: the
`zebra.exe` built from the COMMITTED `selfhost/*.zig` (the previous generation, N-1)
re-emits `selfhost/*.zbr`, and `tools/bootstrap_check.sh` proves the level-2 fixed point
(A emits B, B emits A's bytes). The rule that made the port complete — *"the Zebra
compiler in `selfhost/` must be functionally equivalent to the Zig compiler in `src/`"* —
retired with `src/` on 2026-09-16 (`docs/design/bootstrap_sunset.md`). What replaces it:
**equivalence with the previous release**, which is what `tools/divergence_check.sh`
measures against the N-1 anchor; a feature that stops working is a `REGRESSION` there,
not a gap. `docs/SELFHOST_JOURNAL.md` records how each phase of the port was done.

If `zebra.exe` cannot build itself: `bash tools/regen_recover.sh --install` (optionally
`--from <last good sha>`), then `zig build`, then gates. Compiler changes go in
`selfhost/*.zbr`, always through `tools/rebuild.sh`, never by editing a generated `.zig`.
...
1. Implement it in the appropriate `selfhost/*.zbr` module; `bash tools/rebuild.sh`.
2. Extend the test suite in `test/` and register the fixture in `tools/selfhost_smoke.sh`.
3. Update `QUICKSTART.md` if user-visible syntax or semantics change, and `CHANGELOG.md`
   (plus `docs/SURFACE.md` via `surface_inventory.py --write`) if the surface moved.

**Self-hosting (Phase 22 complete; bootstrap retired 2026-09-16):**
- `zig build` produces `zig-out/bin/zebra.exe` from `selfhost/main.zig`, the only binary.
- `tools/bootstrap_check.sh` regenerates `selfhost/*.zig` from `*.zbr` with that binary
  (N-1) and proves the round-trip; `tools/regen_recover.sh` is the recovery path.
- Keep intermediate Zig files using `zebra --emit-zig` or `--output-dir DIR`.
- The sunset in three steps (bootstrap_sunset.md): Step 1 (2026-09-15) retired the last
  delegations (`--zig-backend`, `--gui-backend=glfw`, `debug --listen`); Step 2 the
  comparison tooling (`selfhost-div`, `interp-escape`, `compile_check --bootstrap`,
  `diagnostic_parity`, `scaling_probe`); Step 3 (2026-09-16) deleted `src/` and the
  `zebra-bootstrap` target with `zig-test`, `grammar-export` and `decl-exhaustive`, whose
  oracles it was.
...
(`src/Binder.zig`, `src/Resolver.zig`, `test/main.zig`). (2) The same morning <!-- doc-lint-ok: a dated receipt; all three files were deleted with the bootstrap 2026-09-16 -->
