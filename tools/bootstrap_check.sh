...
#
# Known quirk (INFRA-001): the first --update run after certain bootstrap states
# can produce "Generated by the Zebra compiler." file headers (bootstrap style)
# instead of the expected "Generated by zebra-selfhost." (selfhost style).
# Subsequent runs are stable and idempotent. Root cause is not fully understood —
# see BUGS.md INFRA-001 for details. Cosmetic only; both forms compile correctly.
#
...
if [[ $QUICK -eq 1 ]]; then
    if [[ $UPDATE -eq 1 ]]; then
        ...
        for f in "${FILES[@]}"; do
            if ! "$SELFHOST_A" --emit-zig "selfhost/$f.zbr" >/dev/null 2>/tmp/bs-update-err; then
