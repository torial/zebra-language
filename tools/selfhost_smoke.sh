...
if got=$("$ZEBRA" test/prog_args_passthrough_test.zbr -- alpha "b c" --verbose 2>&1) && echo "$got" | grep -qF "prog_args: alpha|b c|--verbose|"; then
    echo "  PASS: prog_args_passthrough_test_run"; PASS=$((PASS + 1))
else
    echo "  FAIL: prog_args_passthrough_test_run" >&2; echo "    got: $got" >&2; FAIL=$((FAIL + 1))
fi
...
# BUG-253: two diagnostics ported from the bootstrap. Negatives in separate files -- a
...
smoke_tc_fail test/bug253_struct_destruct_nonclass_fail.zbr "struct destructuring requires a class or struct"
smoke_tc_fail test/bug253_uninit_collection_fail.zbr "requires explicit initialization"
smoke_run     test/bug253_uninit_collection_field_test.zbr "bug253-uninit: OK"
...
smoke_run test/bug337_json_getlist_list_test.zbr "bug337: OK"
...
smoke_tc_fail test/bug380_trailing_colon_header_fail.zbr "a block header ends at the end of the line with no colon"
smoke_tc_fail test/bug380_return_arrow_fail.zbr "the return type is written with a colon"
smoke_tc_fail test/bug381_container_property_fail.zbr "has no property 'length'"
smoke_run test/bug381_container_property_ok_test.zbr "3 3 3 1 9 1"
smoke_tc_fail test/bug382_str_num_compare_fail.zbr "cannot compare 'int' with 'str'"
...
smoke_run test/bug392_stdlib_str_returns_test.zbr "rt=hello hello hello"
smoke_tc_fail test/bug393_regex_unknown_method_fail.zbr "'Regex' has no method 'matches'"
...
smoke_run test/bug401_struct_positional_ctor_test.zbr "1 2 0 5 0 hi 3"
smoke_run test/bug402_enum_list_literal_test.zbr "2 2 2 [red, blue]"
smoke_run test/bug403_split_index_test.zbr "one"
smoke_tc_fail test/bug404_extends_fail.zbr "Zebra has no class inheritance"
smoke_tc_fail test/bug405_python_ternary_fail.zbr "there is no \`a if c else b\`"
smoke_run test/bug406_print_containers_test.zbr "[2, 4]"
smoke_run test/bug407_primitive_loop_var_test.zbr "7"
...
smoke_run test/bug415_dep_module_catch_message_test.zbr "caught negative: -1"
smoke_run test/bug416_regex_split_test.zbr "[\"a\", \"b\", \"c\", \"\", \"d\"]"
smoke_tc_fail test/bug419_arrow_lambda_fail.zbr "there is no \`x => expr\` arrow lambda in Zebra"
...
smoke_run test/bug437_list_append_alias_test.zbr "append: 2 2 a,2 5"
smoke_run test/bug314_nested_copy_mutation_fail.zbr "1 3"
