...
def regen(wt, rel):
    ...
    rc, out = run([str(boot), "--emit-zig", rel], wt, timeout=300)
    if rc != 0:
        return False, out
    marker = "// Generated by the Zebra compiler."
    if marker not in out:
        return False, "bootstrap emitted no source"
...
def main():
    ...
    for n, (rel, ln, c0, c1, op, old, new) in enumerate(sites, 1):
        ...
        ok, msg = regen(wt, rel)
        if not ok:
            verdict, by = "DETECTED", "regen"      # the bootstrap refused it
        else:
            rc, blog = build(wt)
            if rc != 0:
        ...
        results.append(dict(file=rel, line=ln + 1, op=op, old=old, new=new,
                            verdict=verdict, by=by, secs=round(time.time() - t0, 1)))
