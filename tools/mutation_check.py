...
2026-07-31, that is wrong by an order of magnitude:
    ...
    regen ONE module via the selfhost    ~22 s   (not the full 12-module regen)
...
def regen(wt, rel):
    """
    Regenerate ONE module's .zig via the SELFHOST -- the cheap half of a rebuild, the same
    shape as `tools/rebuild.sh --module`: emit the WHOLE program from selfhost/main.zbr
    with --output-dir into a scratch directory and install the one file. (Emitting a
    module as a ROOT gives it scaffolding a dependency does not -- measured, 648 vs 643
    lines for Token.zig -- so the per-module emit must go through main.zbr.)

    Until 2026-09-15 this ran zebra-bootstrap.exe with a stdout redirect (the bootstrap
    had no --output-dir). Two hazards from that era are kept in mind here: a large
    stdout through a PIPE came back EMPTY with rc=0 on Windows and was read as a
    refusal (80% of a published mutation result, later retracted), and rc=0 with no
    emitted source is the silent-failure case -- so the header is checked, not assumed.
    --output-dir sidesteps the pipe; the header check stays.

    Retired with the bootstrap: docs/design/bootstrap_sunset.md Step 2.
    """
    zebra = wt / "zig-out" / "bin" / "zebra.exe"
    if not zebra.exists():
        zebra = wt / "zig-out" / "bin" / "zebra"
    outdir = wt / "_mut_emit"
    shutil.rmtree(outdir, ignore_errors=True)
    outdir.mkdir()
    try:
        pr = subprocess.run([str(zebra), "--emit-zig", "--output-dir", str(outdir), "selfhost/main.zbr"],
                            cwd=str(wt), stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, timeout=600)
    except subprocess.TimeoutExpired:
        return False, "regen TIMEOUT"
    if pr.returncode != 0:
        return False, pr.stderr.decode("utf-8", "replace")[-1500:]
    mod = pathlib.Path(rel).stem
    emitted = outdir / (mod + ".zig")
    if not emitted.exists() or emitted.stat().st_size == 0:
        return False, "the compiler emitted no source for %s (rc=0)" % mod
    out = emitted.read_text(encoding="utf-8", errors="replace")
    if "// Generated by" not in out:
        return False, "emitted file for %s carries no header (rc=0)" % mod
    (wt / rel.replace(".zbr", ".zig")).write_text(out, encoding="utf-8", newline="\n")
    return True, ""
...
def main():
    ...
    for n, (rel, ln, c0, c1, op, old, new) in enumerate(sites, 1):
        ...
        ok, msg = regen(wt, rel)
        if not ok:
            verdict, by, why = "DETECTED", "regen", msg[:300]      # the compiler refused its own mutated source
        else:
            rc, blog = build(wt)
            if rc != 0:
