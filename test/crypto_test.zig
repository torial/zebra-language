...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/crypto_test.zbr:2
    const key: []const u8 = "secret-key";
// zbr:test/crypto_test.zbr:3
    const plaintext: []const u8 = "Hello, Zebra!";
// zbr:test/crypto_test.zbr:4
    const ciphertext: []const u8 = _crypto_encrypt(key, plaintext);
// zbr:test/crypto_test.zbr:5
    const decrypted: ?[]const u8 = _crypto_decrypt(key, ciphertext);
// zbr:test/crypto_test.zbr:6
    if (decrypted) |d| {
        std.debug.print("{s}\n", .{d});
    } else {
        std.debug.print("{s}\n", .{"decrypt failed"});
    }
// zbr:test/crypto_test.zbr:10
    const bad: ?[]const u8 = _crypto_decrypt(key, "deadbeef00112233445566778899aabb");
// zbr:test/crypto_test.zbr:11
    if (bad) |_| {
        std.debug.print("{s}\n", .{"should not decrypt"});
    } else {
        std.debug.print("{s}\n", .{"bad ciphertext rejected"});
    }
    std.debug.print("{s}\n", .{"crypto_test OK"});
}
