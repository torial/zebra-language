...
// Source: test/bug118_struct_ctor_test.zbr
...
pub const RGB = struct {
    ...
    g: i64,
    b: i64,
    pub fn init(r: i64, g: i64, b: i64) RGB {
        return .{ .r = r, .g = g, .b = b };
    }

};

pub fn manhattan(p: Point) i64 {
// zbr:test/bug118_struct_ctor_test.zbr:15
    return (p.x + p.y);
}

pub fn main() void {
// zbr:test/bug118_struct_ctor_test.zbr:18
    const p = Point{ .x = 3, .y = 4 };
// zbr:test/bug118_struct_ctor_test.zbr:19
    if (!((p.x == 3))) @panic(_str_concat("expected x=3, got ", (std.fmt.allocPrint(_allocator, "{}", .{p.x}) catch unreachable), _allocator));
// zbr:test/bug118_struct_ctor_test.zbr:20
    if (!((p.y == 4))) @panic(_str_concat("expected y=4, got ", (std.fmt.allocPrint(_allocator, "{}", .{p.y}) catch unreachable), _allocator));
// zbr:test/bug118_struct_ctor_test.zbr:21
    if (!((manhattan(p) == 7))) @panic(_str_concat("expected manhattan=7, got ", (std.fmt.allocPrint(_allocator, "{}", .{manhattan(p)}) catch unreachable), _allocator));
// zbr:test/bug118_struct_ctor_test.zbr:23
    const color = RGB{ .r = 255, .g = 128, .b = 0 };
// zbr:test/bug118_struct_ctor_test.zbr:24
    if (!((color.r == 255))) @panic(_str_concat("expected r=255, got ", (std.fmt.allocPrint(_allocator, "{}", .{color.r}) catch unreachable), _allocator));
// zbr:test/bug118_struct_ctor_test.zbr:25
    if (!((color.g == 128))) @panic(_str_concat("expected g=128, got ", (std.fmt.allocPrint(_allocator, "{}", .{color.g}) catch unreachable), _allocator));
// zbr:test/bug118_struct_ctor_test.zbr:26
    if (!((color.b == 0))) @panic(_str_concat("expected b=0, got ", (std.fmt.allocPrint(_allocator, "{}", .{color.b}) catch unreachable), _allocator));
    std.debug.print("{s}\n", .{"bug118_struct_ctor_test: ok"});
}
