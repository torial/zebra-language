...
// Source: C:/Projects/zebra-language/test/generic_stack_test.zbr
...
pub const Stack = struct {
    ...
    count: i64,
    pub fn init() *Stack {
        const _self = _allocator.create(Stack) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Stack");
            _self.items = std.ArrayList(T){};
            _self.count = 0;
        return _self;
    }

    pub fn push(self: *Stack, val: T) void {
        self.items.append(_allocator, val) catch @panic("OOM");
    ...
    pub fn pop(self: *Stack) ?T {
        ...
        self.count = (self.count - 1);
        return self.items.items[@intCast(self.count)];
    }
    ...
    pub fn peek(self: *Stack) ?T {
        ...
        return self.items.items[@intCast((self.count - 1))];
    }
...
const _ttag_Stack: u64 = _zbr_hash("Stack");
const _reflect_Stack_name: []const u8 = "Stack";
const _reflect_Stack_fields: []const []const u8 = &.{"items", "count"};
const _reflect_Stack_field_types: []const []const u8 = &.{"List(T)", "int"};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
        var s = Stack.init(int)();
        s.push(10);
        s.push(20);
        s.push(30);
        std.debug.print("{any}\n", .{s.pop()});
