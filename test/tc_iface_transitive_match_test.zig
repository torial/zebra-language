...
// Source: test/tc_iface_transitive_match_test.zbr
...
pub const Dog = struct {
    ...
    pub fn id(self: *Dog) i64 {
        _ = self;
// zbr:test/tc_iface_transitive_match_test.zbr:9
        return 42;
    }

    pub fn extra(self: *Dog) []const u8 {
        _ = self;
// zbr:test/tc_iface_transitive_match_test.zbr:11
        return "woof";
    }

    pub fn init() *Dog {
        const self = _allocator.create(Dog) catch @panic("OOM");
        self.* = .{};
...
fn _shim_Dog_IFoo_extra(ptr: *anyopaque) []const u8 { return Dog.extra(@alignCast(@ptrCast(ptr))); }
const _vtable_Dog_IFoo = IFoo.VTable{ .extra = &_shim_Dog_IFoo_extra, .__as_IBase = &_vtable_Dog_IBase, };
fn _shim_Dog_IBase_id(ptr: *anyopaque) i64 { return Dog.id(@alignCast(@ptrCast(ptr))); }
const _vtable_Dog_IBase = IBase.VTable{ .id = &_shim_Dog_IBase_id, };
const _ttag_Dog: u64 = _zbr_hash("Dog");
const _reflect_Dog_name: []const u8 = "Dog";
const _reflect_Dog_fields: []const []const u8 = &.{};
const _reflect_Dog_field_types: []const []const u8 = &.{};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/tc_iface_transitive_match_test.zbr:14
    const d: *Dog = Dog.init();
// zbr:test/tc_iface_transitive_match_test.zbr:15
    var b: IBase = .{ .ptr = d, .vtable = &_vtable_Dog_IBase };
    std.debug.print("{}\n", .{b.id()});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Dog) return _reflect_Dog_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Dog) return .{ .items = _reflect_Dog_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Dog) return .{ .items = _reflect_Dog_field_types };
    return .{ .items = &.{} };
}
