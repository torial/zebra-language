...
// Source: C:/Projects/zebra-language/test/recursive_type_test.zbr
...
pub const Pair = struct {
    ...
    pub fn init(pa: i64, pb: i64) *Pair {
        const _self = _allocator.create(Pair) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Pair");
            _self.a = pa;
            _self.b = pb;
        return _self;
    }
...
const _ttag_Pair: u64 = _zbr_hash("Pair");
...
const _ttag_PairHolder: u64 = _zbr_hash("PairHolder");
...
const _ttag_Tree: u64 = _zbr_hash("Tree");
...
pub const Main = struct {
    ...
    pub fn main() anyerror!void {
        ...
        std.debug.assert((try n.value == 2));
        const n2 = try n.next;
        std.debug.assert((n2 != null));
        std.debug.assert((try n2.value == 3));
        ...
        std.debug.assert((try rl.value == 10));
        const rr = root.right;
        std.debug.assert((rr != null));
        std.debug.assert((try rr.value == 20));
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    ...
    if (tag == _ttag_Pair) return _reflect_Pair_name;
    if (tag == _ttag_PairHolder) return _reflect_PairHolder_name;
    if (tag == _ttag_Tree) return _reflect_Tree_name;
...
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_Pair) return .{ .items = _reflect_Pair_fields };
    if (tag == _ttag_PairHolder) return .{ .items = _reflect_PairHolder_fields };
    if (tag == _ttag_Tree) return .{ .items = _reflect_Tree_fields };
...
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_Pair) return .{ .items = _reflect_Pair_field_types };
    if (tag == _ttag_PairHolder) return .{ .items = _reflect_PairHolder_field_types };
    if (tag == _ttag_Tree) return .{ .items = _reflect_Tree_field_types };
