...
// zbr:test/char_tostring_test.zbr:7
        var _cp_it_1 = std.unicode.Utf8View.initUnchecked(word).iterator();
        while (_cp_it_1.nextCodepoint()) |c| {
// zbr:test/char_tostring_test.zbr:8
            chars_list.append(_allocator, _intern((blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(c, &_cpbuf) catch 1; const _cpout = _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); break :blk @as([]const u8, _cpout); }))) catch unreachable;
        }
...
// zbr:test/char_tostring_test.zbr:15
        var _cp_it_2 = std.unicode.Utf8View.initUnchecked(greek).iterator();
        while (_cp_it_2.nextCodepoint()) |c| {
// zbr:test/char_tostring_test.zbr:16
            gchars.append(_allocator, _intern((blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(c, &_cpbuf) catch 1; const _cpout = _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); break :blk @as([]const u8, _cpout); }))) catch unreachable;
        }
