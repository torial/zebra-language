...
// Source: C:/Projects/zebra-language/test/char_tostring_test.zbr
    ...
    pub fn main() void {
        ...
        var chars_list = std.ArrayList([]const u8){};
        {
            ...
            while (_cp_it_c.nextCodepoint()) |c| {
                chars_list.append(_allocator, (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(c), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); })) catch @panic("OOM");
            }
        }
        ...
        {
            var _cp_it_c = std.unicode.Utf8View.initUnchecked(greek).iterator();
            while (_cp_it_c.nextCodepoint()) |c| {
                gchars.append(_allocator, (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(c), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); })) catch @panic("OOM");
            }
        }
        std.debug.print("{any}\n", .{@as(i64, @intCast(gchars.items.len))});
        var i: i64 = 0;
        while (_zebra_lt(i, @as(i64, @intCast(chars_list.items.len)))) {
            std.debug.print("{any}\n", .{chars_list.items[@intCast(i)]});
            i = (i + 1);
        }
