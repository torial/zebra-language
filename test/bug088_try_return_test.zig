...
// Source: test/bug088_try_return_test.zbr
...
pub const Outer = struct {
    _type_tag: u64 = _ttag_Outer,
    pub fn getLabel(self: *Outer) []const u8 {
// zbr:test/bug088_try_return_test.zbr:2
        {
            var _try_err: ?anyerror = null;
            _ = &_try_err;
            _try_blk: {
// zbr:test/bug088_try_return_test.zbr:3
                const v: []const u8 = self.compute() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
// zbr:test/bug088_try_return_test.zbr:4
                return _str_concat("ok:", v, _allocator);
            }
            if (_try_err != null) {
// zbr:test/bug088_try_return_test.zbr:6
                return "err";
            }
            unreachable;
        }
    ...
    pub fn compute(self: *Outer) anyerror![]const u8 {
        _ = self;
// zbr:test/bug088_try_return_test.zbr:9
        return "hello";
    }

    pub fn init() *Outer {
        const self = _allocator.create(Outer) catch @panic("OOM");
        self._type_tag = _ttag_Outer;
        return self;
    }

};
const _ttag_Outer: u64 = _zbr_hash("Outer");
const _reflect_Outer_name: []const u8 = "Outer";
const _reflect_Outer_fields: []const []const u8 = &.{};
const _reflect_Outer_field_types: []const []const u8 = &.{};

pub fn main() void {
// zbr:test/bug088_try_return_test.zbr:12
    var o = Outer.init();
// zbr:test/bug088_try_return_test.zbr:13
    const got = o.getLabel();
// zbr:test/bug088_try_return_test.zbr:14
    if (!(std.mem.eql(u8, got, "ok:hello"))) @panic(_str_concat("expected ok:hello, got ", got, _allocator));
    std.debug.print("{s}\n", .{"bug088_try_return_test: ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Outer) return _reflect_Outer_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Outer) return .{ .items = _reflect_Outer_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Outer) return .{ .items = _reflect_Outer_field_types };
    return .{ .items = &.{} };
}
