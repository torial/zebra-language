...
// Source: C:/Projects/zebra-language/test/arena_scope_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        var x: i64 = 0;
        {
            var _scope_arena_0 = std.heap.ArenaAllocator.init(_allocator);
            defer _scope_arena_0.deinit();
            const _saved_alloc_0 = _allocator;
            _allocator = _scope_arena_0.allocator();
            var c = Counter.init();
            c.inc();
            c.inc();
            c.inc();
            x = c.value();
            _allocator = _saved_alloc_0;
        }
        std.debug.assert((x == 3));
        var y: i64 = 0;
        {
            var _scope_arena_1 = std.heap.ArenaAllocator.init(_allocator);
            defer _scope_arena_1.deinit();
            const _saved_alloc_1 = _allocator;
            _allocator = _scope_arena_1.allocator();
            var a = Counter.init();
            a.inc();
            {
                var _scope_arena_2 = std.heap.ArenaAllocator.init(_allocator);
                defer _scope_arena_2.deinit();
                const _saved_alloc_2 = _allocator;
                _allocator = _scope_arena_2.allocator();
                var b = Counter.init();
                b.inc();
                b.inc();
                y = (a.value() + b.value());
                _allocator = _saved_alloc_2;
            }
            _allocator = _saved_alloc_1;
        }
