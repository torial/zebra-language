...
// Source: test/bug094_hashmap_kv_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/bug094_hashmap_kv_test.zbr:8
        var m = std.StringHashMap(i64).init(_allocator);
        m.put(_intern("a"), 1) catch @panic("OOM");
        m.put(_intern("b"), 2) catch @panic("OOM");
// zbr:test/bug094_hashmap_kv_test.zbr:13
        var sum1: i64 = 0;
// zbr:test/bug094_hashmap_kv_test.zbr:14
        {
            var _it_k = m.iterator();
            while (_it_k.next()) |_ent_k| {
                const k = _ent_k.key_ptr.*;
                const v = _ent_k.value_ptr.*;
// zbr:test/bug094_hashmap_kv_test.zbr:15
                if ((std.mem.eql(u8, k, "a") or std.mem.eql(u8, k, "b"))) {
// zbr:test/bug094_hashmap_kv_test.zbr:16
                    sum1 = (sum1 + v);
                }
            }
        }
// zbr:test/bug094_hashmap_kv_test.zbr:17
        if (!((sum1 == 3))) @panic("both used");
// zbr:test/bug094_hashmap_kv_test.zbr:20
        var keys = std.ArrayList([]const u8){};
// zbr:test/bug094_hashmap_kv_test.zbr:21
        {
            var _it_k = m.iterator();
            while (_it_k.next()) |_ent_k| {
                const k = _ent_k.key_ptr.*;
                const v = _ent_k.value_ptr.*;
                _ = v;
                keys.append(_allocator, _intern(k)) catch @panic("OOM");
            }
        }
// zbr:test/bug094_hashmap_kv_test.zbr:23
        if (!((@as(i64, @intCast(keys.items.len)) == 2))) @panic("k used v unused");
// zbr:test/bug094_hashmap_kv_test.zbr:26
        var sum3: i64 = 0;
// zbr:test/bug094_hashmap_kv_test.zbr:27
        {
            var _it_k = m.iterator();
            while (_it_k.next()) |_ent_k| {
                const k = _ent_k.key_ptr.*;
                _ = k;
                const v = _ent_k.value_ptr.*;
// zbr:test/bug094_hashmap_kv_test.zbr:28
                sum3 = (sum3 + v);
            }
        }
// zbr:test/bug094_hashmap_kv_test.zbr:29
        if (!((sum3 == 3))) @panic("k unused v used");
// zbr:test/bug094_hashmap_kv_test.zbr:32
        var count: i64 = 0;
// zbr:test/bug094_hashmap_kv_test.zbr:33
        {
            var _it_k = m.iterator();
            while (_it_k.next()) |_ent_k| {
                const k = _ent_k.key_ptr.*;
                ...
                const v = _ent_k.value_ptr.*;
                _ = v;
// zbr:test/bug094_hashmap_kv_test.zbr:34
                count = (count + 1);
            }
        }
// zbr:test/bug094_hashmap_kv_test.zbr:35
        if (!((count == 2))) @panic("both unused");
        std.debug.print("{s}\n", .{"ok"});
    }
