...
pub var counter: i64 = 0;
pub const MAX: i64 = 100;
pub const LABEL: []const u8 = "module_var";
pub fn bump() void {
...
pub fn main(_zinit: std.process.Init) void {
    ...
    bump();
    bump();
    std.debug.print("{s}\n", .{_str_concat((std.fmt.allocPrint(_allocator, "{s}: counter={} max=", .{LABEL, counter}) catch @panic("OOM")), (std.fmt.allocPrint(_allocator, "{}", .{MAX}) catch unreachable), _allocator)});
// zbr:test/module_var_test.zbr:26
    if (_zebra_lt(counter, MAX)) {
        std.debug.print("{s}\n", .{"module_var_test: OK"});
    }
}
