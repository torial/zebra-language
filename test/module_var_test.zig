...
// Source: test/module_var_test.zbr
...
pub var _zbr_mv_counter: i64 = 0;
pub const _zbr_mv_MAX: i64 = 100;
pub const _zbr_mv_LABEL: []const u8 = "module_var";
pub fn bump() void {
// zbr:test/module_var_test.zbr:17
    _zbr_mv_counter += 1;
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    bump();
    bump();
    std.debug.print("{s}\n", .{_str_concat((std.fmt.allocPrint(_allocator, "{s}: counter={} max=", .{_zbr_mv_LABEL, _zbr_mv_counter}) catch @panic("OOM")), (std.fmt.allocPrint(_allocator, "{}", .{_zbr_mv_MAX}) catch unreachable), _allocator)});
// zbr:test/module_var_test.zbr:26
    if (_zebra_lt(_zbr_mv_counter, _zbr_mv_MAX)) {
        std.debug.print("{s}\n", .{"module_var_test: OK"});
    }
}
