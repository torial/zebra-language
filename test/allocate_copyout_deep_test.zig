...
// Source: test/allocate_copyout_deep_test.zbr
...
pub const Node = struct {
    ...
    name: []const u8,
    next: ?*Node,
    pub fn init(name: []const u8, next: ?*Node) *Node {
        const _self = _allocator.create(Node) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Node");
// zbr:test/allocate_copyout_deep_test.zbr:6
            _self.name = _intern(name);
// zbr:test/allocate_copyout_deep_test.zbr:7
            _self.next = next;
        return _self;
    }

};
const _ttag_Node: u64 = _zbr_hash("Node");
const _reflect_Node_name: []const u8 = "Node";
const _reflect_Node_fields: []const []const u8 = &.{"name", "next"};
const _reflect_Node_field_types: []const []const u8 = &.{"str", "^?Node"};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/allocate_copyout_deep_test.zbr:11
        var ok = true;
// zbr:test/allocate_copyout_deep_test.zbr:14
        var words = std.ArrayList([]const u8).empty;
// zbr:test/allocate_copyout_deep_test.zbr:15
        { // allocate
            var _alloc_src_0 = std.heap.ArenaAllocator.init(_allocator);
            const _saved_alloc_0 = _allocator;
            _allocator = _alloc_src_0.allocator();
            defer { _allocator = _saved_alloc_0; _alloc_src_0.deinit(); }
// zbr:test/allocate_copyout_deep_test.zbr:16
            var inner = std.ArrayList([]const u8).empty;
            inner.append(_allocator, _intern("hello")) catch @panic("OOM");
            inner.append(_allocator, _intern("world")) catch @panic("OOM");
// zbr:test/allocate_copyout_deep_test.zbr:19
            const _co_1 = inner;
            words = _zbr_deep_copy(@TypeOf(_co_1), _saved_alloc_0, _co_1, 0) catch @panic("OOM copy-out");
        }
// zbr:test/allocate_copyout_deep_test.zbr:20
        if ((@as(i64, @intCast(words.items.len)) != 2)) {
// zbr:test/allocate_copyout_deep_test.zbr:21
            ok = false;
            std.debug.print("{s}\n", .{"Test 1 FAIL: count"});
        } else {
// zbr:test/allocate_copyout_deep_test.zbr:23
            if ((!std.mem.eql(u8, words.items[@intCast(0)], "hello") or !std.mem.eql(u8, words.items[@intCast(1)], "world"))) {
// zbr:test/allocate_copyout_deep_test.zbr:24
                ok = false;
                std.debug.print("{s}\n", .{"Test 1 FAIL: values"});
            }
        }
// zbr:test/allocate_copyout_deep_test.zbr:28
        var nums = std.ArrayList(i64).empty;
// zbr:test/allocate_copyout_deep_test.zbr:29
        { // allocate
            var _alloc_src_1 = std.heap.ArenaAllocator.init(_allocator);
            const _saved_alloc_1 = _allocator;
            _allocator = _alloc_src_1.allocator();
            defer { _allocator = _saved_alloc_1; _alloc_src_1.deinit(); }
// zbr:test/allocate_copyout_deep_test.zbr:30
            var inner2 = std.ArrayList(i64).empty;
            inner2.append(_allocator, 10) catch @panic("OOM");
            inner2.append(_allocator, 20) catch @panic("OOM");
            inner2.append(_allocator, 30) catch @panic("OOM");
// zbr:test/allocate_copyout_deep_test.zbr:34
            const _co_2 = inner2;
            nums = _zbr_deep_copy(@TypeOf(_co_2), _saved_alloc_1, _co_2, 0) catch @panic("OOM copy-out");
        }
// zbr:test/allocate_copyout_deep_test.zbr:35
        if ((@as(i64, @intCast(nums.items.len)) != 3)) {
// zbr:test/allocate_copyout_deep_test.zbr:36
            ok = false;
            std.debug.print("{s}\n", .{"Test 2 FAIL: count"});
        } else {
// zbr:test/allocate_copyout_deep_test.zbr:38
            if ((((nums.items[@intCast(0)] != 10) or (nums.items[@intCast(1)] != 20)) or (nums.items[@intCast(2)] != 30))) {
// zbr:test/allocate_copyout_deep_test.zbr:39
                ok = false;
                std.debug.print("{s}\n", .{"Test 2 FAIL: values"});
            }
        }
// zbr:test/allocate_copyout_deep_test.zbr:43
        var head: ?*Node = null;
// zbr:test/allocate_copyout_deep_test.zbr:44
        { // allocate
            var _alloc_src_2 = std.heap.ArenaAllocator.init(_allocator);
            const _saved_alloc_2 = _allocator;
            _allocator = _alloc_src_2.allocator();
            defer { _allocator = _saved_alloc_2; _alloc_src_2.deinit(); }
// zbr:test/allocate_copyout_deep_test.zbr:45
            const n3 = Node.init("c", null);
// zbr:test/allocate_copyout_deep_test.zbr:46
            const n2 = Node.init("b", n3);
// zbr:test/allocate_copyout_deep_test.zbr:47
            const n1 = Node.init("a", n2);
// zbr:test/allocate_copyout_deep_test.zbr:48
            const _co_3 = n1;
            head = _zbr_deep_copy(@TypeOf(_co_3), _saved_alloc_2, _co_3, 0) catch @panic("OOM copy-out");
        }
// zbr:test/allocate_copyout_deep_test.zbr:49
        if ((head == null)) {
// zbr:test/allocate_copyout_deep_test.zbr:50
            ok = false;
            std.debug.print("{s}\n", .{"Test 3 FAIL: head nil"});
        } else {
// zbr:test/allocate_copyout_deep_test.zbr:53
            const h = head.?;
// zbr:test/allocate_copyout_deep_test.zbr:54
            if (!std.mem.eql(u8, h.name, "a")) {
// zbr:test/allocate_copyout_deep_test.zbr:55
                ok = false;
                std.debug.print("{s}\n", .{"Test 3 FAIL: head name"});
            } else {
// zbr:test/allocate_copyout_deep_test.zbr:58
                const mid = h.next;
// zbr:test/allocate_copyout_deep_test.zbr:59
                if ((mid == null)) {
// zbr:test/allocate_copyout_deep_test.zbr:60
                    ok = false;
                    std.debug.print("{s}\n", .{"Test 3 FAIL: mid nil"});
                } else {
// zbr:test/allocate_copyout_deep_test.zbr:63
                    const m = mid.?;
// zbr:test/allocate_copyout_deep_test.zbr:64
                    if (!std.mem.eql(u8, m.name, "b")) {
// zbr:test/allocate_copyout_deep_test.zbr:65
                        ok = false;
                        std.debug.print("{s}\n", .{"Test 3 FAIL: mid name"});
                    } else {
// zbr:test/allocate_copyout_deep_test.zbr:68
                        const tail = m.next;
// zbr:test/allocate_copyout_deep_test.zbr:69
                        if ((tail == null)) {
// zbr:test/allocate_copyout_deep_test.zbr:70
                            ok = false;
                            std.debug.print("{s}\n", .{"Test 3 FAIL: tail nil"});
                        } else {
// zbr:test/allocate_copyout_deep_test.zbr:73
                            const t = tail.?;
// zbr:test/allocate_copyout_deep_test.zbr:74
                            if (!std.mem.eql(u8, t.name, "c")) {
// zbr:test/allocate_copyout_deep_test.zbr:75
                                ok = false;
                                std.debug.print("{s}\n", .{"Test 3 FAIL: tail name"});
                            }
                        }
                    }
                }
...
// zbr:test/allocate_copyout_deep_test.zbr:78
        if (ok) {
            std.debug.print("{s}\n", .{"allocate_copyout_deep: OK"});
        } else {
            std.debug.print("{s}\n", .{"allocate_copyout_deep: FAIL"});
        }
    }
