...
// zbr:test/try_postfix_test.zbr:5
            { _error_ctx = .{ .message = "division by zero" }; return error.ZebraError; }
        }
// zbr:test/try_postfix_test.zbr:6
    ...
    pub fn init() *Math {
        const self = _allocator.create(Math) catch @panic("OOM");
        self._type_tag = _ttag_Math;
        return self;
    }

};
const _ttag_Math: u64 = _zbr_hash("Math");
...
pub fn testSafeDiv() void {
// zbr:test/try_postfix_test.zbr:8
    {
        var _try_err: ?anyerror = null;
        _ = &_try_err;
        _try_blk: {
// zbr:test/try_postfix_test.zbr:9
            const quotient = Math.safeDiv(10, 2) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
            std.debug.print("{}\n", .{quotient});
            break :_try_blk;
        }
        if (_try_err != null) {
...
pub fn testSafeDivFail() void {
// zbr:test/try_postfix_test.zbr:14
    {
        var _try_err: ?anyerror = null;
        _ = &_try_err;
        _try_blk: {
// zbr:test/try_postfix_test.zbr:15
            const result2 = Math.safeDiv(10, 0) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
            std.debug.print("{}\n", .{result2});
            break :_try_blk;
        }
        if (_try_err != null) {
            std.debug.print("{s}\n", .{(std.mem.concat(_allocator, u8, &.{ "caught: ", _zbr_error_msg() }) catch unreachable)});
        }
    }
}
