...
// Source: test/tc_iface_i2i_match_test.zbr
...
pub const IBase = struct {
    ...
    pub const VTable = struct {
        id: *const fn (ptr: *anyopaque) i64,
    };

    pub fn id(self: @This()) i64 {
        return self.vtable.id(self.ptr);
    }

    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "id")) @compileError("type " ++ @typeName(T) ++ " does not implement IBase.id");
        }
    }
};
...
pub const IFoo = struct {
    ...
    pub const VTable = struct {
        extra: *const fn (ptr: *anyopaque) []const u8,
        __as_IBase: *const IBase.VTable,
    };

    pub fn extra(self: @This()) []const u8 {
        return self.vtable.extra(self.ptr);
    }

    pub fn id(self: @This()) i64 {
        return self.vtable.__as_IBase.id(self.ptr);
    }

    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "extra")) @compileError("type " ++ @typeName(T) ++ " does not implement IFoo.extra");
        }
    }
};

pub const FooImpl = struct {
    _type_tag: u64 = _ttag_FooImpl,
    pub fn id(self: *FooImpl) i64 {
        _ = self;
// zbr:test/tc_iface_i2i_match_test.zbr:9
        return 1;
    }

    pub fn extra(self: *FooImpl) []const u8 {
        _ = self;
// zbr:test/tc_iface_i2i_match_test.zbr:11
        return "hi";
    }

    pub fn init() *FooImpl {
        const self = _allocator.create(FooImpl) catch @panic("OOM");
        self.* = .{};
        self._type_tag = _ttag_FooImpl;
        return self;
    }
...
fn _shim_FooImpl_IFoo_extra(ptr: *anyopaque) []const u8 { return FooImpl.extra(@alignCast(@ptrCast(ptr))); }
const _vtable_FooImpl_IFoo = IFoo.VTable{ .extra = &_shim_FooImpl_IFoo_extra, .__as_IBase = &_vtable_FooImpl_IBase, };
fn _shim_FooImpl_IBase_id(ptr: *anyopaque) i64 { return FooImpl.id(@alignCast(@ptrCast(ptr))); }
const _vtable_FooImpl_IBase = IBase.VTable{ .id = &_shim_FooImpl_IBase_id, };
const _ttag_FooImpl: u64 = _zbr_hash("FooImpl");
const _reflect_FooImpl_name: []const u8 = "FooImpl";
const _reflect_FooImpl_fields: []const []const u8 = &.{};
const _reflect_FooImpl_field_types: []const []const u8 = &.{};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/tc_iface_i2i_match_test.zbr:14
    const f: IFoo = .{ .ptr = FooImpl.init(), .vtable = &_vtable_FooImpl_IFoo };
// zbr:test/tc_iface_i2i_match_test.zbr:15
    const _ifsrc_1 = f;
    var b: IBase = .{ .ptr = _ifsrc_1.ptr, .vtable = _ifsrc_1.vtable.__as_IBase };
    std.debug.print("{}\n", .{b.id()});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_FooImpl) return _reflect_FooImpl_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_FooImpl) return .{ .items = _reflect_FooImpl_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_FooImpl) return .{ .items = _reflect_FooImpl_field_types };
    return .{ .items = &.{} };
}
