...
// Source: test/stop_method_test.zbr
...
pub const Anim = struct {
    _type_tag: u64 = _ttag_Anim,
    playing: bool,
    pub fn init() *Anim {
        const _self = _allocator.create(Anim) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Anim");
// zbr:test/stop_method_test.zbr:6
            _self.playing = true;
        return _self;
    }

    pub fn stop(self: *Anim) void {
// zbr:test/stop_method_test.zbr:8
        self.playing = false;
    }

};
const _ttag_Anim: u64 = _zbr_hash("Anim");
const _reflect_Anim_name: []const u8 = "Anim";
const _reflect_Anim_fields: []const []const u8 = &.{"playing"};
const _reflect_Anim_field_types: []const []const u8 = &.{"bool"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/stop_method_test.zbr:11
    var a = Anim.init();
    a.stop();
    std.debug.print("{s}\n", .{"stop-method: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Anim) return _reflect_Anim_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Anim) return .{ .items = _reflect_Anim_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Anim) return .{ .items = _reflect_Anim_field_types };
    return .{ .items = &.{} };
}
