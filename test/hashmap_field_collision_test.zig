...
// Source: test/hashmap_field_collision_test.zbr
...
pub const MapHolder = struct {
    _type_tag: u64 = _ttag_MapHolder,
    bag: std.StringHashMap(i64),
    pub fn init() *MapHolder {
        const _self = _allocator.create(MapHolder) catch @panic("OOM");
        _self._type_tag = _zbr_hash("MapHolder");
// zbr:test/hashmap_field_collision_test.zbr:10
            _self.bag = std.StringHashMap(i64).init(_allocator);
        return _self;
    }

    pub fn set_(self: *MapHolder, k: []const u8, v: i64) void {
        self.bag.put(_intern(k), v) catch @panic("OOM");
    }

    pub fn get_(self: *MapHolder, k: []const u8) i64 {
// zbr:test/hashmap_field_collision_test.zbr:14
        return self.bag.get(k).?;
    }

    pub fn size(self: *MapHolder) i64 {
// zbr:test/hashmap_field_collision_test.zbr:16
        return @as(i64, @intCast(self.bag.count()));
    }

};
const _ttag_MapHolder: u64 = _zbr_hash("MapHolder");
const _reflect_MapHolder_name: []const u8 = "MapHolder";
const _reflect_MapHolder_fields: []const []const u8 = &.{"bag"};
const _reflect_MapHolder_field_types: []const []const u8 = &.{"HashMap(str, int)"};

pub const ListHolder = struct {
    _type_tag: u64 = _ttag_ListHolder,
    bag: std.ArrayList(i64),
    pub fn init() *ListHolder {
        const _self = _allocator.create(ListHolder) catch @panic("OOM");
        _self._type_tag = _zbr_hash("ListHolder");
// zbr:test/hashmap_field_collision_test.zbr:21
            _self.bag = std.ArrayList(i64).empty;
        return _self;
    }

    pub fn add_(self: *ListHolder, v: i64) void {
        self.bag.append(_allocator, v) catch @panic("OOM");
    }

    pub fn first(self: *ListHolder) i64 {
// zbr:test/hashmap_field_collision_test.zbr:25
        return self.bag.items[@intCast(0)];
    }

    pub fn size(self: *ListHolder) i64 {
// zbr:test/hashmap_field_collision_test.zbr:27
        return @as(i64, @intCast(self.bag.items.len));
    }

};
const _ttag_ListHolder: u64 = _zbr_hash("ListHolder");
const _reflect_ListHolder_name: []const u8 = "ListHolder";
const _reflect_ListHolder_fields: []const []const u8 = &.{"bag"};
const _reflect_ListHolder_field_types: []const []const u8 = &.{"List(int)"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/hashmap_field_collision_test.zbr:30
    var lh = ListHolder.init();
    lh.add_(42);
// zbr:test/hashmap_field_collision_test.zbr:32
    var mh = MapHolder.init();
    mh.set_("a", 7);
    std.debug.print("{s}\n", .{_str_concat("list_first=", (std.fmt.allocPrint(_allocator, "{}", .{lh.first()}) catch unreachable), _allocator)});
    std.debug.print("{s}\n", .{_str_concat("list_size=", (std.fmt.allocPrint(_allocator, "{}", .{lh.size()}) catch unreachable), _allocator)});
    std.debug.print("{s}\n", .{_str_concat("map_get=", (std.fmt.allocPrint(_allocator, "{}", .{mh.get_("a")}) catch unreachable), _allocator)});
    std.debug.print("{s}\n", .{_str_concat("map_size=", (std.fmt.allocPrint(_allocator, "{}", .{mh.size()}) catch unreachable), _allocator)});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_MapHolder) return _reflect_MapHolder_name;
    if (tag == _ttag_ListHolder) return _reflect_ListHolder_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_MapHolder) return .{ .items = _reflect_MapHolder_fields };
    if (tag == _ttag_ListHolder) return .{ .items = _reflect_ListHolder_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_MapHolder) return .{ .items = _reflect_MapHolder_field_types };
    if (tag == _ttag_ListHolder) return .{ .items = _reflect_ListHolder_field_types };
    return .{ .items = &.{} };
}
