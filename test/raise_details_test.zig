...
// Source: C:/Projects/zebra-language/test/raise_details_test.zbr
...
pub const FileInfo = struct {
    ...
    pub fn init(p: []const u8, c: i64) *FileInfo {
        const _self = _allocator.create(FileInfo) catch @panic("OOM");
        _self._type_tag = _zbr_hash("FileInfo");
            _self.path = p;
            _self.code = c;
        return _self;
    }
...
const _ttag_FileInfo: u64 = _zbr_hash("FileInfo");
...
pub const Main = struct {
    ...
    pub fn tryRead(path: []const u8) anyerror!void {
        { const _rdet_1 = FileInfo.init(path, 404);
          const _rshim_1 = struct { fn call(p: *anyopaque) []const u8 {
              return @as(*FileInfo, @alignCast(@ptrCast(p))).toString();
          } };
          _error_ctx = .{ .message = "file not found", .details = .{ .ptr = @ptrCast(_rdet_1), .toString_fn = _rshim_1.call } };
    ...
    pub fn main() void {
        {
            ...
            _ = &_try_err;
            _try_blk: {
                Main.tryRead("data.txt") catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_zbr_error_msg()});
                std.debug.print("{s}\n", .{(if (_error_ctx.details) |_det| _det.toString() else "")});
            }
        }
        {
            var _try_err: ?anyerror = null;
            _ = &_try_err;
            _try_blk: {
                { _error_ctx = .{ .message = "simple error" }; _try_err = error.ZebraError; break :_try_blk; }
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_zbr_error_msg()});
