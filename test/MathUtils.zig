...
const _ttag_MathUtils: u64 = _zbr_hash("MathUtils");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_MathUtils) return _reflect_MathUtils_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_MathUtils) return .{ .items = _reflect_MathUtils_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_MathUtils) return .{ .items = _reflect_MathUtils_field_types };
    return .{ .items = &.{} };
}
