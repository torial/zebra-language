...
// Source: test/type_alias_test.zbr
...
pub fn main(init: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/type_alias_test.zbr:7
    const name: []const u8 = "hello";
    {
        const value = name;
        if (!(_zebra_gt(@as(i64, @intCast(value.len)), 0))) std.debug.panic("type constraint 'NonEmptyStr' failed\n", .{});
    }
// zbr:test/type_alias_test.zbr:8
    const count: i64 = 42;
    {
        const value = count;
        if (!(_zebra_gt(value, 0))) std.debug.panic("type constraint 'PositiveInt' failed\n", .{});
    }
// zbr:test/type_alias_test.zbr:9
    const ratio: f64 = 0.5;
    {
        const value = ratio;
        if (!((_zebra_ge(value, 0.0) and _zebra_le(value, 1.0)))) std.debug.panic("type constraint 'Ratio' failed\n", .{});
    }
// zbr:test/type_alias_test.zbr:10
    const raw: i64 = (-7);
// zbr:test/type_alias_test.zbr:13
    const doubled: i64 = (count * 2);
// zbr:test/type_alias_test.zbr:14
    const combined: []const u8 = _str_concat(name, " world", _allocator);
    std.debug.print("{any}\n", .{name});
    std.debug.print("{s}\n", .{combined});
    std.debug.print("{}\n", .{doubled});
    std.debug.print("{any}\n", .{raw});
