...
// Source: test/bug089_mixin_method_test.zbr
...
pub const Foo = struct {
    _type_tag: u64 = _ttag_Foo,
    pub fn init() *Foo {
        const _self = _allocator.create(Foo) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Foo");
            // pass
        return _self;
    }

    pub fn hi(self: *Foo) []const u8 {
        _ = self;
// zbr:test/bug089_mixin_method_test.zbr:6
        return "hi";
    }

    pub fn count(self: *Foo) i64 {
        _ = self;
// zbr:test/bug089_mixin_method_test.zbr:8
        return 42;
    }

};
const _ttag_Foo: u64 = _zbr_hash("Foo");
const _reflect_Foo_name: []const u8 = "Foo";
const _reflect_Foo_fields: []const []const u8 = &.{};
const _reflect_Foo_field_types: []const []const u8 = &.{};

pub fn main() void {
// zbr:test/bug089_mixin_method_test.zbr:15
    var f = Foo.init();
// zbr:test/bug089_mixin_method_test.zbr:16
    if (!(std.mem.eql(u8, f.hi(), "hi"))) @panic("mixin str method");
// zbr:test/bug089_mixin_method_test.zbr:17
    if (!((f.count() == 42))) @panic("mixin int method");
    std.debug.print("{s}\n", .{"ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Foo) return _reflect_Foo_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Foo) return .{ .items = _reflect_Foo_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Foo) return .{ .items = _reflect_Foo_field_types };
    return .{ .items = &.{} };
}
