...
// Source: test/stdlib_file_test.zbr
...
pub fn main() void {
// zbr:test/stdlib_file_test.zbr:2
    const path = "test_stdlib_file_tmp.txt";
// zbr:test/stdlib_file_test.zbr:5
    var lines = std.ArrayList([]const u8){};
// zbr:test/stdlib_file_test.zbr:6
    lines.append(_allocator, _intern("line one")) catch unreachable;
// zbr:test/stdlib_file_test.zbr:7
    lines.append(_allocator, _intern("line two")) catch unreachable;
// zbr:test/stdlib_file_test.zbr:8
    lines.append(_allocator, _intern("line three")) catch unreachable;
// zbr:test/stdlib_file_test.zbr:9
    _file_write_lines(path, lines);
// zbr:test/stdlib_file_test.zbr:12
    std.debug.assert((blk: { std.fs.cwd().access(path, .{}) catch break :blk false; break :blk true; }));
// zbr:test/stdlib_file_test.zbr:13
    std.debug.assert((blk: { const _fi_stat = std.fs.cwd().statFile(path) catch break :blk false; break :blk _fi_stat.kind == .file; }));
// zbr:test/stdlib_file_test.zbr:14
    std.debug.assert((!(blk: { var _fd_dir = std.fs.cwd().openDir(path, .{}) catch break :blk false; _fd_dir.close(); break :blk true; })));
// zbr:test/stdlib_file_test.zbr:17
    const sz = (blk: { const _fs_stat = std.fs.cwd().statFile(path) catch break :blk @as(i64, -1); break :blk @as(i64, @intCast(_fs_stat.size)); });
// zbr:test/stdlib_file_test.zbr:18
    std.debug.assert(_zebra_gt(sz, 0));
// zbr:test/stdlib_file_test.zbr:21
    std.debug.assert((blk: { var _fd_dir = std.fs.cwd().openDir("test", .{}) catch break :blk false; _fd_dir.close(); break :blk true; }));
// zbr:test/stdlib_file_test.zbr:22
    std.debug.assert((!(blk: { const _fi_stat = std.fs.cwd().statFile("test") catch break :blk false; break :blk _fi_stat.kind == .file; })));
// zbr:test/stdlib_file_test.zbr:25
    (std.fs.cwd().deleteFile(path) catch |_fd_err| { if (_fd_err != error.FileNotFound) @panic("File.delete error"); });
// zbr:test/stdlib_file_test.zbr:26
    std.debug.assert((!(blk: { std.fs.cwd().access(path, .{}) catch break :blk false; break :blk true; })));
// zbr:test/stdlib_file_test.zbr:28
    std.debug.print("{s}\n", .{"stdlib_file OK"});
}
