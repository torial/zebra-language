...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/json_test.zbr:3
    const json: []const u8 = "{"name": "Alice", "age": 30, "score": 9.5, "active": true}";
// zbr:test/json_test.zbr:4
    const data = _json_parse(json);
// zbr:test/json_test.zbr:5
    if (!((data != null))) @panic("parse returned nil");
// zbr:test/json_test.zbr:7
    var d = data.?;
// zbr:test/json_test.zbr:8
    if (!(std.mem.eql(u8, _json_get_str(d, "name"), "Alice"))) @panic("getStr name");
// zbr:test/json_test.zbr:9
    if (!((_json_get_int(d, "age") == 30))) @panic("getInt age");
// zbr:test/json_test.zbr:10
    if (!(_zebra_gt(_json_get_float(d, "score"), 9.4))) @panic("getFloat score");
// zbr:test/json_test.zbr:11
    if (!((_json_get_bool(d, "active") == true))) @panic("getBool active");
// zbr:test/json_test.zbr:14
    if (!(std.mem.eql(u8, _json_get_str(d, "missing"), ""))) @panic("missing str key");
// zbr:test/json_test.zbr:15
    if (!((_json_get_int(d, "missing") == 0))) @panic("missing int key");
// zbr:test/json_test.zbr:16
    if (!((_json_get_bool(d, "missing") == false))) @panic("missing bool key");
// zbr:test/json_test.zbr:19
    const nested: []const u8 = "{"user": {"id": 42, "tag": "admin"}}";
// zbr:test/json_test.zbr:20
    var nd = _json_parse(nested).?;
// zbr:test/json_test.zbr:21
    var user = _json_get_obj(nd, "user");
// zbr:test/json_test.zbr:22
    if (!((user.getInt("id") == 42))) @panic("nested getInt");
// zbr:test/json_test.zbr:23
    if (!(std.mem.eql(u8, user.getStr("tag"), "admin"))) @panic("nested getStr");
// zbr:test/json_test.zbr:26
    const arr_json: []const u8 = "{"tags": ["alpha", "beta", "gamma"]}";
// zbr:test/json_test.zbr:27
    var ad = _json_parse(arr_json).?;
...
// zbr:test/json_test.zbr:35
    if (!((count == 3))) @panic("array element count");
// zbr:test/json_test.zbr:38
    var obj = _json_object();
// zbr:test/json_test.zbr:39
    if (!((_json_is_object(obj) == true))) @panic("isObject");
// zbr:test/json_test.zbr:40
    if (!((_json_is_array(obj) == false))) @panic("isArray on object");
// zbr:test/json_test.zbr:41
    if (!((_json_is_null(obj) == false))) @panic("isNull on object");
// zbr:test/json_test.zbr:43
    var arr = _json_array();
// zbr:test/json_test.zbr:44
    if (!((_json_is_array(arr) == true))) @panic("isArray");
// zbr:test/json_test.zbr:47
...
// zbr:test/json_test.zbr:54
    if (!((std.mem.indexOf(u8, out, "Springfield") != null))) @panic("stringify contains city");
// zbr:test/json_test.zbr:55
    if (!((std.mem.indexOf(u8, out, "50000") != null))) @panic("stringify contains pop");
// zbr:test/json_test.zbr:58
...
// zbr:test/json_test.zbr:65
    if (!((std.mem.indexOf(u8, list_out, "one") != null))) @panic("array stringify contains one");
// zbr:test/json_test.zbr:68
    const rt_json: []const u8 = "{"x": 1, "y": 2}";
// zbr:test/json_test.zbr:69
    const rt = _json_parse(rt_json).?;
// zbr:test/json_test.zbr:70
    const rt_str = _json_stringify(rt);
// zbr:test/json_test.zbr:71
    if (!((std.mem.indexOf(u8, rt_str, "\"x\"") != null))) @panic("roundtrip contains x key");
