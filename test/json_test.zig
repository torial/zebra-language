...
// Source: C:/Projects/zebra-language/test/json_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        var d = data.?;
        if (!(std.mem.eql(u8, d.getStr("name"), "Alice"))) @panic("getStr name");
        if (!((d.getInt("age") == 30))) @panic("getInt age");
        if (!(_zebra_gt(d.getFloat("score"), 9.4))) @panic("getFloat score");
        if (!((d.getBool("active") == true))) @panic("getBool active");
        if (!(std.mem.eql(u8, d.getStr("missing"), ""))) @panic("missing str key");
        if (!((d.getInt("missing") == 0))) @panic("missing int key");
        if (!((d.getBool("missing") == false))) @panic("missing bool key");
        ...
        if (!((obj.isObject() == true))) @panic("isObject");
        if (!((obj.isArray() == false))) @panic("isArray on object");
        if (!((obj.isNull() == false))) @panic("isNull on object");
        var arr = _json_array();
        if (!((arr.isArray() == true))) @panic("isArray");
        var builder = _json_object();
        builder.put("city", "Springfield") catch @panic("OOM");
        builder.putInt("pop", 50000);
        builder.putFloat("lat", 37.5);
        builder.putBool("capital", false);
        const out = _json_stringify(builder);
        if (!(out.contains("Springfield"))) @panic("stringify contains city");
        if (!(out.contains("50000"))) @panic("stringify contains pop");
        var list = _json_array();
        list.appendSlice(_allocator, "one") catch @panic("OOM");
        list.appendInt(2);
        list.appendFloat(3.0);
        list.appendBool(true);
        const list_out = _json_stringify(list);
        if (!(list_out.contains("one"))) @panic("array stringify contains one");
        const rt_json: []const u8 = "{"x": 1, "y": 2}";
        const rt = _json_parse(rt_json).?;
        const rt_str = _json_stringify(rt);
        if (!(rt_str.contains("\"x\""))) @panic("roundtrip contains x key");
