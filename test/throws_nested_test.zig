...
// zbr:test/throws_nested_test.zbr:15
        { _error_ctx = .{ .message = "from compute" }; return error.ZebraError; }
    }
...
pub const Engine = struct {
    ...
    helper: *Helper,
    pub fn init() *Engine {
        const _self = _allocator.create(Engine) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Engine");
// zbr:test/throws_nested_test.zbr:21
            _self.helper = Helper.init();
        return _self;
    }
...
// zbr:test/throws_nested_test.zbr:25
        { _error_ctx = .{ .message = "from doWork" }; return error.ZebraError; }
    }
...
const _ttag_Engine: u64 = _zbr_hash("Engine");
...
pub fn testCase1() void {
// zbr:test/throws_nested_test.zbr:38
    {
        var _try_err: ?anyerror = null;
        _ = &_try_err;
        _try_blk: {
// zbr:test/throws_nested_test.zbr:39
            var e = Engine.init();
            e.run() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
            break :_try_blk;
        }
        if (_try_err != null) {
            // pass
...
// zbr:test/throws_nested_test.zbr:45
            const e = Engine.init();
            e.helper.compute() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
            break :_try_blk;
        }
        if (_try_err != null) {
            // pass
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    ...
    if (tag == _ttag_Engine) return _reflect_Engine_name;
...
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_Engine) return .{ .items = _reflect_Engine_fields };
...
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_Engine) return .{ .items = _reflect_Engine_field_types };
