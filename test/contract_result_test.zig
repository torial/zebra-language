...
// Source: test/contract_result_test.zbr
...
pub const Main = struct {
    ...
    pub fn doubleIt(x: i64) i64 {
        var _result: i64 = undefined;
        var _ensure_armed: bool = false;
        defer {
            if (_ensure_armed and !((_result == (x * 2)))) std.debug.panic("ensure failed in 'doubleIt'\n", .{});
        }
// zbr:test/contract_result_test.zbr:6
        { _result = (x * 2); _ensure_armed = true; return _result; }
        _ensure_armed = true;
    }
    ...
    pub fn absolute(n: i64) i64 {
        ...
        var _ensure_armed: bool = false;
        defer {
            if (_ensure_armed and !(_zebra_ge(_result, 0))) std.debug.panic("ensure failed in 'absolute'\n", .{});
        }
// zbr:test/contract_result_test.zbr:11
        if (_zebra_lt(n, 0)) {
// zbr:test/contract_result_test.zbr:12
            { _result = (-n); _ensure_armed = true; return _result; }
        }
// zbr:test/contract_result_test.zbr:13
        { _result = n; _ensure_armed = true; return _result; }
        _ensure_armed = true;
    }

    pub fn main() void {
// zbr:test/contract_result_test.zbr:16
        std.debug.assert((Main.doubleIt(5) == 10));
// zbr:test/contract_result_test.zbr:17
        std.debug.assert((Main.doubleIt((-3)) == (-6)));
// zbr:test/contract_result_test.zbr:18
        std.debug.assert((Main.absolute(7) == 7));
// zbr:test/contract_result_test.zbr:19
        std.debug.assert((Main.absolute((-7)) == 7));
// zbr:test/contract_result_test.zbr:20
        std.debug.assert((Main.absolute(0) == 0));
        std.debug.print("{s}\n", .{"contract_result_test OK"});
    }
