...
// Source: C:/Projects/zebra-language/test/tc_infer_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        const opt_resp_sym = TcSymbol.init("maybe_resp", TcSymKind.local, TcType{ .optional = TcType{ .http_response = {} } });
        ...
        std.debug.assert(TcTypes.eql(nil_t, TcType{ .optional = TcType{ .void_ = {} } }));
        ...
        const t_status = inf.inferExpr(TcExpr{ .member = mem_status });
        std.debug.assert(TcTypes.eql(t_status, TcType{ .uint_ = {} }));
        const mem_text = TcExprMember.init(resp_expr, "text");
        const t_text = inf.inferExpr(TcExpr{ .member = mem_text });
        ...
        const t_get = inf.inferExpr(TcExpr{ .member = mem_get });
        ...
        const http_call = TcExprCall.init(TcExpr{ .member = http_mem }, http_args);
        const t_http_get = inf.inferExpr(TcExpr{ .call = http_call });
        std.debug.assert(TcTypes.eql(t_http_get, TcType{ .optional = TcType{ .http_response = {} } }));
        ...
        const tm_call = TcExprCall.init(TcExpr{ .member = tm_mem }, tm_args);
        const t_timer = inf.inferExpr(TcExpr{ .call = tm_call });
        ...
        std.debug.assert(TcTypes.eql(inf.inferExpr(TcExpr{ .binary = bi_arith }), TcType{ .int_ = {} }));
        const bi_cmp = TcExprBinary.init("==", TcExpr{ .int_lit = {} }, TcExpr{ .int_lit = {} });
        std.debug.assert(TcTypes.eql(inf.inferExpr(TcExpr{ .binary = bi_cmp }), TcType{ .bool_ = {} }));
        const bi_str = TcExprBinary.init("+", TcExpr{ .string_lit = {} }, TcExpr{ .string_lit = {} });
        std.debug.assert(TcTypes.eql(inf.inferExpr(TcExpr{ .binary = bi_str }), TcType{ .string_ = {} }));
        const bi_lt = TcExprBinary.init("<", TcExpr{ .float_lit = {} }, TcExpr{ .float_lit = {} });
        std.debug.assert(TcTypes.eql(inf.inferExpr(TcExpr{ .binary = bi_lt }), TcType{ .bool_ = {} }));
        std.debug.print("{s}\n", .{"7. binary operator inference OK"});
        const un_neg = TcExprUnary.init("-", TcExpr{ .int_lit = {} });
        std.debug.assert(TcTypes.eql(inf.inferExpr(TcExpr{ .unary = un_neg }), TcType{ .int_ = {} }));
        const un_not = TcExprUnary.init("not", TcExpr{ .bool_lit = {} });
        std.debug.assert(TcTypes.eql(inf.inferExpr(TcExpr{ .unary = un_not }), TcType{ .bool_ = {} }));
        ...
        const chain_expr = TcExpr{ .optional_chain = opt_chain };
