...
pub fn main(init: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/compress_test.zbr:3
    const original: []const u8 = "hello world";
// zbr:test/compress_test.zbr:4
    const compressed: []const u8 = _compress_gzip(original);
// zbr:test/compress_test.zbr:5
    std.debug.assert(_zebra_gt(@as(i64, @intCast(compressed.len)), 0));
    std.debug.print("{s}\n", .{"gzip produces output: OK"});
// zbr:test/compress_test.zbr:8
    const recovered = _compress_gunzip(compressed);
// zbr:test/compress_test.zbr:9
    std.debug.assert((recovered != null));
// zbr:test/compress_test.zbr:10
    std.debug.assert(std.mem.eql(u8, recovered.?, original));
    std.debug.print("{s}\n", .{"gzip round-trip: OK"});
// zbr:test/compress_test.zbr:14
    const big: []const u8 = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
// zbr:test/compress_test.zbr:15
    const big_gz: []const u8 = _compress_gzip(big);
// zbr:test/compress_test.zbr:16
    std.debug.assert(_zebra_gt(@as(i64, @intCast(big_gz.len)), 0));
// zbr:test/compress_test.zbr:17
    std.debug.assert(_zebra_lt(@as(i64, @intCast(big_gz.len)), @as(i64, @intCast(big.len))));
// zbr:test/compress_test.zbr:18
    const big_back = _compress_gunzip(big_gz);
// zbr:test/compress_test.zbr:19
    std.debug.assert((big_back != null));
// zbr:test/compress_test.zbr:20
    std.debug.assert(std.mem.eql(u8, big_back.?, big));
    std.debug.print("{s}\n", .{"gzip compressible data round-trip: OK"});
// zbr:test/compress_test.zbr:24
    const bad = _compress_gunzip("not valid gzip");
// zbr:test/compress_test.zbr:25
