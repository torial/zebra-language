...
// Source: C:/Projects/zebra-language/test/compress_test.zbr
...
pub const CompressTest = struct {
    ...
    pub fn init() *CompressTest {
        const self = _allocator.create(CompressTest) catch @panic("OOM");
        self._type_tag = _ttag_CompressTest;
        return self;
    }

};
const _ttag_CompressTest: u64 = _zbr_hash("CompressTest");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_CompressTest) return _reflect_CompressTest_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_CompressTest) return .{ .items = _reflect_CompressTest_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_CompressTest) return .{ .items = _reflect_CompressTest_field_types };
    return .{ .items = &.{} };
}
