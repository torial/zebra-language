...
pub const Main = struct {
    ...
    pub fn main() void {
        var anchored = _regex_compile("^hello$", "");
        std.debug.print("{}\n", .{anchored.match("hello")});
        std.debug.print("{}\n", .{anchored.match("hello world")});
        var starts = _regex_compile("^\\d+", "");
        std.debug.print("{}\n", .{starts.find("42 abc")});
        std.debug.print("{}\n", .{starts.find("abc 42")});
        var exact = _regex_compile("\\d{4}", "");
        std.debug.print("{}\n", .{exact.match("2026")});
        std.debug.print("{}\n", .{exact.match("123")});
        var range_re = _regex_compile("a{2,4}", "");
        std.debug.print("{}\n", .{range_re.match("aa")});
        std.debug.print("{}\n", .{range_re.match("aaaa")});
        std.debug.print("{}\n", .{range_re.match("a")});
        var nc = _regex_compile("(?:ab)+", "");
        std.debug.print("{}\n", .{nc.match("abab")});
        std.debug.print("{}\n", .{nc.match("ab")});
    }
