...
pub fn classify(c: Color) []const u8 {
// zbr:test/is_not_test.zbr:11
    if ((!(c == .red))) {
// zbr:test/is_not_test.zbr:12
        return "non-red";
    }
// zbr:test/is_not_test.zbr:13
    return "red";
}
...
// zbr:test/is_not_test.zbr:17
    const c = Color.red;
// zbr:test/is_not_test.zbr:18
    if ((!(c == .blue))) {
        std.debug.print("{s}\n", .{"not blue"});
    } else {
        std.debug.print("{s}\n", .{"blue"});
    }
    std.debug.print("{s}\n", .{classify(Color.green)});
    std.debug.print("{s}\n", .{classify(Color.red)});
// zbr:test/is_not_test.zbr:28
    const s = Shape{ .square = 5 };
// zbr:test/is_not_test.zbr:29
    if ((!(s == .circle))) {
        std.debug.print("{s}\n", .{"not a circle"});
    }
// zbr:test/is_not_test.zbr:33
    const c2 = Color.blue;
// zbr:test/is_not_test.zbr:34
    const result: bool = (!(c2 == .red));
    std.debug.print("{}\n", .{result});
// zbr:test/is_not_test.zbr:38
    const c3 = Color.green;
// zbr:test/is_not_test.zbr:39
    if ((c3 == .green)) {
        std.debug.print("{s}\n", .{"is green"});
    }
    std.debug.print("{s}\n", .{"is_not: OK"});
