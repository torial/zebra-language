...
// Source: test/unless_until_test.zbr
...
pub const UnlessUntilTest = struct {
    _type_tag: u64 = _ttag_UnlessUntilTest,
    pub fn run(self: *UnlessUntilTest) void {
        _ = self;
// zbr:test/unless_until_test.zbr:4
        var x: i64 = 5;
// zbr:test/unless_until_test.zbr:5
        if ((!_zebra_gt(x, 10))) {
// zbr:test/unless_until_test.zbr:6
            x = (x + 1);
        }
// zbr:test/unless_until_test.zbr:7
        std.debug.assert((x == 6));
// zbr:test/unless_until_test.zbr:10
        if ((!_zebra_lt(x, 10))) {
// zbr:test/unless_until_test.zbr:11
            x = 99;
        }
// zbr:test/unless_until_test.zbr:12
        std.debug.assert((x == 6));
// zbr:test/unless_until_test.zbr:15
        var n: i64 = 0;
// zbr:test/unless_until_test.zbr:16
        while ((!_zebra_ge(n, 3))) {
// zbr:test/unless_until_test.zbr:17
            n = (n + 1);
        }
// zbr:test/unless_until_test.zbr:18
        std.debug.assert((n == 3));
        std.debug.print("{s}\n", .{"unless/until OK"});
    }

    pub fn init() *UnlessUntilTest {
        const self = _allocator.create(UnlessUntilTest) catch @panic("OOM");
        self._type_tag = _ttag_UnlessUntilTest;
        return self;
    }

};
const _ttag_UnlessUntilTest: u64 = _zbr_hash("UnlessUntilTest");
const _reflect_UnlessUntilTest_name: []const u8 = "UnlessUntilTest";
const _reflect_UnlessUntilTest_fields: []const []const u8 = &.{};
const _reflect_UnlessUntilTest_field_types: []const []const u8 = &.{};

pub fn main() void {
    var _mc_1 = UnlessUntilTest.init();
    _mc_1.run();
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_UnlessUntilTest) return _reflect_UnlessUntilTest_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_UnlessUntilTest) return .{ .items = _reflect_UnlessUntilTest_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_UnlessUntilTest) return .{ .items = _reflect_UnlessUntilTest_field_types };
    return .{ .items = &.{} };
}
