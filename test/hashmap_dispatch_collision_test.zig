...
// Source: test/hashmap_dispatch_collision_test.zbr
...
pub const Registry = struct {
    _type_tag: u64 = _ttag_Registry,
    n: i64,
    pub fn init() *Registry {
        const _self = _allocator.create(Registry) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Registry");
// zbr:test/hashmap_dispatch_collision_test.zbr:13
            _self.n = 0;
        return _self;
    }

    pub fn set(self: *Registry, val: i64) void {
// zbr:test/hashmap_dispatch_collision_test.zbr:15
        self.n = val;
    }

    pub fn get(self: *Registry) i64 {
// zbr:test/hashmap_dispatch_collision_test.zbr:17
        return self.n;
    }

    pub fn contains(self: *Registry, val: i64) bool {
// zbr:test/hashmap_dispatch_collision_test.zbr:19
        return (self.n == val);
    }

    pub fn remove(self: *Registry, val: i64) void {
// zbr:test/hashmap_dispatch_collision_test.zbr:21
        if ((self.n == val)) {
// zbr:test/hashmap_dispatch_collision_test.zbr:22
            self.n = 0;
        }
    }

    pub fn count(self: *Registry) i64 {
// zbr:test/hashmap_dispatch_collision_test.zbr:24
        return self.n;
    }

    pub fn keys(self: *Registry) []const u8 {
// zbr:test/hashmap_dispatch_collision_test.zbr:26
        return (std.fmt.allocPrint(_allocator, "{}", .{self.n}) catch unreachable);
    }

    pub fn values(self: *Registry) []const u8 {
// zbr:test/hashmap_dispatch_collision_test.zbr:28
        return _str_concat((std.fmt.allocPrint(_allocator, "{}", .{self.n}) catch unreachable), (std.fmt.allocPrint(_allocator, "{}", .{self.n}) catch unreachable), _allocator);
    }

};
const _ttag_Registry: u64 = _zbr_hash("Registry");
const _reflect_Registry_name: []const u8 = "Registry";
const _reflect_Registry_fields: []const []const u8 = &.{"n"};
const _reflect_Registry_field_types: []const []const u8 = &.{"int"};

pub const Cache = struct {
    _type_tag: u64 = _ttag_Cache,
    hits: i64,
    misses: i64,
    pub fn init() *Cache {
        const _self = _allocator.create(Cache) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Cache");
// zbr:test/hashmap_dispatch_collision_test.zbr:34
            _self.hits = 0;
// zbr:test/hashmap_dispatch_collision_test.zbr:35
            _self.misses = 0;
        return _self;
    }

    pub fn get(self: *Cache, key: []const u8) i64 {
// zbr:test/hashmap_dispatch_collision_test.zbr:37
        if (std.mem.eql(u8, key, "hit")) {
// zbr:test/hashmap_dispatch_collision_test.zbr:38
            self.hits = (self.hits + 1);
// zbr:test/hashmap_dispatch_collision_test.zbr:39
            return self.hits;
        }
// zbr:test/hashmap_dispatch_collision_test.zbr:40
        self.misses = (self.misses + 1);
// zbr:test/hashmap_dispatch_collision_test.zbr:41
        return (-self.misses);
    }

    pub fn put(self: *Cache, k: []const u8, v: i64) bool {
        _ = self;
// zbr:test/hashmap_dispatch_collision_test.zbr:43
        return (_zebra_gt(@as(i64, @intCast(k.len)), 0) and _zebra_ge(v, 0));
    }

    pub fn size(self: *Cache) i64 {
// zbr:test/hashmap_dispatch_collision_test.zbr:45
        return (self.hits + self.misses);
    }

};
const _ttag_Cache: u64 = _zbr_hash("Cache");
const _reflect_Cache_name: []const u8 = "Cache";
const _reflect_Cache_fields: []const []const u8 = &.{"hits", "misses"};
const _reflect_Cache_field_types: []const []const u8 = &.{"int", "int"};
...
pub fn main(init: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/hashmap_dispatch_collision_test.zbr:48
    var r = Registry.init();
    r.set(42);
// zbr:test/hashmap_dispatch_collision_test.zbr:51
    if (!((r.get() == 42))) @panic("get: expected 42");
// zbr:test/hashmap_dispatch_collision_test.zbr:52
    if (!(r.contains(42))) @panic("contains true: expected true");
// zbr:test/hashmap_dispatch_collision_test.zbr:53
    if (!((!r.contains(7)))) @panic("contains false: expected false");
// zbr:test/hashmap_dispatch_collision_test.zbr:54
    if (!((r.count() == 42))) @panic("count: expected 42");
// zbr:test/hashmap_dispatch_collision_test.zbr:55
    if (!(std.mem.eql(u8, r.keys(), "42"))) @panic("keys: expected '42'");
// zbr:test/hashmap_dispatch_collision_test.zbr:56
    if (!(std.mem.eql(u8, r.values(), "4242"))) @panic("values: expected '4242'");
    r.remove(42);
// zbr:test/hashmap_dispatch_collision_test.zbr:59
    if (!((r.get() == 0))) @panic("after remove get: expected 0");
    r.set(10);
// zbr:test/hashmap_dispatch_collision_test.zbr:62
    if (!((r.count() == 10))) @panic("count after set: expected 10");
// zbr:test/hashmap_dispatch_collision_test.zbr:64
    var c = Cache.init();
// zbr:test/hashmap_dispatch_collision_test.zbr:65
    const v1 = c.get("hit");
// zbr:test/hashmap_dispatch_collision_test.zbr:66
    if (!((v1 == 1))) @panic("cache get hit 1: expected 1");
// zbr:test/hashmap_dispatch_collision_test.zbr:67
    const v2 = c.get("hit");
// zbr:test/hashmap_dispatch_collision_test.zbr:68
    if (!((v2 == 2))) @panic("cache get hit 2: expected 2");
// zbr:test/hashmap_dispatch_collision_test.zbr:69
    const v3 = c.get("miss");
// zbr:test/hashmap_dispatch_collision_test.zbr:70
    if (!((v3 == (-1)))) @panic("cache get miss: expected -1");
// zbr:test/hashmap_dispatch_collision_test.zbr:71
    if (!((c.size() == 3))) @panic("cache size: expected 3");
// zbr:test/hashmap_dispatch_collision_test.zbr:72
    if (!(c.put("key", 99))) @panic("put valid: expected true");
// zbr:test/hashmap_dispatch_collision_test.zbr:73
    if (!((!c.put("", 99)))) @panic("put empty key: expected false");
    std.debug.print("{s}\n", .{"hashmap_dispatch_collision: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Registry) return _reflect_Registry_name;
    if (tag == _ttag_Cache) return _reflect_Cache_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Registry) return .{ .items = _reflect_Registry_fields };
    if (tag == _ttag_Cache) return .{ .items = _reflect_Cache_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Registry) return .{ .items = _reflect_Registry_field_types };
    if (tag == _ttag_Cache) return .{ .items = _reflect_Cache_field_types };
    return .{ .items = &.{} };
}
