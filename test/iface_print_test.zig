...
// Source: test/iface_print_test.zbr
...
pub const IGreeter = struct {
    ptr:    *anyopaque,
    vtable: *const VTable,

    pub const VTable = struct {
        greet: *const fn (ptr: *anyopaque, name: []const u8) []const u8,
    };
    ...
    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "greet")) @compileError("type " ++ @typeName(T) ++ " does not implement IGreeter.greet");
        }
    }
};

pub const ICounter = struct {
    ptr:    *anyopaque,
    vtable: *const VTable,

    pub const VTable = struct {
        count: *const fn (ptr: *anyopaque) i64,
    };

    pub fn count(self: @This()) i64 {
        return self.vtable.count(self.ptr);
    }

    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "count")) @compileError("type " ++ @typeName(T) ++ " does not implement ICounter.count");
        }
    }
};

pub const HelloGreeter = struct {
    _type_tag: u64 = _ttag_HelloGreeter,
    pub fn greet(self: *HelloGreeter, name: []const u8) []const u8 {
        _ = self;
// zbr:test/iface_print_test.zbr:9
        return _str_concat(_str_concat("Hello, ", name, _allocator), "!", _allocator);
    }

    pub fn init() *HelloGreeter {
        const self = _allocator.create(HelloGreeter) catch @panic("OOM");
        self._type_tag = _ttag_HelloGreeter;
        return self;
    }
...
fn _shim_HelloGreeter_IGreeter_greet(ptr: *anyopaque, name: []const u8) []const u8 { return HelloGreeter.greet(@alignCast(@ptrCast(ptr)), name); }
const _vtable_HelloGreeter_IGreeter = IGreeter.VTable{ .greet = &_shim_HelloGreeter_IGreeter_greet, };
const _ttag_HelloGreeter: u64 = _zbr_hash("HelloGreeter");
const _reflect_HelloGreeter_name: []const u8 = "HelloGreeter";
const _reflect_HelloGreeter_fields: []const []const u8 = &.{};
const _reflect_HelloGreeter_field_types: []const []const u8 = &.{};

pub const StepCounter = struct {
    _type_tag: u64 = _ttag_StepCounter,
    n: i64,
    pub fn init() *StepCounter {
        const _self = _allocator.create(StepCounter) catch @panic("OOM");
        _self._type_tag = _zbr_hash("StepCounter");
// zbr:test/iface_print_test.zbr:14
            _self.n = 0;
        return _self;
    }

    pub fn count(self: *StepCounter) i64 {
// zbr:test/iface_print_test.zbr:16
        self.n = (self.n + 1);
// zbr:test/iface_print_test.zbr:17
        return self.n;
    }
...
fn _shim_StepCounter_ICounter_count(ptr: *anyopaque) i64 { return StepCounter.count(@alignCast(@ptrCast(ptr))); }
const _vtable_StepCounter_ICounter = ICounter.VTable{ .count = &_shim_StepCounter_ICounter_count, };
const _ttag_StepCounter: u64 = _zbr_hash("StepCounter");
const _reflect_StepCounter_name: []const u8 = "StepCounter";
...
// zbr:test/iface_print_test.zbr:20
    var g: IGreeter = .{ .ptr = HelloGreeter.init(), .vtable = &_vtable_HelloGreeter_IGreeter };
    std.debug.print("{s}\n", .{g.greet("World")});
// zbr:test/iface_print_test.zbr:22
    const c: ICounter = .{ .ptr = StepCounter.init(), .vtable = &_vtable_StepCounter_ICounter };
    std.debug.print("{}\n", .{c.count()});
    std.debug.print("{}\n", .{c.count()});
    std.debug.print("{s}\n", .{"iface_print: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_HelloGreeter) return _reflect_HelloGreeter_name;
    if (tag == _ttag_StepCounter) return _reflect_StepCounter_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_HelloGreeter) return .{ .items = _reflect_HelloGreeter_fields };
    if (tag == _ttag_StepCounter) return .{ .items = _reflect_StepCounter_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_HelloGreeter) return .{ .items = _reflect_HelloGreeter_field_types };
    if (tag == _ttag_StepCounter) return .{ .items = _reflect_StepCounter_field_types };
    return .{ .items = &.{} };
}
