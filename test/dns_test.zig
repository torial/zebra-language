...
// Source: C:/Projects/zebra-language/test/dns_test.zbr
...
pub const Main = struct {
    ...
    pub fn testMultipleHosts() void {
        ...
        hosts.append(_allocator, "localhost") catch @panic("OOM");
        hosts.append(_allocator, "127.0.0.1") catch @panic("OOM");
        for (hosts.items) |hostname| {
            std.debug.print("{s}\n", .{(std.mem.concat(_allocator, u8, &.{ "Resolving: ", hostname }) catch unreachable)});
            for (_net_resolve(hostname).items) |ip| {
                std.debug.print("{s}\n", .{(std.mem.concat(_allocator, u8, &.{ "  -> ", ip }) catch unreachable)});
            }
        }
    }
