...
// zbr:test/dns_test.zbr:3
    std.debug.print("{s}\n", .{"Resolving localhost:"});
// zbr:test/dns_test.zbr:4
    for (_net_resolve("localhost").items) |ip| {
...
// zbr:test/dns_test.zbr:8
    var hosts = std.ArrayList([]const u8).empty;
// zbr:test/dns_test.zbr:9
    hosts.append(_allocator, _intern("localhost")) catch unreachable;
// zbr:test/dns_test.zbr:10
    hosts.append(_allocator, _intern("127.0.0.1")) catch unreachable;
...
// zbr:test/dns_test.zbr:13
        for (_net_resolve(hostname).items) |ip| {
...
// zbr:test/dns_test.zbr:18
    std.debug.print("{s}\n", .{"Attempting to resolve invalid host:"});
// zbr:test/dns_test.zbr:19
    const results = _net_resolve("invalid.hostname.that.does.not.exist.local");
// zbr:test/dns_test.zbr:20
    {
        const _pt0 = (std.mem.concat(_allocator, u8, &.{ (std.mem.concat(_allocator, u8, &.{ "Got ", (std.fmt.allocPrint(_allocator, "{}", .{@as(i64, @intCast(results.items.len))}) catch unreachable) }) catch unreachable), " results" }) catch unreachable);
        std.debug.print("{s}\n", .{_pt0});
    }
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/dns_test.zbr:23
    testLocalhostResolve();
// zbr:test/dns_test.zbr:24
    testMultipleHosts();
