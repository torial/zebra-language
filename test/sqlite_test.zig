...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/sqlite_test.zbr:2
    const db: ?SqliteDb = _sqlite_open(":memory:");
// zbr:test/sqlite_test.zbr:3
    if ((db == null)) {
        std.debug.print("{s}\n", .{"sqlite_test SKIP (no sqlite3)"});
// zbr:test/sqlite_test.zbr:5
        return;
    }
// zbr:test/sqlite_test.zbr:6
    var d: SqliteDb = db.?;
    d.exec_("CREATE TABLE t (id INTEGER, name TEXT)");
    d.exec_p_("INSERT INTO t VALUES (?, ?)", &[_]_SqliteParam{.{ .int = @as(i64, @intCast(1)) }, .{ .text = "alice" }});
    d.exec_p_("INSERT INTO t VALUES (?, ?)", &[_]_SqliteParam{.{ .int = @as(i64, @intCast(2)) }, .{ .text = "bob" }});
// zbr:test/sqlite_test.zbr:10
    const rows = d.query_("SELECT * FROM t ORDER BY id");
// zbr:test/sqlite_test.zbr:11
    {
        const _it_row = rows;
        for (_it_row.items) |row| {
            std.debug.print("{s}\n", .{_str_concat(_str_concat((std.fmt.allocPrint(_allocator, "{}", .{row.int_("id")}) catch unreachable), " ", _allocator), row.str_("name"), _allocator)});
        }
    }
    d.begin_();
    d.exec_p_("INSERT INTO t VALUES (?, ?)", &[_]_SqliteParam{.{ .int = @as(i64, @intCast(3)) }, .{ .text = "carol" }});
    d.commit_();
// zbr:test/sqlite_test.zbr:16
    const rows2 = d.query_("SELECT id, name FROM t ORDER BY id");
// zbr:test/sqlite_test.zbr:17
    {
        const _it_row2 = rows2;
        for (_it_row2.items) |row2| {
            std.debug.print("{s}\n", .{_str_concat(_str_concat((std.fmt.allocPrint(_allocator, "{}", .{row2.int_("id")}) catch unreachable), " ", _allocator), row2.str_("name"), _allocator)});
        }
    }
    d.close_();
    std.debug.print("{s}\n", .{"sqlite_test OK"});
