...
// zbr:test/type_infer_chain_test.zbr:28
    const words: std.ArrayList([]const u8) = blk092_1: { var _ll_1: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_1 = std.mem.splitSequence(u8, "zero one two three four", " "); while (_split_iter_1.next()) |_se_1| { _ll_1.append(_allocator, _se_1) catch @panic("OOM"); } break :blk092_1 _ll_1; };
...
// zbr:test/type_infer_chain_test.zbr:64
    const csv_parts: std.ArrayList([]const u8) = blk092_2: { var _ll_2: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_2 = std.mem.splitSequence(u8, csv, ","); while (_split_iter_2.next()) |_se_2| { _ll_2.append(_allocator, _se_2) catch @panic("OOM"); } break :blk092_2 _ll_2; };
...
// zbr:test/type_infer_chain_test.zbr:71
    const rows: std.ArrayList([]const u8) = blk092_3: { var _ll_3: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_3 = std.mem.splitSequence(u8, "a b,c d,e f", ","); while (_split_iter_3.next()) |_se_3| { _ll_3.append(_allocator, _se_3) catch @panic("OOM"); } break :blk092_3 _ll_3; };
// zbr:test/type_infer_chain_test.zbr:72
    const mid_row = rows.items[@intCast(1)];
// zbr:test/type_infer_chain_test.zbr:73
    const mid_parts: std.ArrayList([]const u8) = blk092_4: { var _ll_4: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_4 = std.mem.splitSequence(u8, mid_row, " "); while (_split_iter_4.next()) |_se_4| { _ll_4.append(_allocator, _se_4) catch @panic("OOM"); } break :blk092_4 _ll_4; };
