...
// Source: test/type_infer_chain_test.zbr
...
pub const Box = struct {
    ...
    val: i64,
    pub fn init(v: i64) *Box {
        const _self = _allocator.create(Box) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Box");
// zbr:test/type_infer_chain_test.zbr:18
            _self.val = v;
        return _self;
    }

    pub fn doubled(self: *Box) i64 {
// zbr:test/type_infer_chain_test.zbr:20
        return (self.val * 2);
    }

    pub fn asStr(self: *Box) []const u8 {
// zbr:test/type_infer_chain_test.zbr:22
        return (std.fmt.allocPrint(_allocator, "{}", .{self.val}) catch unreachable);
    }

};
const _ttag_Box: u64 = _zbr_hash("Box");
const _reflect_Box_name: []const u8 = "Box";
const _reflect_Box_fields: []const []const u8 = &.{"val"};
const _reflect_Box_field_types: []const []const u8 = &.{"int"};

pub fn makeBox(n: i64) *Box {
// zbr:test/type_infer_chain_test.zbr:25
    return Box.init(n);
}

pub fn getWord(idx: i64) []const u8 {
// zbr:test/type_infer_chain_test.zbr:28
    const words: std.ArrayList([]const u8) = blk092_1: { var _ll_1: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_1 = std.mem.splitSequence(u8, "zero one two three four", " "); while (_split_iter_1.next()) |_se_1| { _ll_1.append(_allocator, _se_1) catch @panic("OOM"); } break :blk092_1 _ll_1; };
// zbr:test/type_infer_chain_test.zbr:29
...
// zbr:test/type_infer_chain_test.zbr:33
    ...
    strs.append(_allocator, _intern("hello")) catch @panic("OOM");
    strs.append(_allocator, _intern("world")) catch @panic("OOM");
    strs.append(_allocator, _intern("!")) catch @panic("OOM");
// zbr:test/type_infer_chain_test.zbr:37
    const w0 = strs.items[@intCast(0)];
// zbr:test/type_infer_chain_test.zbr:38
    if (!((@as(i64, @intCast(w0.len)) == 5))) @panic("List(str).at().len: expected 5");
// zbr:test/type_infer_chain_test.zbr:39
    if (!(std.mem.startsWith(u8, w0, "hel"))) @panic("List(str).at().startsWith: expected true");
// zbr:test/type_infer_chain_test.zbr:42
    var ints = std.ArrayList(i64){};
    ints.append(_allocator, 7) catch @panic("OOM");
    ints.append(_allocator, 14) catch @panic("OOM");
    ints.append(_allocator, 21) catch @panic("OOM");
// zbr:test/type_infer_chain_test.zbr:46
    const elem1 = ints.items[@intCast(1)];
// zbr:test/type_infer_chain_test.zbr:47
    if (!(((elem1 * 2) == 28))) @panic("List(int).at() * 2: expected 28");
// zbr:test/type_infer_chain_test.zbr:50
    const word = getWord(2);
// zbr:test/type_infer_chain_test.zbr:51
    if (!(std.mem.eql(u8, word, "two"))) @panic("getWord(2): expected 'two'");
// zbr:test/type_infer_chain_test.zbr:52
    if (!((@as(i64, @intCast(word.len)) == 3))) @panic("getWord(2).len: expected 3");
// zbr:test/type_infer_chain_test.zbr:55
    var b = makeBox(5);
// zbr:test/type_infer_chain_test.zbr:56
    const d = b.doubled();
// zbr:test/type_infer_chain_test.zbr:57
    if (!((d == 10))) @panic("makeBox(5).doubled(): expected 10");
// zbr:test/type_infer_chain_test.zbr:58
    const sv = b.asStr();
// zbr:test/type_infer_chain_test.zbr:59
    if (!((@as(i64, @intCast(sv.len)) == 1))) @panic("makeBox(5).asStr().len: expected 1");
// zbr:test/type_infer_chain_test.zbr:60
    if (!(std.mem.eql(u8, sv, "5"))) @panic("makeBox(5).asStr(): expected '5'");
// zbr:test/type_infer_chain_test.zbr:63
    const csv: []const u8 = "alpha,beta,gamma,delta";
// zbr:test/type_infer_chain_test.zbr:64
    const csv_parts: std.ArrayList([]const u8) = blk092_2: { var _ll_2: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_2 = std.mem.splitSequence(u8, csv, ","); while (_split_iter_2.next()) |_se_2| { _ll_2.append(_allocator, _se_2) catch @panic("OOM"); } break :blk092_2 _ll_2; };
// zbr:test/type_infer_chain_test.zbr:65
    if (!((@as(i64, @intCast(csv_parts.items.len)) == 4))) @panic("csv_parts.count: expected 4");
// zbr:test/type_infer_chain_test.zbr:66
    const last = csv_parts.items[@intCast(3)];
// zbr:test/type_infer_chain_test.zbr:67
    if (!(std.mem.eql(u8, last, "delta"))) @panic("csv_parts.at(3): expected 'delta'");
// zbr:test/type_infer_chain_test.zbr:68
    if (!((@as(i64, @intCast(last.len)) == 5))) @panic("csv_parts.at(3).len: expected 5");
// zbr:test/type_infer_chain_test.zbr:71
    const rows: std.ArrayList([]const u8) = blk092_3: { var _ll_3: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_3 = std.mem.splitSequence(u8, "a b,c d,e f", ","); while (_split_iter_3.next()) |_se_3| { _ll_3.append(_allocator, _se_3) catch @panic("OOM"); } break :blk092_3 _ll_3; };
// zbr:test/type_infer_chain_test.zbr:72
    const mid_row = rows.items[@intCast(1)];
// zbr:test/type_infer_chain_test.zbr:73
    const mid_parts: std.ArrayList([]const u8) = blk092_4: { var _ll_4: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_4 = std.mem.splitSequence(u8, mid_row, " "); while (_split_iter_4.next()) |_se_4| { _ll_4.append(_allocator, _se_4) catch @panic("OOM"); } break :blk092_4 _ll_4; };
// zbr:test/type_infer_chain_test.zbr:74
    if (!((@as(i64, @intCast(mid_parts.items.len)) == 2))) @panic("nested split count: expected 2");
// zbr:test/type_infer_chain_test.zbr:75
    if (!(std.mem.eql(u8, mid_parts.items[@intCast(0)], "c"))) @panic("nested split at(0): expected 'c'");
    std.debug.print("{s}\n", .{"type_infer_chain: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Box) return _reflect_Box_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Box) return .{ .items = _reflect_Box_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Box) return .{ .items = _reflect_Box_field_types };
    return .{ .items = &.{} };
}
