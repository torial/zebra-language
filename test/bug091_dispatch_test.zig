...
// Source: test/bug091_dispatch_test.zbr
...
pub const Filler = struct {
    _type_tag: u64 = _ttag_Filler,
    pub fn helperList(self: *Filler, out: *std.ArrayList([]const u8)) void {
        _ = self;
        out.append(_allocator, _intern("a")) catch @panic("OOM");
        out.append(_allocator, _intern("b")) catch @panic("OOM");
    }

    pub fn runSelf(self: *Filler) void {
// zbr:test/bug091_dispatch_test.zbr:12
        var items = std.ArrayList([]const u8){};
        self.helperList(&items);
// zbr:test/bug091_dispatch_test.zbr:14
        if (!((@as(i64, @intCast(items.items.len)) == 2))) @panic("self count");
    }

    pub fn init() *Filler {
        const self = _allocator.create(Filler) catch @panic("OOM");
        self._type_tag = _ttag_Filler;
        return self;
    }

};
const _ttag_Filler: u64 = _zbr_hash("Filler");
const _reflect_Filler_name: []const u8 = "Filler";
const _reflect_Filler_fields: []const []const u8 = &.{};
const _reflect_Filler_field_types: []const []const u8 = &.{};

pub fn main() void {
// zbr:test/bug091_dispatch_test.zbr:17
    var f = Filler.init();
    f.runSelf();
// zbr:test/bug091_dispatch_test.zbr:21
    var items2 = std.ArrayList([]const u8){};
    f.helperList(&items2);
// zbr:test/bug091_dispatch_test.zbr:23
    if (!((@as(i64, @intCast(items2.items.len)) == 2))) @panic("instance count");
    std.debug.print("{s}\n", .{"ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Filler) return _reflect_Filler_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Filler) return .{ .items = _reflect_Filler_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Filler) return .{ .items = _reflect_Filler_field_types };
    return .{ .items = &.{} };
}
