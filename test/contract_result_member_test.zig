...
// Source: test/contract_result_member_test.zbr
...
pub const Main = struct {
    ...
    pub fn fmtSum(x: i64, y: i64) []const u8 {
        var _result: []const u8 = undefined;
        var _ensure_armed: bool = false;
        defer {
            if (_ensure_armed and !(_zebra_gt(_result.len, 0))) std.debug.panic("ensure failed in 'fmtSum'\n", .{});
            if (_ensure_armed and !(std.mem.startsWith(u8, _result, "sum"))) std.debug.panic("ensure failed in 'fmtSum'\n", .{});
        }
// zbr:test/contract_result_member_test.zbr:11
        { _result = (std.fmt.allocPrint(_allocator, "sum={}+{}", .{x, y}) catch @panic("OOM")); _ensure_armed = true; return _result; }
        _ensure_armed = true;
    }

    pub fn negate(n: i64) i64 {
        var _result: i64 = undefined;
        var _ensure_armed: bool = false;
        defer {
            if (_ensure_armed and !((_result == (-n)))) std.debug.panic("ensure failed in 'negate'\n", .{});
        }
// zbr:test/contract_result_member_test.zbr:16
        { _result = (-n); _ensure_armed = true; return _result; }
        _ensure_armed = true;
    }

    pub fn takeFirst(xs: std.ArrayList(i64)) i64 {
        var _result: i64 = undefined;
        var _ensure_armed: bool = false;
        defer {
            if (_ensure_armed and !((_result == xs.items[@as(usize, @intCast(0))]))) std.debug.panic("ensure failed in 'takeFirst'\n", .{});
        }
// zbr:test/contract_result_member_test.zbr:21
        { _result = xs.items[@as(usize, @intCast(0))]; _ensure_armed = true; return _result; }
        _ensure_armed = true;
    }

    pub fn main() void {
// zbr:test/contract_result_member_test.zbr:24
        std.debug.assert(std.mem.eql(u8, Main.fmtSum(2, 3), "sum=2+3"));
// zbr:test/contract_result_member_test.zbr:25
        std.debug.assert((Main.negate(7) == (-7)));
// zbr:test/contract_result_member_test.zbr:26
        var nums = std.ArrayList(i64){};
// zbr:test/contract_result_member_test.zbr:27
        nums.append(_allocator, 42) catch unreachable;
// zbr:test/contract_result_member_test.zbr:28
        nums.append(_allocator, 99) catch unreachable;
// zbr:test/contract_result_member_test.zbr:29
        std.debug.assert((Main.takeFirst(nums) == 42));
// zbr:test/contract_result_member_test.zbr:30
        std.debug.print("{s}\n", .{"contract_result_member_test OK"});
    }
