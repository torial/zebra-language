...
// Source: test/tc_iface_match_test.zbr
...
pub const Printable = struct {
    ...
    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "describe")) @compileError("type " ++ @typeName(T) ++ " does not implement Printable.describe");
        }
    }
};
...
pub const Dog = struct {
    ...
    pub fn describe(self: *Dog) []const u8 {
        _ = self;
// zbr:test/tc_iface_match_test.zbr:6
        return "Woof";
    }
...
fn _shim_Dog_Printable_describe(ptr: *anyopaque) []const u8 { return Dog.describe(@alignCast(@ptrCast(ptr))); }
const _vtable_Dog_Printable = Printable.VTable{ .describe = &_shim_Dog_Printable_describe, };
pub fn main() void {
// zbr:test/tc_iface_match_test.zbr:9
        var p: Printable = .{ .ptr = Dog.init(), .vtable = &_vtable_Dog_Printable };
// zbr:test/tc_iface_match_test.zbr:10
    std.debug.print("{s}\n", .{p.describe()});
}
