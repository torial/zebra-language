...
// Source: C:/Projects/zebra-language/test/hash_test.zbr
...
pub const HashTest = struct {
    ...
    pub fn init() *HashTest {
        const self = _allocator.create(HashTest) catch @panic("OOM");
        self._type_tag = _ttag_HashTest;
        return self;
    }

};
const _ttag_HashTest: u64 = _zbr_hash("HashTest");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_HashTest) return _reflect_HashTest_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_HashTest) return .{ .items = _reflect_HashTest_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_HashTest) return .{ .items = _reflect_HashTest_field_types };
    return .{ .items = &.{} };
}
