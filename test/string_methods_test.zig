...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/string_methods_test.zbr:3
    const greeting: []const u8 = "hello";
    std.debug.print("{any}\n", .{@as(i64, @intCast(greeting.len))});
// zbr:test/string_methods_test.zbr:7
    const padded: []const u8 = "  hi  ";
    std.debug.print("{s}\n", .{std.mem.trimStart(u8, padded, &std.ascii.whitespace)});
    std.debug.print("{s}\n", .{std.mem.trimEnd(u8, padded, &std.ascii.whitespace)});
// zbr:test/string_methods_test.zbr:12
    const empty: []const u8 = "";
    std.debug.print("{}\n", .{(empty.len == 0)});
    std.debug.print("{}\n", .{(greeting.len == 0)});
// zbr:test/string_methods_test.zbr:17
    const sentence: []const u8 = "banana";
    std.debug.print("{}\n", .{@as(i64, @intCast(std.mem.count(u8, sentence, "an")))});
// zbr:test/string_methods_test.zbr:21
    var words = std.ArrayList([]const u8).empty;
    words.append(_allocator, _intern("one")) catch @panic("OOM");
    words.append(_allocator, _intern("two")) catch @panic("OOM");
    words.append(_allocator, _intern("three")) catch @panic("OOM");
    std.debug.print("{s}\n", .{(std.mem.join(_allocator, words, ", ".items) catch @panic("OOM"))});
// zbr:test/string_methods_test.zbr:28
    const text: []const u8 = "line1\nline2\nline3";
// zbr:test/string_methods_test.zbr:29
    {
...
// zbr:test/string_methods_test.zbr:36
...
// zbr:test/string_methods_test.zbr:44
    const alpha: []const u8 = "hello";
// zbr:test/string_methods_test.zbr:45
...
// zbr:test/string_methods_test.zbr:46
    ...
    std.debug.print("{}\n", .{(blk_in: { if (mixed.len == 0) break :blk_in false; for (mixed) |_nc| { if (!std.ascii.isDigit(_nc)) break :blk_in false; } break :blk_in true; })});
}
