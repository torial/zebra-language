...
pub const TimerTest = struct {
    ...
    pub fn main() void {
        var t = _timer_start();
        const ms = t.elapsed();
        std.debug.assert((ms >= 0.0));
        std.debug.print("{s}\n", .{"elapsed: OK"});
        const us = t.elapsedMicros();
        std.debug.assert((us >= 0));
        std.debug.print("{s}\n", .{"elapsedMicros: OK"});
        t.reset();
        const ms2 = t.elapsed();
        std.debug.assert((ms2 >= 0.0));
        std.debug.print("{s}\n", .{"reset: OK"});
        var t1 = _timer_start();
        var t2 = _timer_start();
        const e1 = t1.elapsed();
        const e2 = t2.elapsed();
        std.debug.assert((e1 >= 0.0));
        std.debug.assert((e2 >= 0.0));
        std.debug.print("{s}\n", .{"independent timers: OK"});
        std.debug.print("{s}\n", .{"All timer tests passed."});
    }

    pub fn init() *TimerTest {
        const self = _allocator.create(TimerTest) catch @panic("OOM");
        self._type_tag = _ttag_TimerTest;
        return self;
    }

};
const _ttag_TimerTest: u64 = _zbr_hash("TimerTest");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_TimerTest) return _reflect_TimerTest_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_TimerTest) return .{ .items = _reflect_TimerTest_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_TimerTest) return .{ .items = _reflect_TimerTest_field_types };
    return .{ .items = &.{} };
}
