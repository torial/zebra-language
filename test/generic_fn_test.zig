...
// Source: test/generic_fn_test.zbr
...
// zbr:test/generic_fn_test.zbr:2
    return x;
}

pub fn clamp(comptime T: type, v: T, lo: T, hi: T) T {
// zbr:test/generic_fn_test.zbr:5
    if (_zebra_lt(v, lo)) {
// zbr:test/generic_fn_test.zbr:6
        return lo;
    }
// zbr:test/generic_fn_test.zbr:7
    if (_zebra_gt(v, hi)) {
// zbr:test/generic_fn_test.zbr:8
        return hi;
    }
// zbr:test/generic_fn_test.zbr:9
    return v;
}

pub fn first(comptime T: type, a: T, b: T) T {
    _ = b;
// zbr:test/generic_fn_test.zbr:12
    return a;
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
    std.debug.print("{}\n", .{identity(i64, 42)});
    std.debug.print("{s}\n", .{identity([]const u8, "hi")});
    std.debug.print("{}\n", .{clamp(i64, 5, 0, 10)});
    std.debug.print("{}\n", .{clamp(i64, (-3), 0, 10)});
    std.debug.print("{}\n", .{clamp(i64, 15, 0, 10)});
    std.debug.print("{}\n", .{first(i64, 7, 99)});
    std.debug.print("{s}\n", .{first([]const u8, "a", "b")});
// zbr:test/generic_fn_test.zbr:22
    const x: i64 = (identity(i64, 7) + 1);
    std.debug.print("{}\n", .{x});
    std.debug.print("{s}\n", .{"generic functions: OK"});
}
