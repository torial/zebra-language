...
// zbr:test/hashmap_remove_test.zbr:5
    ...
    m.put(_intern("a"), 1) catch @panic("OOM");
    m.put(_intern("b"), 2) catch @panic("OOM");
    m.put(_intern("c"), 3) catch @panic("OOM");
// zbr:test/hashmap_remove_test.zbr:9
    if (!((@as(i64, @intCast(m.count())) == 3))) @panic("initial count");
    _ = m.remove("b");
// zbr:test/hashmap_remove_test.zbr:12
    if (!((@as(i64, @intCast(m.count())) == 2))) @panic("count after remove");
// zbr:test/hashmap_remove_test.zbr:13
    if (!((!m.contains("b")))) @panic("b removed");
// zbr:test/hashmap_remove_test.zbr:14
    if (!(m.contains("a"))) @panic("a still present");
// zbr:test/hashmap_remove_test.zbr:15
    if (!(m.contains("c"))) @panic("c still present");
    _ = m.remove("z");
// zbr:test/hashmap_remove_test.zbr:19
    if (!((@as(i64, @intCast(m.count())) == 2))) @panic("count unchanged after no-op remove");
// zbr:test/hashmap_remove_test.zbr:22
    var tags = std.StringHashMap([]const u8).init(_allocator);
    tags.put(_intern("lang"), _intern("zebra")) catch @panic("OOM");
    tags.put(_intern("tier"), _intern("alpha")) catch @panic("OOM");
// zbr:test/hashmap_remove_test.zbr:25
    if (!((@as(i64, @intCast(tags.count())) == 2))) @panic("str-str count");
    _ = tags.remove("tier");
// zbr:test/hashmap_remove_test.zbr:27
    if (!((@as(i64, @intCast(tags.count())) == 1))) @panic("str-str count after remove");
// zbr:test/hashmap_remove_test.zbr:28
    if (!(tags.contains("lang"))) @panic("lang still there");
    std.debug.print("{s}\n", .{"ok"});
}
