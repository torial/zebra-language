...
pub fn main() void {
// zbr:test/dir_walk_test.zbr:4
    const files = (blk_dw: {
        const _dw_root = "examples";
...
// zbr:test/dir_walk_test.zbr:5
    var found = false;
// zbr:test/dir_walk_test.zbr:6
    for (files.items) |f| {
// zbr:test/dir_walk_test.zbr:7
        if (std.mem.endsWith(u8, f, ".zbr")) {
// zbr:test/dir_walk_test.zbr:8
            found = true;
        }
    }
// zbr:test/dir_walk_test.zbr:9
    if (!(found)) @panic("Dir.walk should find at least one .zbr file under examples/");
// zbr:test/dir_walk_test.zbr:12
    var all_fwd = true;
// zbr:test/dir_walk_test.zbr:13
    for (files.items) |f| {
// zbr:test/dir_walk_test.zbr:14
        if ((std.mem.indexOf(u8, f, "\\") != null)) {
// zbr:test/dir_walk_test.zbr:15
            all_fwd = false;
        }
    }
// zbr:test/dir_walk_test.zbr:16
    if (!(all_fwd)) @panic("Dir.walk paths should use '/' separators, not '\\'");
// zbr:test/dir_walk_test.zbr:19
    var all_rooted = true;
// zbr:test/dir_walk_test.zbr:20
    for (files.items) |f| {
// zbr:test/dir_walk_test.zbr:21
        if ((!std.mem.startsWith(u8, f, "examples/"))) {
// zbr:test/dir_walk_test.zbr:22
            all_rooted = false;
        }
    }
// zbr:test/dir_walk_test.zbr:23
    if (!(all_rooted)) @panic("Dir.walk paths should be prefixed with the root argument");
    std.debug.print("{s}\n", .{_str_concat(_str_concat("dir_walk_test: ok (", (std.fmt.allocPrint(_allocator, "{}", .{@as(i64, @intCast(files.items.len))}) catch unreachable), _allocator), " files found)", _allocator)});
}
