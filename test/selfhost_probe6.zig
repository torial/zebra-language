...
pub fn test_multiline() void {
    const s = preamble();
    std.debug.assert(s.contains("const std"));
    std.debug.assert(s.contains("pub fn main"));
    std.debug.print("{s}\n", .{"1. multiline """: OK"});
}
...
pub const GenCtx = struct {
    ...
    pub fn init(owner: []const u8, indent_: i64, throws_: bool) GenCtx {
        var _self: GenCtx = undefined;
            _self.owner = owner;
            _self.indent_ = indent_;
            _self.throws_ = throws_;
        return _self;
    }

    pub fn withOwner(self: *GenCtx, new_owner: []const u8) GenCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.owner = new_owner; break :blk _except_tmp; };
        return copy;
    }

    pub fn withIndent(self: *GenCtx, delta: i64) GenCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.indent_ = (self.indent_ + delta); break :blk _except_tmp; };
        return copy;
    }

    pub fn withThrows(self: *GenCtx, t: bool) GenCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.throws_ = t; break :blk _except_tmp; };
        return copy;
    }
...
pub fn test_string_builder() void {
    ...
    const result = (b.toOwnedSlice(_allocator) catch "");
    std.debug.assert(result.contains("pub fn init"));
    std.debug.assert(result.contains("void"));
    std.debug.print("{s}\n", .{"3. StringBuilder: OK"});
}
...
pub fn test_dispatch_chain() void {
    ...
    std.debug.assert(dispatchMethod("List", "count").contains("intCast"));
    std.debug.assert(std.mem.eql(u8, dispatchMethod("HashMap", "has"), "map.contains"));
    std.debug.assert(dispatchMethod("str", "startsWith").contains("startsWith"));
...
pub const EmitCtx = struct {
    ...
    pub fn write_(self: *EmitCtx, s: []const u8) EmitCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.buf_ = _str_concat(self.buf_, s, _allocator); break :blk _except_tmp; };
        return copy;
    }

    pub fn indented(self: *EmitCtx) EmitCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.depth_ = (self.depth_ + 1); break :blk _except_tmp; };
        return copy;
    }
    ...
    pub fn writeLine(self: *EmitCtx, s: []const u8) EmitCtx {
        const pad = (blk_rep: { var _rep = std.ArrayList([]const u8){}; defer _rep.deinit(_allocator); var _ri: i64 = 0; while (_ri < self.depth_) : (_ri += 1) _rep.append(_allocator, "    ") catch unreachable; break :blk_rep std.mem.concat(_allocator, u8, _rep.items) catch unreachable; });
        const copy = blk: { var _except_tmp = self.*; _except_tmp.buf_ = _str_concat(_str_concat(_str_concat(self.buf_, pad, _allocator), s, _allocator), "\n", _allocator); break :blk _except_tmp; };
        return copy;
    }
...
pub const SubCtx = struct {
    ...
    pub fn sub(self: *SubCtx, extra: i64) SubCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.indent_ = (self.indent_ + extra); _except_tmp.out_ = ""; break :blk _except_tmp; };
        return copy;
    }

    pub fn emit_(self: *SubCtx, s: []const u8) SubCtx {
        const copy = blk: { var _except_tmp = self.*; _except_tmp.out_ = _str_concat(self.out_, s, _allocator); break :blk _except_tmp; };
        return copy;
    }
