...
// Source: test/stdlib_str_test.zbr
...
pub fn main() void {
// zbr:test/stdlib_str_test.zbr:2
    const s = "Hello World";
// zbr:test/stdlib_str_test.zbr:5
    std.debug.assert(std.ascii.eqlIgnoreCase(s, "hello world"));
// zbr:test/stdlib_str_test.zbr:6
    std.debug.assert((!std.ascii.eqlIgnoreCase(s, "hello")));
// zbr:test/stdlib_str_test.zbr:9
...
// zbr:test/stdlib_str_test.zbr:10
    std.debug.assert((!(blk: { if ("abc!".len == 0) break :blk false; for ("abc!") |_an| { if (!std.ascii.isAlphanumeric(_an)) break :blk false; } break :blk true; })));
// zbr:test/stdlib_str_test.zbr:11
...
// zbr:test/stdlib_str_test.zbr:14
    std.debug.assert(std.ascii.startsWithIgnoreCase(s, "hello"));
// zbr:test/stdlib_str_test.zbr:15
    std.debug.assert(std.ascii.endsWithIgnoreCase(s, "WORLD"));
// zbr:test/stdlib_str_test.zbr:16
    std.debug.assert((std.ascii.indexOfIgnoreCase(s, "lo wo") != null));
// zbr:test/stdlib_str_test.zbr:17
    std.debug.assert((!(std.ascii.indexOfIgnoreCase(s, "xyz") != null)));
// zbr:test/stdlib_str_test.zbr:20
    const idx = (if (std.ascii.indexOfIgnoreCase(s, "world")) |_ii| @as(i64, @intCast(_ii)) else @as(i64, -1));
// zbr:test/stdlib_str_test.zbr:21
    std.debug.assert((idx == 6));
// zbr:test/stdlib_str_test.zbr:22
    const idx2 = (if (std.ascii.indexOfIgnoreCase(s, "xyz")) |_ii| @as(i64, @intCast(_ii)) else @as(i64, -1));
// zbr:test/stdlib_str_test.zbr:23
    std.debug.assert((idx2 == (-1)));
// zbr:test/stdlib_str_test.zbr:26
    const rep = "aababc";
// zbr:test/stdlib_str_test.zbr:27
    std.debug.assert(((if (std.mem.lastIndexOf(u8, rep, "ab")) |_li| @as(i64, @intCast(_li)) else @as(i64, -1)) == 3));
// zbr:test/stdlib_str_test.zbr:30
    const iof = (if (std.mem.indexOfPos(u8, "hello hello", @intCast(1), "hello")) |_if| @as(i64, @intCast(_if)) else @as(i64, -1));
// zbr:test/stdlib_str_test.zbr:31
    std.debug.assert((iof == 6));
// zbr:test/stdlib_str_test.zbr:34
    std.debug.assert(((std.fmt.parseInt(i64, "ff", @intCast(16)) catch 0) == 255));
// zbr:test/stdlib_str_test.zbr:35
    std.debug.assert(((std.fmt.parseInt(i64, "10", @intCast(2)) catch 0) == 2));
// zbr:test/stdlib_str_test.zbr:36
    std.debug.assert(((std.fmt.parseInt(i64, "77", @intCast(8)) catch 0) == 63));
// zbr:test/stdlib_str_test.zbr:39
    const tokens = (blk_tok: { var _tok_it = std.mem.tokenizeSequence(u8, "one  two   three", " "); var _tok_list = std.ArrayList([]const u8){}; while (_tok_it.next()) |_tok_t| { _tok_list.append(_allocator, _tok_t) catch unreachable; } break :blk_tok _tok_list; });
// zbr:test/stdlib_str_test.zbr:40
    std.debug.assert((@as(i64, @intCast(tokens.items.len)) == 3));
// zbr:test/stdlib_str_test.zbr:42
    std.debug.print("{s}\n", .{"stdlib_str OK"});
}
