...
// Source: C:/Projects/zebra-language/test/raise_auto.zbr
...
pub const App = struct {
    ...
    pub fn risky(x: i64) anyerror!i64 {
        if (_zebra_lt(x, 0)) {
            { const _rdet_1: []const u8 = std.fmt.allocPrint(_allocator, "{}", .{x}) catch @panic("OOM");
              ...
              _error_ctx = .{ .message = "negative value", .details = .{ .ptr = @ptrCast(_rdet_ptr_1), .toString_fn = _rshim_1.call } };
    ...
    pub fn main() void {
        var result: i64 = 0;
        {
            ...
            _ = &_try_err;
            _try_blk: {
                result = App.risky(5) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                break :_try_blk;
            }
            if (_try_err != null) {
                result = (-1);
