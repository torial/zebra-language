...
// Source: C:/Projects/zebra-language/test/tcp_advanced_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        var conn = _tcp_connect("httpbin.org", 80);
        if ((conn != null)) {
            conn.write("GET /get HTTP/1.0\r\nHost: httpbin.org\r\n\r\n");
            const status_line: []const u8 = conn.readLine();
            conn.close();
            std.debug.print("{}\n", .{std.mem.startsWith(u8, status_line, "HTTP/1")});
            std.debug.print("{}\n", .{_zebra_gt(status_line.len, 0)});
        }
        var conn2 = _tcp_connect("httpbin.org", 80);
        if ((conn2 != null)) {
            conn2.write("GET /get HTTP/1.0\r\nHost: httpbin.org\r\n\r\n");
            const chunk: []const u8 = conn2.readBytes(12);
            conn2.close();
