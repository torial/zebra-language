...
pub const PartialTest = struct {
    ...
    pub fn init() *PartialTest {
        const self = _allocator.create(PartialTest) catch @panic("OOM");
        self._type_tag = _ttag_PartialTest;
        return self;
    }

};
const _ttag_PartialTest: u64 = _zbr_hash("PartialTest");
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        if (!(std.mem.eql(u8, g, "Hello, Alice"))) @panic("greet works");
        const d = PartialTest.describe("Bob", 42);
        if (!(std.mem.eql(u8, d, "Bob: 42"))) @panic("describe from .meta partial");
        const s = PartialTest.score(10, 3);
        if (!((s == 7))) @panic("score from .math partial");
