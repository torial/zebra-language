...
// zbr:test/typechecker_test.zbr:41
        ...
        stmts_ok.append(_allocator, TcStmt{ .return_ = TcStmtReturn.init(TcExpr{ .int_lit = {} }) }) catch @panic("OOM");
        tc.checkMethod(TcType{ .int_ = {} }, stmts_ok);
// zbr:test/typechecker_test.zbr:44
        if (!(tc.ok())) @panic("int method returning int should be valid");
        std.debug.print("{s}\n", .{"4. checkMethod valid int return OK"});
// zbr:test/typechecker_test.zbr:48
        var stmts_bad = std.ArrayList(TcStmt){};
        stmts_bad.append(_allocator, TcStmt{ .return_ = TcStmtReturn.init(TcExpr{ .string_lit = {} }) }) catch @panic("OOM");
        tc.checkMethod(TcType{ .int_ = {} }, stmts_bad);
// zbr:test/typechecker_test.zbr:51
        if (!((!tc.ok()))) @panic("int method returning string should have error");
...
// zbr:test/typechecker_test.zbr:60
        ...
        stmts_void.append(_allocator, TcStmt{ .return_void = {} }) catch @panic("OOM");
...
// zbr:test/typechecker_test.zbr:67
        ...
        stmts_if.append(_allocator, TcStmt{ .if_ = TcStmtIf.init(TcExpr{ .int_lit = {} }) }) catch @panic("OOM");
...
// zbr:test/typechecker_test.zbr:77
        ...
        stmts_assert.append(_allocator, TcStmt{ .assert_ = TcStmtAssert.init(TcExpr{ .binary = blk_box_1: { const _bv: std.meta.Child(@FieldType(TcExpr, "binary")) = bi_cmp; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_1 _bp; } }) }) catch @panic("OOM");
...
// zbr:test/typechecker_test.zbr:84
        ...
        stmts_multi.append(_allocator, TcStmt{ .var_decl = TcStmtVarDecl.init("a", TcType{ .int_ = {} }, TcExpr{ .string_lit = {} }) }) catch @panic("OOM");
        stmts_multi.append(_allocator, TcStmt{ .var_decl = TcStmtVarDecl.init("b", TcType{ .bool_ = {} }, TcExpr{ .float_lit = {} }) }) catch @panic("OOM");
        stmts_multi.append(_allocator, TcStmt{ .pass_ = {} }) catch @panic("OOM");
...
// zbr:test/typechecker_test.zbr:95
        std.debug.assert(std.mem.eql(u8, tc.typeName(TcType{ .optional = blk_box_2: { const _bv: std.meta.Child(@FieldType(TcType, "optional")) = TcType{ .int_ = {} }; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_2 _bp; } }), "?int"));
// zbr:test/typechecker_test.zbr:96
        std.debug.assert((blk_in: { if (tc.len == 0) break :blk_in false; for (tc) |_nc| { if (!std.ascii.isDigit(_nc)) break :blk_in false; } break :blk_in true; }));
...
// zbr:test/typechecker_test.zbr:106
        ...
        stmts_inner.append(_allocator, TcStmt{ .return_ = TcStmtReturn.init(TcExpr{ .int_lit = {} }) }) catch @panic("OOM");
...
fn _zbr_error_msg() []const u8 {
    ...
    if (@import("tc_check.zig")._error_ctx.message.len > 0) return @import("tc_check.zig")._error_ctx.message;
