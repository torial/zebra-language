...
// zbr:test/typechecker_test.zbr:77
        ...
        stmts_assert.append(_allocator, TcStmt{ .assert_ = TcStmtAssert.init(TcExpr{ .binary = blk_box: { const _bv: std.meta.Child(@FieldType(TcExpr, "binary")) = bi_cmp; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box _bp; } }) }) catch @panic("OOM");
...
// zbr:test/typechecker_test.zbr:95
        std.debug.assert(std.mem.eql(u8, tc.typeName(TcType{ .optional = blk_box: { const _bv: std.meta.Child(@FieldType(TcType, "optional")) = TcType{ .int_ = {} }; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box _bp; } }), "?int"));
