...
// Source: test/list_index_test.zbr
...
pub const Holder = struct {
    _type_tag: u64 = _ttag_Holder,
    xs: std.ArrayList(i64),
    pub fn init() *Holder {
        const _self = _allocator.create(Holder) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Holder");
// zbr:test/list_index_test.zbr:8
            _self.xs = std.ArrayList(i64).empty;
        return _self;
    }

    pub fn add_(self: *Holder, v: i64) void {
        self.xs.append(_allocator, v) catch @panic("OOM");
    }

    pub fn at(self: *Holder, i: i64) i64 {
// zbr:test/list_index_test.zbr:12
        return self.xs.items[@as(usize, @intCast(i))];
    }

};
const _ttag_Holder: u64 = _zbr_hash("Holder");
const _reflect_Holder_name: []const u8 = "Holder";
const _reflect_Holder_fields: []const []const u8 = &.{"xs"};
const _reflect_Holder_field_types: []const []const u8 = &.{"List(int)"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/list_index_test.zbr:15
...
// zbr:test/list_index_test.zbr:19
    const s: []const u8 = "hello";
// zbr:test/list_index_test.zbr:20
    var h = Holder.init();
    h.add_(99);
    h.add_(88);
    std.debug.print("{s}\n", .{_str_concat("local=", (std.fmt.allocPrint(_allocator, "{}", .{nums.items[@as(usize, @intCast(1))]}) catch unreachable), _allocator)});
    std.debug.print("{s}\n", .{_str_concat("field=", (std.fmt.allocPrint(_allocator, "{}", .{h.at(0)}) catch unreachable), _allocator)});
    std.debug.print("{s}\n", .{_str_concat("str=", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(s[@as(usize, @intCast(1))]), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator)});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Holder) return _reflect_Holder_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Holder) return .{ .items = _reflect_Holder_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Holder) return .{ .items = _reflect_Holder_field_types };
    return .{ .items = &.{} };
}
