...
// Source: test/bug119_list_field_param_test.zbr
...
pub const State = struct {
    _type_tag: u64 = _ttag_State,
    items: std.ArrayList(i64) = std.ArrayList(i64).empty,
    pub fn init() *State {
        const _self = _allocator.create(State) catch @panic("OOM");
        _self._type_tag = _zbr_hash("State");
            _self.items = std.ArrayList(i64).empty;
            // pass
        return _self;
    }

};
const _ttag_State: u64 = _zbr_hash("State");
const _reflect_State_name: []const u8 = "State";
const _reflect_State_fields: []const []const u8 = &.{"items"};
const _reflect_State_field_types: []const []const u8 = &.{"List(int)"};

pub fn itemCount(s: *State) i64 {
// zbr:test/bug119_list_field_param_test.zbr:10
...
// zbr:test/bug119_list_field_param_test.zbr:13
    const st: *State = State.init();
    st.items.append(_allocator, 10) catch @panic("OOM");
    st.items.append(_allocator, 20) catch @panic("OOM");
    st.items.append(_allocator, 30) catch @panic("OOM");
// zbr:test/bug119_list_field_param_test.zbr:17
    const n: i64 = itemCount(st);
// zbr:test/bug119_list_field_param_test.zbr:18
    std.debug.assert((n == 3));
    std.debug.print("{s}\n", .{"bug119_list_field_param: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_State) return _reflect_State_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_State) return .{ .items = _reflect_State_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_State) return .{ .items = _reflect_State_field_types };
    return .{ .items = &.{} };
}
