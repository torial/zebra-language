...
// Source: C:/Projects/zebra-language/test/extend_test.zbr
...
fn _ext_String_shout(self: []const u8) []const u8 {
    return (std.mem.concat(_allocator, u8, &.{ (std.ascii.allocUpperString(_allocator, self) catch unreachable), "!" }) catch unreachable);
}
fn _ext_String_isLong(self: []const u8) bool {
    return _zebra_gt(self.len, 5);
}
fn _ext_String_greet(self: []const u8) []const u8 {
    return (std.mem.concat(_allocator, u8, &.{ (std.mem.concat(_allocator, u8, &.{ "Hello, ", self }) catch unreachable), "!" }) catch unreachable);
}
