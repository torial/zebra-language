...
pub const Point = struct {
    ...
    pub fn toString(self: *const Point) []const u8 {
        return std.fmt.allocPrint(_allocator, "Point(x={}, y={})", .{self.x, self.y}) catch unreachable;
    }
    pub fn eql(self: *const Point, other: *const Point) bool {
        return (self.x == other.x) and (self.y == other.y);
    }
    pub fn hash(self: *const Point) u64 {
...
pub const Greeting = struct {
    ...
    count: i64 = undefined,
    pub fn init(name: []const u8, count: i64) Greeting {
        return .{ .name = name, .count = count };
    }

    pub fn toString(self: *const Greeting) []const u8 {
        return std.fmt.allocPrint(_allocator, "Greeting(name={s}, count={})", .{self.name, self.count}) catch unreachable;
    }
};
...
pub const Key = struct {
    ...
    pub fn init(id: i64, label: []const u8) Key {
        return .{ .id = id, .label = label };
    }

    pub fn eql(self: *const Key, other: *const Key) bool {
        return (self.id == other.id) and std.mem.eql(u8, self.label, other.label);
    }
    pub fn hash(self: *const Key) u64 {
        var hasher = std.hash.Wyhash.init(0);
        std.hash.autoHashStrat(&hasher, self.id, .Deep);
        std.hash.autoHashStrat(&hasher, self.label, .Deep);
        return hasher.final();
    }
};

pub fn main() void {
// zbr:test/derive_test.zbr:17
    const p1 = Point.init(3, 4);
// zbr:test/derive_test.zbr:18
    const p2 = Point.init(3, 4);
// zbr:test/derive_test.zbr:19
    const p3 = Point.init(1, 2);
// zbr:test/derive_test.zbr:22
    {
        const _pt0 = p1.toString();
        std.debug.print("{s}\n", .{_pt0});
    }
// zbr:test/derive_test.zbr:25
    if (p1.eql(&p2)) {
// zbr:test/derive_test.zbr:26
        std.debug.print("{s}\n", .{"p1 == p2: true"});
    }
// zbr:test/derive_test.zbr:27
    if (!p1.eql(&p3)) {
// zbr:test/derive_test.zbr:28
        std.debug.print("{s}\n", .{"p1 != p3: true"});
    }
// zbr:test/derive_test.zbr:31
    const g = Greeting.init("Alice", 5);
// zbr:test/derive_test.zbr:32
    {
        const _pt0 = g.toString();
        std.debug.print("{s}\n", .{_pt0});
    }
// zbr:test/derive_test.zbr:35
    const k1 = Key.init(42, "hello");
// zbr:test/derive_test.zbr:36
    const k2 = Key.init(42, "hello");
// zbr:test/derive_test.zbr:37
    const k3 = Key.init(99, "world");
// zbr:test/derive_test.zbr:38
    const h1 = k1.hash();
// zbr:test/derive_test.zbr:39
    const h2 = k2.hash();
// zbr:test/derive_test.zbr:40
    const h3 = k3.hash();
// zbr:test/derive_test.zbr:41
    if ((h1 == h2)) {
// zbr:test/derive_test.zbr:42
        std.debug.print("{s}\n", .{"hash consistent: ok"});
    }
// zbr:test/derive_test.zbr:43
    if ((h1 != h3)) {
// zbr:test/derive_test.zbr:44
        std.debug.print("{s}\n", .{"hash distinct: ok"});
    }
}
