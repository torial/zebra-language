...
const _ttag_DottedLib: u64 = _zbr_hash("DottedLib");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_DottedLib) return _reflect_DottedLib_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_DottedLib) return .{ .items = _reflect_DottedLib_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_DottedLib) return .{ .items = _reflect_DottedLib_field_types };
    return .{ .items = &.{} };
}
