...
// Source: C:/Projects/zebra-language/test/sys_run_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        argv.append(_allocator, "echo hello") catch @panic("OOM");
        const result: SysRunResult = _sys_run(argv);
        std.debug.print("{}\n", .{(result.exit_code == 0)});
        std.debug.print("{}\n", .{(std.mem.indexOf(u8, result.stdout, "hello") != null)});
        std.debug.print("{}\n", .{(result.stderr.len == 0)});
        var bad = std.ArrayList([]const u8){};
        bad.append(_allocator, "cmd") catch @panic("OOM");
        bad.append(_allocator, "/c") catch @panic("OOM");
        bad.append(_allocator, "exit 42") catch @panic("OOM");
