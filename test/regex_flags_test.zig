...
pub const Main = struct {
    ...
    pub fn main() void {
        var re_i = _regex_compile("hello", "i");
        std.debug.print("{}\n", .{re_i.match("Hello")});
        std.debug.print("{}\n", .{re_i.match("HELLO")});
        std.debug.print("{}\n", .{re_i.match("world")});
        var re_s = _regex_compile("a.b", "s");
        std.debug.print("{}\n", .{re_s.match("a\nb")});
        var re_no_s = _regex_compile("a.b", "");
        std.debug.print("{}\n", .{re_no_s.match("a\nb")});
        var re_m = _regex_compile("^\\w+", "m");
        std.debug.print("{}\n", .{re_m.find("hello\nworld")});
        var re_m2 = _regex_compile("\\w+$", "m");
        std.debug.print("{}\n", .{re_m2.find("hello\nworld")});
        var re_lazy = _regex_compile("<.*?>", "");
        std.debug.print("{}\n", .{re_lazy.find("<a><b>")});
        var re_greedy = _regex_compile("<.*>", "");
        std.debug.print("{}\n", .{re_greedy.find("<a><b>")});
        var re_lazy2 = _regex_compile("a+?", "");
        std.debug.print("{}\n", .{re_lazy2.find("aaa")});
        var re_greedy2 = _regex_compile("a+", "");
        std.debug.print("{}\n", .{re_greedy2.find("aaa")});
        var re_wb = _regex_compile("\\bcat\\b", "");
        std.debug.print("{}\n", .{re_wb.match("cat")});
        std.debug.print("{}\n", .{re_wb.find("catfish")});
        std.debug.print("{}\n", .{re_wb.find("my cat sat")});
        var re_wB = _regex_compile("\\Bcat\\B", "");
        std.debug.print("{}\n", .{re_wB.find("concatenate")});
        std.debug.print("{}\n", .{re_wB.find("my cat sat")});
        var re_big = _regex_compile("a{1,200}", "");
        std.debug.print("{}\n", .{re_big.match("a")});
        var re_u = _regex_compile("a{1,200}", "U");
        std.debug.print("{}\n", .{re_u.match("a")});
    }
