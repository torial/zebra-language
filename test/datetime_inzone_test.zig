...
// Source: test/datetime_inzone_test.zbr
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/datetime_inzone_test.zbr:3
    const epoch = _DateTime{ .epoch_ms = 0 };
// zbr:test/datetime_inzone_test.zbr:6
    const utc = _dt_in_zone(epoch, "UTC");
    std.debug.print("{any}\n", .{_dt_to_gregorian(utc.epoch_ms).year});
    std.debug.print("{any}\n", .{_dt_to_gregorian(utc.epoch_ms).month});
    std.debug.print("{any}\n", .{_dt_to_gregorian(utc.epoch_ms).day});
    std.debug.print("{any}\n", .{_dt_to_gregorian(utc.epoch_ms).hour});
// zbr:test/datetime_inzone_test.zbr:13
    const est = _dt_in_zone(epoch, "America/New_York");
    std.debug.print("{any}\n", .{_dt_to_gregorian(est.epoch_ms).year});
    std.debug.print("{any}\n", .{_dt_to_gregorian(est.epoch_ms).month});
    std.debug.print("{any}\n", .{_dt_to_gregorian(est.epoch_ms).day});
    std.debug.print("{any}\n", .{_dt_to_gregorian(est.epoch_ms).hour});
// zbr:test/datetime_inzone_test.zbr:20
    const jst = _dt_in_zone(epoch, "Asia/Tokyo");
    std.debug.print("{any}\n", .{_dt_to_gregorian(jst.epoch_ms).year});
    std.debug.print("{any}\n", .{_dt_to_gregorian(jst.epoch_ms).month});
    std.debug.print("{any}\n", .{_dt_to_gregorian(jst.epoch_ms).day});
    std.debug.print("{any}\n", .{_dt_to_gregorian(jst.epoch_ms).hour});
// zbr:test/datetime_inzone_test.zbr:27
    const summer = _dt_from_gregorian(2024, 7, 15, 12, 0, 0);
// zbr:test/datetime_inzone_test.zbr:28
    const paris_summer = _dt_in_zone(summer, "Europe/Paris");
    std.debug.print("{any}\n", .{_dt_to_gregorian(paris_summer.epoch_ms).hour});
// zbr:test/datetime_inzone_test.zbr:32
    const winter = _dt_from_gregorian(2024, 1, 15, 12, 0, 0);
// zbr:test/datetime_inzone_test.zbr:33
    const paris_winter = _dt_in_zone(winter, "Europe/Paris");
    std.debug.print("{any}\n", .{_dt_to_gregorian(paris_winter.epoch_ms).hour});
// zbr:test/datetime_inzone_test.zbr:37
    const july4 = _dt_from_gregorian(2024, 7, 4, 12, 0, 0);
// zbr:test/datetime_inzone_test.zbr:38
    const ny_summer = _dt_in_zone(july4, "America/New_York");
    std.debug.print("{any}\n", .{_dt_to_gregorian(ny_summer.epoch_ms).hour});
// zbr:test/datetime_inzone_test.zbr:42
    const unknown = _dt_in_zone(epoch, "Etc/Unknown");
    std.debug.print("{any}\n", .{_dt_to_gregorian(unknown.epoch_ms).year});
    std.debug.print("{any}\n", .{_dt_to_gregorian(unknown.epoch_ms).hour});
}
