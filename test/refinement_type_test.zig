...
// Source: test/refinement_type_test.zbr
...
pub const Range = struct {
    lo: i64,
    hi: i64,
    pub fn init(lo: i64, hi: i64) Range {
        return .{ .lo = lo, .hi = hi };
    }
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/refinement_type_test.zbr:17
    const score: i64 = 85;
    {
        const lo: i64 = 0;
        const hi: i64 = 100;
        const value = score;
        if (!((_zebra_ge(value, lo) and _zebra_le(value, hi)))) std.debug.panic("type constraint 'Bounded' failed\n", .{});
    }
// zbr:test/refinement_type_test.zbr:18
    const neg: i64 = (-20);
    {
        const lo: i64 = -50;
        const hi: i64 = 50;
...
// zbr:test/refinement_type_test.zbr:19
    const temp: i64 = 37;
    {
        const min: i64 = -273;
        const max: i64 = 1000;
        const value = temp;
        if (!((_zebra_ge(value, min) and _zebra_le(value, max)))) std.debug.panic("type constraint 'Temperature' failed\n", .{});
    }
// zbr:test/refinement_type_test.zbr:21
    const r: Range = Range{ .lo = 0, .hi = 10 };
    {
        const value = r;
        if (!(_zebra_lt(value.lo, value.hi))) std.debug.panic("type constraint 'ValidRange' failed\n", .{});
    }
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}", .{score}) catch unreachable)});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}", .{neg}) catch unreachable)});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}", .{temp}) catch unreachable)});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}", .{r.lo}) catch unreachable)});
}
