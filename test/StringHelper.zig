...
pub const StringHelper = struct {
    ...
    pub fn init() *StringHelper {
        const self = _allocator.create(StringHelper) catch @panic("OOM");
        self._type_tag = _ttag_StringHelper;
        return self;
    }

};
const _ttag_StringHelper: u64 = _zbr_hash("StringHelper");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_StringHelper) return _reflect_StringHelper_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_StringHelper) return .{ .items = _reflect_StringHelper_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_StringHelper) return .{ .items = _reflect_StringHelper_field_types };
    return .{ .items = &.{} };
}
