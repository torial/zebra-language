...
// Source: test/hashmap_init_patterns_test.zbr
...
pub const Config = struct {
    ...
    pub fn init(cache: std.StringHashMap([]const u8), tags: std.ArrayList([]const u8)) Config {
        return .{ .cache = cache, .tags = tags };
    }

};

pub const Config2 = struct {
    cache: std.StringHashMap([]const u8),
    tags: std.ArrayList([]const u8),
    pub fn init(cache: std.StringHashMap([]const u8), tags: std.ArrayList([]const u8)) Config2 {
        var _self: Config2 = undefined;
// zbr:test/hashmap_init_patterns_test.zbr:17
            _self.cache = cache;
// zbr:test/hashmap_init_patterns_test.zbr:18
            _self.tags = tags;
        return _self;
    }
...
pub fn getEmptyMap() std.StringHashMap([]const u8) {
// zbr:test/hashmap_init_patterns_test.zbr:22
    return std.StringHashMap([]const u8).init(_allocator);
}

pub fn getEmptyList() std.ArrayList([]const u8) {
// zbr:test/hashmap_init_patterns_test.zbr:25
    return std.ArrayList([]const u8){};
}

pub fn makeConfig1a() Config {
// zbr:test/hashmap_init_patterns_test.zbr:29
    return Config{ .cache = std.StringHashMap([]const u8).init(_allocator), .tags = std.ArrayList([]const u8){} };
}

pub fn makeConfig1b() Config2 {
// zbr:test/hashmap_init_patterns_test.zbr:33
    return Config2.init(std.StringHashMap([]const u8).init(_allocator), std.ArrayList([]const u8){});
}

pub fn makeConfig1bPos() Config2 {
// zbr:test/hashmap_init_patterns_test.zbr:37
    return Config2.init(std.StringHashMap([]const u8).init(_allocator), std.ArrayList([]const u8){});
}

pub fn main() void {
// zbr:test/hashmap_init_patterns_test.zbr:41
    const c1 = makeConfig1a();
// zbr:test/hashmap_init_patterns_test.zbr:42
    std.debug.assert((@as(i64, @intCast(c1.cache.count())) == 0));
// zbr:test/hashmap_init_patterns_test.zbr:43
    std.debug.assert((@as(i64, @intCast(c1.tags.items.len)) == 0));
// zbr:test/hashmap_init_patterns_test.zbr:46
    const c2 = makeConfig1b();
// zbr:test/hashmap_init_patterns_test.zbr:47
    std.debug.assert((@as(i64, @intCast(c2.cache.count())) == 0));
// zbr:test/hashmap_init_patterns_test.zbr:48
    std.debug.assert((@as(i64, @intCast(c2.tags.items.len)) == 0));
// zbr:test/hashmap_init_patterns_test.zbr:51
    const c3 = makeConfig1bPos();
// zbr:test/hashmap_init_patterns_test.zbr:52
    std.debug.assert((@as(i64, @intCast(c3.cache.count())) == 0));
// zbr:test/hashmap_init_patterns_test.zbr:55
    var m = std.StringHashMap([]const u8).init(_allocator);
    m.put(_intern("hello"), _intern("world")) catch @panic("OOM");
// zbr:test/hashmap_init_patterns_test.zbr:57
    std.debug.assert(m.contains("hello"));
// zbr:test/hashmap_init_patterns_test.zbr:58
    var lst = std.ArrayList([]const u8){};
    lst.append(_allocator, _intern("abc")) catch @panic("OOM");
// zbr:test/hashmap_init_patterns_test.zbr:60
    std.debug.assert((@as(i64, @intCast(lst.items.len)) == 1));
// zbr:test/hashmap_init_patterns_test.zbr:63
    var rm = getEmptyMap();
// zbr:test/hashmap_init_patterns_test.zbr:64
    std.debug.assert((@as(i64, @intCast(rm.count())) == 0));
    rm.put(_intern("k"), _intern("v")) catch @panic("OOM");
// zbr:test/hashmap_init_patterns_test.zbr:66
    std.debug.assert((@as(i64, @intCast(rm.count())) == 1));
// zbr:test/hashmap_init_patterns_test.zbr:67
    var rl = getEmptyList();
// zbr:test/hashmap_init_patterns_test.zbr:68
    std.debug.assert((@as(i64, @intCast(rl.items.len)) == 0));
    rl.append(_allocator, _intern("x")) catch @panic("OOM");
// zbr:test/hashmap_init_patterns_test.zbr:70
    std.debug.assert((@as(i64, @intCast(rl.items.len)) == 1));
    std.debug.print("{s}\n", .{"hashmap_init_patterns: all OK"});
}
