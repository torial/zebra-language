...
// Source: test/with_call_test.zbr
...
pub const Logger = struct {
    _type_tag: u64 = _ttag_Logger,
    log: std.ArrayList([]const u8),
    pub fn init() *Logger {
        const _self = _allocator.create(Logger) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Logger");
// zbr:test/with_call_test.zbr:8
            _self.log = std.ArrayList([]const u8).empty;
        return _self;
    }

    pub fn write(self: *Logger, msg: []const u8) void {
        self.log.append(_allocator, _intern(msg)) catch @panic("OOM");
    }

    pub fn count(self: *Logger) i64 {
// zbr:test/with_call_test.zbr:14
        return @as(i64, @intCast(self.log.items.len));
    }

};
const _ttag_Logger: u64 = _zbr_hash("Logger");
const _reflect_Logger_name: []const u8 = "Logger";
const _reflect_Logger_fields: []const []const u8 = &.{"log"};
const _reflect_Logger_field_types: []const []const u8 = &.{"List(str)"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/with_call_test.zbr:17
    var logger = Logger.init();
// zbr:test/with_call_test.zbr:19
    { // with
        logger.write("hello");
        logger.write("world");
    }
// zbr:test/with_call_test.zbr:24
    if (!((logger.count() == 2))) @panic("2 calls");
// zbr:test/with_call_test.zbr:25
    std.debug.assert(std.mem.eql(u8, logger.log.items[@intCast(0)], "hello"));
// zbr:test/with_call_test.zbr:26
    std.debug.assert(std.mem.eql(u8, logger.log.items[@intCast(1)], "world"));
// zbr:test/with_call_test.zbr:29
    var other = Logger.init();
// zbr:test/with_call_test.zbr:30
    { // with
        other.write("test");
    }
// zbr:test/with_call_test.zbr:33
    if (!((other.count() == 1))) @panic("1 call");
    std.debug.print("{s}\n", .{"with_call_test PASS"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Logger) return _reflect_Logger_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Logger) return .{ .items = _reflect_Logger_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Logger) return .{ .items = _reflect_Logger_field_types };
    return .{ .items = &.{} };
}
