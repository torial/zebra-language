...
// Source: test/bug093_strlen_test.zbr
...
pub fn main() void {
// zbr:test/bug093_strlen_test.zbr:6
    const s: []const u8 = "hello.md";
// zbr:test/bug093_strlen_test.zbr:9
    const n: i64 = @as(i64, @intCast(s.len));
// zbr:test/bug093_strlen_test.zbr:10
    if (!((n == 8))) @panic("length");
// zbr:test/bug093_strlen_test.zbr:13
    if (!(_zebra_gt(@as(i64, @intCast(s.len)), 3))) @panic("len > 3");
// zbr:test/bug093_strlen_test.zbr:14
    if (!((@as(i64, @intCast(s.len)) == 8))) @panic("len == 8");
// zbr:test/bug093_strlen_test.zbr:17
    const trimmed_len: i64 = (@as(i64, @intCast(s.len)) - 3);
// zbr:test/bug093_strlen_test.zbr:18
    if (!((trimmed_len == 5))) @panic("len - 3");
// zbr:test/bug093_strlen_test.zbr:21
    const stem: []const u8 = s[@as(usize, @intCast(0))..@as(usize, @intCast((@as(i64, @intCast(s.len)) - 3)))];
// zbr:test/bug093_strlen_test.zbr:22
    if (!(std.mem.eql(u8, stem, "hello"))) @panic("stem");
    std.debug.print("{s}\n", .{"ok"});
}
