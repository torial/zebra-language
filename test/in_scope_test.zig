...
pub const CountGroup = struct {
    _type_tag: u64 = _ttag_CountGroup,
    id: []const u8,
    entered: i64,
    exited: i64,
    pub fn init(id: []const u8) *CountGroup {
        const _self = _allocator.create(CountGroup) catch @panic("OOM");
        _self._type_tag = _zbr_hash("CountGroup");
// zbr:test/in_scope_test.zbr:10
            _self.id = _intern(id);
// zbr:test/in_scope_test.zbr:11
            _self.entered = 0;
// zbr:test/in_scope_test.zbr:12
            _self.exited = 0;
        return _self;
    }

    pub fn begin(self: *CountGroup) void {
// zbr:test/in_scope_test.zbr:15
        self.entered = (self.entered + 1);
    }

    pub fn end(self: *CountGroup) void {
// zbr:test/in_scope_test.zbr:18
        self.exited = (self.exited + 1);
    }

};
const _ttag_CountGroup: u64 = _zbr_hash("CountGroup");
const _reflect_CountGroup_name: []const u8 = "CountGroup";
const _reflect_CountGroup_fields: []const []const u8 = &.{"id", "entered", "exited"};
const _reflect_CountGroup_field_types: []const []const u8 = &.{"str", "int", "int"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/in_scope_test.zbr:21
    const outer = CountGroup.init("outer");
// zbr:test/in_scope_test.zbr:22
    const inner = CountGroup.init("inner");
// zbr:test/in_scope_test.zbr:24
    {
        const _in_1 = outer;
        _in_1.begin();
        defer _in_1.end();
// zbr:test/in_scope_test.zbr:25
        {
            const _in_2 = inner;
            _in_2.begin();
            defer _in_2.end();
...
// zbr:test/in_scope_test.zbr:28
    std.debug.assert((outer.entered == 1));
// zbr:test/in_scope_test.zbr:29
    std.debug.assert((outer.exited == 1));
// zbr:test/in_scope_test.zbr:30
    std.debug.assert((inner.entered == 1));
// zbr:test/in_scope_test.zbr:31
    std.debug.assert((inner.exited == 1));
    std.debug.print("{s}\n", .{"in_scope_test PASS"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_CountGroup) return _reflect_CountGroup_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_CountGroup) return .{ .items = _reflect_CountGroup_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_CountGroup) return .{ .items = _reflect_CountGroup_field_types };
    return .{ .items = &.{} };
}
