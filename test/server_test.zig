...
// zbr:test/server_test.zbr:8
        if (_tcp_connect("127.0.0.1", 8080)) |probe| {
            probe.close();
            std.debug.print("{s}\n", .{"Port 8080 is already in use; exiting."});
        } else {
...
// zbr:test/server_test.zbr:15
                if (std.mem.eql(u8, req.path, "/greet")) {
// zbr:test/server_test.zbr:16
                    const msg: []const u8 = (std.fmt.allocPrint(_allocator, "Hello, {} {s}", .{req.method, req.path}) catch @panic("OOM"));
// zbr:test/server_test.zbr:17
                    return HttpResponse{ .status = 200, .text = msg };
                }
// zbr:test/server_test.zbr:18
                if (std.mem.eql(u8, req.path, "/echo")) {
// zbr:test/server_test.zbr:19
                    return HttpResponse{ .status = 200, .text = req.content };
                }
// zbr:test/server_test.zbr:20
