...
// Source: test/try_parse_test.zbr
...
pub fn test_tryFloat_parses_number() anyerror!void {
// zbr:test/try_parse_test.zbr:8
    const v: ?f64 = (std.fmt.parseFloat(f64, "3.14") catch null);
// zbr:test/try_parse_test.zbr:9
    try _zebra_assert_bool((v != null), true);
// zbr:test/try_parse_test.zbr:10
    if (v) |f| {
// zbr:test/try_parse_test.zbr:11
        try _zebra_assert_cmp(f, 3.14, true);
    }
}

pub fn test_tryFloat_nil_on_nonnumber() anyerror!void {
// zbr:test/try_parse_test.zbr:14
    try _zebra_assert_bool(((std.fmt.parseFloat(f64, "car") catch null) == null), true);
// zbr:test/try_parse_test.zbr:15
    try _zebra_assert_bool(((std.fmt.parseFloat(f64, "+") catch null) == null), true);
// zbr:test/try_parse_test.zbr:16
    try _zebra_assert_bool(((std.fmt.parseFloat(f64, "") catch null) == null), true);
}

pub fn test_tryInt_parses_and_fails() anyerror!void {
// zbr:test/try_parse_test.zbr:19
    const n: ?i64 = (std.fmt.parseInt(i64, "42", 10) catch null);
// zbr:test/try_parse_test.zbr:20
    try _zebra_assert_bool((n != null), true);
// zbr:test/try_parse_test.zbr:21
    if (n) |i| {
// zbr:test/try_parse_test.zbr:22
        try _zebra_assert_cmp(i, 42, true);
    }
// zbr:test/try_parse_test.zbr:23
    try _zebra_assert_bool(((std.fmt.parseInt(i64, "xyz", 10) catch null) == null), true);
}

pub fn test_zero_is_a_value_not_failure() anyerror!void {
// zbr:test/try_parse_test.zbr:27
    try _zebra_assert_bool(((std.fmt.parseInt(i64, "0", 10) catch null) != null), true);
// zbr:test/try_parse_test.zbr:28
    if ((std.fmt.parseInt(i64, "0", 10) catch null)) |z| {
// zbr:test/try_parse_test.zbr:29
        try _zebra_assert_cmp(z, 0, true);
    }
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    if (test_tryFloat_parses_number()) |_| {
        _test_pass += 1;
        std.debug.print("PASS: test_tryFloat_parses_number\n", .{});
    } else |_terr| {
        _test_fail += 1;
        if (_terr == error.ZebraError) {
            std.debug.print("FAIL: test_tryFloat_parses_number: {s}\n", .{_zbr_error_msg()});
        } else {
            std.debug.print("FAIL: test_tryFloat_parses_number: {}\n", .{_terr});
        }
    }
    if (test_tryFloat_nil_on_nonnumber()) |_| {
        _test_pass += 1;
        std.debug.print("PASS: test_tryFloat_nil_on_nonnumber\n", .{});
    } else |_terr| {
        _test_fail += 1;
        if (_terr == error.ZebraError) {
            std.debug.print("FAIL: test_tryFloat_nil_on_nonnumber: {s}\n", .{_zbr_error_msg()});
        } else {
            std.debug.print("FAIL: test_tryFloat_nil_on_nonnumber: {}\n", .{_terr});
        }
    }
    if (test_tryInt_parses_and_fails()) |_| {
        _test_pass += 1;
        std.debug.print("PASS: test_tryInt_parses_and_fails\n", .{});
    } else |_terr| {
        _test_fail += 1;
        if (_terr == error.ZebraError) {
            std.debug.print("FAIL: test_tryInt_parses_and_fails: {s}\n", .{_zbr_error_msg()});
        } else {
            std.debug.print("FAIL: test_tryInt_parses_and_fails: {}\n", .{_terr});
        }
    }
    if (test_zero_is_a_value_not_failure()) |_| {
        _test_pass += 1;
        std.debug.print("PASS: test_zero_is_a_value_not_failure\n", .{});
    } else |_terr| {
        _test_fail += 1;
        if (_terr == error.ZebraError) {
            std.debug.print("FAIL: test_zero_is_a_value_not_failure: {s}\n", .{_zbr_error_msg()});
        } else {
            std.debug.print("FAIL: test_zero_is_a_value_not_failure: {}\n", .{_terr});
        }
    }
