...
// Source: test/json_parse_typed_test.zbr
...
const _ttag_Point: u64 = _zbr_hash("Point");
...
fn _json_parse_strict_Point(_src: []const u8) ?*Point {
    const _v = _json_parse(_src) orelse return null;
    if (!_json_is_object(_v)) return null;
    var _it = _v.object.iterator();
    while (_it.next()) |_e| {
        const _k = _e.key_ptr.*;
        const _known = std.mem.eql(u8, _k, "x") or std.mem.eql(u8, _k, "y");
        if (!_known) return null;
    }
    { const _x = _v.object.get("x") orelse return null;
      switch (_x) { .integer => {}, else => return null } }
    { const _x = _v.object.get("y") orelse return null;
      switch (_x) { .integer => {}, else => return null } }
    const _r = _allocator.create(Point) catch return null;
    _r.* = .{};
    _r._type_tag = _ttag_Point;
    _r.x = _json_get_int(_v, "x");
    _r.y = _json_get_int(_v, "y");
    return _r;
}
...
fn _json_parse_strict_Person(_src: []const u8) ?*Person {
    const _v = _json_parse(_src) orelse return null;
    if (!_json_is_object(_v)) return null;
    var _it = _v.object.iterator();
    while (_it.next()) |_e| {
        const _k = _e.key_ptr.*;
        const _known = std.mem.eql(u8, _k, "name") or std.mem.eql(u8, _k, "age");
        if (!_known) return null;
    }
    ...
    { const _x = _v.object.get("age") orelse return null;
      switch (_x) { .integer => {}, else => return null } }
    const _r = _allocator.create(Person) catch return null;
    _r.* = .{};
    _r._type_tag = _ttag_Person;
...
pub fn main() void {
// zbr:test/json_parse_typed_test.zbr:13
    const p = _json_parse_strict_Point("{\"x\":3,\"y\":7}");
// zbr:test/json_parse_typed_test.zbr:14
    if (p) |pt| {
        std.debug.print("{any}\n", .{pt.x});
        std.debug.print("{any}\n", .{pt.y});
    }
// zbr:test/json_parse_typed_test.zbr:18
    const q = _json_parse_strict_Person("{\"name\":\"Alice\",\"age\":30}");
// zbr:test/json_parse_typed_test.zbr:19
    if (q) |person| {
        std.debug.print("{s}\n", .{person.name});
        std.debug.print("{any}\n", .{person.age});
    }
// zbr:test/json_parse_typed_test.zbr:24
    const raw = _json_parse("{\"key\":\"val\"}");
// zbr:test/json_parse_typed_test.zbr:25
    if (raw) |rv| {
// zbr:test/json_parse_typed_test.zbr:26
        const s = rv.getString("key");
// zbr:test/json_parse_typed_test.zbr:27
        if (s) |sv| {
            std.debug.print("{any}\n", .{sv});
        }
    }
