...
// Source: C:/Projects/zebra-language/test/generic_constrained_test.zbr
...
pub const Score = struct {
    ...
    pub fn init(v: i64) *Score {
        const _self = _allocator.create(Score) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Score");
            _self.value = v;
        return _self;
    }
...
const _ttag_Score: u64 = _zbr_hash("Score");
...
pub const MinHeap = struct {
    ...
    best: T,
    pub fn init(seed: T) *MinHeap {
        const _self = _allocator.create(MinHeap) catch @panic("OOM");
        _self._type_tag = _zbr_hash("MinHeap");
            _self.best = seed;
        return _self;
    }

    pub fn top(self: *MinHeap) T {
        return self.best;
    }

};
const _ttag_MinHeap: u64 = _zbr_hash("MinHeap");
const _reflect_MinHeap_name: []const u8 = "MinHeap";
const _reflect_MinHeap_fields: []const []const u8 = &.{"best"};
const _reflect_MinHeap_field_types: []const []const u8 = &.{"T"};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
        var h = MinHeap.init(Score)(Score.init(42));
        std.debug.print("{any}\n", .{h.top().value});
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Score) return _reflect_Score_name;
    if (tag == _ttag_MinHeap) return _reflect_MinHeap_name;
    if (tag == _ttag_Main) return _reflect_Main_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Score) return .{ .items = _reflect_Score_fields };
    if (tag == _ttag_MinHeap) return .{ .items = _reflect_MinHeap_fields };
    if (tag == _ttag_Main) return .{ .items = _reflect_Main_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Score) return .{ .items = _reflect_Score_field_types };
    if (tag == _ttag_MinHeap) return .{ .items = _reflect_MinHeap_field_types };
