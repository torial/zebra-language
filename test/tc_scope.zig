...
pub const TcSymbol = struct {
    ...
    pub fn init(n: []const u8, k: TcSymKind, t: TcType) TcSymbol {
        var _self: TcSymbol = undefined;
// zbr:test/tc_scope.zbr:58
            _self.name = _intern(n);
// zbr:test/tc_scope.zbr:59
            _self.kind = k;
// zbr:test/tc_scope.zbr:60
            _self.type_ = t;
        return _self;
    }
...
pub const TcScope = struct {
    ...
    _names: std.ArrayList([]const u8),
    _syms: std.ArrayList(TcSymbol),
    _limits: std.ArrayList(i64),
    _kinds: std.ArrayList(TcScopeKind),
    pub fn init() *TcScope {
        const _self = _allocator.create(TcScope) catch @panic("OOM");
        _self._type_tag = _zbr_hash("TcScope");
// zbr:test/tc_scope.zbr:72
            _self._names = std.ArrayList([]const u8){};
// zbr:test/tc_scope.zbr:73
            _self._syms = std.ArrayList(TcSymbol){};
// zbr:test/tc_scope.zbr:74
            _self._limits = std.ArrayList(i64){};
// zbr:test/tc_scope.zbr:75
            _self._kinds = std.ArrayList(TcScopeKind){};
        return _self;
    }

    pub fn push(self: *TcScope, kind: TcScopeKind) void {
        self._limits.append(_allocator, @as(i64, @intCast(self._names.items.len))) catch @panic("OOM");
        self._kinds.append(_allocator, kind) catch @panic("OOM");
    }

    pub fn pop(self: *TcScope) void {
// zbr:test/tc_scope.zbr:84
        const frame_start = self._limits.items[@intCast((@as(i64, @intCast(self._limits.items.len)) - 1))];
// zbr:test/tc_scope.zbr:85
        while (_zebra_gt(@as(i64, @intCast(self._names.items.len)), frame_start)) {
            _ = self._names.orderedRemove(@intCast((@as(i64, @intCast(self._names.items.len)) - 1)));
            _ = self._syms.orderedRemove(@intCast((@as(i64, @intCast(self._syms.items.len)) - 1)));
        }
        _ = self._limits.orderedRemove(@intCast((@as(i64, @intCast(self._limits.items.len)) - 1)));
        _ = self._kinds.orderedRemove(@intCast((@as(i64, @intCast(self._kinds.items.len)) - 1)));
    }

    pub fn depth(self: *TcScope) i64 {
// zbr:test/tc_scope.zbr:93
        return @as(i64, @intCast(self._limits.items.len));
    }

    pub fn currentKind(self: *TcScope) TcScopeKind {
// zbr:test/tc_scope.zbr:97
        return self._kinds.items[@intCast((@as(i64, @intCast(self._kinds.items.len)) - 1))];
    }

    pub fn define(self: *TcScope, name: []const u8, sym: TcSymbol) bool {
// zbr:test/tc_scope.zbr:102
        const frame_start = self._limits.items[@intCast((@as(i64, @intCast(self._limits.items.len)) - 1))];
// zbr:test/tc_scope.zbr:103
        var i = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test/tc_scope.zbr:104
        while (_zebra_ge(i, frame_start)) {
// zbr:test/tc_scope.zbr:105
            if (std.mem.eql(u8, self._names.items[@intCast(i)], name)) {
// zbr:test/tc_scope.zbr:106
                return false;
            }
// zbr:test/tc_scope.zbr:107
            i = (i - 1);
        }
        self._names.append(_allocator, _intern(name)) catch @panic("OOM");
        self._syms.append(_allocator, sym) catch @panic("OOM");
// zbr:test/tc_scope.zbr:110
        return true;
    }

    pub fn hasLocal(self: *TcScope, name: []const u8) bool {
// zbr:test/tc_scope.zbr:114
        const frame_start = self._limits.items[@intCast((@as(i64, @intCast(self._limits.items.len)) - 1))];
// zbr:test/tc_scope.zbr:115
        var i = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test/tc_scope.zbr:116
        while (_zebra_ge(i, frame_start)) {
// zbr:test/tc_scope.zbr:117
            if (std.mem.eql(u8, self._names.items[@intCast(i)], name)) {
// zbr:test/tc_scope.zbr:118
                return true;
            }
// zbr:test/tc_scope.zbr:119
            i = (i - 1);
        }
// zbr:test/tc_scope.zbr:120
        return false;
    }

    pub fn getLocal(self: *TcScope, name: []const u8) TcSymbol {
// zbr:test/tc_scope.zbr:125
        const frame_start = self._limits.items[@intCast((@as(i64, @intCast(self._limits.items.len)) - 1))];
// zbr:test/tc_scope.zbr:126
        var i = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test/tc_scope.zbr:127
        while (_zebra_ge(i, frame_start)) {
// zbr:test/tc_scope.zbr:128
            if (std.mem.eql(u8, self._names.items[@intCast(i)], name)) {
// zbr:test/tc_scope.zbr:129
                return self._syms.items[@intCast(i)];
            }
// zbr:test/tc_scope.zbr:130
            i = (i - 1);
        }
// zbr:test/tc_scope.zbr:131
        return TcSymbol.init("", TcSymKind.local, TcType{ .unknown_ = {} });
    }

    pub fn hasSymbol(self: *TcScope, name: []const u8) bool {
// zbr:test/tc_scope.zbr:135
        var i = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test/tc_scope.zbr:136
        while (_zebra_ge(i, 0)) {
// zbr:test/tc_scope.zbr:137
            if (std.mem.eql(u8, self._names.items[@intCast(i)], name)) {
// zbr:test/tc_scope.zbr:138
                return true;
            }
// zbr:test/tc_scope.zbr:139
            i = (i - 1);
        }
// zbr:test/tc_scope.zbr:140
        return false;
    }

    pub fn lookup(self: *TcScope, name: []const u8) TcSymbol {
// zbr:test/tc_scope.zbr:145
        var i = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test/tc_scope.zbr:146
        while (_zebra_ge(i, 0)) {
// zbr:test/tc_scope.zbr:147
            if (std.mem.eql(u8, self._names.items[@intCast(i)], name)) {
// zbr:test/tc_scope.zbr:148
                return self._syms.items[@intCast(i)];
            }
// zbr:test/tc_scope.zbr:149
            i = (i - 1);
        }
// zbr:test/tc_scope.zbr:150
        return TcSymbol.init("", TcSymKind.local, TcType{ .unknown_ = {} });
    }

};
const _ttag_TcScope: u64 = _zbr_hash("TcScope");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_TcScope) return _reflect_TcScope_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_TcScope) return .{ .items = _reflect_TcScope_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_TcScope) return .{ .items = _reflect_TcScope_field_types };
    return .{ .items = &.{} };
}
