...
// Source: test/bug097_ptr_param_chain_test.zbr
...
pub fn hasValue(items: std.ArrayList([]const u8), x: []const u8) bool {
// zbr:test/bug097_ptr_param_chain_test.zbr:12
    for (items.items) |item| {
// zbr:test/bug097_ptr_param_chain_test.zbr:13
        if (std.mem.eql(u8, item, x)) {
// zbr:test/bug097_ptr_param_chain_test.zbr:14
            return true;
        }
    }
// zbr:test/bug097_ptr_param_chain_test.zbr:15
    return false;
}

pub fn appendAll(dst: *std.ArrayList([]const u8), src: std.ArrayList([]const u8)) void {
// zbr:test/bug097_ptr_param_chain_test.zbr:18
    for (src.items) |s| {
        dst.append(_allocator, _intern(s)) catch @panic("OOM");
    }
}

pub fn populate(dst: *std.ArrayList([]const u8), extra: std.ArrayList([]const u8)) void {
    dst.append(_allocator, _intern("seed")) catch @panic("OOM");
    appendAll(dst, extra);
// zbr:test/bug097_ptr_param_chain_test.zbr:28
    if (!(hasValue(dst.*, "alpha"))) @panic("hasValue on ptr-param");
}

pub fn main() void {
// zbr:test/bug097_ptr_param_chain_test.zbr:31
    var out = std.ArrayList([]const u8){};
// zbr:test/bug097_ptr_param_chain_test.zbr:32
    var items = std.ArrayList([]const u8){};
    items.append(_allocator, _intern("alpha")) catch @panic("OOM");
    items.append(_allocator, _intern("beta")) catch @panic("OOM");
    populate(&out, items);
// zbr:test/bug097_ptr_param_chain_test.zbr:36
    if (!((@as(i64, @intCast(out.items.len)) == 3))) @panic("count (seed+alpha+beta)");
// zbr:test/bug097_ptr_param_chain_test.zbr:37
    if (!(std.mem.eql(u8, out.items[@intCast(0)], "seed"))) @panic("first");
// zbr:test/bug097_ptr_param_chain_test.zbr:38
    if (!(std.mem.eql(u8, out.items[@intCast(1)], "alpha"))) @panic("second");
// zbr:test/bug097_ptr_param_chain_test.zbr:39
    if (!(hasValue(out, "beta"))) @panic("hasValue direct");
    std.debug.print("{s}\n", .{"ok"});
}
