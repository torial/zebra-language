...
// Source: test/generic_iface_test.zbr
...
pub const Printable = struct {
    ...
    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "label")) @compileError("type " ++ @typeName(T) ++ " does not implement Printable.label");
        }
    }
};

pub const Num = struct {
    _type_tag: u64 = _ttag_Num,
    val: i64,
    pub fn init(n: i64) *Num {
        const _self = _allocator.create(Num) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Num");
// zbr:test/generic_iface_test.zbr:7
            _self.val = n;
        return _self;
    }

};
const _ttag_Num: u64 = _zbr_hash("Num");
const _reflect_Num_name: []const u8 = "Num";
const _reflect_Num_fields: []const []const u8 = &.{"val"};
const _reflect_Num_field_types: []const []const u8 = &.{"int"};
...
pub fn Box(comptime T: type) type {
    return struct {
        ...
        pub fn init(v: T) *@This() {
            const _self = _allocator.create(@This()) catch @panic("OOM");
            _self._type_tag = _zbr_hash("Box");
// zbr:test/generic_iface_test.zbr:15
                _self.value = v;
            return _self;
        }
        ...
        pub fn label(self: *@This()) []const u8 {
            _ = self;
// zbr:test/generic_iface_test.zbr:18
            return "Box";
        }

        pub fn get(self: *@This()) T {
// zbr:test/generic_iface_test.zbr:21
            return self.value;
        }


        comptime {
            Printable.check(@This());
        }
    };
}
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/generic_iface_test.zbr:26
        const b = Box(*Num).init(Num.init(42));
        std.debug.print("{any}\n", .{b.get().val});
        std.debug.print("{s}\n", .{"generic_iface OK"});
    }
