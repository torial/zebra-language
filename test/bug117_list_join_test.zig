...
// Source: test/bug117_list_join_test.zbr
...
pub fn main() void {
// zbr:test/bug117_list_join_test.zbr:5
    var parts = std.ArrayList([]const u8){};
    parts.append(_allocator, _intern("alpha")) catch @panic("OOM");
    parts.append(_allocator, _intern("beta")) catch @panic("OOM");
    parts.append(_allocator, _intern("gamma")) catch @panic("OOM");
// zbr:test/bug117_list_join_test.zbr:10
    const joined = (std.mem.join(_allocator, ", ", parts.items) catch @panic("OOM"));
// zbr:test/bug117_list_join_test.zbr:11
    if (!(std.mem.eql(u8, joined, "alpha, beta, gamma"))) @panic(_str_concat(_str_concat("expected 'alpha, beta, gamma', got '", joined, _allocator), "'", _allocator));
// zbr:test/bug117_list_join_test.zbr:13
    const newline_joined = (std.mem.join(_allocator, "\n", parts.items) catch @panic("OOM"));
// zbr:test/bug117_list_join_test.zbr:14
    const lines: std.ArrayList([]const u8) = blk092_1: { var _ll_1: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_1 = std.mem.splitSequence(u8, newline_joined, "\n"); while (_split_iter_1.next()) |_se_1| { _ll_1.append(_allocator, _se_1) catch @panic("OOM"); } break :blk092_1 _ll_1; };
// zbr:test/bug117_list_join_test.zbr:15
    if (!((@as(i64, @intCast(lines.items.len)) == 3))) @panic(_str_concat("expected 3 lines, got ", (std.fmt.allocPrint(_allocator, "{}", .{@as(i64, @intCast(lines.items.len))}) catch unreachable), _allocator));
// zbr:test/bug117_list_join_test.zbr:16
    if (!(std.mem.eql(u8, lines.items[@intCast(0)], "alpha"))) @panic(_str_concat("expected alpha, got ", lines.items[@intCast(0)], _allocator));
// zbr:test/bug117_list_join_test.zbr:18
    const empty = std.ArrayList([]const u8){};
// zbr:test/bug117_list_join_test.zbr:19
    const empty_joined = (std.mem.join(_allocator, "-", empty.items) catch @panic("OOM"));
// zbr:test/bug117_list_join_test.zbr:20
    if (!(std.mem.eql(u8, empty_joined, ""))) @panic(_str_concat(_str_concat("expected empty string, got '", empty_joined, _allocator), "'", _allocator));
    std.debug.print("{s}\n", .{"bug117_list_join_test: ok"});
}
