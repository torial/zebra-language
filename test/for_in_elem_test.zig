...
// Source: test/for_in_elem_test.zbr
...
pub fn main() void {
// zbr:test/for_in_elem_test.zbr:15
    var alpha_count: i64 = 0;
// zbr:test/for_in_elem_test.zbr:16
    {
        var _cp_it_c = std.unicode.Utf8View.initUnchecked("hello123").iterator();
        while (_cp_it_c.nextCodepoint()) |c| {
// zbr:test/for_in_elem_test.zbr:17
            if (std.ascii.isAlphabetic(@as(u8, @truncate(c)))) {
// zbr:test/for_in_elem_test.zbr:18
                alpha_count = (alpha_count + 1);
            }
        }
    }
// zbr:test/for_in_elem_test.zbr:19
    if (!((alpha_count == 5))) @panic("chars isAlpha: expected 5");
// zbr:test/for_in_elem_test.zbr:22
    var digit_count: i64 = 0;
// zbr:test/for_in_elem_test.zbr:23
    {
        var _cp_it_c = std.unicode.Utf8View.initUnchecked("hello123").iterator();
        while (_cp_it_c.nextCodepoint()) |c| {
// zbr:test/for_in_elem_test.zbr:24
            if (std.ascii.isDigit(@as(u8, @truncate(c)))) {
// zbr:test/for_in_elem_test.zbr:25
                digit_count = (digit_count + 1);
            }
        }
    }
// zbr:test/for_in_elem_test.zbr:26
    if (!((digit_count == 3))) @panic("chars isDigit: expected 3");
// zbr:test/for_in_elem_test.zbr:29
    var upper_count: i64 = 0;
// zbr:test/for_in_elem_test.zbr:30
    {
        var _cp_it_c = std.unicode.Utf8View.initUnchecked("Hello World").iterator();
        while (_cp_it_c.nextCodepoint()) |c| {
// zbr:test/for_in_elem_test.zbr:31
            if (std.ascii.isUpper(@as(u8, @truncate(c)))) {
// zbr:test/for_in_elem_test.zbr:32
                upper_count = (upper_count + 1);
            }
        }
    }
// zbr:test/for_in_elem_test.zbr:33
    if (!((upper_count == 2))) @panic("chars isUpper: expected 2");
// zbr:test/for_in_elem_test.zbr:36
    var total_len: i64 = 0;
// zbr:test/for_in_elem_test.zbr:37
    const parts: std.ArrayList([]const u8) = blk092_1: { var _ll_1: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_1 = std.mem.splitSequence(u8, "ab,cde,f", ","); while (_split_iter_1.next()) |_se_1| { _ll_1.append(_allocator, _se_1) catch @panic("OOM"); } break :blk092_1 _ll_1; };
// zbr:test/for_in_elem_test.zbr:38
    for (parts.items) |part| {
// zbr:test/for_in_elem_test.zbr:39
        total_len = (total_len + @as(i64, @intCast(part.len)));
    }
// zbr:test/for_in_elem_test.zbr:40
    if (!((total_len == 6))) @panic("split elem .len sum: expected 6");
// zbr:test/for_in_elem_test.zbr:43
    var sw_count: i64 = 0;
// zbr:test/for_in_elem_test.zbr:44
    const words: std.ArrayList([]const u8) = blk092_2: { var _ll_2: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_2 = std.mem.splitSequence(u8, "apple,ant,banana,avocado", ","); while (_split_iter_2.next()) |_se_2| { _ll_2.append(_allocator, _se_2) catch @panic("OOM"); } break :blk092_2 _ll_2; };
// zbr:test/for_in_elem_test.zbr:45
    for (words.items) |word| {
// zbr:test/for_in_elem_test.zbr:46
        if (std.mem.startsWith(u8, word, "a")) {
// zbr:test/for_in_elem_test.zbr:47
            sw_count = (sw_count + 1);
        }
    }
// zbr:test/for_in_elem_test.zbr:48
    if (!((sw_count == 3))) @panic("split startsWith: expected 3");
// zbr:test/for_in_elem_test.zbr:51
    var items = std.ArrayList(i64){};
    items.append(_allocator, 10) catch @panic("OOM");
    items.append(_allocator, 20) catch @panic("OOM");
    items.append(_allocator, 30) catch @panic("OOM");
// zbr:test/for_in_elem_test.zbr:55
    var sum: i64 = 0;
// zbr:test/for_in_elem_test.zbr:56
    for (items.items) |n| {
// zbr:test/for_in_elem_test.zbr:57
        sum = (sum + n);
    }
// zbr:test/for_in_elem_test.zbr:58
    if (!((sum == 60))) @panic("List(int) sum: expected 60");
// zbr:test/for_in_elem_test.zbr:61
    var strs = std.ArrayList([]const u8){};
    strs.append(_allocator, _intern("cat")) catch @panic("OOM");
    strs.append(_allocator, _intern("elephant")) catch @panic("OOM");
    strs.append(_allocator, _intern("ox")) catch @panic("OOM");
// zbr:test/for_in_elem_test.zbr:65
    var max_len: i64 = 0;
// zbr:test/for_in_elem_test.zbr:66
    for (strs.items) |w| {
// zbr:test/for_in_elem_test.zbr:67
        if (_zebra_gt(@as(i64, @intCast(w.len)), max_len)) {
// zbr:test/for_in_elem_test.zbr:68
            max_len = @as(i64, @intCast(w.len));
        }
    }
// zbr:test/for_in_elem_test.zbr:69
    if (!((max_len == 8))) @panic("List(str) max .len: expected 8");
// zbr:test/for_in_elem_test.zbr:72
    var range_sum: i64 = 0;
// zbr:test/for_in_elem_test.zbr:73
    {
        var i: i64 = 0;
        const _stop_i: i64 = 5;
        while (i < _stop_i) : (i += 1) {
// zbr:test/for_in_elem_test.zbr:74
            range_sum = (range_sum + i);
        }
    }
// zbr:test/for_in_elem_test.zbr:75
    if (!((range_sum == 10))) @panic("range sum 0..4: expected 10");
// zbr:test/for_in_elem_test.zbr:78
    var bools = std.ArrayList(bool){};
    bools.append(_allocator, true) catch @panic("OOM");
    bools.append(_allocator, false) catch @panic("OOM");
    bools.append(_allocator, true) catch @panic("OOM");
// zbr:test/for_in_elem_test.zbr:82
    var true_count: i64 = 0;
// zbr:test/for_in_elem_test.zbr:83
    for (bools.items) |b| {
// zbr:test/for_in_elem_test.zbr:84
        if (b) {
// zbr:test/for_in_elem_test.zbr:85
            true_count = (true_count + 1);
        }
    }
// zbr:test/for_in_elem_test.zbr:86
    if (!((true_count == 2))) @panic("List(bool) true_count: expected 2");
    std.debug.print("{s}\n", .{"for_in_elem: OK"});
}
