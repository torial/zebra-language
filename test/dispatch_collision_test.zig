...
// Source: test/dispatch_collision_test.zbr
...
pub const Counter = struct {
    ...
    n: i64,
    pub fn init() *Counter {
        const _self = _allocator.create(Counter) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Counter");
// zbr:test/dispatch_collision_test.zbr:14
            _self.n = 0;
        return _self;
    }

    pub fn add(self: *Counter, x: i64) void {
// zbr:test/dispatch_collision_test.zbr:16
        self.n = (self.n + x);
    }

    pub fn count(self: *Counter) i64 {
// zbr:test/dispatch_collision_test.zbr:18
        return self.n;
    }

    pub fn at(self: *Counter, i: i64) i64 {
// zbr:test/dispatch_collision_test.zbr:20
        return (self.n + i);
    }

    pub fn remove(self: *Counter, x: i64) void {
// zbr:test/dispatch_collision_test.zbr:22
        self.n = (self.n - x);
    }

    pub fn contains(self: *Counter, x: i64) bool {
// zbr:test/dispatch_collision_test.zbr:24
        return (self.n == x);
    }

    pub fn join(self: *Counter, sep: []const u8) []const u8 {
// zbr:test/dispatch_collision_test.zbr:26
        return _str_concat(_str_concat((std.fmt.allocPrint(_allocator, "{}", .{self.n}) catch unreachable), sep, _allocator), (std.fmt.allocPrint(_allocator, "{}", .{self.n}) catch unreachable), _allocator);
    }

    pub fn sort(self: *Counter) void {
// zbr:test/dispatch_collision_test.zbr:28
        self.n = 0;
    }

};
const _ttag_Counter: u64 = _zbr_hash("Counter");
...
pub const Buffer = struct {
    _type_tag: u64 = _ttag_Buffer,
    data: []const u8,
    pub fn init() *Buffer {
        const _self = _allocator.create(Buffer) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Buffer");
// zbr:test/dispatch_collision_test.zbr:33
            _self.data = _intern("");
        return _self;
    }

    pub fn append(self: *Buffer, s: []const u8) void {
// zbr:test/dispatch_collision_test.zbr:35
        self.data = _intern(_str_concat(self.data, s, _allocator));
    }

    pub fn build(self: *Buffer) []const u8 {
// zbr:test/dispatch_collision_test.zbr:37
        return self.data;
    }

    pub fn len(self: *Buffer) i64 {
// zbr:test/dispatch_collision_test.zbr:39
        return @as(i64, @intCast(self.data.len));
    }

};
const _ttag_Buffer: u64 = _zbr_hash("Buffer");
const _reflect_Buffer_name: []const u8 = "Buffer";
...
// zbr:test/dispatch_collision_test.zbr:42
    ...
    c.add(5);
    c.add(3);
// zbr:test/dispatch_collision_test.zbr:46
    if (!((c.count() == 8))) @panic("count: expected 8");
// zbr:test/dispatch_collision_test.zbr:48
    if (!((c.at(10) == 18))) @panic("at: expected 18");
    c.remove(2);
// zbr:test/dispatch_collision_test.zbr:51
    if (!((c.count() == 6))) @panic("remove then count: expected 6");
// zbr:test/dispatch_collision_test.zbr:53
    if (!(c.contains(6))) @panic("contains: expected true");
// zbr:test/dispatch_collision_test.zbr:54
    if (!((!c.contains(5)))) @panic("contains false: expected false");
// zbr:test/dispatch_collision_test.zbr:56
    if (!(std.mem.eql(u8, c.join("-"), "6-6"))) @panic("join: expected '6-6'");
    c.sort();
// zbr:test/dispatch_collision_test.zbr:59
    if (!((c.count() == 0))) @panic("sort resets to 0");
// zbr:test/dispatch_collision_test.zbr:61
    var buf = Buffer.init();
    buf.append("hello");
    buf.append(" world");
// zbr:test/dispatch_collision_test.zbr:64
    if (!(std.mem.eql(u8, buf.build(), "hello world"))) @panic("build: expected 'hello world'");
// zbr:test/dispatch_collision_test.zbr:65
    if (!((buf.len() == 11))) @panic("len: expected 11");
    std.debug.print("{s}\n", .{"dispatch_collision: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Counter) return _reflect_Counter_name;
    if (tag == _ttag_Buffer) return _reflect_Buffer_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Counter) return .{ .items = _reflect_Counter_fields };
    if (tag == _ttag_Buffer) return .{ .items = _reflect_Buffer_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Counter) return .{ .items = _reflect_Counter_field_types };
    if (tag == _ttag_Buffer) return .{ .items = _reflect_Buffer_field_types };
    return .{ .items = &.{} };
}
