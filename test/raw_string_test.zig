...
pub const Main = struct {
    ...
    pub fn main() void {
        var re1 = _regex_compile("\\d+", "");
        std.debug.print("{}\n", .{re1.match("42")});
        std.debug.print("{}\n", .{re1.match("abc")});
        var re2 = _regex_compile("\\d+", "");
        std.debug.print("{}\n", .{(re1.match("99") == re2.match("99"))});
        var email = _regex_compile("[\\w.+-]+@[\\w-]+\\.[a-z]{2,}", "");
        std.debug.print("{}\n", .{email.match("user@example.com")});
        std.debug.print("{}\n", .{email.match("notanemail")});
        var wb = _regex_compile("\\bword\\b", "");
        std.debug.print("{}\n", .{wb.find("a word here")});
        std.debug.print("{}\n", .{wb.find("swordfish")});
        var re3 = _regex_compile("\\w+", "");
        std.debug.print("{}\n", .{re3.find("hello world")});
    }
