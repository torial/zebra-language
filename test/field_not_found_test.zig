...
// Source: test/field_not_found_test.zbr
...
pub const P = struct {
    _type_tag: u64 = _ttag_P,
    x: i64,
    pub fn init() *P {
        const _self = _allocator.create(P) catch @panic("OOM");
        _self._type_tag = _zbr_hash("P");
// zbr:test/field_not_found_test.zbr:6
            _self.x = 1;
        return _self;
    }

};
const _ttag_P: u64 = _zbr_hash("P");
const _reflect_P_name: []const u8 = "P";
const _reflect_P_fields: []const []const u8 = &.{"x"};
const _reflect_P_field_types: []const []const u8 = &.{"int"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/field_not_found_test.zbr:8
    const p = P.init();
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}", .{p.y}) catch unreachable)});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_P) return _reflect_P_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_P) return .{ .items = _reflect_P_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_P) return .{ .items = _reflect_P_field_types };
    return .{ .items = &.{} };
}
