...
const _ttag_Guard: u64 = _zbr_hash("Guard");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Guard) return _reflect_Guard_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Guard) return .{ .items = _reflect_Guard_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Guard) return .{ .items = _reflect_Guard_field_types };
    return .{ .items = &.{} };
}
