...
pub const Guard = struct {
    _type_tag: u64 = _ttag_Guard,
    x: i64 = 0,
    pub fn bump(self: *Guard) void {
        defer self._check_invariant();
        if (!(_zebra_lt(self.x, 1000))) std.debug.panic("require failed in 'bump'\n", .{});
        const _old_0 = self.x;
        defer {
            if (!(_zebra_gt(self.x, _old_0))) std.debug.panic("ensure failed in 'bump'\n", .{});
        }
// zbr:test/turbo_test.zbr:18
        self.x = (self.x + 1);
    }

    pub fn idle(self: *Guard) void {
        defer self._check_invariant();
        // pass
    }

    pub fn init() *Guard {
        const self = _allocator.create(Guard) catch @panic("OOM");
        self.* = .{};
        self._type_tag = _ttag_Guard;
        return self;
    }

    fn _check_invariant(self: *Guard) void {
        if (!(_zebra_ge(self.x, 0))) std.debug.panic("invariant failed in 'Guard'\n", .{});
    }

};

const _ttag_Guard: u64 = 3515770382;
const _reflect_Guard_name: []const u8 = "Guard";
const _reflect_Guard_fields: []const []const u8 = &.{"x"};
const _reflect_Guard_field_types: []const []const u8 = &.{"int"};

pub fn main() void {
// zbr:test/turbo_test.zbr:26
    const g = Guard.init();
// zbr:test/turbo_test.zbr:27
    g.bump();
// zbr:test/turbo_test.zbr:28
    g.idle();
// zbr:test/turbo_test.zbr:29
    std.debug.assert((g.x == 1));
// zbr:test/turbo_test.zbr:30
    std.debug.print("{s}\n", .{"turbo: OK"});
}
