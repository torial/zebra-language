...
// Source: C:/Projects/zebra-language/test/ref_struct_test.zbr
...
const _reflect_Holder_fields: []const []const u8 = &.{"p"};
const _reflect_Holder_field_types: []const []const u8 = &.{"^Point"};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
        const pt = Point.init(3, 4);
        const h = Holder.init(pt);
        std.debug.assert((h.p.x == 3));
        std.debug.assert((h.p.y == 4));
        std.debug.print("{s}\n", .{"ref_struct_test OK"});
    }
