...
// Source: C:/Projects/zebra-language/test/result_methods_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        if (!((ok_r.isOk() == true))) @panic("ok.isOk");
        if (!((ok_r.isErr() == false))) @panic("ok.isErr");
        if (!((err_r.isOk() == false))) @panic("err.isOk");
        if (!((err_r.isErr() == true))) @panic("err.isErr");
        if (!((ok_r.unwrap() == 42))) @panic("unwrap ok");
        if (!((ok_r.unwrapOr(0) == 42))) @panic("unwrapOr ok");
        if (!((err_r.unwrapOr(99) == 99))) @panic("unwrapOr err");
        const ov = ok_r.okValue();
        if (!((ov != null))) @panic("okValue non-nil");
        const ev = err_r.errValue();
        if (!((ev != null))) @panic("errValue non-nil");
        var mapped = ok_r.map(struct { fn call(x: i64) i64 {
            return (x * 2);
         } }.call);
        if (!((mapped.isOk() == true))) @panic("map isOk");
        if (!((mapped.unwrap() == 84))) @panic("map value");
        var mapped_err = err_r.map(struct { fn call(x: i64) i64 {
            return (x * 2);
         } }.call);
        if (!((mapped_err.isErr() == true))) @panic("map err propagates");
        var chained = ok_r.flatMap(struct { fn call(x: i64) _Result(i64, []const u8) {
            if ((x == 0)) {
                return .{ .err = "zero" };
            }
            return .{ .ok = @divTrunc(100, x) };
         } }.call);
        if (!((chained.isOk() == true))) @panic("flatMap ok");
        if (!((chained.unwrap() == 2))) @panic("flatMap value (100/42 = 2)");
        var zero_r: _Result(i64, []const u8) = .{ .ok = 0 };
        var chained_err = zero_r.flatMap(struct { fn call(x: i64) _Result(i64, []const u8) {
            if ((x == 0)) {
                return .{ .err = "zero" };
            }
            return .{ .ok = @divTrunc(100, x) };
         } }.call);
        if (!((chained_err.isErr() == true))) @panic("flatMap propagates err from f");
