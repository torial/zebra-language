...
// Source: C:/Projects/zebra-language/test/selfhost_probe4.zbr
...
pub const Span = struct {
    ...
    pub fn init(l: i64, c: i64) *Span {
        const _self = _allocator.create(Span) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Span");
            _self.line = l;
            _self.col = c;
        return _self;
    }
...
const _ttag_Span: u64 = _zbr_hash("Span");
...
pub const Diag = struct {
    ...
    pub fn init(msg: []const u8, l: i64, c: i64) *Diag {
        const _self = _allocator.create(Diag) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Diag");
            _self.message = msg;
            _self.line = l;
            _self.col = c;
        return _self;
...
const _ttag_Diag: u64 = _zbr_hash("Diag");
...
pub const Helper = struct {
    ...
    pub fn init() *Helper {
        const self = _allocator.create(Helper) catch @panic("OOM");
        self._type_tag = _ttag_Helper;
        return self;
    }

};
const _ttag_Helper: u64 = _zbr_hash("Helper");
...
pub const Lexer = struct {
    ...
    pub fn init() *Lexer {
        ...
        _self._type_tag = _zbr_hash("Lexer");
            _self.tokens = std.ArrayList(*Token){};
            _self.pos = 0;
        return _self;
    }

    pub fn push(self: *Lexer, k: TokenKind, t: []const u8, l: i64, c: i64) void {
        self.tokens.appendSlice(_allocator, Token.init(k, t, l, c)) catch @panic("OOM");
    }

    pub fn peek(self: *Lexer) TokenKind {
        if (_zebra_lt(self.pos, @as(i64, @intCast(self.tokens.items.len)))) {
            return self.tokens[@as(usize, @intCast(self.pos))].kind;
        }
        return TokenKind{ .eof = {} };
    }

    pub fn advance(self: *Lexer) *Token {
        const t = self.tokens[@as(usize, @intCast(self.pos))];
        self.pos = (self.pos + 1);
        return t;
    }
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Span) return _reflect_Span_name;
    if (tag == _ttag_Token) return _reflect_Token_name;
    if (tag == _ttag_Diag) return _reflect_Diag_name;
    if (tag == _ttag_Helper) return _reflect_Helper_name;
...
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Span) return .{ .items = _reflect_Span_fields };
    if (tag == _ttag_Token) return .{ .items = _reflect_Token_fields };
    if (tag == _ttag_Diag) return .{ .items = _reflect_Diag_fields };
    if (tag == _ttag_Helper) return .{ .items = _reflect_Helper_fields };
...
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Span) return .{ .items = _reflect_Span_field_types };
    if (tag == _ttag_Token) return .{ .items = _reflect_Token_field_types };
    if (tag == _ttag_Diag) return .{ .items = _reflect_Diag_field_types };
    if (tag == _ttag_Helper) return .{ .items = _reflect_Helper_field_types };
