...
// Source: test/bug092_split_to_list_test.zbr
...
pub fn main() void {
// zbr:test/bug092_split_to_list_test.zbr:5
    const content: []const u8 = "a\nb\nc";
// zbr:test/bug092_split_to_list_test.zbr:6
    const lines: std.ArrayList([]const u8) = blk092_1: { var _ll_1: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_1 = std.mem.splitSequence(u8, content, "\n"); while (_split_iter_1.next()) |_se_1| { _ll_1.append(_allocator, _se_1) catch @panic("OOM"); } break :blk092_1 _ll_1; };
// zbr:test/bug092_split_to_list_test.zbr:7
    if (!((@as(i64, @intCast(lines.items.len)) == 3))) @panic("split count");
// zbr:test/bug092_split_to_list_test.zbr:8
    if (!(std.mem.eql(u8, lines.items[@intCast(0)], "a"))) @panic("a");
// zbr:test/bug092_split_to_list_test.zbr:9
    if (!(std.mem.eql(u8, lines.items[@intCast(1)], "b"))) @panic("b");
// zbr:test/bug092_split_to_list_test.zbr:10
    if (!(std.mem.eql(u8, lines.items[@intCast(2)], "c"))) @panic("c");
// zbr:test/bug092_split_to_list_test.zbr:12
    const ls: std.ArrayList([]const u8) = blk092_2: { var _ll_2: std.ArrayList([]const u8) = std.ArrayList([]const u8){}; var _split_iter_2 = std.mem.splitScalar(u8, content, '\n'); while (_split_iter_2.next()) |_se_2| { _ll_2.append(_allocator, _se_2) catch @panic("OOM"); } break :blk092_2 _ll_2; };
// zbr:test/bug092_split_to_list_test.zbr:13
    if (!((@as(i64, @intCast(ls.items.len)) == 3))) @panic("lines count");
    std.debug.print("{s}\n", .{"ok"});
}
