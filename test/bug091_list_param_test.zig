...
// Source: test/bug091_list_param_test.zbr
...
pub fn fillList(out: *std.ArrayList([]const u8)) void {
    out.append(_allocator, _intern("alpha")) catch @panic("OOM");
    out.append(_allocator, _intern("beta")) catch @panic("OOM");
}

pub fn fillMap(out: *std.StringHashMap(i64)) void {
    out.put(_intern("one"), 1) catch @panic("OOM");
    out.put(_intern("two"), 2) catch @panic("OOM");
}

pub fn main() void {
// zbr:test/bug091_list_param_test.zbr:15
    var items = std.ArrayList([]const u8){};
    fillList(&items);
// zbr:test/bug091_list_param_test.zbr:17
    if (!((@as(i64, @intCast(items.items.len)) == 2))) @panic("list count mismatch");
// zbr:test/bug091_list_param_test.zbr:18
    if (!(std.mem.eql(u8, items.items[@intCast(0)], "alpha"))) @panic("alpha");
// zbr:test/bug091_list_param_test.zbr:19
    if (!(std.mem.eql(u8, items.items[@intCast(1)], "beta"))) @panic("beta");
// zbr:test/bug091_list_param_test.zbr:21
    var counts = std.StringHashMap(i64).init(_allocator);
    fillMap(&counts);
// zbr:test/bug091_list_param_test.zbr:23
    if (!((@as(i64, @intCast(counts.count())) == 2))) @panic("map count mismatch");
    std.debug.print("{s}\n", .{"ok"});
}
