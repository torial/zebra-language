...
// Source: test/tuple_smoke_test.zbr
...
pub fn minmax(a: i64, b: i64) struct { i64, i64 } {
// zbr:test/tuple_smoke_test.zbr:2
    if (_zebra_lt(a, b)) {
// zbr:test/tuple_smoke_test.zbr:3
        return .{a, b};
    }
// zbr:test/tuple_smoke_test.zbr:4
    return .{b, a};
}

pub fn add_pair(x: i64, y: i64) struct { i64, i64 } {
// zbr:test/tuple_smoke_test.zbr:7
    return .{(x + y), (x - y)};
}

pub fn stringify_pair(n: i64, m: i64) struct { []const u8, []const u8 } {
// zbr:test/tuple_smoke_test.zbr:10
    return .{(std.fmt.allocPrint(_allocator, "{}", .{n}) catch unreachable), (std.fmt.allocPrint(_allocator, "{}", .{m}) catch unreachable)};
}

pub fn mixed(n: i64, s: []const u8) struct { []const u8, i64 } {
// zbr:test/tuple_smoke_test.zbr:13
    return .{_str_concat(s, "!", _allocator), (n * 2)};
}

pub fn main() void {
// zbr:test/tuple_smoke_test.zbr:17
    const _zbr_dt_1 = minmax(7, 3);
    const lo = _zbr_dt_1.@"0";
    const hi = _zbr_dt_1.@"1";
    std.debug.print("{}\n", .{lo});
    std.debug.print("{}\n", .{hi});
// zbr:test/tuple_smoke_test.zbr:22
    const _zbr_dt_2 = add_pair(10, 4);
    const sum = _zbr_dt_2.@"0";
    const diff = _zbr_dt_2.@"1";
    std.debug.print("{}\n", .{sum});
    std.debug.print("{}\n", .{diff});
// zbr:test/tuple_smoke_test.zbr:27
    const t = minmax(1, 9);
    std.debug.print("{}\n", .{t.@"0"});
    std.debug.print("{}\n", .{t.@"1"});
// zbr:test/tuple_smoke_test.zbr:32
    const _zbr_dt_3 = stringify_pair(42, 99);
    const ns = _zbr_dt_3.@"0";
    const ms = _zbr_dt_3.@"1";
    std.debug.print("{s}\n", .{ns});
    std.debug.print("{s}\n", .{ms});
// zbr:test/tuple_smoke_test.zbr:37
    const _zbr_dt_4 = mixed(5, "hello");
    const greeting = _zbr_dt_4.@"0";
    const doubled = _zbr_dt_4.@"1";
    std.debug.print("{s}\n", .{greeting});
    std.debug.print("{}\n", .{doubled});
    std.debug.print("{s}\n", .{"done"});
}
