...
// Source: test/contract_require.zbr
...
pub const Counter = struct {
    ...
    count: i64 = 0,
    pub fn push(self: *Counter, n: i64) void {
        if (!(_zebra_gt(n, 0))) std.debug.panic("require failed in 'push'\n", .{});
// zbr:test/contract_require.zbr:7
        self.count = (self.count + n);
    }

    pub fn reset(self: *Counter, limit: i64) void {
        if (!(_zebra_ge(limit, 0))) std.debug.panic("require failed in 'reset'\n", .{});
        if (!(_zebra_le(limit, self.count))) std.debug.panic("require failed in 'reset'\n", .{});
// zbr:test/contract_require.zbr:13
        self.count = limit;
    }
...
pub const Box = struct {
    value: i64 = 0,
    pub fn set(self: *Box, v: i64) void {
        if (!(_zebra_ge(v, 0))) std.debug.panic("require failed in 'set'\n", .{});
// zbr:test/contract_require.zbr:21
        self.value = v;
    }
...
pub fn main() void {
// zbr:test/contract_require.zbr:24
    var c = Counter.init();
    c.push(5);
    c.push(3);
    c.reset(2);
// zbr:test/contract_require.zbr:28
    var b = Box.init();
    b.put(10) catch @panic("OOM");
    std.debug.print("{s}\n", .{"ok"});
}
