...
pub fn testOptionalTypes() void {
// zbr:test/tc_smoke_main.zbr:5
    {
        var _try_err: ?anyerror = null;
        _ = &_try_err;
        _try_blk: {
// zbr:test/tc_smoke_main.zbr:6
            const opt = TcType{ .optional = blk_box_1: { const _bv: std.meta.Child(@FieldType(TcType, "optional")) = TcType{ .str_ = {} }; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_1 _bp; } };
// zbr:test/tc_smoke_main.zbr:7
            std.debug.assert(std.mem.eql(u8, TcTypes.describe(opt), "?str"));
// zbr:test/tc_smoke_main.zbr:8
            const nested = TcType{ .optional = blk_box_2: { const _bv: std.meta.Child(@FieldType(TcType, "optional")) = TcType{ .optional = blk_box_3: { const _bv: std.meta.Child(@FieldType(TcType, "optional")) = TcType{ .int_ = {} }; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_3 _bp; } }; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_2 _bp; } };
// zbr:test/tc_smoke_main.zbr:9
            std.debug.assert(std.mem.eql(u8, TcTypes.describe(nested), "??int"));
            break :_try_blk;
        }
        if (_try_err != null) {
            std.debug.print("{s}\n", .{_str_concat("FAIL: ", _zbr_error_msg(), _allocator)});
        }
    }
}
...
fn _zbr_error_msg() []const u8 {
    ...
    if (@import("tc_smoke_types.zig")._error_ctx.message.len > 0) return @import("tc_smoke_types.zig")._error_ctx.message;
    return "";
}
