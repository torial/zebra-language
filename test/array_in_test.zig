...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/array_in_test.zbr:5
        const name = "findAll";
// zbr:test/array_in_test.zbr:8
        if (_zebra_in(name, .{"match", "find", "findAll", "groups"})) {
// zbr:test/array_in_test.zbr:9
            std.debug.print("{s}\n", .{"is regex method"});
        } else {
// zbr:test/array_in_test.zbr:11
            std.debug.print("{s}\n", .{"not regex method"});
        }
// zbr:test/array_in_test.zbr:14
        const other = "unknown";
// zbr:test/array_in_test.zbr:15
        if (_zebra_in(other, .{"match", "find", "findAll", "groups"})) {
// zbr:test/array_in_test.zbr:16
            std.debug.print("{s}\n", .{"found"});
        } else {
// zbr:test/array_in_test.zbr:18
            std.debug.print("{s}\n", .{"not found"});
        }
    }
