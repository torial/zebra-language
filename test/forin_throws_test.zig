...
// Source: test/forin_throws_test.zbr
...
pub fn makeNums() anyerror!std.ArrayList(i64) {
// zbr:test/forin_throws_test.zbr:9
    var xs = std.ArrayList(i64).empty;
    xs.append(_allocator, 1) catch @panic("OOM");
    xs.append(_allocator, 2) catch @panic("OOM");
    xs.append(_allocator, 3) catch @panic("OOM");
// zbr:test/forin_throws_test.zbr:13
    return xs;
}

pub fn sumViaForIn() anyerror!i64 {
// zbr:test/forin_throws_test.zbr:17
    var total: i64 = 0;
// zbr:test/forin_throws_test.zbr:18
    for ((try makeNums()).items) |a| {
// zbr:test/forin_throws_test.zbr:19
        total = (total + a);
    }
// zbr:test/forin_throws_test.zbr:20
    return total;
}

pub fn test_forin_over_throws_call() anyerror!void {
// zbr:test/forin_throws_test.zbr:23
    const got = sumViaForIn() catch 0;
// zbr:test/forin_throws_test.zbr:24
    try _zebra_assert_cmp(got, 6, true);
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    if (test_forin_over_throws_call()) |_| {
        _test_pass += 1;
        std.debug.print("PASS: test_forin_over_throws_call\n", .{});
    } else |_terr| {
        _test_fail += 1;
        if (_terr == error.ZebraError) {
            std.debug.print("FAIL: test_forin_over_throws_call: {s}\n", .{_zbr_error_msg()});
        } else {
            std.debug.print("FAIL: test_forin_over_throws_call: {}\n", .{_terr});
        }
    }
