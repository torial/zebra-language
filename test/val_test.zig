...
const val_lib = @import("val_lib.zig");
const Val = val_lib.Val;
const Container = val_lib.Container;
const makeContainer = val_lib.makeContainer;
pub fn extractFromParam(c: Container) Val {
// zbr:test/val_test.zbr:4
    return c.opt_val.?.*;
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
    @import("val_lib.zig")._initAllocator(_allocator);
    @import("val_lib.zig")._initIo(_io);
// zbr:test/val_test.zbr:8
    const c1 = makeContainer(42);
// zbr:test/val_test.zbr:9
    const v1: Val = extractFromParam(c1);
// zbr:test/val_test.zbr:10
    if (v1 == .int_val) {
        const n = v1.int_val;
// zbr:test/val_test.zbr:11
        std.debug.assert((n == 42));
    }
// zbr:test/val_test.zbr:14
    const c2 = makeContainer(99);
// zbr:test/val_test.zbr:15
    const v2: Val = c2.opt_val.?.*;
// zbr:test/val_test.zbr:16
    if (v2 == .int_val) {
        const n2 = v2.int_val;
// zbr:test/val_test.zbr:17
        std.debug.assert((n2 == 99));
    }
    std.debug.print("{s}\n", .{"val_test: OK"});
}
...
fn _zbr_error_msg() []const u8 {
    ...
    if (@import("val_lib.zig")._error_ctx.message.len > 0) return @import("val_lib.zig")._error_ctx.message;
    return "";
}
