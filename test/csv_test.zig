...
// Source: C:/Projects/zebra-language/test/csv_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        const simple: []const u8 = "name,score,city\nAlice,95,Boston\nBob,87,Austin\nCarla,100,Denver";
        var csv = _csv_parse(simple);
        if (!((csv.rowCount() == 4))) @panic("rowCount includes header");
        if (!((csv.colCount() == 3))) @panic("colCount = 3 columns");
        const hdr = csv.header();
        if (!(std.mem.eql(u8, hdr.items[@intCast(0)], "name"))) @panic("header col 0");
        if (!(std.mem.eql(u8, hdr.items[@intCast(1)], "score"))) @panic("header col 1");
        if (!(std.mem.eql(u8, hdr.items[@intCast(2)], "city"))) @panic("header col 2");
        if (!((@as(i64, @intCast(hdr.items.len)) == 3))) @panic("header count");
        const r1 = csv.row(1);
        if (!(std.mem.eql(u8, r1.items[@intCast(0)], "Alice"))) @panic("row 1 name");
        if (!(std.mem.eql(u8, r1.items[@intCast(1)], "95"))) @panic("row 1 score");
        if (!(std.mem.eql(u8, r1.items[@intCast(2)], "Boston"))) @panic("row 1 city");
        const r3 = csv.row(3);
        if (!(std.mem.eql(u8, r3.items[@intCast(0)], "Carla"))) @panic("row 3 name");
        if (!(std.mem.eql(u8, r3.items[@intCast(1)], "100"))) @panic("row 3 score");
        const r2 = csv.row(2);
        if (!(std.mem.eql(u8, csv.get(r2), "Bob"))) @panic("get name");
        if (!(std.mem.eql(u8, csv.get(r2), "Austin"))) @panic("get city");
        if (!(std.mem.eql(u8, csv.get(r2), "87"))) @panic("get score");
        if (!(std.mem.eql(u8, csv.get(r2), ""))) @panic("get missing col");
        if (!(((csv.rowCount() - 1) == 3))) @panic("dataRows count");
        var high: i64 = 0;
        for (csv.dataRows().items) |row| {
            const s = csv.get(row);
            if ((std.mem.eql(u8, s, "95") or std.mem.eql(u8, s, "100"))) {
                high = (high + 1);
            }
        }
        if (!((high == 2))) @panic("high scorers");
        if (!((csv.rowCount() == 4))) @panic("rows() count matches rowCount");
        const comma_csv: []const u8 = "name,city\n\"Smith, Jr\",Boston";
        var cc = _csv_parse(comma_csv);
        if (!((cc.rowCount() == 2))) @panic("comma csv rowCount");
        const cr = cc.row(1);
        if (!(std.mem.eql(u8, cr.items[@intCast(0)], "Smith, Jr"))) @panic("embedded comma in field");
        if (!(std.mem.eql(u8, cr.items[@intCast(1)], "Boston"))) @panic("field after quoted comma-field");
        const quote_csv: []const u8 = "name,note\nAlice,\"She said \"\"hello\"\"\"";
        var qc = _csv_parse(quote_csv);
        if (!((qc.rowCount() == 2))) @panic("quote csv rowCount");
        const qr = qc.row(1);
        if (!(std.mem.eql(u8, qr.items[@intCast(0)], "Alice"))) @panic("quote csv name");
        if (!(std.mem.eql(u8, qr.items[@intCast(1)], "She said \"hello\""))) @panic("embedded double-quote");
        const mixed_csv: []const u8 = "key,value\n\"Jones, \"\"Jr\"\"\",\"New York\"";
        var mc = _csv_parse(mixed_csv);
        const mr = mc.row(1);
        if (!(std.mem.eql(u8, mr.items[@intCast(0)], "Jones, \"Jr\""))) @panic("mixed: comma + quote");
        if (!(std.mem.eql(u8, mr.items[@intCast(1)], "New York"))) @panic("mixed: second field");
        var file_csv = _csv_parse_file("test/texts/MAR.csv");
        if (!(_zebra_gt(file_csv.rowCount(), 1))) @panic("MAR.csv has rows");
        if (!((file_csv.colCount() == 3))) @panic("MAR.csv has 3 columns");
        const fhdr = file_csv.header();
        if (!(std.mem.eql(u8, fhdr.items[@intCast(0)], "book"))) @panic("MAR col 0");
        if (!(std.mem.eql(u8, fhdr.items[@intCast(1)], "chapter:verse"))) @panic("MAR col 1");
        if (!(std.mem.eql(u8, fhdr.items[@intCast(2)], "text"))) @panic("MAR col 2");
        const first_data = file_csv.row(1);
        if (!(std.mem.eql(u8, first_data.items[@intCast(0)], "MAR"))) @panic("first data row book");
        if (!(std.mem.eql(u8, file_csv.get(first_data), "MAR"))) @panic("get book col");
        if (!(_zebra_gt(file_csv.rowCount(), 1))) @panic("MAR has data rows");
        var writer = CsvWriter();
        writer.writeRow(csv.header());
        for (csv.dataRows().items) |row| {
            writer.writeRow(row);
        }
        const rebuilt = (writer.toOwnedSlice(_allocator) catch "");
        if (!(_zebra_gt(rebuilt.len, 0))) @panic("writer produced output");
        if (!(rebuilt.contains("Alice"))) @panic("writer output has Alice");
        if (!(rebuilt.contains("name"))) @panic("writer output has header");
        if (!(rebuilt.contains("Boston"))) @panic("writer output has Boston");
        var writer2 = CsvWriter();
        writer2.writeRow(cc.header());
        for (cc.dataRows().items) |row| {
            writer2.writeRow(row);
        }
        const rt = (writer2.toOwnedSlice(_allocator) catch "");
        var rt_csv = _csv_parse(rt);
        if (!((rt_csv.rowCount() == 2))) @panic("round-trip rowCount");
        const rt_row = rt_csv.row(1);
        if (!(std.mem.eql(u8, rt_row.items[@intCast(0)], "Smith, Jr"))) @panic("round-trip comma field");
        var resp = HttpResponse{ .status = 200, .text = "hello" };
        if (!((resp.status == 200))) @panic("ok status");
        if (!(std.mem.eql(u8, resp.text, "hello"))) @panic("ok text");
        var resp2 = resp.withHeader("Content-Type", "application/json");
        if (!((resp2.status == 200))) @panic("withHeader preserves status");
        if (!(std.mem.eql(u8, resp2.text, "hello"))) @panic("withHeader preserves text");
        const resp3 = resp2.withHeader("X-Custom", "value");
        if (!((resp3.status == 200))) @panic("chained withHeader status");
