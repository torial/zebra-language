...
// Source: test/str_method_chain_test.zbr
...
pub fn wordsIn(s: []const u8) i64 {
// zbr:test/str_method_chain_test.zbr:15
    const parts: std.ArrayList([]const u8) = blk092_1: { var _ll_1: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_1 = std.mem.splitSequence(u8, s, " "); while (_split_iter_1.next()) |_se_1| { _ll_1.append(_allocator, _se_1) catch @panic("OOM"); } break :blk092_1 _ll_1; };
// zbr:test/str_method_chain_test.zbr:16
    return @as(i64, @intCast(parts.items.len));
}

pub fn firstWord(s: []const u8) []const u8 {
// zbr:test/str_method_chain_test.zbr:19
    const parts: std.ArrayList([]const u8) = blk092_2: { var _ll_2: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_2 = std.mem.splitSequence(u8, s, " "); while (_split_iter_2.next()) |_se_2| { _ll_2.append(_allocator, _se_2) catch @panic("OOM"); } break :blk092_2 _ll_2; };
// zbr:test/str_method_chain_test.zbr:20
    return parts.items[@intCast(0)];
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/str_method_chain_test.zbr:23
    const s: []const u8 = "  Hello, World!  ";
// zbr:test/str_method_chain_test.zbr:26
    const trimmed = std.mem.trim(u8, s, " \t\n\r");
// zbr:test/str_method_chain_test.zbr:27
    if (!((@as(i64, @intCast(trimmed.len)) == 13))) @panic("trimmed.len: expected 13");
// zbr:test/str_method_chain_test.zbr:30
    const up = (std.ascii.allocUpperString(_allocator, "hello") catch unreachable);
// zbr:test/str_method_chain_test.zbr:31
    if (!(std.mem.startsWith(u8, up, "HE"))) @panic("upper startsWith: expected true");
// zbr:test/str_method_chain_test.zbr:32
    if (!(std.mem.eql(u8, up, "HELLO"))) @panic("upper result: expected HELLO");
// zbr:test/str_method_chain_test.zbr:35
    const lo = (std.ascii.allocLowerString(_allocator, "WORLD") catch unreachable);
// zbr:test/str_method_chain_test.zbr:36
    if (!(std.mem.endsWith(u8, lo, "ld"))) @panic("lower endsWith: expected true");
// zbr:test/str_method_chain_test.zbr:39
    if (!((wordsIn("the quick brown fox") == 4))) @panic("wordsIn: expected 4");
// zbr:test/str_method_chain_test.zbr:42
    if (!(std.mem.eql(u8, firstWord("hello world"), "hello"))) @panic("firstWord: expected hello");
// zbr:test/str_method_chain_test.zbr:45
    const parts: std.ArrayList([]const u8) = blk092_3: { var _ll_3: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_3 = std.mem.splitSequence(u8, "a,b,c,d", ","); while (_split_iter_3.next()) |_se_3| { _ll_3.append(_allocator, _se_3) catch @panic("OOM"); } break :blk092_3 _ll_3; };
// zbr:test/str_method_chain_test.zbr:46
    const joined = (std.mem.join(_allocator, "-", parts.items) catch @panic("OOM"));
// zbr:test/str_method_chain_test.zbr:47
    if (!(std.mem.eql(u8, joined, "a-b-c-d"))) @panic("split join: expected a-b-c-d");
// zbr:test/str_method_chain_test.zbr:50
    const words: std.ArrayList([]const u8) = blk092_4: { var _ll_4: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_4 = std.mem.splitSequence(u8, "one two three four five", " "); while (_split_iter_4.next()) |_se_4| { _ll_4.append(_allocator, _se_4) catch @panic("OOM"); } break :blk092_4 _ll_4; };
// zbr:test/str_method_chain_test.zbr:51
    if (!((@as(i64, @intCast(words.items.len)) == 5))) @panic("words count: expected 5");
// zbr:test/str_method_chain_test.zbr:54
    const tokens: std.ArrayList([]const u8) = blk092_5: { var _ll_5: std.ArrayList([]const u8) = std.ArrayList([]const u8).empty; var _split_iter_5 = std.mem.splitSequence(u8, "foo:bar:baz", ":"); while (_split_iter_5.next()) |_se_5| { _ll_5.append(_allocator, _se_5) catch @panic("OOM"); } break :blk092_5 _ll_5; };
// zbr:test/str_method_chain_test.zbr:55
    const mid = tokens.items[@intCast(1)];
// zbr:test/str_method_chain_test.zbr:56
    if (!((@as(i64, @intCast(mid.len)) == 3))) @panic("tokens.at(1).len: expected 3");
// zbr:test/str_method_chain_test.zbr:57
    if (!(std.mem.startsWith(u8, mid, "ba"))) @panic("tokens.at(1).startsWith: expected true");
// zbr:test/str_method_chain_test.zbr:60
    if (!((std.mem.indexOf(u8, "hello", "ell") != null))) @panic("in: expected true");
// zbr:test/str_method_chain_test.zbr:61
    if (!((!(std.mem.indexOf(u8, "hello", "xyz") != null)))) @panic("not in: expected false");
// zbr:test/str_method_chain_test.zbr:64
    const inner = std.mem.trim(u8, "  spaces  ", " \t\n\r");
// zbr:test/str_method_chain_test.zbr:65
    if (!((@as(i64, @intCast(inner.len)) == 6))) @panic("trim().len: expected 6");
    std.debug.print("{s}\n", .{"str_method_chain: OK"});
}
