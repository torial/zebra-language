...
// Source: C:/Projects/zebra-language/test/expose_dotted_test.zbr
...
const expose_dotted.lib = @import("expose_dotted.lib.zig");
const DottedLib = expose_dotted.lib.DottedLib;
const Point = expose_dotted.lib.Point;
pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
        const g = DottedLib.greet();
        if (!(std.mem.eql(u8, g, "hello from dotted lib"))) @panic("greet failed");
        std.debug.print("{s}\n", .{"1. dotted.lib static str call: OK"});
        const sum = DottedLib.add(7, 3);
        if (!((sum == 10))) @panic("add failed");
...
fn _zbr_error_msg() []const u8 {
    ...
    if (@import("expose_dotted.lib.zig")._error_ctx.message.len > 0) return @import("expose_dotted.lib.zig")._error_ctx.message;
    return "";
}
