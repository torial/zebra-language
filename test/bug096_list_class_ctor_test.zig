...
// Source: test/bug096_list_class_ctor_test.zbr
...
pub const Item = struct {
    _type_tag: u64 = _ttag_Item,
    label: []const u8 = "",
    pub fn init(s: []const u8) *Item {
        const _self = _allocator.create(Item) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Item");
            _self.label = "";
// zbr:test/bug096_list_class_ctor_test.zbr:7
            _self.label = _intern(s);
        return _self;
    }

};
const _ttag_Item: u64 = _zbr_hash("Item");
const _reflect_Item_name: []const u8 = "Item";
const _reflect_Item_fields: []const []const u8 = &.{"label"};
const _reflect_Item_field_types: []const []const u8 = &.{"str"};

pub const Container = struct {
    _type_tag: u64 = _ttag_Container,
    items: std.ArrayList(*Item),
    pub fn init() *Container {
        const _self = _allocator.create(Container) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Container");
// zbr:test/bug096_list_class_ctor_test.zbr:12
            _self.items = std.ArrayList(*Item){};
        return _self;
    }

};
const _ttag_Container: u64 = _zbr_hash("Container");
const _reflect_Container_name: []const u8 = "Container";
const _reflect_Container_fields: []const []const u8 = &.{"items"};
const _reflect_Container_field_types: []const []const u8 = &.{"List(Item)"};

pub fn main() void {
// zbr:test/bug096_list_class_ctor_test.zbr:15
    const c = Container.init();
    c.items.append(_allocator, Item.init("alpha")) catch @panic("OOM");
    c.items.append(_allocator, Item.init("beta")) catch @panic("OOM");
// zbr:test/bug096_list_class_ctor_test.zbr:18
    if (!((@as(i64, @intCast(c.items.items.len)) == 2))) @panic("count");
// zbr:test/bug096_list_class_ctor_test.zbr:19
    if (!(std.mem.eql(u8, c.items.items[@intCast(0)].label, "alpha"))) @panic("first");
// zbr:test/bug096_list_class_ctor_test.zbr:20
    if (!(std.mem.eql(u8, c.items.items[@intCast(1)].label, "beta"))) @panic("second");
    std.debug.print("{s}\n", .{"ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Item) return _reflect_Item_name;
    if (tag == _ttag_Container) return _reflect_Container_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Item) return .{ .items = _reflect_Item_fields };
    if (tag == _ttag_Container) return .{ .items = _reflect_Container_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Item) return .{ .items = _reflect_Item_field_types };
    if (tag == _ttag_Container) return .{ .items = _reflect_Container_field_types };
    return .{ .items = &.{} };
}
