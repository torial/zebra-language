...
pub const IntBag = struct {
    _type_tag: u64 = _ttag_IntBag,
    _items: std.ArrayList(i64),
    len: i64,
    pub fn init() *IntBag {
        const _self = _allocator.create(IntBag) catch @panic("OOM");
        _self._type_tag = _zbr_hash("IntBag");
// zbr:test/len_size_test.zbr:12
            _self._items = std.ArrayList(i64).empty;
// zbr:test/len_size_test.zbr:13
            _self.len = 0;
        return _self;
    }

    pub fn add(self: *IntBag, v: i64) void {
        self._items.append(_allocator, v) catch @panic("OOM");
// zbr:test/len_size_test.zbr:17
        self.len = @as(i64, @intCast(self._items.items.len));
    }

    pub fn removeAll(self: *IntBag) void {
// zbr:test/len_size_test.zbr:20
        self._items = std.ArrayList(i64).empty;
// zbr:test/len_size_test.zbr:21
        self.len = 0;
    }

};
const _ttag_IntBag: u64 = _zbr_hash("IntBag");
const _reflect_IntBag_name: []const u8 = "IntBag";
const _reflect_IntBag_fields: []const []const u8 = &.{"_items", "len"};
const _reflect_IntBag_field_types: []const []const u8 = &.{"List(int)", "int"};

pub fn test_list_len() anyerror!void {
// zbr:test/len_size_test.zbr:24
    var xs = std.ArrayList([]const u8).empty;
    xs.append(_allocator, _intern("a")) catch @panic("OOM");
    xs.append(_allocator, _intern("b")) catch @panic("OOM");
    xs.append(_allocator, _intern("c")) catch @panic("OOM");
// zbr:test/len_size_test.zbr:28
    try _zebra_assert_cmp(@as(i64, @intCast(xs.items.len)), 3, true);
}

pub fn test_hashmap_len() anyerror!void {
// zbr:test/len_size_test.zbr:31
    var m = std.StringHashMap(i64).init(_allocator);
    m.put(_intern("x"), 1) catch @panic("OOM");
    m.put(_intern("y"), 2) catch @panic("OOM");
// zbr:test/len_size_test.zbr:34
    try _zebra_assert_cmp(@as(i64, @intCast(m.count())), 2, true);
}

pub fn test_str_count_substring() anyerror!void {
// zbr:test/len_size_test.zbr:38
    try _zebra_assert_cmp(@as(i64, @intCast(std.mem.count(u8, "banana", "a"))), 3, true);
// zbr:test/len_size_test.zbr:39
    try _zebra_assert_cmp(@as(i64, @intCast(std.mem.count(u8, "banana", "na"))), 2, true);
}

pub fn test_maintained_len_field() anyerror!void {
// zbr:test/len_size_test.zbr:42
    var bag = IntBag.init();
// zbr:test/len_size_test.zbr:43
    try _zebra_assert_cmp(@as(i64, @intCast(bag.len)), 0, true);
    bag.add(10);
    bag.add(20);
// zbr:test/len_size_test.zbr:46
    try _zebra_assert_cmp(@as(i64, @intCast(bag.len)), 2, true);
    bag.removeAll();
// zbr:test/len_size_test.zbr:48
    try _zebra_assert_cmp(@as(i64, @intCast(bag.len)), 0, true);
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
    std.debug.print("{s}\n", .{"len_size_test: run with 'zebra test'"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_IntBag) return _reflect_IntBag_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_IntBag) return .{ .items = _reflect_IntBag_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_IntBag) return .{ .items = _reflect_IntBag_field_types };
    return .{ .items = &.{} };
}
