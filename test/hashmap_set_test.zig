...
pub const Gauge = struct {
    ...
    pub fn init(v: i64) *Gauge {
        const _self = _allocator.create(Gauge) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Gauge");
...
const _ttag_Gauge: u64 = _zbr_hash("Gauge");
...
// zbr:test/hashmap_set_test.zbr:20
    ...
    scores.put("alice", 95) catch @panic("OOM");
    scores.put("bob", 87) catch @panic("OOM");
    std.debug.print("{any}\n", .{(scores.get("alice") orelse undefined)});
    std.debug.print("{any}\n", .{(scores.get("bob") orelse undefined)});
    std.debug.print("{s}\n", .{"ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Gauge) return _reflect_Gauge_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Gauge) return .{ .items = _reflect_Gauge_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Gauge) return .{ .items = _reflect_Gauge_field_types };
    return .{ .items = &.{} };
}
