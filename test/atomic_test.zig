...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/atomic_test.zbr:2
    var counter: *_Atomic(i64) = _atomic_create(i64, 0);
    counter.store(5);
    std.debug.print("{any}\n", .{counter.load()});
// zbr:test/atomic_test.zbr:5
    const old: i64 = counter.add(3);
    std.debug.print("{}\n", .{old});
    std.debug.print("{any}\n", .{counter.load()});
// zbr:test/atomic_test.zbr:8
    const swapped: i64 = counter.swap(0);
    std.debug.print("{}\n", .{swapped});
    std.debug.print("{any}\n", .{counter.load()});
// zbr:test/atomic_test.zbr:11
    const ok: bool = counter.cas(0, 42);
    std.debug.print("{}\n", .{ok});
    std.debug.print("{any}\n", .{counter.load()});
// zbr:test/atomic_test.zbr:14
    var flag: *_Atomic(bool) = _atomic_create(bool, false);
    flag.store(true);
    std.debug.print("{any}\n", .{flag.load()});
    std.debug.print("{s}\n", .{"atomic_test OK"});
}
