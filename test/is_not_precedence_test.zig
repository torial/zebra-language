...
// Source: test/is_not_precedence_test.zbr
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/is_not_precedence_test.zbr:12
    const c = Color.red;
// zbr:test/is_not_precedence_test.zbr:18
    const r1: bool = (!(!(c == .blue)));
    std.debug.print("{}\n", .{r1});
// zbr:test/is_not_precedence_test.zbr:25
    const r2: bool = (!(!(c == .red)));
    std.debug.print("{}\n", .{r2});
// zbr:test/is_not_precedence_test.zbr:29
    const r3: bool = (!(c == .green));
    std.debug.print("{}\n", .{r3});
// zbr:test/is_not_precedence_test.zbr:33
    const r4: bool = (!(c == .blue));
    std.debug.print("{}\n", .{r4});
// zbr:test/is_not_precedence_test.zbr:38
    const c2 = Color.green;
// zbr:test/is_not_precedence_test.zbr:40
    const r5: bool = ((!(c == .blue)) or (!(c2 == .red)));
    std.debug.print("{}\n", .{r5});
// zbr:test/is_not_precedence_test.zbr:43
    const r6: bool = ((!(c == .red)) or (!(c2 == .green)));
    std.debug.print("{}\n", .{r6});
    std.debug.print("{s}\n", .{"is_not_precedence: OK"});
}
