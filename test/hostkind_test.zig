...
pub const Widget = struct {
    _type_tag: u64 = _ttag_Widget,
    w: i64,
    pub fn init() *Widget {
        const _self = _allocator.create(Widget) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Widget");
// zbr:test/hostkind_test.zbr:8
            _self.w = 1;
        return _self;
    }

};
const _ttag_Widget: u64 = _zbr_hash("Widget");
const _reflect_Widget_name: []const u8 = "Widget";
const _reflect_Widget_fields: []const []const u8 = &.{"w"};
const _reflect_Widget_field_types: []const []const u8 = &.{"int"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/hostkind_test.zbr:11
    const n: i64 = 5;
// zbr:test/hostkind_test.zbr:12
    const f: f64 = 1.5;
// zbr:test/hostkind_test.zbr:13
    const s: []const u8 = "hi";
// zbr:test/hostkind_test.zbr:14
    const b = true;
// zbr:test/hostkind_test.zbr:15
    const wgt = Widget.init();
    std.debug.print("{any}\n", .{switch (@typeInfo(@TypeOf(n))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }});
    std.debug.print("{any}\n", .{switch (@typeInfo(@TypeOf(f))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }});
    std.debug.print("{any}\n", .{switch (@typeInfo(@TypeOf(s))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }});
    std.debug.print("{any}\n", .{switch (@typeInfo(@TypeOf(b))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }});
    std.debug.print("{any}\n", .{switch (@typeInfo(@TypeOf(wgt))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }});
    std.debug.print("{any}\n", .{switch (@typeInfo(@TypeOf(Widget.init()))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }});
// zbr:test/hostkind_test.zbr:24
    if (std.mem.eql(u8, switch (@typeInfo(@TypeOf(s))) { .bool => "bool", .int, .comptime_int => "int", .float, .comptime_float => "float", .pointer => |_p| _sw: { const _ci = @typeInfo(_p.child); break :_sw if (_p.child == u8 or (_ci == .array and _ci.array.child == u8)) "string" else "ref"; }, .@"fn" => "function", .optional, .null => "nil", else => "ref" }, "string")) {
        std.debug.print("{s}\n", .{"hostKind: OK"});
    }
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Widget) return _reflect_Widget_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Widget) return .{ .items = _reflect_Widget_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Widget) return .{ .items = _reflect_Widget_field_types };
    return .{ .items = &.{} };
}
