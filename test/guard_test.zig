...
// zbr:test/guard_test.zbr:6
            { _error_ctx = .{ .message = "expected positive number" }; return error.ZebraError; }
        }
...
// zbr:test/guard_test.zbr:12
        return s[@as(usize, @intCast(0))..@as(usize, @intCast(1))];
    }

    pub fn init() *Validator {
        const self = _allocator.create(Validator) catch @panic("OOM");
        self._type_tag = _ttag_Validator;
        return self;
    }

};
const _ttag_Validator: u64 = _zbr_hash("Validator");
...
// zbr:test/guard_test.zbr:19
                const x = Validator.checkPositive(42) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                std.debug.print("{}\n", .{x});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{(std.mem.concat(_allocator, u8, &.{ "unexpected error: ", _zbr_error_msg() }) catch unreachable)});
            }
        }
// zbr:test/guard_test.zbr:25
        const c = Validator.firstChar("hello");
        std.debug.print("{s}\n", .{c});
// zbr:test/guard_test.zbr:28
        const empty = Validator.firstChar("");
        std.debug.print("{s}\n", .{empty});
// zbr:test/guard_test.zbr:32
        {
            var _try_err: ?anyerror = null;
            _ = &_try_err;
            _try_blk: {
// zbr:test/guard_test.zbr:33
                const bad = Validator.checkPositive((-5)) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                std.debug.print("{}\n", .{bad});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{(std.mem.concat(_allocator, u8, &.{ "guard raised: ", _zbr_error_msg() }) catch unreachable)});
            }
        }
        std.debug.print("{s}\n", .{"ok"});
    }
