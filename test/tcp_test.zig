...
// Source: C:/Projects/zebra-language/test/tcp_test.zbr
...
fn _udp_socket() UdpSocket {
    const s = std.c.socket(@intFromEnum(std.posix.AF.INET), @intFromEnum(std.posix.SOCK.DGRAM), 0);
    if (s < 0) @panic("Udp.socket: socket() failed");
    return .{ .handle = @intCast(s) };
}
fn _udp_send(sock: UdpSocket, host: []const u8, port: u16, data: []const u8) void {
    ...
    switch (addr) {
        .ip4 => |a| {
            ...
            _ = std.c.sendto(sock.handle, data.ptr, data.len, 0, @ptrCast(&sa), @sizeOf(std.posix.sockaddr.in));
        },
        .ip6 => std.debug.print("UDP send: IPv6 not supported\n", .{}),
    }
}
fn _udp_recv(sock: UdpSocket, max_bytes: usize) []const u8 {
    const buf = std.heap.page_allocator.alloc(u8, max_bytes) catch @panic("OOM");
    const n = std.c.recv(sock.handle, buf.ptr, buf.len, 0);
    if (n < 0) return buf[0..0];
    return buf[0..@intCast(n)];
}
fn _udp_close(sock: UdpSocket) void { _ = std.c.close(sock.handle); }
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:C:/Projects/zebra-language/test/tcp_test.zbr:3
    const conn = _tcp_connect("httpbin.org", 80);
// zbr:C:/Projects/zebra-language/test/tcp_test.zbr:4
    if (conn) |c| {
        _tcp_write(c, "GET /get HTTP/1.0\r\nHost: httpbin.org\r\n\r\n");
// zbr:C:/Projects/zebra-language/test/tcp_test.zbr:6
        const data: []const u8 = _tcp_read(c);
        _tcp_close(c);
        std.debug.print("{any}\n", .{@as(i64, @intCast(data.len))});
