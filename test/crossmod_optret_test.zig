...
// Source: test/crossmod_optret_test.zbr
...
const crossmod_optret_geom = @import("crossmod_optret_geom.zig");
const Vec3 = crossmod_optret_geom.Vec3;
const crossmod_optret_lib = @import("crossmod_optret_lib.zig");
const World = crossmod_optret_lib.World;
pub fn sizeOrDefault(wld: *World, found: bool) *Vec3 {
// zbr:test/crossmod_optret_test.zbr:8
    if (wld.getSize(found)) |s| {
// zbr:test/crossmod_optret_test.zbr:9
        return s;
    }
// zbr:test/crossmod_optret_test.zbr:10
    return Vec3.init(9, 9, 9);
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
    @import("crossmod_optret_geom.zig")._initAllocator(_allocator);
    @import("crossmod_optret_geom.zig")._initIo(_io);
    @import("crossmod_optret_lib.zig")._initAllocator(_allocator);
    @import("crossmod_optret_lib.zig")._initIo(_io);
// zbr:test/crossmod_optret_test.zbr:13
    const wld: *World = World.init(5);
// zbr:test/crossmod_optret_test.zbr:14
    const hit: *Vec3 = sizeOrDefault(wld, true);
// zbr:test/crossmod_optret_test.zbr:15
    std.debug.assert((hit.x == 1));
// zbr:test/crossmod_optret_test.zbr:16
    std.debug.assert((hit.z == 3));
// zbr:test/crossmod_optret_test.zbr:17
    const miss: *Vec3 = sizeOrDefault(wld, false);
// zbr:test/crossmod_optret_test.zbr:18
    std.debug.assert((miss.x == 9));
    std.debug.print("{s}\n", .{"crossmod_optret: OK"});
}
...
fn _zbr_error_msg() []const u8 {
    ...
    if (@import("crossmod_optret_geom.zig")._error_ctx.message.len > 0) return @import("crossmod_optret_geom.zig")._error_ctx.message;
    if (@import("crossmod_optret_lib.zig")._error_ctx.message.len > 0) return @import("crossmod_optret_lib.zig")._error_ctx.message;
    return "";
}
