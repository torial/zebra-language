...
pub fn main(init: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/string_format_test.zbr:4
    const n: i32 = 255;
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{x:0>8}", .{@as(u32, @bitCast(n))}) catch @panic("OOM"))});
// zbr:test/string_format_test.zbr:8
    const fval: f64 = 3.14159;
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{d:.2}", .{fval}) catch @panic("OOM"))});
// zbr:test/string_format_test.zbr:12
    const greeting: []const u8 = "hello";
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{s:>20}", .{greeting}) catch @panic("OOM"))});
// zbr:test/string_format_test.zbr:16
    const tag: []const u8 = "ok";
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{s:-<15}", .{tag}) catch @panic("OOM"))});
// zbr:test/string_format_test.zbr:20
...
// zbr:test/string_format_test.zbr:26
    const s: []const u8 = "ab";
// zbr:test/string_format_test.zbr:27
    for (s) |c| {
        std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{c}", .{c}) catch @panic("OOM"))});
    }
}
