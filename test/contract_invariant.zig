...
// Source: test/contract_invariant.zbr
...
pub const Account = struct {
    _type_tag: u64 = _ttag_Account,
    balance: i64 = 0,
    pub fn init(starting: i64) *Account {
        const _self = _allocator.create(Account) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Account");
// zbr:test/contract_invariant.zbr:8
            _self.balance = starting;
    ...
    pub fn deposit(self: *Account, amount: i64) void {
        ...
        if (!(_zebra_gt(amount, 0))) std.debug.panic("require failed in 'deposit'\n", .{});
// zbr:test/contract_invariant.zbr:13
        self.balance = (self.balance + amount);
    }

    pub fn withdraw(self: *Account, amount: i64) void {
        defer self._check_invariant();
        if (!(_zebra_gt(amount, 0))) std.debug.panic("require failed in 'withdraw'\n", .{});
        if (!(_zebra_le(amount, self.balance))) std.debug.panic("require failed in 'withdraw'\n", .{});
// zbr:test/contract_invariant.zbr:19
        self.balance = (self.balance - amount);
    }

    fn _check_invariant(self: *Account) void {
        if (!(_zebra_ge(self.balance, 0))) std.debug.panic("invariant failed in 'Account'\n", .{});
    }

};
const _ttag_Account: u64 = _zbr_hash("Account");
const _reflect_Account_name: []const u8 = "Account";
const _reflect_Account_fields: []const []const u8 = &.{"balance"};
const _reflect_Account_field_types: []const []const u8 = &.{"int"};

pub const Gauge = struct {
    value: i64 = 0,
    max_: i64 = 100,
    pub fn init(initial: i64, cap: i64) Gauge {
        var _self: Gauge = undefined;
// zbr:test/contract_invariant.zbr:30
            _self.value = initial;
// zbr:test/contract_invariant.zbr:31
            _self.max_ = cap;
        _self._check_invariant();
        return _self;
    }
    ...
    pub fn set(self: *Gauge, v: i64) void {
        defer self._check_invariant();
        if (!(_zebra_ge(v, 0))) std.debug.panic("require failed in 'set'\n", .{});
        if (!(_zebra_le(v, self.max_))) std.debug.panic("require failed in 'set'\n", .{});
// zbr:test/contract_invariant.zbr:37
        self.value = v;
    }

    fn _check_invariant(self: *Gauge) void {
        if (!(_zebra_ge(self.value, 0))) std.debug.panic("invariant failed in 'Gauge'\n", .{});
        if (!(_zebra_le(self.value, self.max_))) std.debug.panic("invariant failed in 'Gauge'\n", .{});
    }

};

pub fn main() void {
// zbr:test/contract_invariant.zbr:40
    var a = Account.init(10);
    a.deposit(5);
    a.withdraw(3);
// zbr:test/contract_invariant.zbr:43
    var g = Gauge.init(50, 100);
    g.set(75);
    std.debug.print("{s}\n", .{"ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Account) return _reflect_Account_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Account) return .{ .items = _reflect_Account_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Account) return .{ .items = _reflect_Account_field_types };
    return .{ .items = &.{} };
}
