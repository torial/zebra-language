...
// zbr:test/fuzzy_selfhost.zbr:7
        var ngrams = std.StringHashMap(i64).init(_allocator);
// zbr:test/fuzzy_selfhost.zbr:8
        var clist = std.ArrayList([]const u8){};
// zbr:test/fuzzy_selfhost.zbr:9
        var _cp_it_1 = std.unicode.Utf8View.initUnchecked(text).iterator();
        while (_cp_it_1.nextCodepoint()) |c| {
// zbr:test/fuzzy_selfhost.zbr:10
            clist.append(_allocator, _intern((blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(c, &_cpbuf) catch 1; const _cpout = _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); break :blk @as([]const u8, _cpout); }))) catch unreachable;
        }
// zbr:test/fuzzy_selfhost.zbr:11
        var i: i64 = 0;
// zbr:test/fuzzy_selfhost.zbr:12
        while (_zebra_lt(i, (@as(i64, @intCast(clist.items.len)) - 2))) {
// zbr:test/fuzzy_selfhost.zbr:13
...
// zbr:test/fuzzy_selfhost.zbr:14
            if (ngrams.contains(tri)) {
// zbr:test/fuzzy_selfhost.zbr:15
                ngrams.put(_intern(tri), ((ngrams.get(tri) orelse undefined) + 1)) catch unreachable;
            } else {
// zbr:test/fuzzy_selfhost.zbr:17
                ngrams.put(_intern(tri), 1) catch unreachable;
            }
// zbr:test/fuzzy_selfhost.zbr:18
            i = (i + 1);
        }
// zbr:test/fuzzy_selfhost.zbr:19
        return ngrams;
    }

    pub fn intersection(a: std.StringHashMap(i64), b: std.StringHashMap(i64)) i64 {
// zbr:test/fuzzy_selfhost.zbr:22
        var total: i64 = 0;
// zbr:test/fuzzy_selfhost.zbr:23
        var _it_tri = a.iterator();
        while (_it_tri.next()) |_e_tri| {
            const tri = _e_tri.key_ptr.*;
            const ca = _e_tri.value_ptr.*;
// zbr:test/fuzzy_selfhost.zbr:24
            if (b.contains(tri)) {
// zbr:test/fuzzy_selfhost.zbr:25
                const cb = (b.get(tri) orelse undefined);
// zbr:test/fuzzy_selfhost.zbr:26
                if (_zebra_lt(ca, cb)) {
// zbr:test/fuzzy_selfhost.zbr:27
                    total = (total + ca);
                } else {
// zbr:test/fuzzy_selfhost.zbr:29
                    total = (total + cb);
                }
            }
        }
// zbr:test/fuzzy_selfhost.zbr:30
        return total;
    }

    pub fn loadVerses(path: []const u8) std.ArrayList([]const u8) {
// zbr:test/fuzzy_selfhost.zbr:33
        var verses = std.ArrayList([]const u8){};
// zbr:test/fuzzy_selfhost.zbr:34
