...
pub const IDE = struct {
    ...
    pub fn loadFileList() void {
        const entries: std.ArrayList([]const u8) = @compileError("selfhost: unknown File.listDir");
        IDE.file_list = std.ArrayList([]const u8){};
        for (entries.items) |f| {
            if ((std.mem.endsWith(u8, f, ".zbr") and (!std.mem.endsWith(u8, f, "zebra_ide.zbr")))) {
                IDE.file_list.append(_allocator, f) catch @panic("OOM");
            }
        }
    }
    ...
    pub fn runFile() void {
        ...
        IDE.run_output = (blk_sh: {
            ...
            break :blk_sh std.mem.concat(_allocator, u8, &[_][]const u8{ _sh_res.stdout, _sh_res.stderr }) catch @panic("OOM");
        });
    }

    pub fn frame(g: Gui) void {
        g.text("Files");
        g.separator();
    ...
    pub fn init() *IDE {
        const self = _allocator.create(IDE) catch @panic("OOM");
        self._type_tag = _ttag_IDE;
        return self;
    }

};
const _ttag_IDE: u64 = _zbr_hash("IDE");
const _reflect_IDE_name: []const u8 = "IDE";
const _reflect_IDE_fields: []const []const u8 = &.{"current_file", "source_code", "run_output", "file_list"};
const _reflect_IDE_field_types: []const []const u8 = &.{"str", "str", "str", "List(str)"};
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_IDE) return _reflect_IDE_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_IDE) return .{ .items = _reflect_IDE_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_IDE) return .{ .items = _reflect_IDE_field_types };
    return .{ .items = &.{} };
}
