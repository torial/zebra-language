...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        std.debug.print("{}\n", .{((atan2_val > 0.785) and (atan2_val < 0.786))});
        std.debug.print("{}\n", .{(std.math.sqrt(9.0) > 2.999)});
        std.debug.print("{}\n", .{(std.math.pow(f64, 2.0, 10.0) > 1023.9)});
        std.debug.print("{}\n", .{(std.math.log(Math.E) > 0.999)});
        std.debug.print("{}\n", .{(std.math.log2(8.0) > 2.999)});
        std.debug.print("{}\n", .{(std.math.log10(1000.0) > 2.999)});
        std.debug.print("{}\n", .{(std.math.exp(0.0) > 0.999)});
        std.debug.print("{}\n", .{(@floor(3.9) > 2.999)});
        std.debug.print("{}\n", .{(@ceil(3.1) > 3.999)});
        std.debug.print("{}\n", .{(std.math.round(3.5) > 3.999)});
        std.debug.print("{}\n", .{(std.math.trunc(3.9) > 2.999)});
        std.debug.print("{}\n", .{(@abs((-5.0)) > 4.999)});
        std.debug.print("{}\n", .{(@abs((-7)) == 7)});
        std.debug.print("{}\n", .{(@min(3.0, 7.0) > 2.999)});
        std.debug.print("{}\n", .{(@max(3.0, 7.0) > 6.999)});
        std.debug.print("{}\n", .{(std.math.clamp(15.0, 0.0, 10.0) > 9.999)});
