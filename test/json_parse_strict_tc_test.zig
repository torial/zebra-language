...
// Source: test/json_parse_strict_tc_test.zbr
...
pub const User = struct {
    _type_tag: u64 = _ttag_User,
    name: []const u8 = "",
    age: i64 = 0,
    rate: f64 = 0.0,
    ok: bool = false,
    pub fn init() *User {
        const self = _allocator.create(User) catch @panic("OOM");
        self.* = .{};
        self._type_tag = _ttag_User;
        return self;
    }

};

const _ttag_User: u64 = 3768991250;
const _reflect_User_name: []const u8 = "User";
const _reflect_User_fields: []const []const u8 = &.{"name", "age", "rate", "ok"};
const _reflect_User_field_types: []const []const u8 = &.{"str", "int", "float", "bool"};

fn _json_parse_strict_User(_src: []const u8) ?*User {
    const _v = _json_parse(_src) orelse return null;
    if (!_json_is_object(_v)) return null;
    var _it = _v.object.iterator();
    while (_it.next()) |_e| {
        const _k = _e.key_ptr.*;
        const _known = std.mem.eql(u8, _k, "name") or std.mem.eql(u8, _k, "age") or std.mem.eql(u8, _k, "rate") or std.mem.eql(u8, _k, "ok");
        if (!_known) return null;
    }
    { const _x = _v.object.get("name") orelse return null;
      switch (_x) { .string => {}, else => return null } }
    { const _x = _v.object.get("age") orelse return null;
      switch (_x) { .integer => {}, else => return null } }
    { const _x = _v.object.get("rate") orelse return null;
      switch (_x) { .float, .integer => {}, else => return null } }
    { const _x = _v.object.get("ok") orelse return null;
      switch (_x) { .bool => {}, else => return null } }
    const _r = _allocator.create(User) catch return null;
    _r.* = .{};
    _r._type_tag = _ttag_User;
    _r.name = _json_get_str(_v, "name");
    _r.age = _json_get_int(_v, "age");
    _r.rate = _json_get_float(_v, "rate");
    _r.ok = _json_get_bool(_v, "ok");
    return _r;
}

pub fn takesOpt(u: ?*User) []const u8 {
// zbr:test/json_parse_strict_tc_test.zbr:21
    if (u) |user| {
// zbr:test/json_parse_strict_tc_test.zbr:22
        return user.name;
    }
// zbr:test/json_parse_strict_tc_test.zbr:23
    return "(none)";
}

pub fn main() void {
// zbr:test/json_parse_strict_tc_test.zbr:26
    const src = "{\"name\":\"Alice\",\"age\":30,\"rate\":1.5,\"ok\":true}";
// zbr:test/json_parse_strict_tc_test.zbr:29
    const maybe = _json_parse_strict_User(src);
// zbr:test/json_parse_strict_tc_test.zbr:30
    if (maybe) |u| {
// zbr:test/json_parse_strict_tc_test.zbr:31
        std.debug.assert(std.mem.eql(u8, u.name, "Alice"));
    }
// zbr:test/json_parse_strict_tc_test.zbr:34
    std.debug.assert(std.mem.eql(u8, takesOpt(_json_parse_strict_User(src)), "Alice"));
// zbr:test/json_parse_strict_tc_test.zbr:37
    const u_typed: ?*User = _json_parse_strict_User(src);
// zbr:test/json_parse_strict_tc_test.zbr:38
    if (u_typed) |inner| {
// zbr:test/json_parse_strict_tc_test.zbr:39
        std.debug.assert((inner.age == 30));
    }
// zbr:test/json_parse_strict_tc_test.zbr:42
    if (_json_parse_strict_User(src)) |direct| {
// zbr:test/json_parse_strict_tc_test.zbr:43
        std.debug.assert((direct.rate == 1.5));
    }
// zbr:test/json_parse_strict_tc_test.zbr:45
    std.debug.print("{s}\n", .{"json_parse_strict_tc_test OK"});
}
