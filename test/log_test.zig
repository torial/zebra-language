...
// Source: C:/Projects/zebra-language/test/log_test.zbr
...
pub const LogTest = struct {
    ...
    pub fn main() void {
        ...
        _log_set_output_stderr("stdout");
        _log_set_level("info");
        _log_info("writing to stdout now");
        _log_set_output_stderr("stderr");
        std.debug.print("{s}\n", .{"All log tests passed."});
    }

    pub fn init() *LogTest {
        const self = _allocator.create(LogTest) catch @panic("OOM");
        self._type_tag = _ttag_LogTest;
        return self;
    }

};
const _ttag_LogTest: u64 = _zbr_hash("LogTest");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_LogTest) return _reflect_LogTest_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_LogTest) return .{ .items = _reflect_LogTest_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_LogTest) return .{ .items = _reflect_LogTest_field_types };
    return .{ .items = &.{} };
}
