...
// Source: test/dynlib_iface_throws_test.zbr
...
pub const IParser = struct {
    ...
    pub const VTable = struct {
        parse: *const fn (ptr: *anyopaque, input: []const u8) anyerror!i64,
    };

    pub fn parse(self: @This(), input: []const u8) anyerror!i64 {
        return try self.vtable.parse(self.ptr, input);
    }

    pub fn check(comptime T: type) void {
        comptime {
            if (!@hasDecl(T, "parse")) @compileError("type " ++ @typeName(T) ++ " does not implement IParser.parse");
        }
    }
};

pub const SafeParser = struct {
    _type_tag: u64 = _ttag_SafeParser,
    pub fn parse(self: *SafeParser, input: []const u8) anyerror!i64 {
        _ = self;
// zbr:test/dynlib_iface_throws_test.zbr:6
        if (std.mem.eql(u8, input, "")) {
// zbr:test/dynlib_iface_throws_test.zbr:7
            { _error_ctx = .{ .message = "empty input" }; return error.ZebraError; }
        }
// zbr:test/dynlib_iface_throws_test.zbr:8
        return @as(i64, @intCast(input.len));
    }

    pub fn init() *SafeParser {
        const self = _allocator.create(SafeParser) catch @panic("OOM");
        self.* = .{};
        self._type_tag = _ttag_SafeParser;
        return self;
    }
...
fn _shim_SafeParser_IParser_parse(ptr: *anyopaque, input: []const u8) anyerror!i64 { return try SafeParser.parse(@alignCast(@ptrCast(ptr)), input); }
const _vtable_SafeParser_IParser = IParser.VTable{ .parse = &_shim_SafeParser_IParser_parse, };
const _ttag_SafeParser: u64 = _zbr_hash("SafeParser");
const _reflect_SafeParser_name: []const u8 = "SafeParser";
...
// zbr:test/dynlib_iface_throws_test.zbr:12
    const r1: i64 = p.parse("hello") catch (-1);
// zbr:test/dynlib_iface_throws_test.zbr:13
    if (!((r1 == 5))) @panic("expected 5");
// zbr:test/dynlib_iface_throws_test.zbr:14
    const r2: i64 = p.parse("") catch (-1);
// zbr:test/dynlib_iface_throws_test.zbr:15
    if (!((r2 == (-1)))) @panic("expected -1 from throw path");
    std.debug.print("{s}\n", .{"dynlib_iface_throws: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_SafeParser) return _reflect_SafeParser_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_SafeParser) return .{ .items = _reflect_SafeParser_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_SafeParser) return .{ .items = _reflect_SafeParser_field_types };
    return .{ .items = &.{} };
}
