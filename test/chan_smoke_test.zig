...
// Source: test/chan_smoke_test.zbr
...
pub fn main(init: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/chan_smoke_test.zbr:5
    var ch: *_Chan(i64) = _chan_create(i64, 3);
    ch.send(10);
    ch.send(20);
    ch.send(30);
// zbr:test/chan_smoke_test.zbr:10
    const a: ?i64 = ch.recv();
// zbr:test/chan_smoke_test.zbr:11
    const b: ?i64 = ch.recv();
// zbr:test/chan_smoke_test.zbr:12
    const c: ?i64 = ch.recv();
// zbr:test/chan_smoke_test.zbr:14
    std.debug.assert((a == 10));
// zbr:test/chan_smoke_test.zbr:15
    std.debug.assert((b == 20));
// zbr:test/chan_smoke_test.zbr:16
    std.debug.assert((c == 30));
// zbr:test/chan_smoke_test.zbr:19
    var ch2: *_Chan([]const u8) = _chan_create([]const u8, 2);
    ch2.send("hello");
    ch2.send("world");
    ch2.close();
// zbr:test/chan_smoke_test.zbr:24
    const s1: ?[]const u8 = ch2.recv();
// zbr:test/chan_smoke_test.zbr:25
    const s2: ?[]const u8 = ch2.recv();
// zbr:test/chan_smoke_test.zbr:26
    const s3: ?[]const u8 = ch2.recv();
// zbr:test/chan_smoke_test.zbr:28
    if (s1) |sv1| {
// zbr:test/chan_smoke_test.zbr:29
        std.debug.assert(std.mem.eql(u8, sv1, "hello"));
    } else {
// zbr:test/chan_smoke_test.zbr:31
        std.debug.assert(false);
    }
// zbr:test/chan_smoke_test.zbr:32
    if (s2) |sv2| {
// zbr:test/chan_smoke_test.zbr:33
        std.debug.assert(std.mem.eql(u8, sv2, "world"));
    } else {
// zbr:test/chan_smoke_test.zbr:35
        std.debug.assert(false);
    }
// zbr:test/chan_smoke_test.zbr:36
    std.debug.assert((s3 == null));
// zbr:test/chan_smoke_test.zbr:39
    var ch3: *_Chan(i64) = _chan_create(i64, 2);
// zbr:test/chan_smoke_test.zbr:40
    ch3.send(42);
// zbr:test/chan_smoke_test.zbr:41
    ch3.send(99);
// zbr:test/chan_smoke_test.zbr:43
    var x: ?i64 = null;
// zbr:test/chan_smoke_test.zbr:44
    x = ch3.recv();
// zbr:test/chan_smoke_test.zbr:45
    var y: ?i64 = null;
// zbr:test/chan_smoke_test.zbr:46
    y = ch3.recv();
// zbr:test/chan_smoke_test.zbr:48
    std.debug.assert((x == 42));
// zbr:test/chan_smoke_test.zbr:49
    std.debug.assert((y == 99));
    std.debug.print("{s}\n", .{"chan: OK"});
}
