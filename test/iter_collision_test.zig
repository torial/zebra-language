...
// Source: test/iter_collision_test.zbr
...
pub fn main() void {
// zbr:test/iter_collision_test.zbr:4
    const s1: []const u8 = "a,b,c";
// zbr:test/iter_collision_test.zbr:5
    const s2: []const u8 = "x,y,z";
// zbr:test/iter_collision_test.zbr:6
    var out1: []const u8 = "";
// zbr:test/iter_collision_test.zbr:7
    var out2: []const u8 = "";
// zbr:test/iter_collision_test.zbr:8
    {
        var _it_p = std.mem.splitSequence(u8, s1, ",");
        while (_it_p.next()) |p| {
// zbr:test/iter_collision_test.zbr:9
            out1 = _str_concat(out1, p, _allocator);
        }
    }
// zbr:test/iter_collision_test.zbr:10
    {
        var _it_p = std.mem.splitSequence(u8, s2, ",");
        while (_it_p.next()) |p| {
// zbr:test/iter_collision_test.zbr:11
            out2 = _str_concat(out2, p, _allocator);
        }
    }
// zbr:test/iter_collision_test.zbr:12
    var m = std.StringHashMap([]const u8).init(_allocator);
    m.put(_intern("k1"), _intern("hello")) catch @panic("OOM");
    m.put(_intern("k2"), _intern("world")) catch @panic("OOM");
// zbr:test/iter_collision_test.zbr:15
    var m2 = std.StringHashMap([]const u8).init(_allocator);
    m2.put(_intern("k1"), _intern("foo")) catch @panic("OOM");
// zbr:test/iter_collision_test.zbr:17
    var count1: i64 = 0;
// zbr:test/iter_collision_test.zbr:18
    var count2: i64 = 0;
// zbr:test/iter_collision_test.zbr:19
    {
        var _it_k = m.iterator();
        while (_it_k.next()) |_ent_k| {
            const k = _ent_k.key_ptr.*;
            _ = k;
            const v = _ent_k.value_ptr.*;
// zbr:test/iter_collision_test.zbr:20
            if (_zebra_gt(@as(i64, @intCast(v.len)), 0)) {
// zbr:test/iter_collision_test.zbr:21
                count1 += 1;
            }
        }
    }
// zbr:test/iter_collision_test.zbr:22
    {
        var _it_k = m2.iterator();
        while (_it_k.next()) |_ent_k| {
            const k = _ent_k.key_ptr.*;
            _ = k;
            const v = _ent_k.value_ptr.*;
// zbr:test/iter_collision_test.zbr:23
            if (_zebra_gt(@as(i64, @intCast(v.len)), 0)) {
// zbr:test/iter_collision_test.zbr:24
                count2 += 1;
            }
        }
    }
    std.debug.print("{s}\n", .{_str_concat(_str_concat(_str_concat(_str_concat(_str_concat(_str_concat(out1, " ", _allocator), out2, _allocator), " ", _allocator), (std.fmt.allocPrint(_allocator, "{}", .{count1}) catch unreachable), _allocator), " ", _allocator), (std.fmt.allocPrint(_allocator, "{}", .{count2}) catch unreachable), _allocator)});
}
