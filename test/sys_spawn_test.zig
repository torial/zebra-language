...
// Source: test/sys_spawn_test.zbr
...
pub fn main() void {
// zbr:test/sys_spawn_test.zbr:5
    var argv = std.ArrayList([]const u8){};
    argv.append(_allocator, _intern("zig")) catch @panic("OOM");
    argv.append(_allocator, _intern("version")) catch @panic("OOM");
// zbr:test/sys_spawn_test.zbr:8
    const proc = _sys_spawn(argv);
// zbr:test/sys_spawn_test.zbr:11
    if (!(_zebra_ge(proc.pid, (-1)))) @panic(_str_concat("expected pid >= -1, got ", (std.fmt.allocPrint(_allocator, "{}", .{proc.pid}) catch unreachable), _allocator));
// zbr:test/sys_spawn_test.zbr:14
    var waited: i64 = 0;
// zbr:test/sys_spawn_test.zbr:15
    while (_sys_process_is_running(proc)) {
        std.Thread.sleep(@as(u64, @intCast(100)) * std.time.ns_per_ms);
// zbr:test/sys_spawn_test.zbr:17
        waited = (waited + 1);
// zbr:test/sys_spawn_test.zbr:18
        if (_zebra_ge(waited, 20)) {
            break;
        }
    }
// zbr:test/sys_spawn_test.zbr:22
    if (!((!_sys_process_is_running(proc)))) @panic("expected process to have exited");
// zbr:test/sys_spawn_test.zbr:25
    var argv2 = std.ArrayList([]const u8){};
    argv2.append(_allocator, _intern("zig")) catch @panic("OOM");
    argv2.append(_allocator, _intern("version")) catch @panic("OOM");
// zbr:test/sys_spawn_test.zbr:28
    const proc2 = _sys_spawn(argv2);
    _sys_process_kill(proc2);
// zbr:test/sys_spawn_test.zbr:30
    if (!((!_sys_process_is_running(proc2)))) @panic("expected killed process to not be running");
    std.debug.print("{s}\n", .{"sys_spawn_test: ok"});
}
