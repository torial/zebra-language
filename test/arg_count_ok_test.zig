...
// Source: test/arg_count_ok_test.zbr
...
pub fn greet(name: []const u8, greeting: []const u8) []const u8 {
// zbr:test/arg_count_ok_test.zbr:3
    return _str_concat(_str_concat(greeting, ", ", _allocator), name, _allocator);
}

pub fn twice(x: i64) i64 {
// zbr:test/arg_count_ok_test.zbr:5
    return (x + x);
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    std.debug.print("{s}\n", .{greet("World", "Hello")});
    std.debug.print("{s}\n", .{greet("World", "Hi")});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}", .{twice(21)}) catch unreachable)});
}
