...
// Source: test/contract_old_compound_test.zbr
...
pub const Ratchet = struct {
    _type_tag: u64 = _ttag_Ratchet,
    val: i64 = 0,
    pub fn advance(self: *Ratchet, n: i64) void {
        const _old_0 = self.val;
        defer {
            if (!(_zebra_in(self.val, .{_old_0, n}))) std.debug.panic("ensure failed in 'advance'\n", .{});
        }
// zbr:test/contract_old_compound_test.zbr:11
        self.val = n;
    }

    pub fn init() *Ratchet {
        const self = _allocator.create(Ratchet) catch @panic("OOM");
        self._type_tag = _ttag_Ratchet;
        self.val = 0;
        return self;
    }

};
const _ttag_Ratchet: u64 = _zbr_hash("Ratchet");
const _reflect_Ratchet_name: []const u8 = "Ratchet";
const _reflect_Ratchet_fields: []const []const u8 = &.{"val"};
const _reflect_Ratchet_field_types: []const []const u8 = &.{"int"};

pub fn main() void {
// zbr:test/contract_old_compound_test.zbr:14
    var r = Ratchet.init();
    r.advance(3);
// zbr:test/contract_old_compound_test.zbr:16
    std.debug.assert((r.val == 3));
    r.advance(7);
// zbr:test/contract_old_compound_test.zbr:18
    std.debug.assert((r.val == 7));
    std.debug.print("{s}\n", .{"contract_old_compound: OK"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Ratchet) return _reflect_Ratchet_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Ratchet) return .{ .items = _reflect_Ratchet_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Ratchet) return .{ .items = _reflect_Ratchet_field_types };
    return .{ .items = &.{} };
}
