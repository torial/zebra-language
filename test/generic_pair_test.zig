...
// Source: C:/Projects/zebra-language/test/generic_pair_test.zbr
...
pub const Pair = struct {
    ...
    pub fn swap(self: *Pair) Pair(B, A) {
        return Pair.init(B, A)(self.second, self.first);
    }

};
const _ttag_Pair: u64 = _zbr_hash("Pair");
const _reflect_Pair_name: []const u8 = "Pair";
const _reflect_Pair_fields: []const []const u8 = &.{"first", "second"};
const _reflect_Pair_field_types: []const []const u8 = &.{"A", "B"};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
        const p = Pair.init(str, int)("hello", 42);
        std.debug.print("{any}\n", .{p.second});
        const q = Pair.init(int, bool)(100, true);
        std.debug.print("{any}\n", .{q.first});
        std.debug.print("{any}\n", .{q.second});
        var r = Pair.init(int, str)(1, "one");
        std.debug.print("{any}\n", .{r.first});
        const s = r.swap();
        std.debug.print("{any}\n", .{s.second});
    }
