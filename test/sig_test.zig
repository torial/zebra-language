...
// Source: C:/Projects/zebra-language/test/sig_test.zbr
...
pub fn spanWhile(src: []const u8, pos: i64, pred: CharPred) i64 {
    var i = pos;
    while ((_zebra_lt(i, src.len) and pred(src[@as(usize, @intCast(i))]))) {
        i = (i + 1);
    }
    return i;
}
...
pub const SigTest = struct {
    ...
    pub fn init() *SigTest {
        const self = _allocator.create(SigTest) catch @panic("OOM");
        self._type_tag = _ttag_SigTest;
        return self;
    }

};
const _ttag_SigTest: u64 = _zbr_hash("SigTest");
