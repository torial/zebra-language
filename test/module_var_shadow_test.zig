...
// Source: test/module_var_shadow_test.zbr
...
pub var _zbr_mv_item: i64 = 100;
pub var _zbr_mv_idx: i64 = 50;
pub fn getOpt() ?i64 {
// zbr:test/module_var_shadow_test.zbr:11
    return 7;
}

pub fn loopSum() i64 {
// zbr:test/module_var_shadow_test.zbr:14
    var total: i64 = 0;
// zbr:test/module_var_shadow_test.zbr:15
    for ((blk_ll_1: { var _ll_1: std.ArrayList(i64) = std.ArrayList(i64).empty; _ll_1.append(_allocator, 1) catch @panic("OOM"); _ll_1.append(_allocator, 2) catch @panic("OOM"); _ll_1.append(_allocator, 3) catch @panic("OOM"); break :blk_ll_1 _ll_1; }).items) |item| {
// zbr:test/module_var_shadow_test.zbr:16
        total = (total + item);
    }
// zbr:test/module_var_shadow_test.zbr:17
    return total;
}

pub fn numLoop() i64 {
// zbr:test/module_var_shadow_test.zbr:20
    var acc: i64 = 0;
// zbr:test/module_var_shadow_test.zbr:21
    {
        var idx: i64 = 1;
        const _stop_idx: i64 = 4;
        while (idx < _stop_idx) : (idx += 1) {
// zbr:test/module_var_shadow_test.zbr:22
            acc = (acc + idx);
        }
    }
// zbr:test/module_var_shadow_test.zbr:23
    return acc;
}

pub fn captured() i64 {
// zbr:test/module_var_shadow_test.zbr:26
    if (getOpt()) |idx| {
// zbr:test/module_var_shadow_test.zbr:27
        return idx;
    }
// zbr:test/module_var_shadow_test.zbr:28
    return (-1);
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "loopSum={}", .{loopSum()}) catch @panic("OOM"))});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "numLoop={}", .{numLoop()}) catch @panic("OOM"))});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "captured={}", .{captured()}) catch @panic("OOM"))});
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "item={} idx={}", .{_zbr_mv_item, _zbr_mv_idx}) catch @panic("OOM"))});
// zbr:test/module_var_shadow_test.zbr:35
    if (((_zbr_mv_item == 100) and (_zbr_mv_idx == 50))) {
        std.debug.print("{s}\n", .{"module_var_shadow_test: OK"});
    }
}
