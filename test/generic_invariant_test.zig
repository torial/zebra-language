...
pub fn BoundedSlot(comptime T: type) type {
    return struct {
        _type_tag: u64 = _ttag_BoundedSlot,
        item: ?T = undefined,
        count: i64 = undefined,
        limit: i64 = undefined,
        pub fn init(cap: i64) *@This() {
            const self = _allocator.create(@This()) catch @panic("OOM");
            self._type_tag = _ttag_BoundedSlot;
// zbr:test/generic_invariant_test.zbr:13
            self.item = null;
// zbr:test/generic_invariant_test.zbr:14
            self.count = 0;
// zbr:test/generic_invariant_test.zbr:15
            self.limit = cap;
        ...
        pub fn store(self: *@This(), v: T) void {
            ...
            if (!(_zebra_lt(self.count, self.limit))) std.debug.panic("require failed in 'store'\n", .{});
// zbr:test/generic_invariant_test.zbr:19
            self.item = v;
// zbr:test/generic_invariant_test.zbr:20
...
// zbr:test/generic_invariant_test.zbr:22
            self.item = null;
// zbr:test/generic_invariant_test.zbr:23
            self.count = 0;
        }

        pub fn full(self: *@This()) bool {
            defer self._check_invariant();
// zbr:test/generic_invariant_test.zbr:25
            return _zebra_ge(self.count, self.limit);
        }

        fn _check_invariant(self: *@This()) void {
            if (!(_zebra_ge(self.count, 0))) std.debug.panic("invariant failed in 'BoundedSlot'\n", .{});
            if (!(_zebra_le(self.count, self.limit))) std.debug.panic("invariant failed in 'BoundedSlot'\n", .{});
        }

    };
}

const _ttag_BoundedSlot: u64 = 4024928590;

pub const Tag = struct {
    _type_tag: u64 = _ttag_Tag,
    id: i64 = undefined,
    pub fn init(n: i64) *Tag {
        const self = _allocator.create(Tag) catch @panic("OOM");
        self._type_tag = _ttag_Tag;
// zbr:test/generic_invariant_test.zbr:30
        self.id = n;
        return self;
    }
...
const _ttag_Tag: u64 = 4169356339;
const _reflect_Tag_name: []const u8 = "Tag";
const _reflect_Tag_fields: []const []const u8 = &.{"id"};
const _reflect_Tag_field_types: []const []const u8 = &.{"int"};

pub fn main() void {
// zbr:test/generic_invariant_test.zbr:33
    var s = BoundedSlot(*Tag).init(2);
// zbr:test/generic_invariant_test.zbr:34
    s.store(Tag.init(1));
// zbr:test/generic_invariant_test.zbr:35
    s.store(Tag.init(2));
// zbr:test/generic_invariant_test.zbr:36
    s.clear();
// zbr:test/generic_invariant_test.zbr:37
    std.debug.print("{}\n", .{s.full()});
// zbr:test/generic_invariant_test.zbr:38
    std.debug.print("{s}\n", .{"ok"});
}
