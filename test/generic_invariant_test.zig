...
// Source: test/generic_invariant_test.zbr
...
pub const BoundedCounter = struct {
    _type_tag: u64 = _ttag_BoundedCounter,
    value: i64,
    lo: i64,
    hi: i64,
    pub fn init(start: i64, low: i64, high: i64) *BoundedCounter {
        const _self = _allocator.create(BoundedCounter) catch @panic("OOM");
        _self._type_tag = _zbr_hash("BoundedCounter");
// zbr:test/generic_invariant_test.zbr:12
            _self.lo = low;
// zbr:test/generic_invariant_test.zbr:13
            _self.hi = high;
// zbr:test/generic_invariant_test.zbr:14
            _self.value = start;
        _self._check_invariant();
        return _self;
    }

    pub fn inc(self: *BoundedCounter) void {
        defer self._check_invariant();
        if (!(_zebra_lt(self.value, self.hi))) std.debug.panic("require failed in 'inc'\n", .{});
// zbr:test/generic_invariant_test.zbr:18
        self.value = (self.value + 1);
    }

    pub fn dec(self: *BoundedCounter) void {
        defer self._check_invariant();
        if (!(_zebra_gt(self.value, self.lo))) std.debug.panic("require failed in 'dec'\n", .{});
// zbr:test/generic_invariant_test.zbr:22
        self.value = (self.value - 1);
    }

    pub fn get(self: *BoundedCounter) i64 {
        defer self._check_invariant();
// zbr:test/generic_invariant_test.zbr:24
        return self.value;
    }

    fn _check_invariant(self: *BoundedCounter) void {
        if (!(_zebra_ge(self.value, self.lo))) std.debug.panic("invariant failed in 'BoundedCounter'\n", .{});
        if (!(_zebra_le(self.value, self.hi))) std.debug.panic("invariant failed in 'BoundedCounter'\n", .{});
    }

};
const _ttag_BoundedCounter: u64 = _zbr_hash("BoundedCounter");
const _reflect_BoundedCounter_name: []const u8 = "BoundedCounter";
const _reflect_BoundedCounter_fields: []const []const u8 = &.{"value", "lo", "hi"};
const _reflect_BoundedCounter_field_types: []const []const u8 = &.{"int", "int", "int"};

pub fn main() void {
// zbr:test/generic_invariant_test.zbr:27
    var c = BoundedCounter.init(int)(5, 0, 10);
    c.inc();
    c.inc();
    c.dec();
    std.debug.print("{any}\n", .{c.get()});
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_BoundedCounter) return _reflect_BoundedCounter_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_BoundedCounter) return .{ .items = _reflect_BoundedCounter_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_BoundedCounter) return .{ .items = _reflect_BoundedCounter_field_types };
    return .{ .items = &.{} };
}
