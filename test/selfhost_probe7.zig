...
pub const Writer = struct {
    ...
    pub fn result(self: *Writer) []const u8 {
        return (self.buf.toOwnedSlice(_allocator) catch "");
    }
...
pub const SharedState = struct {
    ...
    pub fn init() *SharedState {
        const _self = _allocator.create(SharedState) catch @panic("OOM");
        _self._type_tag = _zbr_hash("SharedState");
            _self.count = 0;
        return _self;
    }
...
const _ttag_SharedState: u64 = _zbr_hash("SharedState");
...
pub const Generator = struct {
    ...
    pub fn withTryLabel(self: *Generator, label: []const u8, err_var: []const u8) Generator {
        const g = blk: { var _except_tmp = self.*; _except_tmp.try_label = label; _except_tmp.catch_var = err_var; break :blk _except_tmp; };
        return g;
    }

    pub fn emit(self: *Generator, s: []const u8) void {
        self.w.appendSlice(_allocator, s) catch @panic("OOM");
    }
...
pub const Main = struct {
    ...
    pub fn test1SharedClassViaStructCopy() void {
        ...
        g2.state.increment();
        g3.state.increment();
        if (!((g1.state.count == 3))) @panic("shared count via g1 should be 3");
        if (!((g2.state.count == 3))) @panic("shared count via g2 should be 3");
        if (!((g3.state.count == 3))) @panic("shared count via g3 should be 3");
        std.debug.print("{s}\n", .{"test1 OK"});
    }
    ...
    pub fn test3OptionalStrField() void {
        ...
        if (!((g.try_label == null))) @panic("try_label should start nil");
        if (!(std.mem.eql(u8, g.catch_var, ""))) @panic("catch_var should start empty");
        const g2 = g.withTryLabel("blk_try", "_err");
        if (!(std.mem.eql(u8, g2.try_label.?, "blk_try"))) @panic("try_label after withTryLabel");
        if (!(std.mem.eql(u8, g2.catch_var, "_err"))) @panic("catch_var after withTryLabel");
        if (!((g.try_label == null))) @panic("g.try_label should still be nil");
        std.debug.print("{s}\n", .{"test3 OK"});
    }
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    ...
    if (tag == _ttag_SharedState) return _reflect_SharedState_name;
...
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_SharedState) return .{ .items = _reflect_SharedState_fields };
...
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_SharedState) return .{ .items = _reflect_SharedState_field_types };
