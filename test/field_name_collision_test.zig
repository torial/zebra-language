...
// Source: test/field_name_collision_test.zbr
...
pub const NodeList = struct {
    _type_tag: u64 = _ttag_NodeList,
    source: std.ArrayList(i64),
    pub fn init() *NodeList {
        const _self = _allocator.create(NodeList) catch @panic("OOM");
        _self._type_tag = _zbr_hash("NodeList");
// zbr:test/field_name_collision_test.zbr:9
            _self.source = std.ArrayList(i64).empty;
        return _self;
    }

    pub fn size(self: *NodeList) i64 {
// zbr:test/field_name_collision_test.zbr:11
        return @as(i64, @intCast(self.source.items.len));
    }

};
const _ttag_NodeList: u64 = _zbr_hash("NodeList");
const _reflect_NodeList_name: []const u8 = "NodeList";
const _reflect_NodeList_fields: []const []const u8 = &.{"source"};
const _reflect_NodeList_field_types: []const []const u8 = &.{"List(int)"};

pub const TextBuf = struct {
    _type_tag: u64 = _ttag_TextBuf,
    source: []const u8,
    pub fn init(s: []const u8) *TextBuf {
        const _self = _allocator.create(TextBuf) catch @panic("OOM");
        _self._type_tag = _zbr_hash("TextBuf");
// zbr:test/field_name_collision_test.zbr:16
            _self.source = _intern(s);
        return _self;
    }

    pub fn size(self: *TextBuf) i64 {
// zbr:test/field_name_collision_test.zbr:18
        return @as(i64, @intCast(self.source.len));
    }

};
const _ttag_TextBuf: u64 = _zbr_hash("TextBuf");
const _reflect_TextBuf_name: []const u8 = "TextBuf";
const _reflect_TextBuf_fields: []const []const u8 = &.{"source"};
const _reflect_TextBuf_field_types: []const []const u8 = &.{"str"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/field_name_collision_test.zbr:21
    var t = TextBuf.init("hello");
// zbr:test/field_name_collision_test.zbr:22
    var nl = NodeList.init();
    nl.source.append(_allocator, 1) catch @panic("OOM");
    nl.source.append(_allocator, 2) catch @panic("OOM");
    nl.source.append(_allocator, 3) catch @panic("OOM");
    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "text={} list={}", .{t.size(), nl.size()}) catch @panic("OOM"))});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_NodeList) return _reflect_NodeList_name;
    if (tag == _ttag_TextBuf) return _reflect_TextBuf_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_NodeList) return .{ .items = _reflect_NodeList_fields };
    if (tag == _ttag_TextBuf) return .{ .items = _reflect_TextBuf_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_NodeList) return .{ .items = _reflect_NodeList_field_types };
    if (tag == _ttag_TextBuf) return .{ .items = _reflect_TextBuf_field_types };
    return .{ .items = &.{} };
}
