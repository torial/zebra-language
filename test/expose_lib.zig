...
pub const Palette = struct {
    ...
    pub fn init(n: []const u8) *Palette {
        const _self = _allocator.create(Palette) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Palette");
            _self.name = n;
        return _self;
    }
...
const _ttag_Palette: u64 = _zbr_hash("Palette");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Palette) return _reflect_Palette_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Palette) return .{ .items = _reflect_Palette_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Palette) return .{ .items = _reflect_Palette_field_types };
    return .{ .items = &.{} };
}
