...
// Source: test/contract_result_throws_test.zbr
...
pub const Main = struct {
    ...
    caught_count: i64 = 0,
    ok_count: i64 = 0,
    pub fn safeAbs(n: i64) anyerror!i64 {
        var _result: i64 = undefined;
        var _ensure_armed: bool = false;
        defer {
            if (_ensure_armed and !(_zebra_ge(_result, 0))) std.debug.panic("ensure failed in 'safeAbs'\n", .{});
        }
// zbr:test/contract_result_throws_test.zbr:13
        if (_zebra_lt(n, (-1000))) {
// zbr:test/contract_result_throws_test.zbr:14
            { _error_ctx = .{ .message = "out of range" }; return error.ZebraError; }
        }
// zbr:test/contract_result_throws_test.zbr:15
        if (_zebra_lt(n, 0)) {
// zbr:test/contract_result_throws_test.zbr:16
            { _result = (-n); _ensure_armed = true; return _result; }
        }
// zbr:test/contract_result_throws_test.zbr:17
        { _result = n; _ensure_armed = true; return _result; }
        _ensure_armed = true;
    }

    pub fn runOne(m: *Main, n: i64) void {
// zbr:test/contract_result_throws_test.zbr:19
        {
            var _try_err: ?anyerror = null;
            _ = &_try_err;
            _try_blk: {
// zbr:test/contract_result_throws_test.zbr:20
                const v: i64 = Main.safeAbs(n) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
// zbr:test/contract_result_throws_test.zbr:21
                m.ok_count = (m.ok_count + 1);
                std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "ok={}", .{v}) catch @panic("OOM"))});
                break :_try_blk;
            }
            if (_try_err != null) {
// zbr:test/contract_result_throws_test.zbr:24
    ...
    pub fn main() void {
// zbr:test/contract_result_throws_test.zbr:28
        const m = Main.init();
        Main.runOne(m, 5);
        Main.runOne(m, (-7));
        Main.runOne(m, (-2000));
// zbr:test/contract_result_throws_test.zbr:34
        std.debug.assert((m.ok_count == 2));
// zbr:test/contract_result_throws_test.zbr:35
        std.debug.assert((m.caught_count == 1));
        std.debug.print("{s}\n", .{"contract_result_throws_test OK"});
    }
...
const _reflect_Main_fields: []const []const u8 = &.{"caught_count", "ok_count"};
const _reflect_Main_field_types: []const []const u8 = &.{"int", "int"};
