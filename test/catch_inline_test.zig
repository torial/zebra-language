...
// Source: test/catch_inline_test.zbr
...
pub fn risky(n: i64) anyerror!i64 {
// zbr:test/catch_inline_test.zbr:2
    if (_zebra_lt(n, 0)) {
// zbr:test/catch_inline_test.zbr:3
        { _error_ctx = .{ .message = "negative" }; return error.ZebraError; }
    }
// zbr:test/catch_inline_test.zbr:4
    return (n * 2);
}

pub fn main() void {
// zbr:test/catch_inline_test.zbr:8
    const a = risky((-1)) catch 99;
// zbr:test/catch_inline_test.zbr:9
    if (!((a == 99))) @panic("expected fallback 99");
// zbr:test/catch_inline_test.zbr:12
    const c = risky(5) catch 99;
// zbr:test/catch_inline_test.zbr:13
    if (!((c == 10))) @panic("expected 10 from risky(5)");
    std.debug.print("{s}\n", .{"catch_inline: OK"});
}
