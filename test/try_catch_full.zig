...
// Source: C:/Projects/zebra-language/test/try_catch_full.zbr
...
pub const App = struct {
    ...
    pub fn risky(x: i64) anyerror!i64 {
        if (_zebra_lt(x, 0)) {
            { const _rdet_1: []const u8 = std.fmt.allocPrint(_allocator, "{}", .{x}) catch @panic("OOM");
              const _rdet_ptr_1 = _allocator.create([]const u8) catch @panic("OOM");
              _rdet_ptr_1.* = _rdet_1;
              const _rshim_1 = struct { fn call(p: *anyopaque) []const u8 {
                  return @as(*[]const u8, @alignCast(@ptrCast(p))).*;
              } };
              _error_ctx = .{ .message = "negative input", .details = .{ .ptr = @ptrCast(_rdet_ptr_1), .toString_fn = _rshim_1.call } };
    ...
    pub fn main() void {
        {
            ...
            _ = &_try_err;
            _try_blk: {
                const result: i64 = App.risky(5) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                std.debug.print("{}\n", .{result});
                break :_try_blk;
            }
            if (_try_err != null) {
    ...
    pub fn init() *App {
        const self = _allocator.create(App) catch @panic("OOM");
        self._type_tag = _ttag_App;
        return self;
    }

};
const _ttag_App: u64 = _zbr_hash("App");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_App) return _reflect_App_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_App) return .{ .items = _reflect_App_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_App) return .{ .items = _reflect_App_field_types };
    return .{ .items = &.{} };
}
