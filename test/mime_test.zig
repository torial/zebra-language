...
// Source: C:/Projects/zebra-language/test/mime_test.zbr
...
pub const MimeTest = struct {
    ...
    pub fn init() *MimeTest {
        const self = _allocator.create(MimeTest) catch @panic("OOM");
        self._type_tag = _ttag_MimeTest;
        return self;
    }

};
const _ttag_MimeTest: u64 = _zbr_hash("MimeTest");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_MimeTest) return _reflect_MimeTest_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_MimeTest) return .{ .items = _reflect_MimeTest_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_MimeTest) return .{ .items = _reflect_MimeTest_field_types };
    return .{ .items = &.{} };
}
