...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/regex_groups_test.zbr:3
    const re = _regex_compile("(\\w+)\\s+(\\w+)", "");
// zbr:test/regex_groups_test.zbr:4
    for (_regex_groups(re, "hello world").items) |g| {
// zbr:test/regex_groups_test.zbr:5
        std.debug.print("{s}\n", .{g});
    }
// zbr:test/regex_groups_test.zbr:8
    const date_re = _regex_compile("(\\d+)-(\\d+)-(\\d+)", "");
// zbr:test/regex_groups_test.zbr:9
    for (_regex_groups(date_re, "2026-04-06").items) |part| {
// zbr:test/regex_groups_test.zbr:10
        std.debug.print("{s}\n", .{part});
    }
// zbr:test/regex_groups_test.zbr:13
    for (_regex_groups(re, "nope").items) |x| {
// zbr:test/regex_groups_test.zbr:14
        std.debug.print("{s}\n", .{x});
    }
}
