...
// Source: test/crossmod_expose_test.zbr
...
pub fn _initAllocator(a: std.mem.Allocator) void {
    _allocator = a;
    @import("crossmod_expose_lib.zig")._initAllocator(a);
}
...
pub fn _zbr_error_msg() []const u8 {
    ...
    if (@import("crossmod_expose_lib.zig")._zbr_error_msg().len > 0) return @import("crossmod_expose_lib.zig")._zbr_error_msg();
    return "";
}
...
const crossmod_expose_lib = @import("crossmod_expose_lib.zig");
const Tag = crossmod_expose_lib.Tag;
const Bag = crossmod_expose_lib.Bag;
const Shape = crossmod_expose_lib.Shape;
pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/crossmod_expose_test.zbr:15
        Main.test1BranchOnExposedUnion();
// zbr:test/crossmod_expose_test.zbr:16
        Main.test2ForInOverExposedClassList();
// zbr:test/crossmod_expose_test.zbr:17
        std.debug.print("{s}\n", .{"crossmod_expose: all OK"});
    }

    pub fn test1BranchOnExposedUnion() void {
// zbr:test/crossmod_expose_test.zbr:20
        const s: Shape = Shape{ .box = Tag.init("hello", 5) };
// zbr:test/crossmod_expose_test.zbr:21
        switch (s) {
            .box => |r| {
// zbr:test/crossmod_expose_test.zbr:24
                const label = _str_concat(r.name, " label", _allocator);
// zbr:test/crossmod_expose_test.zbr:25
                if (!(std.mem.eql(u8, label, "hello label"))) {
                    std.debug.print("assertion failed\n", .{});
                    unreachable;
                }
// zbr:test/crossmod_expose_test.zbr:26
                if (!((r.weight == 5))) {
                    std.debug.print("assertion failed\n", .{});
                    unreachable;
                }
            ...
            .dot => {
// zbr:test/crossmod_expose_test.zbr:28
                if (!(false)) {
                    std.debug.print("assertion failed\n", .{});
                    unreachable;
                }
...
// zbr:test/crossmod_expose_test.zbr:29
        std.debug.print("{s}\n", .{"test1 OK"});
    }

    pub fn test2ForInOverExposedClassList() void {
// zbr:test/crossmod_expose_test.zbr:32
        var b = Bag.init("test-bag");
// zbr:test/crossmod_expose_test.zbr:33
        b.add(Tag.init("alpha", 10));
// zbr:test/crossmod_expose_test.zbr:34
        b.add(Tag.init("beta", 20));
// zbr:test/crossmod_expose_test.zbr:35
        b.add(Tag.init("gamma", 30));
// zbr:test/crossmod_expose_test.zbr:36
        var total: i64 = 0;
// zbr:test/crossmod_expose_test.zbr:37
        for (b.items.items) |t| {
// zbr:test/crossmod_expose_test.zbr:38
            total += t.weight;
        }
// zbr:test/crossmod_expose_test.zbr:39
        if (!((total == 60))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/crossmod_expose_test.zbr:41
        var names: []const u8 = "";
// zbr:test/crossmod_expose_test.zbr:42
        for (b.items.items) |t| {
// zbr:test/crossmod_expose_test.zbr:43
            if (!std.mem.eql(u8, names, "")) {
// zbr:test/crossmod_expose_test.zbr:44
                names = _str_concat(names, ",", _allocator);
            }
// zbr:test/crossmod_expose_test.zbr:45
            names = _str_concat(names, t.name, _allocator);
        }
// zbr:test/crossmod_expose_test.zbr:46
        if (!(std.mem.eql(u8, names, "alpha,beta,gamma"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/crossmod_expose_test.zbr:47
        std.debug.print("{s}\n", .{"test2 OK"});
    }
...
pub fn main() void {
    ...
    defer _arena.deinit();
    @import("crossmod_expose_lib.zig")._initAllocator(_allocator);
    Main.main();
}
