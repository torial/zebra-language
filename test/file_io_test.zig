...
// Source: C:/Projects/zebra-language/test/file_io_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        std.debug.print("{any}\n", .{content});
        const exists = (blk_fex: { std.fs.cwd().access(path, .{}) catch break :blk_fex false; break :blk_fex true; });
        std.debug.print("{any}\n", .{exists});
        @compileError("selfhost: unknown File.copy");
        std.debug.print("{any}\n", .{(blk_fex: { std.fs.cwd().access(copy_path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        @compileError("selfhost: unknown File.rename");
        std.debug.print("{any}\n", .{(blk_fex: { std.fs.cwd().access(renamed_path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        std.debug.print("{any}\n", .{(blk_fex: { std.fs.cwd().access(copy_path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        (std.fs.cwd().deleteFile(path) catch @panic("File.delete error"));
        (std.fs.cwd().deleteFile(renamed_path) catch @panic("File.delete error"));
        std.debug.print("{any}\n", .{(blk_fex: { std.fs.cwd().access(path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        ...
        std.debug.print("{any}\n", .{(blk_de: { var _de_d = std.fs.cwd().openDir(dir_path, .{}) catch break :blk_de false; _de_d.close(); break :blk_de true; })});
