...
// zbr:test/file_io_test.zbr:6
        const path: []const u8 = "/tmp/zebra_file_test.txt";
// zbr:test/file_io_test.zbr:7
        const copy_path: []const u8 = "/tmp/zebra_file_copy.txt";
// zbr:test/file_io_test.zbr:8
        const renamed_path: []const u8 = "/tmp/zebra_file_renamed.txt";
        (blk_fw: {
            const _fw_path = path;
            const _fw_f = std.Io.Dir.cwd().createFile(_io, _zbr_norm_path(_fw_path), .{}) catch @panic("File.write error");
            defer _fw_f.close(_io);
            _fw_f.writeStreamingAll(_io, "line1") catch @panic("File.write error");
            break :blk_fw {};
        });
        File.appendSlice(_allocator, path) catch @panic("OOM");
        File.appendSlice(_allocator, path) catch @panic("OOM");
// zbr:test/file_io_test.zbr:14
        const content = (std.Io.Dir.cwd().readFileAlloc(_io, path, _allocator, .unlimited) catch @panic("File.read error"));
        std.debug.print("{any}\n", .{content});
// zbr:test/file_io_test.zbr:17
        const exists = (blk_fex: { std.Io.Dir.cwd().access(_io, path, .{}) catch break :blk_fex false; break :blk_fex true; });
        std.debug.print("{any}\n", .{exists});
        @compileError("selfhost: unknown File.copy");
        std.debug.print("{any}\n", .{(blk_fex: { std.Io.Dir.cwd().access(_io, copy_path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        @compileError("selfhost: unknown File.rename");
        std.debug.print("{any}\n", .{(blk_fex: { std.Io.Dir.cwd().access(_io, renamed_path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        std.debug.print("{any}\n", .{(blk_fex: { std.Io.Dir.cwd().access(_io, copy_path, .{}) catch break :blk_fex false; break :blk_fex true; })});
        (std.Io.Dir.cwd().deleteFile(_io, path) catch @panic("File.delete error"));
        (std.Io.Dir.cwd().deleteFile(_io, renamed_path) catch @panic("File.delete error"));
        std.debug.print("{any}\n", .{(blk_fex: { std.Io.Dir.cwd().access(_io, path, .{}) catch break :blk_fex false; break :blk_fex true; })});
// zbr:test/file_io_test.zbr:32
        const dir_path: []const u8 = "/tmp/zebra_dir_test";
        (std.Io.Dir.cwd().createDirPath(_io, dir_path) catch @panic("Dir.create error"));
        std.debug.print("{any}\n", .{(blk_de: { var _de_d = std.Io.Dir.cwd().openDir(_io, dir_path, .{}) catch break :blk_de false; _de_d.close(_io); break :blk_de true; })});
        (std.Io.Dir.cwd().deleteDir(_io, dir_path) catch |_dd_err| { if (_dd_err != error.FileNotFound) @panic("Dir.delete error"); });
        std.debug.print("{any}\n", .{(blk_de: { var _de_d = std.Io.Dir.cwd().openDir(_io, dir_path, .{}) catch break :blk_de false; _de_d.close(_io); break :blk_de true; })});
// zbr:test/file_io_test.zbr:39
        const full_path: []const u8 = "/home/user/documents/report.txt";
