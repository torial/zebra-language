...
// Source: test/for_in_guard_test.zbr
...
pub fn main() void {
// zbr:test/for_in_guard_test.zbr:3
    var nums = std.ArrayList(i64){};
    nums.append(_allocator, 1) catch @panic("OOM");
    nums.append(_allocator, 2) catch @panic("OOM");
    nums.append(_allocator, 3) catch @panic("OOM");
    nums.append(_allocator, 4) catch @panic("OOM");
    nums.append(_allocator, 5) catch @panic("OOM");
// zbr:test/for_in_guard_test.zbr:9
    var evens = std.ArrayList(i64){};
// zbr:test/for_in_guard_test.zbr:10
    for (nums.items) |n| {
        if (!((@mod(n, 2) == 0))) continue;
        evens.append(_allocator, n) catch @panic("OOM");
    }
// zbr:test/for_in_guard_test.zbr:12
    if (!((@as(i64, @intCast(evens.items.len)) == 2))) @panic("expected 2 evens");
// zbr:test/for_in_guard_test.zbr:13
    if (!((evens.items[@intCast(0)] == 2))) @panic("first even");
// zbr:test/for_in_guard_test.zbr:14
    if (!((evens.items[@intCast(1)] == 4))) @panic("second even");
// zbr:test/for_in_guard_test.zbr:17
    var parts = std.ArrayList([]const u8){};
// zbr:test/for_in_guard_test.zbr:18
    {
        var _it_p = std.mem.splitSequence(u8, "a,,b,,c", ",");
        while (_it_p.next()) |p| {
            if (!(!std.mem.eql(u8, p, ""))) continue;
            parts.append(_allocator, p) catch @panic("OOM");
        }
    }
// zbr:test/for_in_guard_test.zbr:20
    if (!((@as(i64, @intCast(parts.items.len)) == 3))) @panic("expected 3 non-empty parts");
// zbr:test/for_in_guard_test.zbr:21
    if (!(std.mem.eql(u8, parts.items[@intCast(0)], "a"))) @panic("split[0]");
// zbr:test/for_in_guard_test.zbr:22
    if (!(std.mem.eql(u8, parts.items[@intCast(1)], "b"))) @panic("split[1]");
// zbr:test/for_in_guard_test.zbr:23
    if (!(std.mem.eql(u8, parts.items[@intCast(2)], "c"))) @panic("split[2]");
// zbr:test/for_in_guard_test.zbr:26
    var letters = std.ArrayList([]const u8){};
// zbr:test/for_in_guard_test.zbr:27
    {
        var _cp_it_c = std.unicode.Utf8View.initUnchecked("h3ll0").iterator();
        while (_cp_it_c.nextCodepoint()) |c| {
            if (!(std.ascii.isAlphabetic(@as(u8, @truncate(c))))) continue;
            letters.append(_allocator, (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(c), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); })) catch @panic("OOM");
        }
    }
// zbr:test/for_in_guard_test.zbr:29
    if (!((@as(i64, @intCast(letters.items.len)) == 3))) @panic("expected 3 alpha chars");
// zbr:test/for_in_guard_test.zbr:30
    if (!(std.mem.eql(u8, letters.items[@intCast(0)], "h"))) @panic("char[0]");
// zbr:test/for_in_guard_test.zbr:31
    if (!(std.mem.eql(u8, letters.items[@intCast(1)], "l"))) @panic("char[1]");
// zbr:test/for_in_guard_test.zbr:32
    if (!(std.mem.eql(u8, letters.items[@intCast(2)], "l"))) @panic("char[2]");
// zbr:test/for_in_guard_test.zbr:35
    var threes = std.ArrayList(i64){};
// zbr:test/for_in_guard_test.zbr:36
    {
        var i: i64 = 0;
        const _stop_i: i64 = 10;
        while (i < _stop_i) : (i += 1) {
            if (!((@mod(i, 3) == 0))) continue;
            threes.append(_allocator, i) catch @panic("OOM");
        }
    }
// zbr:test/for_in_guard_test.zbr:38
    if (!((@as(i64, @intCast(threes.items.len)) == 4))) @panic("expected 4 multiples-of-3 (0,3,6,9)");
// zbr:test/for_in_guard_test.zbr:39
    if (!((threes.items[@intCast(0)] == 0))) @panic("three[0]");
// zbr:test/for_in_guard_test.zbr:40
    if (!((threes.items[@intCast(3)] == 9))) @panic("three[3]");
// zbr:test/for_in_guard_test.zbr:43
    var scores = std.ArrayList(i64){};
    scores.append(_allocator, 3) catch @panic("OOM");
    scores.append(_allocator, 7) catch @panic("OOM");
    scores.append(_allocator, 12) catch @panic("OOM");
    scores.append(_allocator, 5) catch @panic("OOM");
// zbr:test/for_in_guard_test.zbr:48
    const found = _zebra_list_find(std.meta.Child(@TypeOf(scores.items)), struct { fn call(x: anytype) @TypeOf(_zebra_gt(x, 6)) { return _zebra_gt(x, 6); } }.call, scores);
// zbr:test/for_in_guard_test.zbr:49
    if (!((found != null))) @panic("find should succeed");
// zbr:test/for_in_guard_test.zbr:50
    if (!((found.? == 7))) @panic("find value");
// zbr:test/for_in_guard_test.zbr:52
    const not_found = _zebra_list_find(std.meta.Child(@TypeOf(scores.items)), struct { fn call(x: anytype) @TypeOf(_zebra_gt(x, 100)) { return _zebra_gt(x, 100); } }.call, scores);
// zbr:test/for_in_guard_test.zbr:53
    if (!((not_found == null))) @panic("find on no-match returns nil");
    std.debug.print("{s}\n", .{"ok"});
}
