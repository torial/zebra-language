...
pub const TypeChecker = struct {
    ...
    pub fn init() *TypeChecker {
        const _self = _allocator.create(TypeChecker) catch @panic("OOM");
        _self._type_tag = _zbr_hash("TypeChecker");
// zbr:test/typechecker.zbr:28
            _self.errors = std.ArrayList([]const u8){};
// zbr:test/typechecker.zbr:29
            var s = TcScope.init();
            s.push(TcScopeKind.module_);
// zbr:test/typechecker.zbr:31
            _self._scope = s;
        return _self;
    }
...
// zbr:test/typechecker.zbr:90
        return (@as(i64, @intCast(self.errors.items.len)) == 0);
    }

    pub fn clearErrors(self: *TypeChecker) void {
// zbr:test/typechecker.zbr:94
        var i = (@as(i64, @intCast(self.errors.items.len)) - 1);
// zbr:test/typechecker.zbr:95
        while (_zebra_ge(i, 0)) {
            _ = self.errors.orderedRemove(@intCast(i));
// zbr:test/typechecker.zbr:97
            i = (i - 1);
        }
    }

};
const _ttag_TypeChecker: u64 = _zbr_hash("TypeChecker");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_TypeChecker) return _reflect_TypeChecker_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_TypeChecker) return .{ .items = _reflect_TypeChecker_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_TypeChecker) return .{ .items = _reflect_TypeChecker_field_types };
    return .{ .items = &.{} };
}
