...
// Source: test/modtime_optional_test.zbr
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/modtime_optional_test.zbr:5
    const mt = (blk: {
        const _mt_stat = std.Io.Dir.cwd().statFile(_io, "QUICKSTART.md", .{}) catch break :blk @as(?i64, null);
        break :blk @as(?i64, _mt_stat.mtime.toMilliseconds());
    });
// zbr:test/modtime_optional_test.zbr:6
    std.debug.assert((mt != null));
// zbr:test/modtime_optional_test.zbr:9
    const missing = (blk: {
        const _mt_stat = std.Io.Dir.cwd().statFile(_io, "does_not_exist_zzz_42.txt", .{}) catch break :blk @as(?i64, null);
        break :blk @as(?i64, _mt_stat.mtime.toMilliseconds());
    });
// zbr:test/modtime_optional_test.zbr:10
    std.debug.assert((missing == null));
// zbr:test/modtime_optional_test.zbr:12
    std.debug.print("{s}\n", .{"modtime int? OK"});
}
