...
// Source: test/if_unwrap_test.zbr
...
pub const Wrapper = struct {
    value: i64 = undefined,
    pub fn init(v: i64) Wrapper {
        var self: Wrapper = undefined;
// zbr:test/if_unwrap_test.zbr:4
        self.value = v;
        return self;
    }
...
pub fn tryParse(s: []const u8) ?i64 {
// zbr:test/if_unwrap_test.zbr:7
    if (std.mem.eql(u8, s, "42")) {
// zbr:test/if_unwrap_test.zbr:8
        return 42;
    }
// zbr:test/if_unwrap_test.zbr:9
    return null;
}

pub fn tryParseStr(s: []const u8) ?[]const u8 {
// zbr:test/if_unwrap_test.zbr:12
    if (_zebra_gt(s.len, 0)) {
// zbr:test/if_unwrap_test.zbr:13
        return s;
    }
// zbr:test/if_unwrap_test.zbr:14
    return null;
}

pub fn tryWrap(s: []const u8) ?Wrapper {
// zbr:test/if_unwrap_test.zbr:17
    if (std.mem.eql(u8, s, "ok")) {
// zbr:test/if_unwrap_test.zbr:18
        return Wrapper.init(99);
    }
// zbr:test/if_unwrap_test.zbr:19
    return null;
}

pub fn main() void {
// zbr:test/if_unwrap_test.zbr:22
    const v1 = tryParse("42");
// zbr:test/if_unwrap_test.zbr:23
    if (v1) |n| {
// zbr:test/if_unwrap_test.zbr:24
        std.debug.print("{s}\n", .{_str_concat("got: ", (std.fmt.allocPrint(_allocator, "{}", .{n}) catch unreachable), _allocator)});
    } else {
// zbr:test/if_unwrap_test.zbr:26
        std.debug.print("{s}\n", .{"none"});
    }
// zbr:test/if_unwrap_test.zbr:27
    const v2 = tryParse("bad");
// zbr:test/if_unwrap_test.zbr:28
    if (v2) |n| {
// zbr:test/if_unwrap_test.zbr:29
        std.debug.print("{s}\n", .{_str_concat("got: ", (std.fmt.allocPrint(_allocator, "{}", .{n}) catch unreachable), _allocator)});
    } else {
// zbr:test/if_unwrap_test.zbr:31
        std.debug.print("{s}\n", .{"none"});
    }
// zbr:test/if_unwrap_test.zbr:32
    const a = tryParse("bad");
// zbr:test/if_unwrap_test.zbr:33
    const b = tryParse("42");
// zbr:test/if_unwrap_test.zbr:34
    if (a) |x| {
// zbr:test/if_unwrap_test.zbr:35
        std.debug.print("{s}\n", .{_str_concat("a: ", (std.fmt.allocPrint(_allocator, "{}", .{x}) catch unreachable), _allocator)});
    } else if (b) |y| {
// zbr:test/if_unwrap_test.zbr:37
        std.debug.print("{s}\n", .{_str_concat("b: ", (std.fmt.allocPrint(_allocator, "{}", .{y}) catch unreachable), _allocator)});
    } else {
// zbr:test/if_unwrap_test.zbr:39
        std.debug.print("{s}\n", .{"neither"});
    }
// zbr:test/if_unwrap_test.zbr:40
    const s1 = tryParseStr("hello");
// zbr:test/if_unwrap_test.zbr:41
    if (s1) |ss| {
// zbr:test/if_unwrap_test.zbr:42
        std.debug.print("{s}\n", .{_str_concat("str: ", ss, _allocator)});
    }
// zbr:test/if_unwrap_test.zbr:43
    if (tryParseStr("")) |ss| {
// zbr:test/if_unwrap_test.zbr:44
        std.debug.print("{s}\n", .{_str_concat("str: ", ss, _allocator)});
    } else {
// zbr:test/if_unwrap_test.zbr:46
        std.debug.print("{s}\n", .{"empty"});
    }
// zbr:test/if_unwrap_test.zbr:47
    const w1 = tryWrap("ok");
// zbr:test/if_unwrap_test.zbr:48
    if (w1) |w| {
// zbr:test/if_unwrap_test.zbr:49
        std.debug.print("{s}\n", .{_str_concat("wrap: ", (std.fmt.allocPrint(_allocator, "{}", .{w.value}) catch unreachable), _allocator)});
    }
// zbr:test/if_unwrap_test.zbr:50
    const w2 = tryWrap("nope");
// zbr:test/if_unwrap_test.zbr:51
    if (w2) |w| {
// zbr:test/if_unwrap_test.zbr:52
        std.debug.print("{s}\n", .{_str_concat("wrap: ", (std.fmt.allocPrint(_allocator, "{}", .{w.value}) catch unreachable), _allocator)});
    } else {
// zbr:test/if_unwrap_test.zbr:54
        std.debug.print("{s}\n", .{"no wrap"});
    }
// zbr:test/if_unwrap_test.zbr:55
    std.debug.print("{s}\n", .{"ok"});
}
