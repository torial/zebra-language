...
// zbr:test/throws_autoprop_test.zbr:9
        { _error_ctx = .{ .message = "from inner" }; return error.ZebraError; }
    }
...
// zbr:test/throws_autoprop_test.zbr:16
        {
            var _try_err: ?anyerror = null;
            _ = &_try_err;
            _try_blk: {
                self.outer() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{"caught: OK"});
    ...
    pub fn init() *ErrorPropTest {
        const self = _allocator.create(ErrorPropTest) catch @panic("OOM");
        self._type_tag = _ttag_ErrorPropTest;
        return self;
    }

};
const _ttag_ErrorPropTest: u64 = _zbr_hash("ErrorPropTest");
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/throws_autoprop_test.zbr:24
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_ErrorPropTest) return _reflect_ErrorPropTest_name;
    if (tag == _ttag_Main) return _reflect_Main_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_ErrorPropTest) return .{ .items = _reflect_ErrorPropTest_fields };
    if (tag == _ttag_Main) return .{ .items = _reflect_Main_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_ErrorPropTest) return .{ .items = _reflect_ErrorPropTest_field_types };
