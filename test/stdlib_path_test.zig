...
// Source: test/stdlib_path_test.zbr
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:test/stdlib_path_test.zbr:3
    const p: []const u8 = (std.fs.path.join(_allocator, &.{"foo", "bar"}) catch @panic("Path.join error"));
// zbr:test/stdlib_path_test.zbr:4
    std.debug.assert((std.mem.indexOf(u8, p, "foo") != null));
// zbr:test/stdlib_path_test.zbr:5
    std.debug.assert((std.mem.indexOf(u8, p, "bar") != null));
// zbr:test/stdlib_path_test.zbr:8
    std.debug.assert(std.mem.eql(u8, (std.fs.path.basename("/a/b/file.txt")), "file.txt"));
// zbr:test/stdlib_path_test.zbr:11
    std.debug.assert(std.mem.eql(u8, (std.fs.path.dirname("/a/b/file.txt") orelse ""), "/a/b"));
// zbr:test/stdlib_path_test.zbr:12
    std.debug.assert(std.mem.eql(u8, (std.fs.path.dirname("file.txt") orelse ""), ""));
// zbr:test/stdlib_path_test.zbr:15
    std.debug.assert(std.mem.eql(u8, (std.fs.path.extension("file.txt")), ".txt"));
// zbr:test/stdlib_path_test.zbr:16
    std.debug.assert(std.mem.eql(u8, (std.fs.path.extension("no_ext")), ""));
// zbr:test/stdlib_path_test.zbr:19
    std.debug.assert(std.mem.eql(u8, (std.fs.path.extension("archive.tar.gz")), ".gz"));
// zbr:test/stdlib_path_test.zbr:20
    std.debug.assert(std.mem.eql(u8, (std.fs.path.extension("Makefile")), ""));
// zbr:test/stdlib_path_test.zbr:23
    std.debug.assert(std.mem.eql(u8, (blk_ps: {
        const _ps_base = std.fs.path.basename("file.txt");
        const _ps_ext = std.fs.path.extension(_ps_base);
        break :blk_ps _ps_base[0 .. _ps_base.len - _ps_ext.len];
    }), "file"));
// zbr:test/stdlib_path_test.zbr:24
    std.debug.assert(std.mem.eql(u8, (blk_ps: {
        const _ps_base = std.fs.path.basename("/a/b/doc.md");
        const _ps_ext = std.fs.path.extension(_ps_base);
        break :blk_ps _ps_base[0 .. _ps_base.len - _ps_ext.len];
    }), "doc"));
// zbr:test/stdlib_path_test.zbr:27
    std.debug.assert((!(std.fs.path.isAbsolute("rel/path"))));
// zbr:test/stdlib_path_test.zbr:28
