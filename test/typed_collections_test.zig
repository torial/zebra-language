...
// Source: C:/Projects/zebra-language/test/typed_collections_test.zbr
...
pub const Program = struct {
    ...
    pub fn doubleAll(nums: std.ArrayList(i64)) std.ArrayList(i64) {
        ...
        for (nums.items) |n| {
            result.append(_allocator, (n * 2)) catch @panic("OOM");
        }
        return result;
    }
    ...
    pub fn main() void {
        ...
        std.debug.print("{any}\n", .{nums.items[@intCast(0)]});
        const doubled: std.ArrayList(i64) = Program.doubleAll(nums);
        for (doubled.items) |n| {
            std.debug.print("{any}\n", .{n});
        }
        var words = std.ArrayList([]const u8){};
        words.append(_allocator, "alpha") catch @panic("OOM");
        words.append(_allocator, "beta") catch @panic("OOM");
        words.append(_allocator, "gamma") catch @panic("OOM");
        ...
        std.debug.print("{any}\n", .{@as(i64, @intCast(scores.count()))});
        var codes = std.AutoHashMap(i64, []const u8).init(_allocator);
        codes.put(200, "OK") catch @panic("OOM");
        codes.put(404, "Not Found") catch @panic("OOM");
        std.debug.print("{any}\n", .{codes.contains(200)});
