...
// Source: C:/Projects/zebra-language/test/string_methods.zbr
...
pub const Program = struct {
    ...
    pub fn main() void {
        ...
        std.debug.print("{s}\n", .{(std.ascii.allocUpperString(_allocator, s) catch unreachable)});
        std.debug.print("{s}\n", .{(std.ascii.allocLowerString(_allocator, s) catch unreachable)});
        std.debug.print("{any}\n", .{blk: { const _idx = std.mem.indexOf(u8, s, "World"); break :blk if (_idx) |_i| @as(i64, @intCast(_i)) else null; }});
        std.debug.print("{any}\n", .{blk: { const _idx = std.mem.indexOf(u8, s, "xyz"); break :blk if (_idx) |_i| @as(i64, @intCast(_i)) else null; }});
