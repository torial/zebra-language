...
// Source: test/for_tuple_test.zbr
...
pub fn main() void {
// zbr:test/for_tuple_test.zbr:3
    var pairs = std.ArrayList(struct { i64, []const u8 }){};
    pairs.append(_allocator, .{1, "one"}) catch @panic("OOM");
    pairs.append(_allocator, .{2, "two"}) catch @panic("OOM");
    pairs.append(_allocator, .{3, "three"}) catch @panic("OOM");
// zbr:test/for_tuple_test.zbr:7
    for (pairs.items) |_zbr_tup| {
        const n = _zbr_tup.@"0";
        const s = _zbr_tup.@"1";
        std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{}:{s}", .{n, s}) catch @panic("OOM"))});
    }
// zbr:test/for_tuple_test.zbr:11
    for (pairs.items) |_zbr_tup| {
        const n = _zbr_tup.@"0";
        const s = _zbr_tup.@"1";
        _ = s;
        if (!(_zebra_gt(n, 1))) continue;
        std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "big {}", .{n}) catch @panic("OOM"))});
    }
// zbr:test/for_tuple_test.zbr:15
    var triples = std.ArrayList(struct { i64, []const u8, bool }){};
    triples.append(_allocator, .{10, "ten", true}) catch @panic("OOM");
    triples.append(_allocator, .{20, "twenty", false}) catch @panic("OOM");
// zbr:test/for_tuple_test.zbr:18
    for (triples.items) |_zbr_tup| {
        const n = _zbr_tup.@"0";
        const s = _zbr_tup.@"1";
        const b = _zbr_tup.@"2";
        std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{} {s} {}", .{n, s, b}) catch @panic("OOM"))});
    }
// zbr:test/for_tuple_test.zbr:22
    var scores = std.StringHashMap(i64).init(_allocator);
    scores.put(_intern("a"), 1) catch @panic("OOM");
// zbr:test/for_tuple_test.zbr:24
    {
        var _it_k = scores.iterator();
        while (_it_k.next()) |_ent_k| {
            const k = _ent_k.key_ptr.*;
            const v = _ent_k.value_ptr.*;
            std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "{s}={}", .{k, v}) catch @panic("OOM"))});
        }
    }
    std.debug.print("{s}\n", .{"done"});
}
