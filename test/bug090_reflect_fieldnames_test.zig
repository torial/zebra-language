...
// Source: test/bug090_reflect_fieldnames_test.zbr
...
const _ttag_User: u64 = _zbr_hash("User");
const _reflect_User_name: []const u8 = "User";
const _reflect_User_fields: []const []const u8 = &.{"name", "age"};
const _reflect_User_field_types: []const []const u8 = &.{"str", "int"};
...
pub fn main() void {
// zbr:test/bug090_reflect_fieldnames_test.zbr:10
    var u = User.init();
// zbr:test/bug090_reflect_fieldnames_test.zbr:11
    u.name = "Alice";
// zbr:test/bug090_reflect_fieldnames_test.zbr:12
    u.age = 30;
// zbr:test/bug090_reflect_fieldnames_test.zbr:13
    var names = std.ArrayList([]const u8){};
// zbr:test/bug090_reflect_fieldnames_test.zbr:14
    for (_reflect_lookup_fields(u._type_tag).items) |n| {
        names.append(_allocator, _intern(n)) catch @panic("OOM");
    }
// zbr:test/bug090_reflect_fieldnames_test.zbr:16
    if (!((@as(i64, @intCast(names.items.len)) == 2))) @panic("field count");
// zbr:test/bug090_reflect_fieldnames_test.zbr:18
    if (!(std.mem.eql(u8, names.items[@intCast(0)], "name"))) @panic("first field");
// zbr:test/bug090_reflect_fieldnames_test.zbr:19
    if (!(std.mem.eql(u8, names.items[@intCast(1)], "age"))) @panic("second field");
    std.debug.print("{s}\n", .{"ok"});
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_User) return _reflect_User_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_User) return .{ .items = _reflect_User_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_User) return .{ .items = _reflect_User_field_types };
    return .{ .items = &.{} };
}
