...
`action` `beginHBox` `beginMenu` `beginPanel` `beginTabPage` `beginTable` `beginTabs` `beginVBox` `button` `buttonId` `checkbox` `childWindow` `combobox` `endHBox` `endMenu` `endPanel` `endTabPage` `endTable` `endTabs` `endVBox` `every` `field` `getDpi` `hbox` `hotkey` `indent` `input` `inputMultiline` `menuItem` `menuQuit` `menuSeparator` `minSize` `msgBox` `msgBoxError` `openFile` `openFolder` `panel` `progressBar` `sameLine` `saveFile` `scaleAllSizes` `selectTab` `selectable` `send` `separator` `setColor` `setColorsDark` `setStyleFloat` `setVec2` `slider` `spacing` `spinbox` `tabSelected` `tableActivatedRow` `tableHeadersRow` `tableNextColumn` `tableNextRow` `tableSelectedRow` `tableSetupColumn` `takeKey` `text` `textColored` `toggle` `treeNode` `treePop` `unindent` `vbox` `window`
