...
**The rerun did not land either, and the reason is bug 6 above.** It was stopped at 43/60
once its regen refusals were shown to be fabricated by repo-vs-worktree coordinate drift.
Its verdicts (18 regen, 18 NO-EFFECT, 6 boundary, 1 build) are **not** recorded here,
...
| 6 | sites chosen from `REPO/rel`, applied to `wt/rel` -- **two different files** | 18 of 43 "regen refusals" |
...
Bug 6 is the sharpest instance and was found the same day, *while* writing this section.
`candidate_sites(REPO / rel)` chose the mutation coordinates; `src = wt / rel` applied
them. The worktree sits at the commit it was created from; the main checkout carries
uncommitted work, and this repo regularly has a parallel session editing
`selfhost/*.zbr`. `CodeGen.zbr` was **16,539 lines here and 16,521 there**; line 14831 was
`if args.len > 0` in one and `else` in the other. So mutations landed on unrelated lines,
corrupted the source, and the bootstrap's entirely correct refusal was scored as
*"the bootstrap refused this mutant"*.

**That is the second time this harness manufactured "regen refusals" by corrupting the
source it was supposed to be mutating** -- the first was the CRLF restore. Both times the
fabricated verdict was the reassuring-looking one, which is not a coincidence: see rule 2.

It was caught by a discrepancy that had no innocent explanation. Two runs with the same
`--seed 1 --limit 60` produced **0 regen refusals** and then **18**, and identical inputs
do not do that. Hand-checking at the worktree's own commit settled it: six of nine target
lines were not mutable sites there at all, and all three that were emitted cleanly. That is the actual hazard in this repo's QA work — not
...
5. **Read from the file you are going to write to.** A relative path resolved against two
   different roots is silent -- both paths exist, both open fine, and you read A and write
   B. Where a tool has an isolated copy, *every* read must come from the copy, and a
   per-item control should verify the thing at the coordinates is what the coordinates
   claimed before acting on it. That control costs one string comparison and would have
   stopped bug 6 on the first mutant instead of the forty-third.

6. **Check format/marker strings against the thing that actually emits them.** Two
