...
### BUG-315: `protected` is a synonym for `private`, and its documented meaning needs inheritance the language does not have — OPEN (found 2026-08-29)
...
**Control when fixing:** add `test/bug315_protected_freed_test.zbr` naming a
variable, field, parameter and method `protected` and asserting the printed values —
the shape `test/bug280_freed_words_test.zbr` uses for the seven words already freed.
It must be watched FAILING first (today `protected` tokenises as a modifier keyword,
so the parse should refuse it), then passing. A fixture that was green before the
