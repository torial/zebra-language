...
- **Status:** Fixed 2026-04-18
- **Backend:** selfhost only (Zig compiler unaffected)
- **Was:** `parseStmt`'s `var` branch went unconditionally to `parseVarStmt`, which does `.expectText("var")` then `.eatId()` — so `var {x, y} = p` failed at `{` with `expected identifier, got '{'`. Downstream had partial support: `ast.zbr::StmtDestruct {span, names, init_expr}` existed, `Stmt.destruct as ^StmtDestruct` was in `bv` (codegen.zbr:151), and `rf.add("StmtDestruct.init_expr")` was present (codegen.zbr:208). Two missing pieces beyond the parser gap: (1) `StmtDestruct` had no way to distinguish tuple `(x, y)` from struct `{x, y}` (Zig backend uses a `kind: enum { tuple, struct_ }` field on `Ast.StmtDestruct`); (2) `genDestruct` hard-coded a single `_zbr_dt` temporary and always emitted `_zbr_dt.@"N"` positional access, breaking both multi-destructure functions (name collision) and all struct-form destructures (`p.@"0"` is a tuple index, not a class-field access).
- **Fix (5 coordinated edits):**
  1. `parser.zbr` — new `PDestruct {names: List(str), init_expr: List(PNode), is_struct: bool}` holder, new `PNode.stmt_destruct as ^PDestruct` variant, new `parseDestructStmt` method (reads `var`, dispatches on `{`/`(`, parses comma-separated idents, eats closing bracket, `=`, expression, eol). `parseStmt` gains a 1-line lookahead — if `this.textIs("var")` and `.peekAt(1).text` is `{` or `(`, call `parseDestructStmt()` instead of falling through to `parseVarStmt(false)`.
  2. `ast.zbr` — added `var is_struct as bool` field to `StmtDestruct` and extended its `cue init` signature. Zero call-sites in the tree to migrate (no prior parser emitted this variant).
  3. `astbuilder.zbr` — new `on PNode.stmt_destruct as pd` arm constructing `Stmt.destruct(StmtDestruct(zspan(), pd.names, init_expr, pd.is_struct))`. Added `StmtDestruct` to `use ast exposing` and `PDestruct` to `use Parser exposing`. **Gotcha:** first attempt wrote `var names as List(String) = List(String)()` to rebuild the list — emit in selfhost-A raised `undefined name: 'String'` because `String` (while used freely in `ast.zbr`) is not in `astbuilder.zbr`'s use-list scope. Since `List(str)` and `List(String)` are assignable in Zebra, pass `pd.names` straight through instead.
  4. `codegen.zbr::addCrossModuleBoxedVariants` — `bv.add("PNode.stmt_destruct")` alongside the other PNode-stmt boxed variants. **Without this the round-trip fails at step 4** — same bv/rf pairing trap as BUG-069.
  5. `codegen.zbr::genDestruct` — switched to `w.nextUid()` for a monotonic `_zbr_dt_N` temporary, and branches on `d.is_struct`: `true` → `const <name> = <tmp>.<name>;` (struct form, matches Zig backend's `_dt_<hex>.<name>`), `false` → `const <name> = <tmp>.@"<i>";` (tuple form, preserves existing behavior).
  6. `resolver.zbr::resolveStmt` — new `on PNode.stmt_destruct as d` arm that resolves `d.init_expr` and then binds every name in `d.names` into the method scope. Without this, the expressions following the destructure (e.g. `print x`) resolved to `undefined name: 'x'`.
- **Rule re-surfaced:** adding `Stmt.destruct(StmtDestruct(...))` requires bv + rf entries for every `^T` struct field — here `StmtDestruct.init_expr` was already registered, but the new PNode variant required its own bv line. Extending an AST struct shape is transparent to bv/rf (they only track variants and ref-fields, not struct layout).
- **Verification:** Corpus 137/152 → 138/152 (+1 emit: struct_destruct_test). Bootstrap round-trip A/B byte-identical. Emitted Zig matches Zig backend output modulo temp-name format (`_zbr_dt_1` vs `_dt_2c0`) — semantically equivalent, emits still compile together.
