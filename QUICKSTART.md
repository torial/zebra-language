...
| `Gui.registerIcon(name, w, h, rgba)` | void | The program's own tree icon (2026-09-23): straight-alpha RGBA bytes in a `str`, any size; the name then works in `treeNodeIcon` / `treeLeafIcon` beside the built-in four. Queued if called before `Gui.run`. Stub prints; TUI ignores. `examples/icon_smoke.zbr` |
...
| `g.tableSetupCheckColumn(name, on)` / `g.tableCheck(checked)` | void | A checkbox column (2026-09-22): set it up in place of `tableSetupColumn`, write its cell with `tableCheck(checked)` in place of `g.text`; a click sends `on(row, checked)` -- `on: def(row: int, checked: bool): Msg`. The model drives the boxes: a message that changes it re-renders the rows and the diff flips them. One check column per table in this cut. `examples/table_check_smoke.zbr` |
| `g.tableSetupEditColumn(name, on)` / `g.tableSetupButtonColumn(name, on)` | void | An editable text column and a button column (2026-09-23), the check column's shape: set up in place of `tableSetupColumn`, cells are `g.text` as usual (a button column's cells are the labels). Committing an edit sends `on(row, text)` -- `def(row: int, text: str): Msg` -- and the next render shows what the model now holds, so a declined edit snaps back; a click sends `on(row)`. One of each per table. `examples/table_edit_smoke.zbr` |
...
| `g.beginToolbar()` … `g.endToolbar()`      | void     | A native toolbar under the menubar (2026-09-23; GtkToolbar / ToolbarWindow32 / NSToolbar in the title bar): `g.tool(label, msg)`, `g.toolIcon(label, icon, tip, msg)` with a built-in or `Gui.registerIcon` name and a tooltip, `g.toolSeparator()`, and `g.toolEnabled(bool)` for the NEXT item (resets to true). Declared every render like the menus, but the set may CHANGE: same labels/icons/tips means an in-place update, anything else a rebuild. TUI: a row of buttons. `examples/toolbar_smoke.zbr` |
...
icon)` take an icon name from the built-in set (`folder`, `file`, `dot`, `warn`) or one the
program registered: `Gui.registerIcon(name, w, h, rgba)` takes straight-alpha RGBA bytes
(`w * h * 4` of them, in a `str` -- `StringBuilder.appendChar(n)` builds one a byte at a
time, or read a raw `.rgba` file) and the name then works wherever a built-in one does.
Call it before `Gui.run` (main is the natural place; registrations are queued until the
toolkit is up) or from `update`; registering a name again replaces its image. There is no
image decoder yet, so a PNG has to be decoded by the program. Not yet: columns,
drag-and-drop, editing, the tui's keyboard. `examples/icon_smoke.zbr`.
