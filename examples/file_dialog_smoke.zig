...
// Source: examples/file_dialog_smoke.zbr
...
pub const Model = struct {
    ...
    pub fn init(path: []const u8, status: []const u8) Model {
...
pub fn init() Model {
// zbr:examples/file_dialog_smoke.zbr:11
    return Model{ .path = "", .status = "No dialog opened yet" };
}

pub fn update(model: Model, msg: Msg) Model {
    _ = model;
// zbr:examples/file_dialog_smoke.zbr:14
    switch (msg) {
        .opened => |p| {
// zbr:examples/file_dialog_smoke.zbr:16
            return Model{ .path = p, .status = _str_concat("Opened: ", p, _allocator) };
        },
        .saved => |p| {
// zbr:examples/file_dialog_smoke.zbr:18
            return Model{ .path = p, .status = _str_concat("Saved: ", p, _allocator) };
        },
        .foldered => |p| {
// zbr:examples/file_dialog_smoke.zbr:20
            return Model{ .path = p, .status = _str_concat("Folder: ", p, _allocator) };
        },
    }
}

pub fn view(g: GuiContext, model: Model) void {
// zbr:examples/file_dialog_smoke.zbr:23
    g.text("File Dialog Smoke Test");
// zbr:examples/file_dialog_smoke.zbr:24
    g.text(_str_concat("Status: ", model.status, _allocator));
// zbr:examples/file_dialog_smoke.zbr:25
    g.separator();
// zbr:examples/file_dialog_smoke.zbr:26
    if (g.button("Open File")) {
// zbr:examples/file_dialog_smoke.zbr:27
        const p = g.openFile();
// zbr:examples/file_dialog_smoke.zbr:28
        if (p) |path| {
// zbr:examples/file_dialog_smoke.zbr:29
            g.send(Msg{ .opened = path });
        }
    }
// zbr:examples/file_dialog_smoke.zbr:30
    if (g.button("Save File")) {
// zbr:examples/file_dialog_smoke.zbr:31
        const p = g.saveFile();
// zbr:examples/file_dialog_smoke.zbr:32
        if (p) |path| {
// zbr:examples/file_dialog_smoke.zbr:33
            g.send(Msg{ .saved = path });
        }
    }
// zbr:examples/file_dialog_smoke.zbr:34
    if (g.button("Open Folder")) {
// zbr:examples/file_dialog_smoke.zbr:35
        const p = g.openFolder();
// zbr:examples/file_dialog_smoke.zbr:36
        if (p) |path| {
// zbr:examples/file_dialog_smoke.zbr:37
            g.send(Msg{ .foldered = path });
        }
    }
// zbr:examples/file_dialog_smoke.zbr:38
    g.separator();
// zbr:examples/file_dialog_smoke.zbr:39
    if (g.button("Show Alert")) {
// zbr:examples/file_dialog_smoke.zbr:40
        g.msgBox("File Dialogs", "All three dialog types are working.");
    }
// zbr:examples/file_dialog_smoke.zbr:41
    if (g.button("Show Error")) {
// zbr:examples/file_dialog_smoke.zbr:42
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:examples/file_dialog_smoke.zbr:45
    _gui_mvu_run("File Dialog Smoke", 400, 300, init, update, view);
}
