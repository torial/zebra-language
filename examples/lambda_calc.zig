...
// Source: examples/lambda_calc.zbr
...
pub const Token = struct {
    ...
    text: []const u8 = "",
    pub fn init(kind: TokKind, text: []const u8) *Token {
        const _self = _allocator.create(Token) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Token");
            _self.kind = TokKind.eof;
            _self.text = "";
// zbr:examples/lambda_calc.zbr:38
            _self.kind = kind;
// zbr:examples/lambda_calc.zbr:39
...
const _reflect_Token_fields: []const []const u8 = &.{"kind", "text"};
const _reflect_Token_field_types: []const []const u8 = &.{"TokKind", "str"};
...
pub const Lexer = struct {
    ...
    pub fn init(src: []const u8) *Lexer {
        const _self = _allocator.create(Lexer) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Lexer");
            _self.src = "";
            _self.pos = 0;
// zbr:examples/lambda_calc.zbr:49
            _self.src = _intern(src);
// zbr:examples/lambda_calc.zbr:50
            _self.pos = 0;
        return _self;
    }

    pub fn atEnd(self: *Lexer) bool {
// zbr:examples/lambda_calc.zbr:53
        return _zebra_ge(self.pos, self.src.len);
    }

    pub fn isIdentStart(self: *Lexer, c: u21) bool {
        _ = self;
// zbr:examples/lambda_calc.zbr:56
        return (((_zebra_ge(c, 'a') and _zebra_le(c, 'z')) or (_zebra_ge(c, 'A') and _zebra_le(c, 'Z'))) or (c == '_'));
    }

    pub fn isIdentCont(self: *Lexer, c: u21) bool {
// zbr:examples/lambda_calc.zbr:59
        return ((self.isIdentStart(c) or (_zebra_ge(c, '0') and _zebra_le(c, '9'))) or (c == '\''));
    }

    pub fn skipSpace(self: *Lexer) void {
// zbr:examples/lambda_calc.zbr:62
        while ((!self.atEnd())) {
// zbr:examples/lambda_calc.zbr:63
            const c = self.src[@as(usize, @intCast(self.pos))];
// zbr:examples/lambda_calc.zbr:64
            if (((((c == ' ') or (c == '\t')) or (c == '\n')) or (c == '\r'))) {
// zbr:examples/lambda_calc.zbr:65
                self.pos = (self.pos + 1);
            } else {
                break;
            }
    ...
    pub fn nextToken(self: *Lexer) anyerror!*Token {
        self.skipSpace();
// zbr:examples/lambda_calc.zbr:71
        if (self.atEnd()) {
// zbr:examples/lambda_calc.zbr:72
            return Token.init(TokKind.eof, "");
        }
// zbr:examples/lambda_calc.zbr:73
        const c = self.src[@as(usize, @intCast(self.pos))];
// zbr:examples/lambda_calc.zbr:74
        if ((c == '(')) {
// zbr:examples/lambda_calc.zbr:75
            self.pos = (self.pos + 1);
// zbr:examples/lambda_calc.zbr:76
            return Token.init(TokKind.lparen, "(");
        }
// zbr:examples/lambda_calc.zbr:77
        if ((c == ')')) {
// zbr:examples/lambda_calc.zbr:78
            self.pos = (self.pos + 1);
// zbr:examples/lambda_calc.zbr:79
            return Token.init(TokKind.rparen, ")");
        }
// zbr:examples/lambda_calc.zbr:80
        if ((c == '\\')) {
// zbr:examples/lambda_calc.zbr:81
            self.pos = (self.pos + 1);
// zbr:examples/lambda_calc.zbr:82
            return Token.init(TokKind.backslash, "\\");
        }
// zbr:examples/lambda_calc.zbr:83
        if ((c == '.')) {
// zbr:examples/lambda_calc.zbr:84
            self.pos = (self.pos + 1);
// zbr:examples/lambda_calc.zbr:85
            return Token.init(TokKind.dot, ".");
        }
// zbr:examples/lambda_calc.zbr:86
        if (self.isIdentStart(c)) {
// zbr:examples/lambda_calc.zbr:87
            const start = self.pos;
// zbr:examples/lambda_calc.zbr:88
            while (((!self.atEnd()) and self.isIdentCont(self.src[@as(usize, @intCast(self.pos))]))) {
// zbr:examples/lambda_calc.zbr:89
                self.pos = (self.pos + 1);
            }
// zbr:examples/lambda_calc.zbr:90
            const text: []const u8 = self.src[@as(usize, @intCast(start))..@as(usize, @intCast(self.pos))];
// zbr:examples/lambda_calc.zbr:91
            return Token.init(TokKind.ident, text);
        }
// zbr:examples/lambda_calc.zbr:92
        { _error_ctx = .{ .message = "lex: unexpected character" }; return error.ZebraError; }
    }

};
const _ttag_Lexer: u64 = _zbr_hash("Lexer");
const _reflect_Lexer_name: []const u8 = "Lexer";
const _reflect_Lexer_fields: []const []const u8 = &.{"src", "pos"};
const _reflect_Lexer_field_types: []const []const u8 = &.{"str", "int"};
...
pub const Lam = struct {
    param: []const u8,
    body: *Term,
    pub fn init(param: []const u8, body: *Term) Lam {
        var _self: Lam = undefined;
// zbr:examples/lambda_calc.zbr:109
            _self.param = _intern(param);
// zbr:examples/lambda_calc.zbr:110
            _self.body = body;
        return _self;
    }
...
pub const App = struct {
    func: *Term,
    arg: *Term,
    pub fn init(func: *Term, arg: *Term) App {
        var _self: App = undefined;
// zbr:examples/lambda_calc.zbr:117
            _self.func = func;
// zbr:examples/lambda_calc.zbr:118
            _self.arg = arg;
        return _self;
    }
...
// zbr:examples/lambda_calc.zbr:136
            _self.lex = Lexer.init(src);
// zbr:examples/lambda_calc.zbr:137
            _self.cur = Token.init(TokKind.eof, "");
        return _self;
    }

    pub fn start(self: *Parser) anyerror!void {
// zbr:examples/lambda_calc.zbr:140
        self.cur = try self.lex.nextToken();
    }

    pub fn bump(self: *Parser) anyerror!void {
// zbr:examples/lambda_calc.zbr:143
        self.cur = try self.lex.nextToken();
    }

    pub fn parseAtom(self: *Parser) anyerror!Term {
// zbr:examples/lambda_calc.zbr:146
        const t = self.cur;
// zbr:examples/lambda_calc.zbr:147
        if ((t.kind == TokKind.ident)) {
            try self.bump();
// zbr:examples/lambda_calc.zbr:149
            return Term{ .var_ = t.text };
        }
// zbr:examples/lambda_calc.zbr:150
        if ((t.kind == TokKind.lparen)) {
            try self.bump();
// zbr:examples/lambda_calc.zbr:152
            const inner = try self.parseTerm();
// zbr:examples/lambda_calc.zbr:153
            if ((self.cur.kind != TokKind.rparen)) {
// zbr:examples/lambda_calc.zbr:154
                { _error_ctx = .{ .message = "parse: missing ')'" }; return error.ZebraError; }
            }
            try self.bump();
// zbr:examples/lambda_calc.zbr:156
            return inner;
        }
// zbr:examples/lambda_calc.zbr:157
        if ((t.kind == TokKind.backslash)) {
            try self.bump();
// zbr:examples/lambda_calc.zbr:159
            if ((self.cur.kind != TokKind.ident)) {
// zbr:examples/lambda_calc.zbr:160
                { _error_ctx = .{ .message = "parse: expected variable after '\\'" }; return error.ZebraError; }
            }
// zbr:examples/lambda_calc.zbr:161
            const p: []const u8 = self.cur.text;
            try self.bump();
// zbr:examples/lambda_calc.zbr:163
            if ((self.cur.kind != TokKind.dot)) {
// zbr:examples/lambda_calc.zbr:164
                { _error_ctx = .{ .message = "parse: expected '.' after lambda parameter" }; return error.ZebraError; }
            }
            try self.bump();
// zbr:examples/lambda_calc.zbr:166
            const body = try self.parseTerm();
// zbr:examples/lambda_calc.zbr:167
            return Term{ .abs = blk_box_1: { const _bv: std.meta.Child(@FieldType(Term, "abs")) = Lam.init(p, _bx0: { const _bv = body; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_1 _bp; } };
        }
// zbr:examples/lambda_calc.zbr:168
        { _error_ctx = .{ .message = "parse: unexpected token" }; return error.ZebraError; }
    }

    pub fn parseTerm(self: *Parser) anyerror!Term {
// zbr:examples/lambda_calc.zbr:172
        var lhs = try self.parseAtom();
// zbr:examples/lambda_calc.zbr:173
        while (self.canStartAtom()) {
// zbr:examples/lambda_calc.zbr:174
            const rhs = try self.parseAtom();
// zbr:examples/lambda_calc.zbr:175
            lhs = Term{ .app = blk_box_2: { const _bv: std.meta.Child(@FieldType(Term, "app")) = App.init(_bx0: { const _bv = lhs; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }, _bx1: { const _bv = rhs; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx1 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_2 _bp; } };
        }
// zbr:examples/lambda_calc.zbr:176
        return lhs;
    }

    pub fn canStartAtom(self: *Parser) bool {
// zbr:examples/lambda_calc.zbr:179
        const k = self.cur.kind;
// zbr:examples/lambda_calc.zbr:180
...
const _reflect_Parser_fields: []const []const u8 = &.{"lex", "cur"};
const _reflect_Parser_field_types: []const []const u8 = &.{"Lexer", "Token"};

pub fn hasName(list: std.ArrayList([]const u8), name: []const u8) bool {
// zbr:examples/lambda_calc.zbr:194
    for (list.items) |x| {
// zbr:examples/lambda_calc.zbr:195
        const v: []const u8 = x;
// zbr:examples/lambda_calc.zbr:196
        if (std.mem.eql(u8, v, name)) {
// zbr:examples/lambda_calc.zbr:197
            return true;
        }
    }
// zbr:examples/lambda_calc.zbr:198
    return false;
}

pub fn freeVars(t: Term) std.ArrayList([]const u8) {
// zbr:examples/lambda_calc.zbr:207
    var out = std.ArrayList([]const u8){};
// zbr:examples/lambda_calc.zbr:208
    switch (t) {
        .var_ => |name| {
            out.append(_allocator, name) catch @panic("OOM");
        },
        .abs => |_ptr_l| {
            const l = _ptr_l.*;
// zbr:examples/lambda_calc.zbr:212
            const inner = freeVars(l.body.*);
// zbr:examples/lambda_calc.zbr:213
            for (inner.items) |x| {
// zbr:examples/lambda_calc.zbr:214
                const v: []const u8 = x;
// zbr:examples/lambda_calc.zbr:215
                if (!std.mem.eql(u8, v, l.param)) {
// zbr:examples/lambda_calc.zbr:216
                    if ((!hasName(out, v))) {
                        out.append(_allocator, v) catch @panic("OOM");
                    }
                }
            }
        },
        .app => |_ptr_a| {
            const a = _ptr_a.*;
// zbr:examples/lambda_calc.zbr:219
            for (freeVars(a.func.*).items) |x| {
// zbr:examples/lambda_calc.zbr:220
                const fv: []const u8 = x;
// zbr:examples/lambda_calc.zbr:221
                if ((!hasName(out, fv))) {
                    out.append(_allocator, fv) catch @panic("OOM");
                }
            }
// zbr:examples/lambda_calc.zbr:223
            for (freeVars(a.arg.*).items) |x| {
// zbr:examples/lambda_calc.zbr:224
                const av: []const u8 = x;
// zbr:examples/lambda_calc.zbr:225
                if ((!hasName(out, av))) {
                    out.append(_allocator, av) catch @panic("OOM");
                }
            }
        },
    }
// zbr:examples/lambda_calc.zbr:227
    return out;
}

pub fn subst(body: Term, x: []const u8, n: Term) Term {
// zbr:examples/lambda_calc.zbr:238
    switch (body) {
        .var_ => |name| {
// zbr:examples/lambda_calc.zbr:240
            if (std.mem.eql(u8, name, x)) {
// zbr:examples/lambda_calc.zbr:241
                return n;
            }
// zbr:examples/lambda_calc.zbr:242
            return Term{ .var_ = name };
        },
        .abs => |_ptr_l| {
            const l = _ptr_l.*;
// zbr:examples/lambda_calc.zbr:244
            if (std.mem.eql(u8, l.param, x)) {
// zbr:examples/lambda_calc.zbr:246
                return Term{ .abs = blk_box_3: { const _bv: std.meta.Child(@FieldType(Term, "abs")) = l; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_3 _bp; } };
            }
// zbr:examples/lambda_calc.zbr:248
            const n_free = freeVars(n);
// zbr:examples/lambda_calc.zbr:249
            if (hasName(n_free, l.param)) {
// zbr:examples/lambda_calc.zbr:251
                const b_free = freeVars(l.body.*);
// zbr:examples/lambda_calc.zbr:252
                const fresh: []const u8 = freshName(l.param, b_free, n_free);
// zbr:examples/lambda_calc.zbr:253
                const renamed_body: Term = subst(l.body.*, l.param, Term{ .var_ = fresh });
// zbr:examples/lambda_calc.zbr:254
                const rebound: Term = Term{ .abs = blk_box_4: { const _bv: std.meta.Child(@FieldType(Term, "abs")) = Lam.init(fresh, _bx0: { const _bv = renamed_body; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_4 _bp; } };
// zbr:examples/lambda_calc.zbr:255
                return subst(rebound, x, n);
            }
// zbr:examples/lambda_calc.zbr:256
            const new_body: Term = subst(l.body.*, x, n);
// zbr:examples/lambda_calc.zbr:257
            return Term{ .abs = blk_box_5: { const _bv: std.meta.Child(@FieldType(Term, "abs")) = Lam.init(l.param, _bx0: { const _bv = new_body; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_5 _bp; } };
        },
        .app => |_ptr_a| {
            const a = _ptr_a.*;
// zbr:examples/lambda_calc.zbr:259
            const f = subst(a.func.*, x, n);
// zbr:examples/lambda_calc.zbr:260
            const v = subst(a.arg.*, x, n);
// zbr:examples/lambda_calc.zbr:261
            return Term{ .app = blk_box_6: { const _bv: std.meta.Child(@FieldType(Term, "app")) = App.init(_bx0: { const _bv = f; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }, _bx1: { const _bv = v; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx1 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_6 _bp; } };
        },
    }
}

pub fn freshName(base: []const u8, b_free: std.ArrayList([]const u8), n_free: std.ArrayList([]const u8)) []const u8 {
// zbr:examples/lambda_calc.zbr:266
    var candidate: []const u8 = _str_concat(base, "'", _allocator);
// zbr:examples/lambda_calc.zbr:267
    while ((hasName(b_free, candidate) or hasName(n_free, candidate))) {
// zbr:examples/lambda_calc.zbr:268
        candidate = _str_concat(candidate, "'", _allocator);
    }
// zbr:examples/lambda_calc.zbr:269
    return candidate;
}

pub const StepResult = struct {
    _type_tag: u64 = _ttag_StepResult,
    term: Term = Term{ .var_ = "" },
    stepped: bool = false,
    pub fn init(term: Term, stepped: bool) *StepResult {
        const _self = _allocator.create(StepResult) catch @panic("OOM");
        _self._type_tag = _zbr_hash("StepResult");
            _self.term = Term{ .var_ = "" };
            _self.stepped = false;
// zbr:examples/lambda_calc.zbr:286
            _self.term = term;
// zbr:examples/lambda_calc.zbr:287
            _self.stepped = stepped;
        return _self;
    }

};
const _ttag_StepResult: u64 = _zbr_hash("StepResult");
const _reflect_StepResult_name: []const u8 = "StepResult";
const _reflect_StepResult_fields: []const []const u8 = &.{"term", "stepped"};
const _reflect_StepResult_field_types: []const []const u8 = &.{"Term", "bool"};

pub fn reduceOnce(t: Term) *StepResult {
// zbr:examples/lambda_calc.zbr:290
    switch (t) {
        .app => |_ptr_a| {
            const a = _ptr_a.*;
// zbr:examples/lambda_calc.zbr:293
            if (a.func.* == .abs) {
                const l_ptr = a.func.*.abs;
                const l = l_ptr.*;
// zbr:examples/lambda_calc.zbr:295
                const reduced: Term = subst(l.body.*, l.param, a.arg.*);
// zbr:examples/lambda_calc.zbr:296
                return StepResult.init(reduced, true);
            }
// zbr:examples/lambda_calc.zbr:298
            const fres = reduceOnce(a.func.*);
// zbr:examples/lambda_calc.zbr:299
            if (fres.stepped) {
// zbr:examples/lambda_calc.zbr:303
                const new_func: Term = fres.term;
// zbr:examples/lambda_calc.zbr:304
                const same_arg: Term = a.arg.*;
// zbr:examples/lambda_calc.zbr:305
                return StepResult.init(Term{ .app = blk_box_7: { const _bv: std.meta.Child(@FieldType(Term, "app")) = App.init(_bx0: { const _bv = new_func; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }, _bx1: { const _bv = same_arg; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx1 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_7 _bp; } }, true);
            }
// zbr:examples/lambda_calc.zbr:307
            const ares = reduceOnce(a.arg.*);
// zbr:examples/lambda_calc.zbr:308
            if (ares.stepped) {
// zbr:examples/lambda_calc.zbr:309
                const same_func: Term = a.func.*;
// zbr:examples/lambda_calc.zbr:310
                const new_arg: Term = ares.term;
// zbr:examples/lambda_calc.zbr:311
                return StepResult.init(Term{ .app = blk_box_8: { const _bv: std.meta.Child(@FieldType(Term, "app")) = App.init(_bx0: { const _bv = same_func; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }, _bx1: { const _bv = new_arg; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx1 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_8 _bp; } }, true);
            }
// zbr:examples/lambda_calc.zbr:312
            return StepResult.init(t, false);
        },
        .abs => |_ptr_l| {
            const l = _ptr_l.*;
// zbr:examples/lambda_calc.zbr:315
            const bres = reduceOnce(l.body.*);
// zbr:examples/lambda_calc.zbr:316
            if (bres.stepped) {
// zbr:examples/lambda_calc.zbr:317
                const new_body_r: Term = bres.term;
// zbr:examples/lambda_calc.zbr:318
                return StepResult.init(Term{ .abs = blk_box_9: { const _bv: std.meta.Child(@FieldType(Term, "abs")) = Lam.init(l.param, _bx0: { const _bv = new_body_r; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_9 _bp; } }, true);
            }
// zbr:examples/lambda_calc.zbr:319
            return StepResult.init(t, false);
        },
        .var_ => |_| {
// zbr:examples/lambda_calc.zbr:321
            return StepResult.init(t, false);
        },
    }
}

pub fn show(t: Term) []const u8 {
// zbr:examples/lambda_calc.zbr:330
    switch (t) {
        .var_ => |name| {
// zbr:examples/lambda_calc.zbr:332
            return name;
        },
        .abs => |_ptr_l| {
            const l = _ptr_l.*;
// zbr:examples/lambda_calc.zbr:334
            return _str_concat(_str_concat(_str_concat("\\", l.param, _allocator), ".", _allocator), show(l.body.*), _allocator);
        },
        .app => |_ptr_a| {
            const a = _ptr_a.*;
// zbr:examples/lambda_calc.zbr:336
            return _str_concat(_str_concat(showApp(a.func.*), " ", _allocator), showAtom(a.arg.*), _allocator);
        },
    }
}

pub fn showApp(t: Term) []const u8 {
// zbr:examples/lambda_calc.zbr:340
    if ((t == .abs)) {
// zbr:examples/lambda_calc.zbr:341
        return _str_concat(_str_concat("(", show(t), _allocator), ")", _allocator);
    }
// zbr:examples/lambda_calc.zbr:342
    return show(t);
}

pub fn showAtom(t: Term) []const u8 {
// zbr:examples/lambda_calc.zbr:347
    if ((t == .abs)) {
// zbr:examples/lambda_calc.zbr:348
        return _str_concat(_str_concat("(", show(t), _allocator), ")", _allocator);
    }
// zbr:examples/lambda_calc.zbr:349
    if ((t == .app)) {
// zbr:examples/lambda_calc.zbr:350
        return _str_concat(_str_concat("(", show(t), _allocator), ")", _allocator);
    }
// zbr:examples/lambda_calc.zbr:351
    return show(t);
}

pub fn runOne(src: []const u8) void {
// zbr:examples/lambda_calc.zbr:356
    {
        var _try_err: ?anyerror = null;
        _ = &_try_err;
        _try_blk: {
// zbr:examples/lambda_calc.zbr:357
            var p = Parser.init(src);
            p.start() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
// zbr:examples/lambda_calc.zbr:359
            const t = p.parseTerm() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
// zbr:examples/lambda_calc.zbr:360
            if ((p.cur.kind != TokKind.eof)) {
                std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "  {s}  -> error: trailing tokens", .{src}) catch @panic("OOM"))});
// zbr:examples/lambda_calc.zbr:362
                return;
            }
            std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "  {s}", .{src}) catch @panic("OOM"))});
// zbr:examples/lambda_calc.zbr:364
            var steps: std.ArrayList([]const u8) = std.ArrayList([]const u8){};
            steps.append(_allocator, _intern(show(t))) catch @panic("OOM");
// zbr:examples/lambda_calc.zbr:366
            const step_cap: i64 = 64;
// zbr:examples/lambda_calc.zbr:367
            var i: i64 = 0;
// zbr:examples/lambda_calc.zbr:368
            var current: Term = t;
// zbr:examples/lambda_calc.zbr:369
            while (_zebra_lt(i, step_cap)) {
// zbr:examples/lambda_calc.zbr:370
                const step = reduceOnce(current);
// zbr:examples/lambda_calc.zbr:371
                if ((!step.stepped)) {
                    break;
                }
// zbr:examples/lambda_calc.zbr:373
                current = step.term;
                steps.append(_allocator, _intern(show(current))) catch @panic("OOM");
// zbr:examples/lambda_calc.zbr:375
                i = (i + 1);
            }
// zbr:examples/lambda_calc.zbr:376
            var k: i64 = 0;
// zbr:examples/lambda_calc.zbr:377
            while (_zebra_lt(k, @as(i64, @intCast(steps.items.len)))) {
// zbr:examples/lambda_calc.zbr:378
                const line: []const u8 = steps.items[@intCast(k)];
// zbr:examples/lambda_calc.zbr:379
                if ((k == 0)) {
                    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "    = {s}", .{line}) catch @panic("OOM"))});
                } else {
                    std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "    → {s}", .{line}) catch @panic("OOM"))});
                }
// zbr:examples/lambda_calc.zbr:383
                k = (k + 1);
            }
// zbr:examples/lambda_calc.zbr:384
            if ((i == step_cap)) {
                std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "    (stopped after {} steps — no normal form found)", .{step_cap}) catch @panic("OOM"))});
            }
            break :_try_blk;
        }
        if (_try_err != null) {
            std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "  {s}  -> error: {s}", .{src, _zbr_error_msg()}) catch @panic("OOM"))});
        }
    }
}

pub fn main() void {
    std.debug.print("{s}\n", .{"lambda_calc — untyped lambda calculus"});
    std.debug.print("{s}\n", .{"---"});
    runOne("(\\x.x) y");
    runOne("(\\x.\\y.x) a b");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    ...
    if (tag == _ttag_StepResult) return _reflect_StepResult_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_StepResult) return .{ .items = _reflect_StepResult_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    ...
    if (tag == _ttag_StepResult) return .{ .items = _reflect_StepResult_field_types };
    return .{ .items = &.{} };
}
