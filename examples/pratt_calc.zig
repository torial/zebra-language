...
// Source: examples/pratt_calc.zbr
...
pub const Token = struct {
    ...
    value: f64 = 0.0,
    pub fn init(kind: TokKind, value: f64) *Token {
        const self = _allocator.create(Token) catch @panic("OOM");
        self._type_tag = _ttag_Token;
// zbr:examples/pratt_calc.zbr:51
        self.kind = kind;
// zbr:examples/pratt_calc.zbr:52
        self.value = value;
        return self;
    }
...
const _reflect_Token_fields: []const []const u8 = &.{"kind", "value"};
const _reflect_Token_field_types: []const []const u8 = &.{"TokKind", "float"};
...
// zbr:examples/pratt_calc.zbr:66
        self.src = _intern(src);
// zbr:examples/pratt_calc.zbr:67
        self.pos = 0;
        return self;
    }

    pub fn peek(self: *Lexer) u21 {
// zbr:examples/pratt_calc.zbr:72
        return self.src[@intCast(self.pos)];
    }

    pub fn atEnd(self: *Lexer) bool {
// zbr:examples/pratt_calc.zbr:75
        return _zebra_ge(self.pos, self.src.len);
    }

    pub fn advance(self: *Lexer) u21 {
// zbr:examples/pratt_calc.zbr:78
        const c = self.peek();
// zbr:examples/pratt_calc.zbr:79
        self.pos = (self.pos + 1);
// zbr:examples/pratt_calc.zbr:80
        return c;
    }

    pub fn skipSpace(self: *Lexer) void {
// zbr:examples/pratt_calc.zbr:83
        while (_zebra_lt(self.pos, self.src.len)) {
// zbr:examples/pratt_calc.zbr:84
            const c = self.peek();
// zbr:examples/pratt_calc.zbr:85
            if (((((c == ' ') or (c == '\t')) or (c == '\n')) or (c == '\r'))) {
// zbr:examples/pratt_calc.zbr:86
                self.pos = (self.pos + 1);
            } else {
                break;
            }
    ...
    pub fn isDigit(self: *Lexer, c: u21) bool {
        _ = self;
// zbr:examples/pratt_calc.zbr:91
        return (_zebra_ge(c, '0') and _zebra_le(c, '9'));
    }

    pub fn nextToken(self: *Lexer) anyerror!*Token {
// zbr:examples/pratt_calc.zbr:94
        self.skipSpace();
// zbr:examples/pratt_calc.zbr:95
        if (self.atEnd()) {
// zbr:examples/pratt_calc.zbr:96
            return Token.init(TokKind.eof, 0.0);
        }
// zbr:examples/pratt_calc.zbr:98
        const c = self.peek();
// zbr:examples/pratt_calc.zbr:99
        if (self.isDigit(c)) {
// zbr:examples/pratt_calc.zbr:100
            return try self.readNumber();
        }
// zbr:examples/pratt_calc.zbr:105
        self.pos = (self.pos + 1);
// zbr:examples/pratt_calc.zbr:106
        if ((c == '+')) {
// zbr:examples/pratt_calc.zbr:107
            return Token.init(TokKind.plus, 0.0);
        }
// zbr:examples/pratt_calc.zbr:108
        if ((c == '-')) {
// zbr:examples/pratt_calc.zbr:109
            return Token.init(TokKind.minus, 0.0);
        }
// zbr:examples/pratt_calc.zbr:110
        if ((c == '*')) {
// zbr:examples/pratt_calc.zbr:111
            return Token.init(TokKind.star, 0.0);
        }
// zbr:examples/pratt_calc.zbr:112
        if ((c == '/')) {
// zbr:examples/pratt_calc.zbr:113
            return Token.init(TokKind.slash, 0.0);
        }
// zbr:examples/pratt_calc.zbr:114
        if ((c == '^')) {
// zbr:examples/pratt_calc.zbr:115
            return Token.init(TokKind.caret, 0.0);
        }
// zbr:examples/pratt_calc.zbr:116
        if ((c == '(')) {
// zbr:examples/pratt_calc.zbr:117
            return Token.init(TokKind.lparen, 0.0);
        }
// zbr:examples/pratt_calc.zbr:118
        if ((c == ')')) {
// zbr:examples/pratt_calc.zbr:119
            return Token.init(TokKind.rparen, 0.0);
        }
// zbr:examples/pratt_calc.zbr:120
        _error_ctx = .{ .message = "lex: unexpected character", .details = null };
        return error.ZebraError;
    }

    pub fn readNumber(self: *Lexer) anyerror!*Token {
// zbr:examples/pratt_calc.zbr:123
        const start = self.pos;
// zbr:examples/pratt_calc.zbr:124
        while ((_zebra_lt(self.pos, self.src.len) and self.isDigit(self.peek()))) {
// zbr:examples/pratt_calc.zbr:125
            self.pos = (self.pos + 1);
        }
// zbr:examples/pratt_calc.zbr:127
        if ((_zebra_lt(self.pos, self.src.len) and (self.peek() == '.'))) {
// zbr:examples/pratt_calc.zbr:128
            self.pos = (self.pos + 1);
// zbr:examples/pratt_calc.zbr:129
            while ((_zebra_lt(self.pos, self.src.len) and self.isDigit(self.peek()))) {
// zbr:examples/pratt_calc.zbr:130
                self.pos = (self.pos + 1);
            }
        }
// zbr:examples/pratt_calc.zbr:134
        const text: []const u8 = self.src[@intCast(start)..@intCast(self.pos)];
// zbr:examples/pratt_calc.zbr:135
        const maybe: ?f64 = (std.fmt.parseFloat(f64, text) catch 0.0);
// zbr:examples/pratt_calc.zbr:136
        if (maybe) |v| {
// zbr:examples/pratt_calc.zbr:137
            return Token.init(TokKind.num, v);
        }
// zbr:examples/pratt_calc.zbr:138
        _error_ctx = .{ .message = "lex: malformed number", .details = null };
        return error.ZebraError;
    }
...
pub const Bin = struct {
    ...
    pub fn init(op: u21, left: *Expr, right: *Expr) Bin {
        var self: Bin = undefined;
// zbr:examples/pratt_calc.zbr:158
        self.op = op;
// zbr:examples/pratt_calc.zbr:159
        self.left = left;
// zbr:examples/pratt_calc.zbr:160
        self.right = right;
        return self;
    }
...
pub const Un = struct {
    ...
    pub fn init(op: u21, operand: *Expr) Un {
        var self: Un = undefined;
// zbr:examples/pratt_calc.zbr:167
        self.op = op;
// zbr:examples/pratt_calc.zbr:168
...
// zbr:examples/pratt_calc.zbr:194
        self.lex = Lexer.init(src);
// zbr:examples/pratt_calc.zbr:195
        self.cur = Token.init(TokKind.eof, 0.0);
        return self;
    }

    pub fn start(self: *Parser) anyerror!void {
// zbr:examples/pratt_calc.zbr:198
        self.cur = try self.lex.nextToken();
    }

    pub fn bumpToken(self: *Parser) anyerror!void {
// zbr:examples/pratt_calc.zbr:201
        self.cur = try self.lex.nextToken();
    }

    pub fn infixBp(self: *Parser, kind: TokKind, left_out: *std.ArrayList(i64), right_out: *std.ArrayList(i64)) void {
        _ = self;
// zbr:examples/pratt_calc.zbr:207
        left_out.clearRetainingCapacity();
// zbr:examples/pratt_calc.zbr:208
        right_out.clearRetainingCapacity();
// zbr:examples/pratt_calc.zbr:209
        if (((kind == TokKind.plus) or (kind == TokKind.minus))) {
// zbr:examples/pratt_calc.zbr:210
            left_out.append(_allocator, 10) catch unreachable;
// zbr:examples/pratt_calc.zbr:211
            right_out.append(_allocator, 11) catch unreachable;
// zbr:examples/pratt_calc.zbr:212
            return;
        }
// zbr:examples/pratt_calc.zbr:213
        if (((kind == TokKind.star) or (kind == TokKind.slash))) {
// zbr:examples/pratt_calc.zbr:214
            left_out.append(_allocator, 20) catch unreachable;
// zbr:examples/pratt_calc.zbr:215
            right_out.append(_allocator, 21) catch unreachable;
// zbr:examples/pratt_calc.zbr:216
            return;
        }
// zbr:examples/pratt_calc.zbr:217
        if ((kind == TokKind.caret)) {
// zbr:examples/pratt_calc.zbr:218
            left_out.append(_allocator, 30) catch unreachable;
// zbr:examples/pratt_calc.zbr:219
            right_out.append(_allocator, 30) catch unreachable;
// zbr:examples/pratt_calc.zbr:220
            return;
        }
// zbr:examples/pratt_calc.zbr:221
        left_out.append(_allocator, (-1)) catch unreachable;
// zbr:examples/pratt_calc.zbr:222
        right_out.append(_allocator, (-1)) catch unreachable;
    }

    pub fn opChar(self: *Parser, kind: TokKind) u21 {
        _ = self;
// zbr:examples/pratt_calc.zbr:225
        if ((kind == TokKind.plus)) {
// zbr:examples/pratt_calc.zbr:226
            return '+';
        }
// zbr:examples/pratt_calc.zbr:227
        if ((kind == TokKind.minus)) {
// zbr:examples/pratt_calc.zbr:228
            return '-';
        }
// zbr:examples/pratt_calc.zbr:229
        if ((kind == TokKind.star)) {
// zbr:examples/pratt_calc.zbr:230
            return '*';
        }
// zbr:examples/pratt_calc.zbr:231
        if ((kind == TokKind.slash)) {
// zbr:examples/pratt_calc.zbr:232
            return '/';
        }
// zbr:examples/pratt_calc.zbr:233
        if ((kind == TokKind.caret)) {
// zbr:examples/pratt_calc.zbr:234
            return '^';
        }
// zbr:examples/pratt_calc.zbr:235
        return '?';
    }

    pub fn parsePrimary(self: *Parser) anyerror!Expr {
// zbr:examples/pratt_calc.zbr:238
        const t = self.cur;
// zbr:examples/pratt_calc.zbr:239
        if ((t.kind == TokKind.num)) {
// zbr:examples/pratt_calc.zbr:240
            try self.bumpToken();
// zbr:examples/pratt_calc.zbr:241
            return Expr{ .num_ = t.value };
        }
// zbr:examples/pratt_calc.zbr:242
        if ((t.kind == TokKind.minus)) {
// zbr:examples/pratt_calc.zbr:246
            try self.bumpToken();
// zbr:examples/pratt_calc.zbr:247
            const rhs = try self.parseExpr(25);
// zbr:examples/pratt_calc.zbr:248
            return Expr{ .unary = _box_1: { const _bp_1 = _allocator.create(Un) catch @panic("OOM"); _bp_1.* = Un.init('-', _box_2: { const _bp_2 = _allocator.create(Expr) catch @panic("OOM"); _bp_2.* = rhs; break :_box_2 _bp_2; }); break :_box_1 _bp_1; } };
        }
// zbr:examples/pratt_calc.zbr:249
        if ((t.kind == TokKind.lparen)) {
// zbr:examples/pratt_calc.zbr:250
            try self.bumpToken();
// zbr:examples/pratt_calc.zbr:251
            const inner = try self.parseExpr(0);
// zbr:examples/pratt_calc.zbr:252
            if ((self.cur.kind != TokKind.rparen)) {
// zbr:examples/pratt_calc.zbr:253
                _error_ctx = .{ .message = "parse: missing ')'", .details = null };
                return error.ZebraError;
            }
// zbr:examples/pratt_calc.zbr:254
            try self.bumpToken();
// zbr:examples/pratt_calc.zbr:255
            return inner;
        }
// zbr:examples/pratt_calc.zbr:256
        _error_ctx = .{ .message = "parse: unexpected token in primary position", .details = null };
        return error.ZebraError;
    }

    pub fn parseExpr(self: *Parser, min_bp: i64) anyerror!Expr {
// zbr:examples/pratt_calc.zbr:259
        var lhs = try self.parsePrimary();
// zbr:examples/pratt_calc.zbr:260
        while (true) {
// zbr:examples/pratt_calc.zbr:261
            const k = self.cur.kind;
// zbr:examples/pratt_calc.zbr:262
            var lefts = std.ArrayList(i64){};
// zbr:examples/pratt_calc.zbr:263
            var rights = std.ArrayList(i64){};
// zbr:examples/pratt_calc.zbr:264
            self.infixBp(k, &lefts, &rights);
// zbr:examples/pratt_calc.zbr:265
            const lbp = lefts.items[@as(usize, @intCast(0))];
// zbr:examples/pratt_calc.zbr:266
            const rbp = rights.items[@as(usize, @intCast(0))];
// zbr:examples/pratt_calc.zbr:267
            if (_zebra_lt(lbp, 0)) {
                break;
            }
// zbr:examples/pratt_calc.zbr:269
            if (_zebra_lt(lbp, min_bp)) {
                break;
            }
// zbr:examples/pratt_calc.zbr:271
            const op = self.opChar(k);
// zbr:examples/pratt_calc.zbr:272
            try self.bumpToken();
// zbr:examples/pratt_calc.zbr:273
            const rhs = try self.parseExpr(rbp);
// zbr:examples/pratt_calc.zbr:274
            lhs = Expr{ .bin = _box_3: { const _bp_3 = _allocator.create(Bin) catch @panic("OOM"); _bp_3.* = Bin.init(op, _box_4: { const _bp_4 = _allocator.create(Expr) catch @panic("OOM"); _bp_4.* = lhs; break :_box_4 _bp_4; }, _box_5: { const _bp_5 = _allocator.create(Expr) catch @panic("OOM"); _bp_5.* = rhs; break :_box_5 _bp_5; }); break :_box_3 _bp_3; } };
        }
// zbr:examples/pratt_calc.zbr:275
        return lhs;
    }
...
pub fn evalExpr(e: Expr) f64 {
// zbr:examples/pratt_calc.zbr:284
    switch (e) {
        .num_ => |v| {
// zbr:examples/pratt_calc.zbr:286
            return v;
        },
        .bin => |b_ptr| {
            const b = b_ptr.*;
// zbr:examples/pratt_calc.zbr:288
            const l = evalExpr(b.left);
// zbr:examples/pratt_calc.zbr:289
            const r = evalExpr(b.right);
// zbr:examples/pratt_calc.zbr:290
            if ((b.op == '+')) {
// zbr:examples/pratt_calc.zbr:291
                return (l + r);
            }
// zbr:examples/pratt_calc.zbr:292
            if ((b.op == '-')) {
// zbr:examples/pratt_calc.zbr:293
                return (l - r);
            }
// zbr:examples/pratt_calc.zbr:294
            if ((b.op == '*')) {
// zbr:examples/pratt_calc.zbr:295
                return (l * r);
            }
// zbr:examples/pratt_calc.zbr:296
            if ((b.op == '/')) {
// zbr:examples/pratt_calc.zbr:297
                return (l / r);
            }
// zbr:examples/pratt_calc.zbr:298
            if ((b.op == '^')) {
// zbr:examples/pratt_calc.zbr:299
                return powFloat(l, r);
            }
// zbr:examples/pratt_calc.zbr:300
            return 0.0;
        },
        .unary => |u_ptr| {
            const u = u_ptr.*;
// zbr:examples/pratt_calc.zbr:302
            const x = evalExpr(u.operand);
// zbr:examples/pratt_calc.zbr:303
            if ((u.op == '-')) {
// zbr:examples/pratt_calc.zbr:304
                return (-x);
            }
// zbr:examples/pratt_calc.zbr:305
            return x;
        },
    }
}

pub fn powFloat(base: f64, exp: f64) f64 {
// zbr:examples/pratt_calc.zbr:312
    var n: i64 = exp.toInt();
// zbr:examples/pratt_calc.zbr:313
    if ((n == 0)) {
// zbr:examples/pratt_calc.zbr:314
        return 1.0;
    }
// zbr:examples/pratt_calc.zbr:315
    var negate: bool = false;
// zbr:examples/pratt_calc.zbr:316
    if (_zebra_lt(n, 0)) {
// zbr:examples/pratt_calc.zbr:317
        negate = true;
// zbr:examples/pratt_calc.zbr:318
        n = (0 - n);
    }
// zbr:examples/pratt_calc.zbr:319
    var acc: f64 = 1.0;
// zbr:examples/pratt_calc.zbr:320
    var i: i64 = 0;
// zbr:examples/pratt_calc.zbr:321
    while (_zebra_lt(i, n)) {
// zbr:examples/pratt_calc.zbr:322
        acc = (acc * base);
// zbr:examples/pratt_calc.zbr:323
        i = (i + 1);
    }
// zbr:examples/pratt_calc.zbr:324
    if (negate) {
// zbr:examples/pratt_calc.zbr:325
        return (1.0 / acc);
    }
// zbr:examples/pratt_calc.zbr:326
    return acc;
}

pub fn formatExpr(e: Expr) []const u8 {
// zbr:examples/pratt_calc.zbr:334
    switch (e) {
        .num_ => |v| {
// zbr:examples/pratt_calc.zbr:336
            return formatFloat(v);
        },
        .bin => |b_ptr| {
            const b = b_ptr.*;
// zbr:examples/pratt_calc.zbr:338
            const ls = formatExpr(b.left);
// zbr:examples/pratt_calc.zbr:339
            const rs = formatExpr(b.right);
// zbr:examples/pratt_calc.zbr:340
            const oc: u21 = b.op;
// zbr:examples/pratt_calc.zbr:341
            return _str_concat(_str_concat(_str_concat(_str_concat(_str_concat(_str_concat("(", ls, _allocator), " ", _allocator), (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(oc, &_cpbuf) catch 1; const _cpout = _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); break :blk @as([]const u8, _cpout); }), _allocator), " ", _allocator), rs, _allocator), ")", _allocator);
        },
        .unary => |u_ptr| {
            const u = u_ptr.*;
// zbr:examples/pratt_calc.zbr:343
            const inner = formatExpr(u.operand);
// zbr:examples/pratt_calc.zbr:344
            const oc: u21 = u.op;
// zbr:examples/pratt_calc.zbr:345
            return _str_concat(_str_concat(_str_concat(_str_concat("(", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(oc, &_cpbuf) catch 1; const _cpout = _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); break :blk @as([]const u8, _cpout); }), _allocator), " ", _allocator), inner, _allocator), ")", _allocator);
        },
    }
}

pub fn formatFloat(v: f64) []const u8 {
// zbr:examples/pratt_calc.zbr:349
    const i: i64 = v.toInt();
// zbr:examples/pratt_calc.zbr:350
    const rebuilt: f64 = i.toFloat();
// zbr:examples/pratt_calc.zbr:351
    if ((rebuilt == v)) {
// zbr:examples/pratt_calc.zbr:352
        return (std.fmt.allocPrint(_allocator, "{}", .{i}) catch unreachable);
    }
// zbr:examples/pratt_calc.zbr:353
    return (std.fmt.allocPrint(_allocator, "{d}", .{v}) catch unreachable);
}

pub fn runOne(src: []const u8) void {
// zbr:examples/pratt_calc.zbr:373
    var _try_err_6: ?anyerror = null;
    _try_blk_6: {
// zbr:examples/pratt_calc.zbr:362
        const p = Parser.init(src);
// zbr:examples/pratt_calc.zbr:363
        p.start() catch |_tc_7| { _try_err_6 = _tc_7; break :_try_blk_6; };
// zbr:examples/pratt_calc.zbr:364
        const ast = p.parseExpr(0) catch |_tc_8| { _try_err_6 = _tc_8; break :_try_blk_6; };
// zbr:examples/pratt_calc.zbr:366
        if ((p.cur.kind != TokKind.eof)) {
// zbr:examples/pratt_calc.zbr:367
            {
                const _pt0 = (std.fmt.allocPrint(_allocator, "  {s}  -> error: trailing tokens", .{src}) catch @panic("OOM"));
                std.debug.print("{s}\n", .{_pt0});
            }
// zbr:examples/pratt_calc.zbr:368
            return;
        }
// zbr:examples/pratt_calc.zbr:369
        const v = evalExpr(ast);
// zbr:examples/pratt_calc.zbr:370
        const fv: []const u8 = formatFloat(v);
// zbr:examples/pratt_calc.zbr:371
        const fa: []const u8 = formatExpr(ast);
// zbr:examples/pratt_calc.zbr:372
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "  {s}  =  {s}    [ {s} ]", .{src, fv, fa}) catch @panic("OOM"));
            std.debug.print("{s}\n", .{_pt0});
        }
        break :_try_blk_6;
    }
    if (_try_err_6 != null) {
// zbr:examples/pratt_calc.zbr:374
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "  {s}  -> error: {s}", .{src, _zbr_error_msg()}) catch @panic("OOM"));
            std.debug.print("{s}\n", .{_pt0});
        }
    }
}
...
pub fn main() void {
// zbr:examples/pratt_calc.zbr:377
    std.debug.print("{s}\n", .{"pratt_calc — operator-precedence parsing demo"});
// zbr:examples/pratt_calc.zbr:378
    std.debug.print("{s}\n", .{"---"});
// zbr:examples/pratt_calc.zbr:379
    runOne("1 + 2 * 3");
// zbr:examples/pratt_calc.zbr:380
    runOne("(1 + 2) * 3");
// zbr:examples/pratt_calc.zbr:381
    runOne("2 ^ 3 ^ 2");
// zbr:examples/pratt_calc.zbr:382
    runOne("-2 ^ 2");
// zbr:examples/pratt_calc.zbr:383
    runOne("1 - 2 - 3");
// zbr:examples/pratt_calc.zbr:384
    runOne("3 + 4 * 2 / (1 - 5) ^ 2 ^ 3");
// zbr:examples/pratt_calc.zbr:385
    runOne("-(3 + 4)");
// zbr:examples/pratt_calc.zbr:386
    runOne("10 / 4");
// zbr:examples/pratt_calc.zbr:387
    runOne("2.5 * 4");
// zbr:examples/pratt_calc.zbr:388
    runOne("1 + ");
// zbr:examples/pratt_calc.zbr:389
    runOne("(1 + 2");
}
