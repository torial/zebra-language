...
// zbr:examples/pratt_calc.zbr:138
        { _error_ctx = .{ .message = "lex: malformed number" }; return error.ZebraError; }
    }
...
// zbr:examples/pratt_calc.zbr:195
            _self.cur = Token.init(TokKind.eof, 0.0);
        return _self;
    }
...
// zbr:examples/pratt_calc.zbr:209
        if (((kind == TokKind.plus) or (kind == TokKind.minus))) {
            left_out.append(_allocator, 10) catch @panic("OOM");
            right_out.append(_allocator, 11) catch @panic("OOM");
// zbr:examples/pratt_calc.zbr:212
            return;
        }
// zbr:examples/pratt_calc.zbr:213
        if (((kind == TokKind.star) or (kind == TokKind.slash))) {
            left_out.append(_allocator, 20) catch @panic("OOM");
            right_out.append(_allocator, 21) catch @panic("OOM");
// zbr:examples/pratt_calc.zbr:216
            return;
        }
// zbr:examples/pratt_calc.zbr:217
        if ((kind == TokKind.caret)) {
            left_out.append(_allocator, 30) catch @panic("OOM");
            right_out.append(_allocator, 30) catch @panic("OOM");
// zbr:examples/pratt_calc.zbr:220
            return;
        }
        left_out.append(_allocator, (-1)) catch @panic("OOM");
        right_out.append(_allocator, (-1)) catch @panic("OOM");
    }
...
// zbr:examples/pratt_calc.zbr:248
            return Expr{ .unary = blk_box_1: { const _bv: std.meta.Child(@FieldType(Expr, "unary")) = Un.init('-', _bx0: { const _bv = rhs; const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :_bx0 _bp; }); const _bp = _allocator.create(@TypeOf(_bv)) catch @panic("OOM"); _bp.* = _bv; break :blk_box_1 _bp; } };
        }
...
// zbr:examples/pratt_calc.zbr:256
        { _error_ctx = .{ .message = "parse: unexpected token in primary position" }; return error.ZebraError; }
    }
...
// zbr:examples/pratt_calc.zbr:265
            const lbp = lefts.items[@intCast(0)];
// zbr:examples/pratt_calc.zbr:266
            const rbp = rights.items[@intCast(0)];
...
// zbr:examples/pratt_calc.zbr:312
    var n: i64 = @as(i64, @intFromFloat(exp));
...
// zbr:examples/pratt_calc.zbr:345
            return _str_concat(_str_concat(_str_concat(_str_concat("(", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(oc), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator), " ", _allocator), inner, _allocator), ")", _allocator);
        },
    }
}
...
// zbr:examples/pratt_calc.zbr:349
    const i: i64 = @as(i64, @intFromFloat(v));
...
// zbr:examples/pratt_calc.zbr:353
    return (std.fmt.allocPrint(_allocator, "{}", .{v}) catch unreachable);
}

pub fn runOne(src: []const u8) void {
// zbr:examples/pratt_calc.zbr:358
    {
...
// zbr:examples/pratt_calc.zbr:364
            const ast = p.parseExpr(0) catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
...
// zbr:examples/pratt_calc.zbr:371
            ...
            std.debug.print("{s}\n", .{(std.fmt.allocPrint(_allocator, "  {s}  =  {s}    [ {s} ]", .{src, fv, fa}) catch @panic("OOM"))});
            break :_try_blk;
        }
        if (_try_err != null) {
