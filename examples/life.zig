...
pub const Life = struct {
    _type_tag: u64 = _ttag_Life,
    width: i64,
    height: i64,
    alive: std.AutoHashMap(i64, bool),
    pub fn init(width: i64, height: i64) *Life {
        const _self = _allocator.create(Life) catch @panic("OOM");
        _self._type_tag = _zbr_hash("Life");
// zbr:examples/life.zbr:17
            _self.width = width;
// zbr:examples/life.zbr:18
            _self.height = height;
// zbr:examples/life.zbr:19
            _self.alive = std.AutoHashMap(i64, bool).init(_allocator);
        return _self;
    }

    pub fn idx(self: *const Life, x: i64, y: i64) i64 {
// zbr:examples/life.zbr:22
        return ((y * self.width) + x);
    }

    pub fn isAlive(self: *Life, x: i64, y: i64) bool {
// zbr:examples/life.zbr:26
        if ((((_zebra_lt(x, 0) or _zebra_ge(x, self.width)) or _zebra_lt(y, 0)) or _zebra_ge(y, self.height))) {
// zbr:examples/life.zbr:27
            return false;
        }
// zbr:examples/life.zbr:28
        return self.alive.contains(self.idx(x, y));
    }

    pub fn setAlive(self: *Life, x: i64, y: i64) void {
        self.alive.put(self.idx(x, y), true) catch @panic("OOM");
    }

    pub fn neighbors(self: *Life, x: i64, y: i64) i64 {
// zbr:examples/life.zbr:35
        var n: i64 = 0;
// zbr:examples/life.zbr:36
        var dy: i64 = (-1);
// zbr:examples/life.zbr:37
        while (_zebra_le(dy, 1)) {
// zbr:examples/life.zbr:38
            var dx: i64 = (-1);
// zbr:examples/life.zbr:39
            while (_zebra_le(dx, 1)) {
// zbr:examples/life.zbr:40
                if ((!((dx == 0) and (dy == 0)))) {
// zbr:examples/life.zbr:41
                    if (self.isAlive((x + dx), (y + dy))) {
// zbr:examples/life.zbr:42
                        n = (n + 1);
                    }
                }
// zbr:examples/life.zbr:43
                dx = (dx + 1);
            }
// zbr:examples/life.zbr:44
            dy = (dy + 1);
        }
// zbr:examples/life.zbr:45
        return n;
    }

    pub fn step(self: *Life) *Life {
// zbr:examples/life.zbr:49
        var nxt = Life.init(self.width, self.height);
// zbr:examples/life.zbr:50
        var y: i64 = 0;
// zbr:examples/life.zbr:51
        while (_zebra_lt(y, self.height)) {
// zbr:examples/life.zbr:52
            var x: i64 = 0;
// zbr:examples/life.zbr:53
            while (_zebra_lt(x, self.width)) {
// zbr:examples/life.zbr:54
                const n = self.neighbors(x, y);
// zbr:examples/life.zbr:55
                if (self.isAlive(x, y)) {
// zbr:examples/life.zbr:57
                    if (((n == 2) or (n == 3))) {
                        nxt.setAlive(x, y);
                    }
                } else {
// zbr:examples/life.zbr:61
                    if ((n == 3)) {
                        nxt.setAlive(x, y);
                    }
                }
// zbr:examples/life.zbr:63
                x = (x + 1);
            }
// zbr:examples/life.zbr:64
            y = (y + 1);
        }
// zbr:examples/life.zbr:65
        return nxt;
    }

    pub fn population(self: *const Life) i64 {
// zbr:examples/life.zbr:69
        return @as(i64, @intCast(self.alive.count()));
    }

    pub fn render(self: *Life) []const u8 {
// zbr:examples/life.zbr:72
        var out: []const u8 = "";
// zbr:examples/life.zbr:73
        var y: i64 = 0;
// zbr:examples/life.zbr:74
        while (_zebra_lt(y, self.height)) {
// zbr:examples/life.zbr:75
            var x: i64 = 0;
// zbr:examples/life.zbr:76
            while (_zebra_lt(x, self.width)) {
// zbr:examples/life.zbr:77
                if (self.isAlive(x, y)) {
// zbr:examples/life.zbr:78
                    out = _str_concat(out, "#", _allocator);
                } else {
// zbr:examples/life.zbr:80
                    out = _str_concat(out, ".", _allocator);
                }
// zbr:examples/life.zbr:81
                x = (x + 1);
            }
// zbr:examples/life.zbr:82
            out = _str_concat(out, "\n", _allocator);
// zbr:examples/life.zbr:83
            y = (y + 1);
        }
// zbr:examples/life.zbr:84
        return out;
    }

};
const _ttag_Life: u64 = _zbr_hash("Life");
const _reflect_Life_name: []const u8 = "Life";
const _reflect_Life_fields: []const []const u8 = &.{"width", "height", "alive"};
const _reflect_Life_field_types: []const []const u8 = &.{"int", "int", "HashMap(int, bool)"};
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:examples/life.zbr:87
    var board = Life.init(10, 10);
    board.setAlive(1, 0);
    board.setAlive(2, 1);
    board.setAlive(0, 2);
    board.setAlive(1, 2);
    ...
    board.setAlive(6, 5);
    board.setAlive(6, 6);
    std.debug.print("{s}\n", .{"Conway's Game of Life — 10x10, glider + blinker"});
// zbr:examples/life.zbr:100
    var gen: i64 = 0;
// zbr:examples/life.zbr:101
    while (_zebra_le(gen, 5)) {
        std.debug.print("{s}\n", .{_str_concat(_str_concat(_str_concat(_str_concat("--- generation ", (std.fmt.allocPrint(_allocator, "{}", .{gen}) catch unreachable), _allocator), "  (pop ", _allocator), (std.fmt.allocPrint(_allocator, "{}", .{board.population()}) catch unreachable), _allocator), ") ---", _allocator)});
        std.debug.print("{s}\n", .{board.render()});
// zbr:examples/life.zbr:104
        board = board.step();
// zbr:examples/life.zbr:105
        gen = (gen + 1);
    }
}
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Life) return _reflect_Life_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Life) return .{ .items = _reflect_Life_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Life) return .{ .items = _reflect_Life_field_types };
    return .{ .items = &.{} };
}
