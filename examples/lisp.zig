...
// zbr:examples/lisp.zbr:110
    ...
    pub fn define(self: *Env, name: []const u8, val: Value) void {
...
const _ttag_Env: u64 = _zbr_hash("Env");
const _reflect_Env_name: []const u8 = "Env";
const _reflect_Env_fields: []const []const u8 = &.{"vars", "parent"};
...
// zbr:examples/lisp.zbr:788
        ...
        if (_try_err != null) {
            std.debug.print("{s}\n", .{_str_concat(_str_concat(_str_concat("  ", label, _allocator), "  =>  error: ", _allocator), _zbr_error_msg(), _allocator)});
        }
    }
}

pub fn main(_zinit: std.process.Init) anyerror!void {
    _io = _zinit.io;
...
// zbr:examples/lisp.zbr:796
    ...
    _ = try runProgram("(define (fact n) (if (= n 0) 1 (* n (fact (- n 1)))))", env);
    runShow("(fact 6)", "(fact 6)", env);
    _ = try runProgram("(define (fib n) (if (< n 2) n (+ (fib (- n 1)) (fib (- n 2)))))", env);
    runShow("(fib 15)", "(fib 15)", env);
    _ = try runProgram("(define (make-adder k) (lambda (x) (+ x k)))", env);
    _ = try runProgram("(define add10 (make-adder 10))", env);
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Env) return _reflect_Env_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Env) return .{ .items = _reflect_Env_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Env) return .{ .items = _reflect_Env_field_types };
    return .{ .items = &.{} };
}
