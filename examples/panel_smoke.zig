...
// Source: examples\panel_smoke.zbr
...
pub fn init() Model {
// zbr:examples\panel_smoke.zbr:9
    return Model{ .count = 0 };
}

pub fn update(model: Model, msg: Msg) Model {
// zbr:examples\panel_smoke.zbr:12
    switch (msg) {
        .inc => {
// zbr:examples\panel_smoke.zbr:13
            return Model{ .count = (model.count + 1) };
        },
        .reset => {
// zbr:examples\panel_smoke.zbr:14
            return Model{ .count = 0 };
        },
    }
}
...
// zbr:examples\panel_smoke.zbr:17
    g.panel("Controls", struct { fn call(g: GuiContext) void {
// zbr:examples\panel_smoke.zbr:18
        if (g.button("+1")) {
// zbr:examples\panel_smoke.zbr:19
            g.send(Msg.inc);
        }
// zbr:examples\panel_smoke.zbr:20
        g.sameLine();
// zbr:examples\panel_smoke.zbr:21
        if (g.button("Reset")) {
// zbr:examples\panel_smoke.zbr:22
            g.send(Msg.reset);
        }
     } }.call);
// zbr:examples\panel_smoke.zbr:24
    g.panel("Status", (struct {
        model: @TypeOf(model),
     fn call(self: @This(), g: GuiContext) void {
// zbr:examples\panel_smoke.zbr:25
        g.text(_str_concat("Count: ", (std.fmt.allocPrint(_allocator, "{}", .{self.model.count}) catch unreachable), _allocator));
     } }{ .model = model, }));
}
...
pub fn main(_zinit: std.process.Init) void {
    ...
    defer _arena.deinit();
// zbr:examples\panel_smoke.zbr:29
    _gui_mvu_run("Panel Smoke", 400, 300, init, update, view);
}
