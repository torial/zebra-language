...
// zbr:selfhost/resolver_test.zbr:43
        const root = try Parser.Parser.parse(src, "<test>");
...
// zbr:selfhost/resolver_test.zbr:73
        if (!((r.errorCount() == 1))) @panic(_str_concat("undefined name: expected 1 error, got ", (std.fmt.allocPrint(_allocator, "{}", .{r.errorCount()}) catch unreachable), _allocator));
...
// zbr:selfhost/resolver_test.zbr:85
        if (!((r.errorCount() == 1))) @panic(_str_concat("pre-decl: expected 1 error, got ", (std.fmt.allocPrint(_allocator, "{}", .{r.errorCount()}) catch unreachable), _allocator));
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:selfhost/resolver_test.zbr:128
        {
