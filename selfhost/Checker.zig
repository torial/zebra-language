...
pub const UnionInfo = struct {
    ...
    pub fn init(name: []const u8, file: []const u8, line: i64) *UnionInfo {
        const self = _allocator.create(UnionInfo) catch @panic("OOM");
        self._type_tag = _ttag_UnionInfo;
...
// zbr:selfhost/Checker.zbr:31
        self.variants = std.ArrayList([]const u8).empty;
// zbr:selfhost/Checker.zbr:32
        self.variant_lines = std.ArrayList(i64).empty;
        return self;
    }

    pub fn addVariant(self: *UnionInfo, v: []const u8, ln: i64) void {
// zbr:selfhost/Checker.zbr:35
        self.variants.append(_allocator, _intern(v)) catch unreachable;
// zbr:selfhost/Checker.zbr:36
        self.variant_lines.append(_allocator, ln) catch unreachable;
    }
...
const _ttag_UnionInfo: u64 = 1036507656;
...
pub const DeadCodeChecker = struct {
    ...
    union_infos: std.StringHashMap(*UnionInfo) = undefined,
    ...
    pub fn init() *DeadCodeChecker {
        const self = _allocator.create(DeadCodeChecker) catch @panic("OOM");
        self._type_tag = _ttag_DeadCodeChecker;
// zbr:selfhost/Checker.zbr:78
        self.union_infos = std.StringHashMap(*UnionInfo).init(_allocator);
// zbr:selfhost/Checker.zbr:79
        self.match_sites = std.StringHashMap(std.ArrayList([]const u8)).init(_allocator);
// zbr:selfhost/Checker.zbr:80
        self.match_key_list = std.ArrayList([]const u8).empty;
// zbr:selfhost/Checker.zbr:81
        self.constructed_set = std.StringHashMap(bool).init(_allocator);
// zbr:selfhost/Checker.zbr:82
        self.constructed_key_list = std.ArrayList([]const u8).empty;
// zbr:selfhost/Checker.zbr:83
        self.module_fns = std.StringHashMap([]const u8).init(_allocator);
// zbr:selfhost/Checker.zbr:84
        self.module_fn_key_list = std.ArrayList([]const u8).empty;
// zbr:selfhost/Checker.zbr:85
        self.fn_edges = std.StringHashMap(std.ArrayList([]const u8)).init(_allocator);
// zbr:selfhost/Checker.zbr:86
        self.root_set = std.StringHashMap(bool).init(_allocator);
// zbr:selfhost/Checker.zbr:87
        self.root_key_list = std.ArrayList([]const u8).empty;
// zbr:selfhost/Checker.zbr:88
        self.reachable_set = std.StringHashMap(bool).init(_allocator);
// zbr:selfhost/Checker.zbr:89
        self.exposed_names = std.StringHashMap(bool).init(_allocator);
// zbr:selfhost/Checker.zbr:90
        self.current_file = _intern("");
...
// zbr:selfhost/Checker.zbr:99
            self.match_sites.put(_intern(key), std.ArrayList([]const u8).empty) catch unreachable;
// zbr:selfhost/Checker.zbr:100
            self.match_key_list.append(_allocator, _intern(key)) catch unreachable;
        }
// zbr:selfhost/Checker.zbr:101
        var sites: std.ArrayList([]const u8) = (self.match_sites.get(key).?);
// zbr:selfhost/Checker.zbr:102
        sites.append(_allocator, _intern(_str_concat(_str_concat(file, ":", _allocator), (std.fmt.allocPrint(_allocator, "{}", .{line}) catch unreachable), _allocator))) catch unreachable;
    }
...
// zbr:selfhost/Checker.zbr:107
            self.constructed_set.put(_intern(key), true) catch unreachable;
// zbr:selfhost/Checker.zbr:108
            self.constructed_key_list.append(_allocator, _intern(key)) catch unreachable;
        }
    }
...
// zbr:selfhost/Checker.zbr:116
            self.module_fns.put(_intern(key), _intern(_str_concat(_str_concat(file, ":", _allocator), (std.fmt.allocPrint(_allocator, "{}", .{line}) catch unreachable), _allocator))) catch unreachable;
// zbr:selfhost/Checker.zbr:117
            self.module_fn_key_list.append(_allocator, _intern(key)) catch unreachable;
// zbr:selfhost/Checker.zbr:118
            self.fn_edges.put(_intern(key), std.ArrayList([]const u8).empty) catch unreachable;
        }
    }
...
// zbr:selfhost/Checker.zbr:122
            self.root_set.put(_intern(fn_key), true) catch unreachable;
// zbr:selfhost/Checker.zbr:123
            self.root_key_list.append(_allocator, _intern(fn_key)) catch unreachable;
        }
    }
...
// zbr:selfhost/Checker.zbr:136
                edges.append(_allocator, _intern(to_key)) catch unreachable;
            }
        }
    }
...
// zbr:selfhost/Checker.zbr:140
            ...
            { var _split_iter_1 = std.mem.splitSequence(u8, fn_key, "::"); while (_split_iter_1.next()) |_se_1| { parts.append(_allocator, _se_1) catch @panic("OOM"); } }
...
// zbr:selfhost/Checker.zbr:158
                const ui: *UnionInfo = UnionInfo.init(u.name, file, u.span.line);
...
// zbr:selfhost/Checker.zbr:164
                self.union_infos.put(_intern(u.name), ui) catch unreachable;
            },
            .method => |m_ptr| {
                const m = m_ptr.*;
...
const _ttag_DeadCodeChecker: u64 = 171501169;
