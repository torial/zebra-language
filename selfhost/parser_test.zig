...
pub const ParserTest = struct {
    ...
    pub fn moduleDecls(self: *ParserTest, src: []const u8) anyerror!std.ArrayList(PNode) {
        ...
        switch (root) {
            .module_ => |_ptr_m| {
                ...
                { _error_ctx = .{ .message = "expected module_, got something else" }; return error.ZebraError; }
            },
        }
    }
    ...
    pub fn testUseDecl(self: *ParserTest) anyerror!void {
        ...
        if (!((@as(i64, @intCast(decls.items.len)) == 1))) @panic(_str_concat("use: expected 1 decl, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(@as(i64, @intCast(decls.items.len))), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
        switch (decls.items[@intCast(0)]) {
            .use_ => |_ptr_u| {
                const u = _ptr_u.*;
                if (!(std.mem.eql(u8, u.path, "Foo"))) @panic(_str_concat(_str_concat("use: expected path 'Foo', got '", u.path, _allocator), "'", _allocator));
                if (!((@as(i64, @intCast(u.exposed.items.len)) == 0))) @panic("use: expected 0 exposed");
            },
            else => |_| {
                if (!(false)) @panic("use: expected use_ variant");
            },
        }
        std.debug.print("{s}\n", .{"testUseDecl: OK"});
    }
    ...
    pub fn testEmptyClass(self: *ParserTest) anyerror!void {
        ...
        if (!((@as(i64, @intCast(decls.items.len)) == 1))) @panic("empty class: expected 1 decl");
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                const c = _ptr_c.*;
                if (!(std.mem.eql(u8, c.name, "Empty"))) @panic(_str_concat(_str_concat("empty class: name mismatch, got '", c.name, _allocator), "'", _allocator));
                if (!((@as(i64, @intCast(c.members.items.len)) == 0))) @panic("empty class: expected 0 members");
            },
            else => |_| {
                if (!(false)) @panic("empty class: expected class_ variant");
            },
        }
        std.debug.print("{s}\n", .{"testEmptyClass: OK"});
    }
    ...
    pub fn testClassWithField(self: *ParserTest) anyerror!void {
        const src: []const u8 = "class Point\n    var x as int\n    var y as int\n";
        const decls = try self.moduleDecls(src);
        if (!((@as(i64, @intCast(decls.items.len)) == 1))) @panic("field class: expected 1 decl");
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                const c = _ptr_c.*;
                if (!(std.mem.eql(u8, c.name, "Point"))) @panic("field class: name mismatch");
                if (!((@as(i64, @intCast(c.members.items.len)) == 2))) @panic(_str_concat("field class: expected 2 members, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(@as(i64, @intCast(c.members.items.len))), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                switch (c.members.items[@intCast(0)]) {
                    .field_ => |_ptr_f| {
                        const f = _ptr_f.*;
                        if (!(std.mem.eql(u8, f.name, "x"))) @panic("field class: first field name mismatch");
                        if (!(std.mem.eql(u8, f.type_name, "int"))) @panic("field class: first field type mismatch");
                        if (!((!f.is_const))) @panic("field class: var field should not be const");
                    },
                    else => |_| {
                        if (!(false)) @panic("field class: expected field_ for first member");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("field class: expected class_ variant");
            },
        }
    ...
    pub fn testClassWithMethod(self: *ParserTest) anyerror!void {
        const src: []const u8 = "class Calc\n    def double(n as int) as int\n        return n\n";
        const decls = try self.moduleDecls(src);
        if (!((@as(i64, @intCast(decls.items.len)) == 1))) @panic("method class: expected 1 decl");
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                const c = _ptr_c.*;
                if (!(std.mem.eql(u8, c.name, "Calc"))) @panic("method class: name mismatch");
                if (!((@as(i64, @intCast(c.members.items.len)) == 1))) @panic("method class: expected 1 member");
                switch (c.members.items[@intCast(0)]) {
                    .method_ => |_ptr_m| {
                        const m = _ptr_m.*;
                        if (!(std.mem.eql(u8, m.name, "double"))) @panic(_str_concat(_str_concat("method class: method name mismatch, got '", m.name, _allocator), "'", _allocator));
                        if (!((!m.is_shared))) @panic("method class: should not be shared");
                        if (!((!m.throws_))) @panic("method class: should not throw");
                        if (!((@as(i64, @intCast(m.stmts.items.len)) == 1))) @panic("method class: expected 1 stmt");
                    },
                    else => |_| {
                        if (!(false)) @panic("method class: expected method_ for member");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("method class: expected class_ variant");
            },
        }
        ...
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                ...
                switch (c.members.items[@intCast(0)]) {
                    .method_ => |_ptr_m| {
                        const m = _ptr_m.*;
                        if (!((@as(i64, @intCast(m.stmts.items.len)) == 1))) @panic("return: expected 1 stmt");
                        switch (m.stmts.items[@intCast(0)]) {
                            .stmt_return => |_ptr_ret| {
                                const ret = _ptr_ret.*;
                                if (!((@as(i64, @intCast(ret.value.items.len)) == 1))) @panic("return: expected value");
                                switch (ret.value.items[@intCast(0)]) {
                                    .expr_int => |text| {
                                        if (!(std.mem.eql(u8, text, "42"))) @panic(_str_concat(_str_concat("return: expected '42', got '", text, _allocator), "'", _allocator));
                                    },
                                    else => |_| {
                                        if (!(false)) @panic("return: expected expr_int");
                                    },
                                }
                            },
                            else => |_| {
                                if (!(false)) @panic("return: expected stmt_return");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("return: expected method_");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("return: expected class_");
            },
        }
        std.debug.print("{s}\n", .{"testReturnStmt: OK"});
    }
        ...
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                ...
                switch (c.members.items[@intCast(0)]) {
                    .method_ => |_ptr_m| {
                        const m = _ptr_m.*;
                        if (!((@as(i64, @intCast(m.stmts.items.len)) == 1))) @panic("if: expected 1 stmt");
                        switch (m.stmts.items[@intCast(0)]) {
                            .stmt_if => |_ptr_if_node| {
                                const if_node = _ptr_if_node.*;
                                if (!((if_node.cond.len == 1))) @panic("if: expected cond");
                                if (!((if_node.then_stmts.len == 1))) @panic("if: expected 1 then stmt");
                                if (!((@as(i64, @intCast(if_node.else_stmts.items.len)) == 0))) @panic("if: expected 0 else stmts");
                            },
                            else => |_| {
                                if (!(false)) @panic("if: expected stmt_if");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("if: expected method_");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("if: expected class_");
            },
        }
        std.debug.print("{s}\n", .{"testIfStmt: OK"});
    }
                        ...
                        switch (m.stmts.items[@intCast(0)]) {
                            .stmt_return => |_ptr_ret| {
                                ...
                                switch (ret.value.items[@intCast(0)]) {
                                    .expr_binary => |_ptr_b| {
                                        const b = _ptr_b.*;
                                        if (!(std.mem.eql(u8, b.op, "+"))) @panic(_str_concat(_str_concat("arith: expected '+', got '", b.op, _allocator), "'", _allocator));
                                        if (!((b.left.len == 1))) @panic("arith: left missing");
                                        if (!((b.right.len == 1))) @panic("arith: right missing");
                                        switch (b.left.items[@intCast(0)]) {
                                            .expr_int => |t| {
                                                if (!(std.mem.eql(u8, t, "1"))) @panic("arith: left should be '1'");
                                            },
                                            else => |_| {
                                                if (!(false)) @panic("arith: left should be expr_int");
                                            },
                                        }
                                        switch (b.right.items[@intCast(0)]) {
                                            .expr_int => |t| {
                                                if (!(std.mem.eql(u8, t, "2"))) @panic("arith: right should be '2'");
                                            },
                                            else => |_| {
                                                if (!(false)) @panic("arith: right should be expr_int");
                                            },
                                        }
                                    },
                                    else => |_| {
                                        if (!(false)) @panic("arith: expected expr_binary");
                                    },
                                }
                            },
                            else => |_| {
                                if (!(false)) @panic("arith: expected stmt_return");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("arith: expected method_");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("arith: expected class_");
            },
        }
        std.debug.print("{s}\n", .{"testExprArith: OK"});
    }
                        ...
                        switch (m.stmts.items[@intCast(0)]) {
                            .stmt_return => |_ptr_ret| {
                                ...
                                switch (ret.value.items[@intCast(0)]) {
                                    .expr_binary => |_ptr_b| {
                                        const b = _ptr_b.*;
                                        if (!(std.mem.eql(u8, b.op, "=="))) @panic(_str_concat(_str_concat("cmp: expected '==', got '", b.op, _allocator), "'", _allocator));
                                    },
                                    else => |_| {
                                        if (!(false)) @panic("cmp: expected expr_binary");
                                    },
                                }
                            },
                            else => |_| {
                                if (!(false)) @panic("cmp: expected stmt_return");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("cmp: expected method_");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("cmp: expected class_");
            },
        }
        ...
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                ...
                switch (c.members.items[@intCast(0)]) {
                    .method_ => |_ptr_m| {
                        const m = _ptr_m.*;
                        if (!((@as(i64, @intCast(m.stmts.items.len)) == 1))) @panic("call: expected 1 stmt");
                        switch (m.stmts.items[@intCast(0)]) {
                            .stmt_print => {
                                // pass
                            },
                            else => {
                                if (!(false)) @panic("call: expected stmt_print");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("call: expected method_");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("call: expected class_");
            },
        }
        std.debug.print("{s}\n", .{"testMethodCall: OK"});
    }
        ...
        switch (decls.items[@intCast(0)]) {
            .class_ => |_ptr_c| {
                ...
                switch (c.members.items[@intCast(0)]) {
                    .method_ => |_ptr_m| {
                        const m = _ptr_m.*;
                        if (!((@as(i64, @intCast(m.stmts.items.len)) == 1))) @panic("var: expected 1 stmt");
                        switch (m.stmts.items[@intCast(0)]) {
                            .stmt_var => |_ptr_v| {
                                const v = _ptr_v.*;
                                if (!(std.mem.eql(u8, v.name, "count"))) @panic(_str_concat(_str_concat("var: name mismatch, got '", v.name, _allocator), "'", _allocator));
                                if (!(std.mem.eql(u8, v.type_name, "int"))) @panic(_str_concat(_str_concat("var: type mismatch, got '", v.type_name, _allocator), "'", _allocator));
                                if (!((!v.is_const))) @panic("var: should not be const");
                                if (!((@as(i64, @intCast(v.init_expr.items.len)) == 1))) @panic("var: expected init expr");
                                switch (v.init_expr.items[@intCast(0)]) {
                                    .expr_int => |t| {
                                        if (!(std.mem.eql(u8, t, "0"))) @panic("var: init should be '0'");
                                    },
                                    else => |_| {
                                        if (!(false)) @panic("var: init should be expr_int");
                                    },
                                }
                            },
                            else => |_| {
                                if (!(false)) @panic("var: expected stmt_var");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("var: expected method_");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("var: expected class_");
            },
        }
        std.debug.print("{s}\n", .{"testVarStmt: OK"});
    }
...
const _ttag_ParserTest: u64 = _zbr_hash("ParserTest");
    ...
    pub fn main() void {
        {
            ...
            _try_blk: {
                ...
                t.run() catch |_tc_e| { _try_err = _tc_e; break :_try_blk; };
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
