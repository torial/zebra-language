...
pub const UnionInfo = struct {
    ...
    variant_lines: std.ArrayList(i64),
    pub fn init(name: []const u8, file: []const u8, line: i64) *UnionInfo {
        const _self = _allocator.create(UnionInfo) catch @panic("OOM");
...
// zbr:selfhost/checker.zbr:32
    ...
    pub fn addVariant(self: *UnionInfo, v: []const u8, ln: i64) void {
        self.variants.append(_allocator, _intern(v)) catch @panic("OOM");
        self.variant_lines.append(_allocator, ln) catch @panic("OOM");
    }

    pub fn hasVariant(self: *UnionInfo, v: []const u8) bool {
...
// zbr:selfhost/checker.zbr:41
            if (std.mem.eql(u8, self.variants.items[@intCast(i)], v)) {
...
// zbr:selfhost/checker.zbr:44
    ...
    pub fn lineOf(self: *UnionInfo, v: []const u8) i64 {
...
// zbr:selfhost/checker.zbr:50
                return self.variant_lines.items[@intCast(i)];
            }
...
pub const DeadCodeChecker = struct {
    _type_tag: u64 = _ttag_DeadCodeChecker,
    union_infos: std.StringHashMap(*UnionInfo),
    match_sites: std.StringHashMap(std.ArrayList([]const u8)),
    match_key_list: std.ArrayList([]const u8),
    constructed_set: std.StringHashMap(bool),
    constructed_key_list: std.ArrayList([]const u8),
    module_fns: std.StringHashMap([]const u8),
    module_fn_key_list: std.ArrayList([]const u8),
    fn_edges: std.StringHashMap(std.ArrayList([]const u8)),
    root_set: std.StringHashMap(bool),
    root_key_list: std.ArrayList([]const u8),
    reachable_set: std.StringHashMap(bool),
    exposed_names: std.StringHashMap(bool),
    current_file: []const u8,
    current_fn_key: []const u8,
    pub fn init() *DeadCodeChecker {
        const _self = _allocator.create(DeadCodeChecker) catch @panic("OOM");
        _self._type_tag = _zbr_hash("DeadCodeChecker");
// zbr:selfhost/checker.zbr:78
            _self.union_infos = std.StringHashMap(*UnionInfo).init(_allocator);
...
// zbr:selfhost/checker.zbr:89
            _self.exposed_names = std.StringHashMap(bool).init(_allocator);
...
// zbr:selfhost/checker.zbr:91
    ...
    pub fn isKnownUnion(self: *DeadCodeChecker, name: []const u8) bool {
// zbr:selfhost/checker.zbr:94
        return self.union_infos.contains(name);
    }

    pub fn recordMatch(self: *DeadCodeChecker, union_name: []const u8, variant: []const u8, file: []const u8, line: i64) void {
...
// zbr:selfhost/checker.zbr:101
        var sites: std.ArrayList([]const u8) = (self.match_sites.get(key) orelse undefined);
        sites.append(_allocator, _intern(_str_concat(_str_concat(file, ":", _allocator), (std.fmt.allocPrint(_allocator, "{}", .{line}) catch unreachable), _allocator))) catch @panic("OOM");
    }

    pub fn recordConstruction(self: *DeadCodeChecker, union_name: []const u8, variant: []const u8) void {
...
// zbr:selfhost/checker.zbr:106
        if ((!self.constructed_set.contains(key))) {
            ...
            self.constructed_key_list.append(_allocator, _intern(key)) catch @panic("OOM");
        }
    }
...
// zbr:selfhost/checker.zbr:111
    ...
    pub fn registerModuleFn(self: *DeadCodeChecker, file: []const u8, fn_name: []const u8, line: i64) void {
...
// zbr:selfhost/checker.zbr:115
    ...
    pub fn markAsRoot(self: *DeadCodeChecker, fn_key: []const u8) void {
// zbr:selfhost/checker.zbr:121
        if ((!self.root_set.contains(fn_key))) {
            self.root_set.put(_intern(fn_key), true) catch @panic("OOM");
            self.root_key_list.append(_allocator, _intern(fn_key)) catch @panic("OOM");
        }
    }

    pub fn trackFnCall(self: *DeadCodeChecker, callee_name: []const u8) void {
...
// zbr:selfhost/checker.zbr:135
                var edges: std.ArrayList([]const u8) = (self.fn_edges.get(self.current_fn_key) orelse undefined);
                edges.append(_allocator, _intern(to_key)) catch @panic("OOM");
            }
        }
    }

    pub fn extractFnName(self: *DeadCodeChecker, fn_key: []const u8) []const u8 {
        _ = self;
...
// zbr:selfhost/checker.zbr:142
                return parts.items[@intCast((@as(i64, @intCast(parts.items.len)) - 1))];
            }
        }
// zbr:selfhost/checker.zbr:143
    ...
    pub fn collectDecls(self: *DeadCodeChecker, module: Module) void {
...
// zbr:selfhost/checker.zbr:151
        while (_zebra_lt(i, @as(i64, @intCast(module.decls.items.len)))) {
            self.collectDecl(module.decls.items[@intCast(i)], file);
// zbr:selfhost/checker.zbr:153
            i = (i + 1);
        }
    }

    pub fn collectDecl(self: *DeadCodeChecker, decl: Decl, file: []const u8) void {
// zbr:selfhost/checker.zbr:156
        switch (decl) {
            .union_ => |_ptr_u| {
                const u = _ptr_u.*;
// zbr:selfhost/checker.zbr:158
                var ui: *UnionInfo = UnionInfo.init(u.name, file, u.span.line);
...
// zbr:selfhost/checker.zbr:161
                    const vt: UnionVariant = u.variants.items[@intCast(vi)];
...
// zbr:selfhost/checker.zbr:173
                    const ename: []const u8 = u.exposed.items[@intCast(ei)];
...
// zbr:selfhost/checker.zbr:178
                while (_zebra_lt(ni, @as(i64, @intCast(ns.decls.items.len)))) {
                    self.collectDecl(ns.decls.items[@intCast(ni)], file);
// zbr:selfhost/checker.zbr:180
    ...
    pub fn walkModule(self: *DeadCodeChecker, module: Module) void {
...
// zbr:selfhost/checker.zbr:190
        while (_zebra_lt(i, @as(i64, @intCast(module.decls.items.len)))) {
            self.walkTopDecl(module.decls.items[@intCast(i)], file);
// zbr:selfhost/checker.zbr:192
            i = (i + 1);
        }
    }

    pub fn walkTopDecl(self: *DeadCodeChecker, decl: Decl, file: []const u8) void {
...
// zbr:selfhost/checker.zbr:215
                while (_zebra_lt(ni, @as(i64, @intCast(ns.decls.items.len)))) {
                    self.walkTopDecl(ns.decls.items[@intCast(ni)], file);
// zbr:selfhost/checker.zbr:217
    ...
    pub fn walkMemberDecls(self: *DeadCodeChecker, members: std.ArrayList(Decl), file: []const u8) void {
...
// zbr:selfhost/checker.zbr:226
            switch (members.items[@intCast(i)]) {
                .method => |_ptr_m| {
                    const m = _ptr_m.*;
...
// zbr:selfhost/checker.zbr:239
    ...
    pub fn walkStmts(self: *DeadCodeChecker, stmts: std.ArrayList(Stmt), file: []const u8) void {
// zbr:selfhost/checker.zbr:242
        var i: i64 = 0;
// zbr:selfhost/checker.zbr:243
        while (_zebra_lt(i, @as(i64, @intCast(stmts.items.len)))) {
            self.walkStmt(stmts.items[@intCast(i)], file);
// zbr:selfhost/checker.zbr:245
            i = (i + 1);
        }
    }

    pub fn walkStmt(self: *DeadCodeChecker, stmt: Stmt, file: []const u8) void {
...
// zbr:selfhost/checker.zbr:254
                    const ef: ElseIf = si.else_ifs.items[@intCast(ei)];
...
// zbr:selfhost/checker.zbr:282
                    const bc: BranchOn = sb.cases.items[@intCast(ci)];
...
// zbr:selfhost/checker.zbr:313
                    const cl: CatchClause = stc.clauses.items[@intCast(cli)];
...
// zbr:selfhost/checker.zbr:331
                while (_zebra_lt(pi, @as(i64, @intCast(sp.args.items.len)))) {
                    self.walkExpr(sp.args.items[@intCast(pi)], file, false);
...
// zbr:selfhost/checker.zbr:350
                while (_zebra_lt(cti, @as(i64, @intCast(sct.exprs.items.len)))) {
                    self.walkExpr(sct.exprs.items[@intCast(cti)], file, false);
// zbr:selfhost/checker.zbr:352
                    cti = (cti + 1);
                }
            },
            else => |_| {
    ...
    pub fn walkBranchValues(self: *DeadCodeChecker, values: std.ArrayList(Expr), file: []const u8) void {
// zbr:selfhost/checker.zbr:357
        var vi: i64 = 0;
// zbr:selfhost/checker.zbr:358
        while (_zebra_lt(vi, @as(i64, @intCast(values.items.len)))) {
            self.walkExpr(values.items[@intCast(vi)], file, true);
// zbr:selfhost/checker.zbr:360
            vi = (vi + 1);
        }
    }

    pub fn walkCapture(self: *DeadCodeChecker, cap: DeclVar, file: []const u8) void {
// zbr:selfhost/checker.zbr:365
    ...
    pub fn walkDictEntry(self: *DeadCodeChecker, entry: DictEntry, file: []const u8) void {
        self.walkExpr(entry.key.*, file, false);
        self.walkExpr(entry.value.*, file, false);
    }

    pub fn walkExpr(self: *DeadCodeChecker, expr: Expr, file: []const u8, in_match: bool) void {
...
// zbr:selfhost/checker.zbr:412
                    const aarg: Arg = ec.args.items[@intCast(ai)];
...
// zbr:selfhost/checker.zbr:483
                while (_zebra_lt(ci, @as(i64, @intCast(el.captures.items.len)))) {
                    self.walkCapture(el.captures.items[@intCast(ci)], file);
...
// zbr:selfhost/checker.zbr:493
                while (_zebra_lt(eli, @as(i64, @intCast(ell.elems.items.len)))) {
                    self.walkExpr(ell.elems.items[@intCast(eli)], file, false);
...
// zbr:selfhost/checker.zbr:498
                while (_zebra_lt(dli, @as(i64, @intCast(edl.entries.items.len)))) {
                    self.walkDictEntry(edl.entries.items[@intCast(dli)], file);
...
// zbr:selfhost/checker.zbr:503
                while (_zebra_lt(ali, @as(i64, @intCast(eal.elems.items.len)))) {
                    self.walkExpr(eal.elems.items[@intCast(ali)], file, false);
...
// zbr:selfhost/checker.zbr:509
                    const part: StringPart = esi.parts.items[@intCast(spi)];
...
// zbr:selfhost/checker.zbr:515
                while (_zebra_lt(ti, @as(i64, @intCast(etup.elems.items.len)))) {
                    self.walkExpr(etup.elems.items[@intCast(ti)], file, false);
...
// zbr:selfhost/checker.zbr:522
                while (_zebra_lt(cci, @as(i64, @intCast(ecc.operands.items.len)))) {
                    self.walkExpr(ecc.operands.items[@intCast(cci)], file, false);
...
// zbr:selfhost/checker.zbr:531
                    const exf: ExceptField = eex.fields.items[@intCast(exi)];
...
// zbr:selfhost/checker.zbr:561
        while (_zebra_lt(ri, @as(i64, @intCast(self.root_key_list.items.len)))) {
            pending.append(_allocator, _intern(self.root_key_list.items[@intCast(ri)])) catch @panic("OOM");
...
// zbr:selfhost/checker.zbr:567
            const mfk: []const u8 = self.module_fn_key_list.items[@intCast(mi)];
...
// zbr:selfhost/checker.zbr:569
            if (self.exposed_names.contains(fn_name)) {
                pending.append(_allocator, _intern(mfk)) catch @panic("OOM");
            }
...
// zbr:selfhost/checker.zbr:575
            const key: []const u8 = pending.items[@intCast(qi)];
...
// zbr:selfhost/checker.zbr:582
                        const target: []const u8 = edges.items[@intCast(ei)];
// zbr:selfhost/checker.zbr:583
                        if ((!self.reachable_set.contains(target))) {
                            pending.append(_allocator, _intern(target)) catch @panic("OOM");
                        }
...
// zbr:selfhost/checker.zbr:593
            if ((!self.constructed_set.contains(self.match_key_list.items[@intCast(i)]))) {
...
// zbr:selfhost/checker.zbr:598
            if ((!self.match_sites.contains(self.constructed_key_list.items[@intCast(i)]))) {
...
// zbr:selfhost/checker.zbr:603
            if ((!self.reachable_set.contains(self.module_fn_key_list.items[@intCast(i)]))) {
...
// zbr:selfhost/checker.zbr:621
            const key: []const u8 = self.match_key_list.items[@intCast(i)];
...
// zbr:selfhost/checker.zbr:626
                const vname: []const u8 = kparts.items[@intCast(1)];
...
// zbr:selfhost/checker.zbr:630
                while (_zebra_lt(si, @as(i64, @intCast(sites.items.len)))) {
                    dead_sb.appendSlice(_allocator, _str_concat(_str_concat(_str_concat(_str_concat("    ", CYN, _allocator), sites.items[@intCast(si)], _allocator), RST, _allocator), "\n", _allocator)) catch @panic("OOM");
...
// zbr:selfhost/checker.zbr:641
            const key: []const u8 = self.constructed_key_list.items[@intCast(i)];
...
// zbr:selfhost/checker.zbr:655
            const key: []const u8 = self.module_fn_key_list.items[@intCast(i)];
...
// zbr:selfhost/checker.zbr:664
        if (_zebra_gt(dead_count, 0)) {
            ...
            sb.appendSlice(_allocator, (dead_sb.toOwnedSlice(_allocator) catch "")) catch @panic("OOM");
        }
// zbr:selfhost/checker.zbr:668
        if (_zebra_gt(phantom_count, 0)) {
            ...
            sb.appendSlice(_allocator, (phantom_sb.toOwnedSlice(_allocator) catch "")) catch @panic("OOM");
        }
// zbr:selfhost/checker.zbr:672
        if (_zebra_gt(unreachable_count, 0)) {
            ...
            sb.appendSlice(_allocator, (unreach_sb.toOwnedSlice(_allocator) catch "")) catch @panic("OOM");
...
// zbr:selfhost/checker.zbr:706
        out = _str_concat(_str_concat(out, "/", _allocator), parts.items[@intCast(i)], _allocator);
...
// zbr:selfhost/checker.zbr:713
        if (std.mem.eql(u8, visited.items[@intCast(vi)], path)) {
...
// zbr:selfhost/checker.zbr:715
    ...
    visited.append(_allocator, _intern(path)) catch @panic("OOM");
...
// zbr:selfhost/checker.zbr:731
            if (pm.decls.items[@intCast(di)] == .use_) {
                const u_ptr = pm.decls.items[@intCast(di)].use_;
                const u = u_ptr.*;
...
// zbr:selfhost/checker.zbr:740
        ...
        modules.append(_allocator, module) catch @panic("OOM");
    }
}
...
// zbr:selfhost/checker.zbr:762
    while (_zebra_lt(mi, @as(i64, @intCast(modules.items.len)))) {
        checker.walkModule(modules.items[@intCast(mi)]);
