...
fn _code_editor_render(_ed: *_CodeEditor, _g: GuiContext, id: []const u8, w: f64, h: f64) void {
    _ = _g; _ = w; _ = h;
    _ = _tui_input(id, _ed.text);      // display only: the tui has no editable text control
}
...
// No Scintilla here: the hatch answers 0 for every message (a caller that reads
// back a value it set will see the mismatch, which is the intended tell).
fn _code_editor_sci(_ed: *_CodeEditor, msg: i64, wparam: i64, lparam: i64) i64 { _ = _ed; _ = msg; _ = wparam; _ = lparam; return 0; }
fn _code_editor_sci_str(_ed: *_CodeEditor, msg: i64, wparam: i64, text: []const u8) i64 { _ = _ed; _ = msg; _ = wparam; _ = text; return 0; }
// ─── ZigZag TUI backend ──────────────────────────────────────────────────────
...
fn _tui_password(label: []const u8, text: []const u8, cap: *const anyopaque, cap_len: usize, thunk: *const fn (*const anyopaque, []const u8, *const fn (*anyopaque, *const anyopaque, usize) void, *anyopaque) void, send_fn: *const fn (*anyopaque, *const anyopaque, usize) void, send_ptr: *anyopaque) void {
    _ = cap_len;
    var _mask: [256]u8 = undefined;
    const _n = @min(text.len, _mask.len);
    @memset(_mask[0.._n], '*');
    const now = _tui_input(label, _mask[0.._n]);
    _ = now;
    // no editing in the tui: a masked field never fires
    _ = thunk; _ = send_fn; _ = send_ptr; _ = cap;
}
fn _tui_search(label: []const u8, text: []const u8, cap: *const anyopaque, cap_len: usize, thunk: *const fn (*const anyopaque, []const u8, *const fn (*anyopaque, *const anyopaque, usize) void, *anyopaque) void, send_fn: *const fn (*anyopaque, *const anyopaque, usize) void, send_ptr: *anyopaque) void {
    _ = cap_len;
    const now = _tui_input(label, text);
    if (!std.mem.eql(u8, now, text)) thunk(cap, now, send_fn, send_ptr);
}
...
fn _tui_slider(label: []const u8, value: f64, min: f64, max: f64, cap: *const anyopaque, cap_len: usize, thunk: *const fn (*const anyopaque, f64, *const fn (*anyopaque, *const anyopaque, usize) void, *anyopaque) void, send_fn: *const fn (*anyopaque, *const anyopaque, usize) void, send_ptr: *anyopaque) void {
    // display only: the tui cannot drag, so the thunk never fires here
    _ = min; _ = max; _ = cap; _ = cap_len; _ = thunk; _ = send_fn; _ = send_ptr;
    if (_tui_terminal) |*_t| {
        const _col = _tui_indent_level * 2;
        var _buf: [256]u8 = undefined;
...
fn _tui_input_multiline(label: []const u8, text: []const u8, cap: *const anyopaque, cap_len: usize, thunk: *const fn (*const anyopaque, []const u8, *const fn (*anyopaque, *const anyopaque, usize) void, *anyopaque) void, send_fn: *const fn (*anyopaque, *const anyopaque, usize) void, send_ptr: *anyopaque) void {
    _ = cap_len;
    const now = _tui_input(label, text);
    if (!std.mem.eql(u8, now, text)) thunk(cap, now, send_fn, send_ptr);
}
...
fn _tui_begin_tabs(id: []const u8, stretch: bool) void { _ = id; _ = stretch; }
...
fn _tui_combobox(_l: []const u8, _items: []const []const u8, _sel: i64, _cap: *const anyopaque, _cap_len: usize, _thunk: *const fn (*const anyopaque, i64, *const fn (*anyopaque, *const anyopaque, usize) void, *anyopaque) void, _send_fn: *const fn (*anyopaque, *const anyopaque, usize) void, _send_ptr: *anyopaque) void {
    _ = _cap; _ = _cap_len; _ = _thunk; _ = _send_fn; _ = _send_ptr;
    if (_sel >= 0 and _sel < @as(i64, @intCast(_items.len))) { var _b: [256]u8 = undefined; _tui_text(std.fmt.bufPrint(&_b, "{s}: {s}", .{ _l, _items[@intCast(_sel)] }) catch _l); } else _tui_text(_l);
}
fn _tui_radio(_l: []const u8, _items: []const []const u8, _sel: i64, _cap: *const anyopaque, _cap_len: usize, _thunk: *const fn (*const anyopaque, i64, *const fn (*anyopaque, *const anyopaque, usize) void, *anyopaque) void, _send_fn: *const fn (*anyopaque, *const anyopaque, usize) void, _send_ptr: *anyopaque) void {
    _ = _cap; _ = _cap_len; _ = _thunk; _ = _send_fn; _ = _send_ptr;
    _tui_text(_l);
...
const _gui_tui_backend = _GuiBackend{
    ...
    .fieldFn        = _tui_field,
    .passwordFn     = _tui_password,
    .searchFn       = _tui_search,
