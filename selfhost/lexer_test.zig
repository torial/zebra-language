    ...
    pub fn testKeyword() void {
        {
            ...
            _try_blk: {
                const toks = Lexer.tokenize("class");
                if (!((toks.len == 3))) @panic(_str_concat("keyword: expected 3 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                std.debug.print("{s}\n", .{"testKeyword: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testKeyword FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testIdentifier() void {
        {
            var _try_err: ?anyerror = null;
            _try_blk: {
                const toks = Lexer.tokenize("fooBar");
                if (!((toks.len == 3))) @panic("identifier: expected 3 tokens");
                if (!(std.mem.eql(u8, toks.items[@intCast(0)].text, "fooBar"))) @panic("identifier: text mismatch");
                std.debug.print("{s}\n", .{"testIdentifier: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testIdentifier FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testClass() void {
        {
            ...
            _try_blk: {
                ...
                if (!((toks.len >= 10))) @panic(_str_concat("class: expected >= 10 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                if (!(std.mem.eql(u8, toks.items[@intCast(0)].text, "class"))) @panic("class: first token should be 'class'");
                if (!(std.mem.eql(u8, toks.items[@intCast(1)].text, "Foo"))) @panic("class: second should be 'Foo'");
                std.debug.print("{s}\n", .{_str_concat(_str_concat("testClass: OK (", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator), " tokens)", _allocator)});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testClass FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testIndentation() void {
        {
            ...
            _try_blk: {
                ...
                const toks = Lexer.tokenize(src);
                if (!((toks.len >= 12))) @panic(_str_concat("indentation: expected >= 12 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                std.debug.print("{s}\n", .{_str_concat(_str_concat("testIndentation: OK (", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator), " tokens)", _allocator)});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testIndentation FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testLineComment() void {
        {
            ...
            _try_blk: {
                ...
                const toks = Lexer.tokenize(src);
                if (!((toks.len == 5))) @panic(_str_concat("comment: expected 5 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                if (!(std.mem.eql(u8, toks.items[@intCast(0)].text, "x"))) @panic("comment: first token should be 'x'");
                if (!(std.mem.eql(u8, toks.items[@intCast(2)].text, "y"))) @panic("comment: third should be 'y'");
                std.debug.print("{s}\n", .{"testLineComment: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testLineComment FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testOperators() void {
        {
            ...
            _try_blk: {
                ...
                const toks = Lexer.tokenize(src);
                if (!((toks.len == 7))) @panic(_str_concat("operators: expected 7 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                if (!(std.mem.eql(u8, toks.items[@intCast(1)].text, "+"))) @panic("operators: token 1 should be '+'");
                if (!(std.mem.eql(u8, toks.items[@intCast(3)].text, "*"))) @panic("operators: token 3 should be '*'");
                std.debug.print("{s}\n", .{"testOperators: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testOperators FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testIntLiteral() void {
        {
            ...
            _try_blk: {
                ...
                const toks = Lexer.tokenize(src);
                if (!((toks.len == 3))) @panic(_str_concat("int: expected 3 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                if (!(std.mem.eql(u8, toks.items[@intCast(0)].text, "42"))) @panic("int: text mismatch");
                std.debug.print("{s}\n", .{"testIntLiteral: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testIntLiteral FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testOpenCall() void {
        {
            ...
            _try_blk: {
                ...
                const toks = Lexer.tokenize(src);
                if (!((toks.len == 4))) @panic(_str_concat("open_call: expected 4 tokens, got ", (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(toks.len), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); }), _allocator));
                if (!(std.mem.eql(u8, toks.items[@intCast(0)].text, "foo"))) @panic("open_call: text should be 'foo'");
                std.debug.print("{s}\n", .{"testOpenCall: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testOpenCall FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testSourcePositions() void {
        {
            ...
            _try_blk: {
                ...
                const toks = Lexer.tokenize(src);
                if (!((toks.items[@intCast(0)].line == 1))) @panic("position: first token on line 1");
                if (!((toks.items[@intCast(0)].col == 1))) @panic("position: first token at col 1");
                if (!((toks.items[@intCast(2)].line == 2))) @panic("position: 'y' on line 2");
                std.debug.print("{s}\n", .{"testSourcePositions: OK"});
                break :_try_blk;
            }
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testSourcePositions FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testTokenKinds() void {
        {
            var _try_err: ?anyerror = null;
            _try_blk: {
                const toks = Lexer.tokenize("abc 123");
                const idKind = toks.items[@intCast(0)].kind;
                const intKind = toks.items[@intCast(1)].kind;
                switch (idKind) {
                    .id => {
            ...
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testTokenKinds FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
    ...
    pub fn testExposing() void {
        {
            var _try_err: ?anyerror = null;
            _try_blk: {
                const toks = Lexer.tokenize("use Foo exposing Bar");
                if (!(std.mem.eql(u8, toks.items[@intCast(2)].text, "exposing"))) @panic("exposing: token text mismatch");
                switch (toks.items[@intCast(2)].kind) {
                    .kw_exposing => {
                        std.debug.print("{s}\n", .{"testExposing: OK"});
                    },
                    else => {
            ...
            if (_try_err != null) {
                std.debug.print("{s}\n", .{_str_concat("testExposing FAILED: ", _zbr_error_msg(), _allocator)});
            }
        }
    }
...
const _ttag_LexerTest: u64 = _zbr_hash("LexerTest");
