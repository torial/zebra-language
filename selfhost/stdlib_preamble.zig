...
pub var _io: std.Io = undefined;
pub var _args: std.process.Args = undefined;
...
pub const _TsAlloc = struct {
    child: std.mem.Allocator,
    mutex: std.Io.Mutex = .init,
    pub const _vt: std.mem.Allocator.VTable = .{ .alloc = _a, .resize = _r, .remap = _m, .free = _f };
    pub fn allocator(self: *_TsAlloc) std.mem.Allocator {
        return .{ .ptr = self, .vtable = &_vt };
    }
    pub fn _a(ctx: *anyopaque, len: usize, al: std.mem.Alignment, ra: usize) ?[*]u8 {
    ...
    pub fn _r(ctx: *anyopaque, mem: []u8, al: std.mem.Alignment, new_len: usize, ra: usize) bool {
        const self: *_TsAlloc = @ptrCast(@alignCast(ctx));
        self.mutex.lockUncancelable(_io);
        defer self.mutex.unlock(_io);
        return self.child.vtable.resize(self.child.ptr, mem, al, new_len, ra);
    }
    pub fn _m(ctx: *anyopaque, mem: []u8, al: std.mem.Alignment, new_len: usize, ra: usize) ?[*]u8 {
        const self: *_TsAlloc = @ptrCast(@alignCast(ctx));
        self.mutex.lockUncancelable(_io);
        defer self.mutex.unlock(_io);
        return self.child.vtable.remap(self.child.ptr, mem, al, new_len, ra);
    }
    pub fn _f(ctx: *anyopaque, mem: []u8, al: std.mem.Alignment, ra: usize) void {
        const self: *_TsAlloc = @ptrCast(@alignCast(ctx));
        self.mutex.lockUncancelable(_io);
        defer self.mutex.unlock(_io);
...
pub var _arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
pub var _ts_alloc: _TsAlloc = .{ .child = _arena.allocator() };
...
pub fn _prog_alloc() std.mem.Allocator {
    return if (@import("builtin").single_threaded) _arena.allocator() else _ts_alloc.allocator();
}
pub var _allocator: std.mem.Allocator = _prog_alloc();
...
pub const _GuiVec2 = struct { f64, f64 };
pub const _GuiBackend = struct {
...
pub const GuiContext = struct {
    ...
    pub fn password(self: GuiContext, label: []const u8, initial: []const u8, on: anytype) void { _ = on; self._b.passwordFn(label, initial); }
    pub fn search(self: GuiContext, label: []const u8, initial: []const u8, on: anytype) void { _ = on; self._b.searchFn(label, initial); }
...
pub fn _code_editor_get_cursor_line(_ed: *_CodeEditor) i64 { _ = _ed; return 1; }
pub fn _code_editor_get_cursor_col(_ed: *_CodeEditor) i64 { _ = _ed; return 1; }
pub fn _code_editor_set_cursor_position(_ed: *_CodeEditor, line: i64, col: i64) void { _ = _ed; _ = line; _ = col; }
// ─── Stub backend (single frame, prints to stderr) ───────────────────────────
pub fn _stub_init(title: []const u8, width: i64, height: i64) anyerror!void {
...
pub fn _stub_text(s: []const u8) void { std.debug.print("[gui] text: {s}\n", .{s}); }
pub fn _stub_separator() void { std.debug.print("[gui] ---\n", .{}); }
pub fn _stub_same_line() void { std.debug.print("[gui] sameLine\n", .{}); }
...
pub fn _stub_checkbox(label: []const u8, value: bool) bool {
    std.debug.print("[gui] checkbox: {s} = {}\n", .{ label, value });
    return value;
}
pub fn _stub_slider(label: []const u8, value: f64, min: f64, max: f64) f64 {
    std.debug.print("[gui] slider: {s} = {d} [{d}, {d}]\n", .{ label, value, min, max });
    return value;
}
pub fn _stub_input(label: []const u8, value: []const u8) []const u8 {
    std.debug.print("[gui] input: {s} = {s}\n", .{ label, value });
    return value;
}
pub fn _stub_input_multiline(label: []const u8, value: []const u8, width: f64, height: f64) []const u8 {
...
pub fn _stub_set_vec2(name: []const u8, x: f32, y: f32) void { _ = name; _ = x; _ = y; }
pub fn _stub_scale_all_sizes(scale: f32) void { _ = scale; }
pub fn _stub_get_dpi() f32 { return 1.0; }
pub fn _stub_ll_add_line(x1: f64, y1: f64, x2: f64, y2: f64, col: i64, thickness: f64) void { _ = x1; _ = y1; _ = x2; _ = y2; _ = col; _ = thickness; }
pub fn _stub_ll_add_rect(x1: f64, y1: f64, x2: f64, y2: f64, col: i64, thickness: f64) void { _ = x1; _ = y1; _ = x2; _ = y2; _ = col; _ = thickness; }
pub fn _stub_ll_add_rect_filled(x1: f64, y1: f64, x2: f64, y2: f64, col: i64) void { _ = x1; _ = y1; _ = x2; _ = y2; _ = col; }
pub fn _stub_ll_add_circle(cx: f64, cy: f64, r: f64, col: i64, thickness: f64) void { _ = cx; _ = cy; _ = r; _ = col; _ = thickness; }
pub fn _stub_ll_add_circle_filled(cx: f64, cy: f64, r: f64, col: i64) void { _ = cx; _ = cy; _ = r; _ = col; }
pub fn _stub_ll_add_text(x: f64, y: f64, col: i64, text: []const u8) void { _ = x; _ = y; _ = col; _ = text; }
pub fn _stub_ll_get_window_pos() _GuiVec2 { return .{ 0, 0 }; }
pub fn _stub_ll_get_window_size() _GuiVec2 { return .{ 800, 600 }; }
pub fn _stub_ll_get_cursor_pos() _GuiVec2 { return .{ 0, 0 }; }
pub fn _stub_ll_get_mouse_pos() _GuiVec2 { return .{ 0, 0 }; }
pub fn _stub_ll_begin_group() void {}
pub fn _stub_ll_end_group() void {}
pub fn _stub_begin_hbox(id: []const u8, stretch: bool) void { _ = id; _ = stretch; }
pub fn _stub_end_hbox() void {}
pub fn _stub_begin_vbox(id: []const u8, stretch: bool) void { _ = id; _ = stretch; }
pub fn _stub_end_vbox() void {}
pub fn _stub_progressbar(_l: []const u8, _v: f64) void { std.debug.print("[gui] progressBar: {s} {d:.1}%\n", .{_l, _v * 100.0}); }
...
pub fn _stub_save_file() ?[]const u8 { std.debug.print("[gui] saveFile (no window)\n", .{}); return null; }
pub fn _stub_open_folder() ?[]const u8 { std.debug.print("[gui] openFolder (no window)\n", .{}); return null; }
pub fn _stub_msg_box(_t: []const u8, _m: []const u8) void { std.debug.print("[gui] msgBox: {s}: {s}\n", .{_t, _m}); }
pub fn _stub_msg_box_error(_t: []const u8, _m: []const u8) void { std.debug.print("[gui] msgBoxError: {s}: {s}\n", .{_t, _m}); }
pub const _gui_stub_backend = _GuiBackend{
    .initFn             = _stub_init,
...
pub const _gui_active_backend: _GuiBackend = _gui_stub_backend;
