...
// zbr:selfhost/typechecker_test.zbr:462
    return Decl{ .method = _box_22: { const _bp_22 = _allocator.create(Ast.DeclMethod) catch @panic("OOM"); _bp_22.* = DeclMethod.init(mkSpan(), emptyMods(), name, std.ArrayList([]const u8).empty, std.ArrayList(Param).empty, ret, null, false, false, std.ArrayList(Expr).empty, std.ArrayList(Expr).empty, std.ArrayList([]const u8).empty); break :_box_22 _bp_22; } };
}
