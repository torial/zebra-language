...
pub const PipelineTest = struct {
    ...
    pub fn testEmptyClass(self: *PipelineTest) anyerror!void {
        ...
        if (!(zig.contains("pub const Foo"))) @panic("expected 'pub const Foo' in output");
        if (!(zig.contains("_type_tag"))) @panic("expected _type_tag in class output");
        std.debug.print("{s}\n", .{"testEmptyClass: OK"});
    }
    ...
    pub fn testClassWithField(self: *PipelineTest) anyerror!void {
        ...
        const zig = try runPipeline(src);
        if (!(zig.contains("pub const Point"))) @panic("expected Point struct");
        if (!(zig.contains("x: i64"))) @panic("expected x: i64 field");
        if (!(zig.contains("y: i64"))) @panic("expected y: i64 field");
    ...
    pub fn testMethodWithReturn(self: *PipelineTest) anyerror!void {
        ...
        const zig = try runPipeline(src);
        if (!(zig.contains("pub fn double"))) @panic("expected pub fn double");
        if (!(zig.contains("return"))) @panic("expected return in output");
        std.debug.print("{s}\n", .{"testMethodWithReturn: OK"});
    }

    pub fn testVarWithInit(self: *PipelineTest) anyerror!void {
        _ = self;
        const src: []const u8 = "class Counter\n    def reset()\n        var count = 0\n";
        const zig = try runPipeline(src);
        if (!(zig.contains("pub fn reset"))) @panic("expected pub fn reset");
        if (!(zig.contains("count"))) @panic("expected count in output");
        std.debug.print("{s}\n", .{"testVarWithInit: OK"});
    }

    pub fn testEnumDecl(self: *PipelineTest) anyerror!void {
        _ = self;
        const src: []const u8 = "enum Color\n    red\n    green\n    blue\n";
        const zig = try runPipeline(src);
        if (!(zig.contains("pub const Color"))) @panic("expected pub const Color");
        if (!(zig.contains("red"))) @panic("expected red variant");
        if (!(zig.contains("blue"))) @panic("expected blue variant");
        std.debug.print("{s}\n", .{"testEnumDecl: OK"});
    }

    pub fn testEntryPointNonThrowing(self: *PipelineTest) anyerror!void {
        _ = self;
        const src: []const u8 = "class Main\n    shared def main()\n        pass\n";
        const pm_node = try Parser.Parser.parse(src);
        switch (pm_node) {
            .module_ => |_ptr_pm| {
                const pm = _ptr_pm.*;
                ...
                if (!((std.mem.indexOf(u8, entry, "pub fn main()") != null))) @panic("expected pub fn main()");
                if (!((std.mem.indexOf(u8, entry, "_arena.allocator()") != null))) @panic("expected allocator setup");
                if (!((std.mem.indexOf(u8, entry, "Main.main()") != null))) @panic("expected Main.main() call");
                if (!((!(std.mem.indexOf(u8, entry, "catch") != null)))) @panic("non-throws main should not have catch");
            },
            else => |_| {
    ...
    pub fn testEntryPointThrowing(self: *PipelineTest) anyerror!void {
        ...
        switch (pm_node) {
            .module_ => |_ptr_pm| {
                ...
                const module = try ASTBuilder.build(pm, "<test>");
                const entry: []const u8 = generateEntryPoint(module);
                if (!((std.mem.indexOf(u8, entry, "App.main() catch") != null))) @panic("expected catch for throws main");
                if (!((std.mem.indexOf(u8, entry, "ZebraError") != null))) @panic("expected ZebraError handler");
            },
            else => |_| {
                { _error_ctx = .{ .message = "expected module" }; return error.ZebraError; }
            },
    ...
    pub fn testEntryPointNoMain(self: *PipelineTest) anyerror!void {
        ...
        switch (pm_node) {
            .module_ => |_ptr_pm| {
                ...
                const module = try ASTBuilder.build(pm, "<test>");
                const entry: []const u8 = generateEntryPoint(module);
                if (!(std.mem.eql(u8, entry, ""))) @panic("no entry point for class without shared main");
            },
            else => |_| {
                { _error_ctx = .{ .message = "expected module" }; return error.ZebraError; }
            },
    ...
    pub fn testResolverClean(self: *PipelineTest) anyerror!void {
        _ = self;
        const src: []const u8 = "class Adder\n    def add(a as int, b as int) as int\n        return a\n";
        const pm_node = try Parser.Parser.parse(src);
        var resolver = Resolver.init();
        try resolver.resolve(pm_node);
        if (!((resolver.errorCount() == 0))) @panic("clean program should have 0 resolver errors");
        std.debug.print("{s}\n", .{"testResolverClean: OK"});
    }

    pub fn testResolverError(self: *PipelineTest) anyerror!void {
        _ = self;
        const src: []const u8 = "class Broken\n    def bad()\n        print undeclaredVar\n";
        const pm_node = try Parser.Parser.parse(src);
        var resolver = Resolver.init();
        try resolver.resolve(pm_node);
        if (!((resolver.errorCount() > 0))) @panic("expected resolver error for undeclared name");
    ...
    pub fn testFullRoundTrip(self: *PipelineTest) anyerror!void {
        ...
        if ((!(blk_fex: { std.fs.cwd().access(preamble_path, .{}) catch break :blk_fex false; break :blk_fex true; }))) {
        ...
        if (!(zig.contains("const std"))) @panic("expected preamble std import");
        if (!(zig.contains("_arena"))) @panic("expected arena allocator in preamble");
        if (!(zig.contains("pub const Main"))) @panic("expected Main class declaration");
        if (!(zig.contains("pub fn main()"))) @panic("expected entry thunk");
        if (!(zig.contains("Main.main()"))) @panic("expected Main.main() call in entry");
...
const _ttag_PipelineTest: u64 = _zbr_hash("PipelineTest");
