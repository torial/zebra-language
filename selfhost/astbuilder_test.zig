...
pub const ASTBuilderTest = struct {
    ...
    pub fn buildModule(self: *ASTBuilderTest, src: []const u8) anyerror!Module {
        ...
        switch (pm) {
            .module_ => |_ptr_m| {
                ...
                { _error_ctx = .{ .message = "expected module_ from parser" }; return error.ZebraError; }
            },
        }
    }
    ...
    pub fn firstDecl(self: *ASTBuilderTest, src: []const u8) anyerror!Decl {
        ...
        if (!((@as(i64, @intCast(mod.decls.items.len)) > 0))) @panic("expected at least one decl");
        return mod.decls.items[@intCast(0)];
    }
    ...
    pub fn firstMethodStmt(self: *ASTBuilderTest, src: []const u8) anyerror!Stmt {
        ...
        switch (d) {
            .class_ => |_ptr_c| {
                const c = _ptr_c.*;
                if (!((@as(i64, @intCast(c.members.items.len)) > 0))) @panic("expected class members");
                const meth_decl = c.members.items[@intCast(0)];
                switch (meth_decl) {
                    .method => |_ptr_meth| {
                        const meth = _ptr_meth.*;
                        const stmts: std.ArrayList(Stmt) = meth.stmts.?;
                        if (!((@as(i64, @intCast(stmts.items.len)) > 0))) @panic("expected method stmts");
                        return stmts.items[@intCast(0)];
                    },
                    else => |_| {
                        { _error_ctx = .{ .message = "expected method decl" }; return error.ZebraError; }
                    },
                }
            },
            else => |_| {
                { _error_ctx = .{ .message = "expected class_ decl" }; return error.ZebraError; }
            },
        }
    }
    ...
    pub fn stmtExprValue(self: *ASTBuilderTest, s: Stmt) anyerror!Expr {
        ...
        switch (s) {
            .expr => |_ptr_e| {
                ...
                { _error_ctx = .{ .message = "expected Stmt.expr" }; return error.ZebraError; }
            },
        }
    }
    ...
    pub fn testEmptyClass(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (d) {
            .class_ => |_ptr_c| {
                const c = _ptr_c.*;
                if (!(std.mem.eql(u8, c.name, "Foo"))) @panic("expected class name 'Foo'");
                if (!((@as(i64, @intCast(c.members.items.len)) == 0))) @panic("expected 0 members");
            },
            else => |_| {
                if (!(false)) @panic("expected class_ decl");
            },
        }
        std.debug.print("{s}\n", .{"testEmptyClass: OK"});
    }
    ...
    pub fn testClassWithField(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (d) {
            .class_ => |_ptr_c| {
                const c = _ptr_c.*;
                if (!(std.mem.eql(u8, c.name, "Foo"))) @panic("expected class name 'Foo'");
                if (!((@as(i64, @intCast(c.members.items.len)) == 1))) @panic("expected 1 member");
                const m = c.members.items[@intCast(0)];
                switch (m) {
                    .var_ => |_ptr_v| {
                        const v = _ptr_v.*;
                        if (!(std.mem.eql(u8, v.name, "x"))) @panic("field name should be 'x'");
                        const tr = v.type_.?;
                        switch (tr) {
                            .named => |nr| {
                                if (!(std.mem.eql(u8, nr.name, "int"))) @panic("field type should be 'int'");
                            },
                            else => |_| {
                                if (!(false)) @panic("expected named TypeRef");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("expected var_ decl for field");
                    },
                }
            },
            else => |_| {
    ...
    pub fn testEnumDecl(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (d) {
            .enum_ => |_ptr_e| {
                const e = _ptr_e.*;
                if (!(std.mem.eql(u8, e.name, "Color"))) @panic("expected enum name 'Color'");
                if (!((@as(i64, @intCast(e.members.items.len)) == 3))) @panic("expected 3 enum members");
                if (!(std.mem.eql(u8, e.members.items[@intCast(0)].name, "red"))) @panic("expected member 'red'");
                if (!(std.mem.eql(u8, e.members.items[@intCast(1)].name, "green"))) @panic("expected member 'green'");
                if (!(std.mem.eql(u8, e.members.items[@intCast(2)].name, "blue"))) @panic("expected member 'blue'");
            },
            else => |_| {
                if (!(false)) @panic("expected enum_ decl");
            },
        }
        std.debug.print("{s}\n", .{"testEnumDecl: OK"});
    }
        ...
        switch (d) {
            .class_ => |_ptr_c| {
                ...
                switch (m) {
                    .method => |_ptr_meth| {
                        const meth = _ptr_meth.*;
                        if (!(std.mem.eql(u8, meth.name, "bar"))) @panic("method name should be 'bar'");
                        if (!((meth.mods.is_shared == false))) @panic("bar should not be shared");
                        if (!((@as(i64, @intCast(meth.params.items.len)) == 0))) @panic("expected 0 params");
                    },
                    else => |_| {
                        if (!(false)) @panic("expected method decl");
                    },
                }
            },
            else => |_| {
        ...
        switch (d) {
            .class_ => |_ptr_c| {
                ...
                switch (m) {
                    .method => |_ptr_meth| {
                        const meth = _ptr_meth.*;
                        if (!(std.mem.eql(u8, meth.name, "add"))) @panic("method name should be 'add'");
                        if (!((@as(i64, @intCast(meth.params.items.len)) == 2))) @panic("expected 2 params");
                        if (!(std.mem.eql(u8, meth.params.items[@intCast(0)].name, "x"))) @panic("first param should be 'x'");
                        if (!(std.mem.eql(u8, meth.params.items[@intCast(1)].name, "y"))) @panic("second param should be 'y'");
                        const ret = meth.return_type.?;
                        switch (ret) {
                            .named => |nr| {
                                if (!(std.mem.eql(u8, nr.name, "int"))) @panic("return type should be 'int'");
                            },
                            else => |_| {
                                if (!(false)) @panic("expected named return type");
                            },
                        }
                    },
                    else => |_| {
    ...
    pub fn testIfStmt(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .if_ => |_ptr_si| {
                const si = _ptr_si.*;
                if (!((si.then_stmts.len == 1))) @panic("expected 1 then_stmt");
                if (!((si.else_ifs.len == 0))) @panic("expected 0 else_ifs");
            },
            else => |_| {
                if (!(false)) @panic("expected if_ stmt");
            },
        }
        std.debug.print("{s}\n", .{"testIfStmt: OK"});
    }
    ...
    pub fn testWhileStmt(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .while_ => |_ptr_sw| {
                const sw = _ptr_sw.*;
                if (!((@as(i64, @intCast(sw.stmts.items.len)) == 1))) @panic("expected 1 body stmt");
            },
            else => |_| {
                if (!(false)) @panic("expected while_ stmt");
            },
        }
        std.debug.print("{s}\n", .{"testWhileStmt: OK"});
    }
    ...
    pub fn testForInStmt(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .for_in => |_ptr_sf| {
                const sf = _ptr_sf.*;
                if (!((sf.vars.len == 1))) @panic("expected 1 loop var");
                if (!(std.mem.eql(u8, sf.vars.items[@intCast(0)], "x"))) @panic("loop var should be 'x'");
                if (!((@as(i64, @intCast(sf.stmts.items.len)) == 1))) @panic("expected 1 body stmt");
            },
            else => |_| {
                if (!(false)) @panic("expected for_in stmt");
            },
        }
        std.debug.print("{s}\n", .{"testForInStmt: OK"});
    }
            ...
            .assign => |_ptr_sa| {
                ...
                switch (sa.op) {
                    .assign => {
                        ...
                        if (!(false)) @panic("expected assign op");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("expected assign stmt");
            },
        }
        std.debug.print("{s}\n", .{"testAssignStmt: OK"});
    }
    ...
    pub fn testExprBinary(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (se) {
            .call => |_ptr_ec| {
                const ec = _ptr_ec.*;
                if (!((@as(i64, @intCast(ec.args.items.len)) == 1))) @panic("expected 1 arg");
                const arg_val = ec.args.items[@intCast(0)].value;
                switch (arg_val) {
                    .binary => |_ptr_eb| {
                        const eb = _ptr_eb.*;
                        switch (eb.op) {
                            .add => {
                                ...
                                if (!(false)) @panic("expected add op");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("expected binary expr in arg");
                    },
                }
            },
            else => |_| {
                if (!(false)) @panic("expected call expr");
            },
        }
        std.debug.print("{s}\n", .{"testExprBinary: OK"});
    }
                    ...
                    .unary => |_ptr_eu| {
                        ...
                        switch (eu.op) {
                            .not_ => {
                                ...
                                if (!(false)) @panic("expected not_ op");
                            },
                        }
                    },
                    else => |_| {
                        if (!(false)) @panic("expected unary expr in arg");
                    },
                }
            },
            else => |_| {
    ...
    pub fn testExprCall(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (se) {
            .call => |_ptr_ec| {
                const ec = _ptr_ec.*;
                if (!((@as(i64, @intCast(ec.args.items.len)) == 2))) @panic("expected 2 args");
                switch (ec.callee) {
                    .ident => |id| {
                        if (!(std.mem.eql(u8, id.name, "foo"))) @panic("callee should be 'foo'");
                    },
                    else => |_| {
                        if (!(false)) @panic("expected ident callee");
                    },
                }
            },
            else => |_| {
        ...
        switch (se) {
            .call => |_ptr_ec| {
                ...
                switch (arg_val) {
                    .member => |_ptr_em| {
                        const em = _ptr_em.*;
                        if (!(std.mem.eql(u8, em.member, "field"))) @panic("member should be 'field'");
                    },
                    else => |_| {
                        if (!(false)) @panic("expected member expr in arg");
                    },
                }
            },
            else => |_| {
            ...
            .class_ => |_ptr_c| {
                ...
                switch (m) {
                    .var_ => |_ptr_v| {
                        ...
                        switch (tr) {
                            .named => |nr| {
                                if (!(std.mem.eql(u8, nr.name, "int"))) @panic("expected 'int'");
                            },
                            else => |_| {
                                if (!(false)) @panic("expected named TypeRef");
                            },
                        ...
                        if (!(false)) @panic("expected var_ decl");
                    },
                }
            },
            else => |_| {
                    ...
                    .var_ => |_ptr_v| {
                        ...
                        switch (tr) {
                            .nilable => {
                                ...
                                if (!(false)) @panic("expected nilable TypeRef");
                            },
                        }
                    },
                    else => |_| {
            ...
            .class_ => |_ptr_c| {
                ...
                switch (m) {
                    .var_ => |_ptr_v| {
                        ...
                        switch (tr) {
                            .generic => |g| {
                                if (!(std.mem.eql(u8, g.name, "List"))) @panic("expected 'List'");
                                if (!((@as(i64, @intCast(g.args.items.len)) == 1))) @panic("expected 1 type arg");
                                switch (g.args.items[@intCast(0)]) {
                                    .named => |nr| {
                                        if (!(std.mem.eql(u8, nr.name, "str"))) @panic("expected 'str' arg");
                                    },
                                    else => |_| {
                                        if (!(false)) @panic("expected named type arg");
                                    },
                                }
                            },
                            else => |_| {
                                if (!(false)) @panic("expected generic TypeRef");
                            },
                        }
                    },
                    else => |_| {
    ...
    pub fn testReturnValue(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .return_ => |_ptr_sr| {
                const sr = _ptr_sr.*;
                if (!((sr.value != null))) @panic("return value should not be nil");
            },
            else => |_| {
                if (!(false)) @panic("expected return_ stmt");
            },
        }
        std.debug.print("{s}\n", .{"testReturnValue: OK"});
    }
    ...
    pub fn testVarInit(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .var_ => |_ptr_v| {
                const v = _ptr_v.*;
                if (!((v.init_expr != null))) @panic("var init_expr should not be nil");
            },
            else => |_| {
                if (!(false)) @panic("expected var_ stmt");
            },
        }
        std.debug.print("{s}\n", .{"testVarInit: OK"});
    }
    ...
    pub fn testAssertMessage(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .assert_ => |_ptr_sa| {
                const sa = _ptr_sa.*;
                if (!((sa.message != null))) @panic("assert message should not be nil");
            },
            else => |_| {
                if (!(false)) @panic("expected assert_ stmt");
            },
        }
    ...
    pub fn testRaiseMessage(self: *ASTBuilderTest) anyerror!void {
        ...
        switch (s) {
            .raise_ => |_ptr_sr| {
                const sr = _ptr_sr.*;
                if (!((sr.message != null))) @panic("raise message should not be nil");
            },
            else => |_| {
                if (!(false)) @panic("expected raise_ stmt");
            },
        }
        std.debug.print("{s}\n", .{"testRaiseMessage: OK"});
    }
...
const _ttag_ASTBuilderTest: u64 = _zbr_hash("ASTBuilderTest");
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_ASTBuilderTest) return _reflect_ASTBuilderTest_name;
    if (tag == _ttag_Main) return _reflect_Main_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_ASTBuilderTest) return .{ .items = _reflect_ASTBuilderTest_fields };
    if (tag == _ttag_Main) return .{ .items = _reflect_Main_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_ASTBuilderTest) return .{ .items = _reflect_ASTBuilderTest_field_types };
