...
// zbr:selfhost/cg_helpers.zbr:66
            if ((!std.mem.startsWith(u8, item, prefix))) {
                new_items.append(_allocator, _intern(item)) catch @panic("OOM");
            }
        }
...
pub const ClosureThunk = struct {
    ...
    pub fn init(id: i64, lambda: ExprLambda) ClosureThunk {
        var _self: ClosureThunk = undefined;
...
pub const ThunkList = struct {
    ...
    _items: std.ArrayList(ClosureThunk),
    pub fn init() *ThunkList {
        const _self = _allocator.create(ThunkList) catch @panic("OOM");
        _self._type_tag = _zbr_hash("ThunkList");
// zbr:selfhost/cg_helpers.zbr:95
            _self._items = std.ArrayList(ClosureThunk).empty;
        return _self;
    }

    pub fn add(self: *ThunkList, t: ClosureThunk) void {
        self._items.append(_allocator, t) catch @panic("OOM");
    }
...
// zbr:selfhost/cg_helpers.zbr:104
        return self._items.items[@intCast(i)];
    }
...
// zbr:selfhost/cg_helpers.zbr:747
                        if (_zebra_gt(@as(i64, @intCast(out.count())), before)) {
...
// zbr:selfhost/cg_helpers.zbr:973
                    if (co.target == .member) {
