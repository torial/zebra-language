...
pub const Lexer = struct {
    ...
    lambdaBodyActive: bool,
    lambdaIndentLevel: i64,
    pub fn init() *Lexer {
        ...
        _self._type_tag = _zbr_hash("Lexer");
            ...
            _self.blockDepth = 0;
            _self.parenDepth = 0;
            _self.out = std.ArrayList(*Token.Token){};
            _self.inLambdaParams = false;
            _self.lambdaParamDepth = 0;
            _self.afterLambdaParams = false;
            _self.lambdaBodyActive = false;
            _self.lambdaIndentLevel = 0;
        return _self;
    }
    ...
    pub fn scanWhile(self: *Lexer, pred: CharPred) void {
        while (((self.pos < self.src.len) and pred(self.src[@as(usize, @intCast(self.pos))]))) {
            self.pos = (self.pos + 1);
        }
    }
    ...
    pub fn classifyLine(self: *Lexer) LineKind {
        var i = self.pos;
        if (((i >= self.src.len) or (self.src[@as(usize, @intCast(i))] == '\n'))) {
            return LineKind{ .empty = {} };
        }
        var hasWs = false;
        while (((i < self.src.len) and ((self.src[@as(usize, @intCast(i))] == ' ') or (self.src[@as(usize, @intCast(i))] == '\t')))) {
            hasWs = true;
            i = (i + 1);
        }
        if (((i >= self.src.len) or (self.src[@as(usize, @intCast(i))] == '\n'))) {
    ...
    pub fn processIndentation(self: *Lexer) anyerror!void {
        ...
        if ((self.lambdaBodyActive and (self.indentDepth <= self.lambdaIndentLevel))) {
    ...
    pub fn scanBlockComment(self: *Lexer) anyerror!void {
        ...
        while (((self.pos < self.src.len) and (self.blockDepth > 0))) {
            if ((((self.src[@as(usize, @intCast(self.pos))] == '/') and ((self.pos + 1) < self.src.len)) and (self.src[@as(usize, @intCast((self.pos + 1)))] == '#'))) {
                self.pos = (self.pos + 2);
                self.blockDepth = (self.blockDepth + 1);
            } else {
                if ((((self.src[@as(usize, @intCast(self.pos))] == '#') and ((self.pos + 1) < self.src.len)) and (self.src[@as(usize, @intCast((self.pos + 1)))] == '/'))) {
    ...
    pub fn scanAt(self: *Lexer, ln: i64, cl: i64) anyerror!void {
        ...
        { _error_ctx = .{ .message = "UnexpectedCharacter" }; return error.ZebraError; }
    }
    ...
    pub fn scanIdentOrKeyword(self: *Lexer, ln: i64, cl: i64) anyerror!void {
        ...
        if ((((((word.len > 3) and (word[@as(usize, @intCast(0))] == 'i')) and (word[@as(usize, @intCast(1))] == 'n')) and (word[@as(usize, @intCast(2))] == 't')) and isDigit(word[@as(usize, @intCast(3))]))) {
            self.emit(TokenKind{ .int_size = {} }, word, ln, cl);
            return;
        }
        if (((((((word.len > 4) and (word[@as(usize, @intCast(0))] == 'u')) and (word[@as(usize, @intCast(1))] == 'i')) and (word[@as(usize, @intCast(2))] == 'n')) and (word[@as(usize, @intCast(3))] == 't')) and isDigit(word[@as(usize, @intCast(4))]))) {
            self.emit(TokenKind{ .uint_size = {} }, word, ln, cl);
            return;
        }
        if ((((((((word.len > 5) and (word[@as(usize, @intCast(0))] == 'f')) and (word[@as(usize, @intCast(1))] == 'l')) and (word[@as(usize, @intCast(2))] == 'o')) and (word[@as(usize, @intCast(3))] == 'a')) and (word[@as(usize, @intCast(4))] == 't')) and isDigit(word[@as(usize, @intCast(5))]))) {
    ...
    pub fn scanInterpExpr(self: *Lexer) anyerror!void {
        ...
        while ((depth > 0)) {
            ...
            if ((c == ':')) {
                ...
                self.pos = (self.pos + 1);
                while ((((self.pos < self.src.len) and (self.src[@as(usize, @intCast(self.pos))] != '}')) and (self.src[@as(usize, @intCast(self.pos))] != '\n'))) {
                    self.pos = (self.pos + 1);
                }
                if (((self.pos < self.src.len) and (self.src[@as(usize, @intCast(self.pos))] == '}'))) {
    ...
    pub fn scanDocString(self: *Lexer, ln: i64, cl: i64) anyerror!void {
        const start = self.pos;
        self.pos = (self.pos + 3);
        while (((self.pos < self.src.len) and ((self.src[@as(usize, @intCast(self.pos))] == ' ') or (self.src[@as(usize, @intCast(self.pos))] == '\t')))) {
            self.pos = (self.pos + 1);
        }
        if (((self.pos < self.src.len) and (self.src[@as(usize, @intCast(self.pos))] != '\n'))) {
        ...
        while ((self.pos < self.src.len)) {
            var p = self.pos;
            while (((p < self.src.len) and ((self.src[@as(usize, @intCast(p))] == ' ') or (self.src[@as(usize, @intCast(p))] == '\t')))) {
                p = (p + 1);
            }
            if ((((((p + 2) < self.src.len) and (self.src[@as(usize, @intCast(p))] == '"')) and (self.src[@as(usize, @intCast((p + 1)))] == '"')) and (self.src[@as(usize, @intCast((p + 2)))] == '"'))) {
                self.pos = (p + 3);
                self.emit(TokenKind{ .doc_string_line = {} }, self.src[@as(usize, @intCast(start))..@as(usize, @intCast(self.pos))], ln, cl);
                return;
            }
            while (((self.pos < self.src.len) and (self.src[@as(usize, @intCast(self.pos))] != '\n'))) {
                self.pos = (self.pos + 1);
            }
            if ((self.pos < self.src.len)) {
                self.advanceNewline();
    ...
    pub fn scanNumericLiteral(self: *Lexer, ln: i64, cl: i64) anyerror!void {
        const start = self.pos;
        if ((((self.src[@as(usize, @intCast(self.pos))] == '0') and ((self.pos + 1) < self.src.len)) and (self.src[@as(usize, @intCast((self.pos + 1)))] == 'x'))) {
            self.pos = (self.pos + 2);
        ...
        if (hasDot) {
            self.emit(TokenKind{ .fractional_lit = {} }, self.src[@as(usize, @intCast(start))..@as(usize, @intCast(self.pos))], ln, cl);
        } else {
    ...
    pub fn scanOperator(self: *Lexer, ln: i64, cl: i64) anyerror!void {
        ...
        if ((c == '(')) {
            if (((self.parenDepth > 0) and (@as(i64, @intCast(self.out.items.len)) > 0))) {
